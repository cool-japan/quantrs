...
pub enum ValidationError {
    ...
    #[error("Index out of bounds: register {register} has size {size}, but index {index} was used")]
    IndexOutOfBounds { register: String, size: usize, index: usize },
    
    #[error("Parameter count mismatch: gate {gate} expects {expected} parameters, but {found} were provided")]
    ParameterCountMismatch { gate: String, expected: usize, found: usize },
    
    #[error("Qubit count mismatch: gate {gate} expects {expected} qubits, but {found} were provided")]
    QubitCountMismatch { gate: String, expected: usize, found: usize },
...
enum Symbol {
    ...
    ClassicalRegister { size: usize },
    Gate { params: Vec<String>, qubits: Vec<String> },
    Variable { typ: ValueType },
    Constant { typ: ValueType },
}
...
impl QasmValidator {
    ...
    pub fn validate(&mut self, program: &QasmProgram) -> Result<(), ValidationError> {
        ...
        self.symbols.insert("pi".to_string(), Symbol::Constant { typ: ValueType::Float });
        self.symbols.insert("e".to_string(), Symbol::Constant { typ: ValueType::Float });
        self.symbols.insert("tau".to_string(), Symbol::Constant { typ: ValueType::Float });
    ...
    fn validate_declaration(&mut self, decl: &Declaration) -> Result<(), ValidationError> {
        match decl {
            Declaration::QuantumRegister(reg) => {
                ...
                self.symbols.insert(
                    reg.name.clone(),
                    Symbol::QuantumRegister { size: reg.size }
                );
            }
            ...
            Declaration::GateDefinition(def) => {
                ...
                for param in &def.params {
                    self.add_to_current_scope(
                        param.clone(),
                        Symbol::Variable { typ: ValueType::Angle }
                    );
                }
                ...
                for qubit in &def.qubits {
                    self.add_to_current_scope(
                        qubit.clone(),
                        Symbol::Variable { typ: ValueType::Qubit }
                    );
                }
        ...
        match stmt {
            ...
            QasmStatement::Assignment(var, expr) => {
                ...
                if let Some(symbol) = self.lookup_symbol(var) {
                    match symbol {
                        ...
                        _ => return Err(ValidationError::SemanticError(
            ...
            QasmStatement::For(for_loop) => {
                ...
                self.add_to_current_scope(
                    ...
                    Symbol::Variable { typ: ValueType::Int }
                );
    ...
    fn validate_gate(&mut self, gate: &QasmGate) -> Result<(), ValidationError> {
        ...
        let (expected_params, expected_qubits) = if let Some(&(p, q)) = self.standard_gates.get(&gate.name) {
            (p, q)
        ...
        match classical_ref {
            ClassicalRef::Single { register, index } => {
                match self.lookup_symbol(register) {
                    ...
                    Some(Symbol::Variable { typ: ValueType::Bit }) => {
                        // Single bit variable
                    }
                    _ => return Err(ValidationError::UndefinedRegister(register.clone())),
                }
            }
            ClassicalRef::Slice { register, start, end } => {
                match self.lookup_symbol(register) {
                    Some(Symbol::ClassicalRegister { size }) => {
                        if *start >= *size || *end > *size {
                            return Err(ValidationError::IndexOutOfBounds {
        ...
        match expr {
            ...
            Expression::Binary(op, left, right) => {
                ...
                match op {
                    ...
                    BinaryOp::Gt | BinaryOp::Ge => Ok(ValueType::Bool),
                    BinaryOp::Pow => {
                        if (left_typ == ValueType::Int || left_typ == ValueType::Float) &&
                           (right_typ == ValueType::Int || right_typ == ValueType::Float) {
                            Ok(ValueType::Float)
            ...
            Expression::Unary(op, expr) => {
                ...
                match op {
                    ...
                    UnaryOp::Acos | UnaryOp::Atan | UnaryOp::Exp | UnaryOp::Ln | 
                    UnaryOp::Sqrt => {
                        if typ == ValueType::Int || typ == ValueType::Float {
                            Ok(ValueType::Float)
                        } else {
    ...
    fn types_compatible(&self, typ1: &ValueType, typ2: &ValueType) -> bool {
        match (typ1, typ2) {
            ...
            (ValueType::Duration, ValueType::Float) | (ValueType::Float, ValueType::Duration) => true,
            (t1, t2) => t1 == t2,
        }
    }
...
"#;
        ...
        let program = parse_qasm3(input).unwrap();
        let result = validate_qasm3(&program);
        assert!(matches!(result, Err(ValidationError::IndexOutOfBounds { .. })));
    }
...
"#;
        ...
        let program = parse_qasm3(input).unwrap();
        let result = validate_qasm3(&program);
        assert!(matches!(result, Err(ValidationError::ParameterCountMismatch { .. })));
    }
}