                ...
                match ch {
                    ...
                    '<' => {
                        ...
                        match self.current {
                            ...
                            Some('<') => { self.advance(); Ok(Token::Shl) }
                            _ => Ok(Token::Lt)
                        }
                    }
                    '>' => {
                        self.advance();
                        match self.current {
                            Some('=') => { self.advance(); Ok(Token::Ge) }
                            Some('>') => { self.advance(); Ok(Token::Shr) }
                            _ => Ok(Token::Gt)
                        }
                    }
                    '&' => {
...
impl<'a> QasmParser<'a> {
    ...
    fn parse_quantum_register(&mut self) -> Result<Declaration, ParseError> {
        ...
        // Add to symbol table
        self.symbols.insert(name.clone(), SymbolType::QuantumRegister(size));
    ...
    fn parse_classical_register(&mut self) -> Result<Declaration, ParseError> {
        ...
        // Add to symbol table
        self.symbols.insert(name.clone(), SymbolType::ClassicalRegister(size));
    ...
    fn parse_gate_definition(&mut self) -> Result<Declaration, ParseError> {
        ...
        // Add to symbol table
        self.symbols.insert(name.clone(), SymbolType::Gate(params.clone(), qubits.clone()));
    ...
    fn parse_gate_or_call(&mut self, name: String) -> Result<QasmStatement, ParseError> {
        ...
        match &self.current_token {
            ...
            _ => Err(ParseError::InvalidSyntax("Expected qubits or semicolon".into())),
        }
    }
    ...
    fn parse_qubit_ref(&mut self) -> Result<QubitRef, ParseError> {
        ...
        if self.current_token == Token::LeftBracket {
            ...
            if self.current_token == Token::Colon {
                ...
                Ok(QubitRef::Slice { register, start, end })
            } else {
                // Single index
                self.expect_token(&Token::RightBracket)?;
                Ok(QubitRef::Single { register, index: start })
            }
        } else {
            // Entire register
    ...
    fn parse_classical_ref(&mut self) -> Result<ClassicalRef, ParseError> {
        ...
        if self.current_token == Token::LeftBracket {
            ...
            if self.current_token == Token::Colon {
                ...
                Ok(ClassicalRef::Slice { register, start, end })
            } else {
                // Single index
                self.expect_token(&Token::RightBracket)?;
                Ok(ClassicalRef::Single { register, index: start })
            }
        } else {
            // Entire register
    ...
    fn parse_unary_expression(&mut self) -> Result<Expression, ParseError> {
        match &self.current_token {
            Token::Minus => {
                self.advance()?;
                Ok(Expression::Unary(UnaryOp::Neg, Box::new(self.parse_unary_expression()?)))
            }
            Token::Not => {
                self.advance()?;
                Ok(Expression::Unary(UnaryOp::Not, Box::new(self.parse_unary_expression()?)))
            }
            Token::BitNot => {
                self.advance()?;
                Ok(Expression::Unary(UnaryOp::BitNot, Box::new(self.parse_unary_expression()?)))
            }
            ...
            match &self.current_token {
                Token::LeftBracket => {
                    ...
                    match expr {
                        ...
                        _ => return Err(ParseError::InvalidSyntax("Cannot index non-variable".into())),
                    }
                }
                Token::LeftParen => {
                    // Function call
                    ...
                    match expr {
                        ...
                        _ => return Err(ParseError::InvalidSyntax("Cannot call non-function".into())),
                    }
                }
                _ => break,
            }
