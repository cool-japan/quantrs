...
impl SingleQubitSynthesizer {
    ...
    fn approximate_with_basic_gate<const N: usize>(
        ...
        match best_gate_idx {
            ...
            1 => { circuit.add_gate(Hadamard { target })?; },
            2 => { circuit.add_gate(T { target })?; },
            3 => {
                // T† = T·T·T
            ...
            4 => { circuit.add_gate(Phase { target })?; }, // S gate
            5 => {
                // S† = S·S·S  
    ...
    fn adjoint_gate(&self, gate: &dyn GateOp) -> QuantRS2Result<Box<dyn GateOp>> {
        match gate.name() {
            "H" => Ok(Box::new(Hadamard { target: gate.qubits()[0] })), // H is self-adjoint
            "T" => {
                // T† = T³
...
impl TwoQubitSynthesizer {
    ...
    pub fn shannon_decomposition<const N: usize>(
        ...
        if self.is_significant_block(&u00_block) {
            ...
            if delta.abs() > self.config.tolerance {
                circuit.add_gate(RotationZ { target, theta: delta })?;
            }
            if gamma.abs() > self.config.tolerance {
                circuit.add_gate(RotationY { target, theta: gamma })?;
            }
            if beta.abs() > self.config.tolerance {
                circuit.add_gate(RotationZ { target, theta: beta })?;
            }
        }
        ...
        if self.is_significant_block(&u11_block) {
            ...
            if delta.abs() > self.config.tolerance {
                circuit.add_gate(CRZ { control, target, theta: delta })?;
            }
            if gamma.abs() > self.config.tolerance {
                circuit.add_gate(CRY { control, target, theta: gamma })?;
            }
            if beta.abs() > self.config.tolerance {
                circuit.add_gate(CRZ { control, target, theta: beta })?;
            }
        }
    ...
    fn cosine_sine_decomposition<const N: usize>(
        ...
        if self.is_block_significant(&u22.clone_owned()) {
            for i in half_size..n.min(N) {
                let angle = self.extract_rotation_angle_from_block(&u22.clone_owned(), i - half_size);
    ...
    fn decompose_small_matrix<const N: usize>(
        ...
        match unitary.nrows() {
            ...
            4 => {
                ...
                let two_circ: Circuit<N> = self.two_synth.synthesize(&u4, QubitId(0), QubitId(1))?;
                // Simplified: add basic gates rather than cloning
                circuit.add_gate(CNOT { control: QubitId(0), target: QubitId(1) })?;
            }
            _ => {
