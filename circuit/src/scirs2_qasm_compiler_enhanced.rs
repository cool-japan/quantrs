...
impl EnhancedQASMCompiler {
    ...
    pub fn convert_version(&self, source: &str, target_version: QASMVersion) -> QuantRS2Result<String> {
        let ast = self.parse_with_recovery(&self.lexical_analysis(source)?)?;
        let converted_ast = self.convert_ast_version(ast, target_version)?;
        self.code_generator.generate_qasm(&converted_ast, target_version)
    }
    ...
    pub fn optimize_qasm(&self, source: &str) -> QuantRS2Result<OptimizedQASM> {
        ...
        let optimized_code = self.code_generator.generate_qasm(
            &optimized_ast,
            self.config.base_config.qasm_version,
        )?;
    ...
    fn calculate_improvements(
        ...
        Ok(ImprovementMetrics {
            gate_reduction: (original.gate_count - optimized.gate_count) as f64 / original.gate_count as f64,
    ...
    fn export_openqasm(&self, ast: &AST) -> QuantRS2Result<Vec<u8>> {
        let code = self.code_generator.generate_qasm(ast, QASMVersion::OpenQASM)?;
        Ok(code.into_bytes())
    }
    
    fn export_qiskit(&self, ast: &AST) -> QuantRS2Result<Vec<u8>> {
        let code = self.code_generator.generate(&ast, CompilationTarget::Qiskit)?;
...
impl ErrorRecovery {
    ...
    fn recover_from_parse_error(&self, tokens: &[Token], error: &ParseError) -> QuantRS2Result<AST> {
...
impl fmt::Display for ParseError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "Parse error at {}:{}: {}", self.location.line, self.location.column, self.message)
    }
}
...
impl fmt::Display for TypeError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "Type error: expected {}, found {}", self.expected, self.found)
    }
}
...
mod tests {
    ...
    fn test_qasm_version_detection() {
        ...
        assert_eq!(compiler.detect_version("OPENQASM 3.0;").unwrap(), QASMVersion::QASM3);
        assert_eq!(compiler.detect_version("OPENQASM 2.0;").unwrap(), QASMVersion::QASM2);
    }
