...
use quantrs2_circuit::qc_co_optimization::{
    LinearAlgebraOp, MLModelType as QCMLModelType, UpdateRule, ScheduleType, LoadBalancingStrategy
};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("Added quantum circuit component with {} gates", 
    ...
    println!("Added {} classical processing steps", problem.classical_steps.len());
    ...
    println!("Set up {} data flow connections", problem.data_flow.edges.len());
    
    // Print data flow details
    for (i, (source, target, data_type)) in problem.data_flow.edges.iter().enumerate() {
        println!("  Flow {}: {} -> {} ({:?})", i + 1, source, target, data_type);
    }
    ...
    problem.add_regularization(
        RegularizationType::Sparsity,
        0.005,
        vec![3, 4, 5],
    )?;
    
    println!("Added {} regularization terms", problem.objective.regularization.len());
    
    for (i, reg) in problem.objective.regularization.iter().enumerate() {
        println!("  Regularization {}: {:?} (strength: {:.3})", 
                 i + 1, reg.reg_type, reg.strength);
    }
    ...
    let algorithms = vec![
        ("Coordinate Descent", HybridOptimizationAlgorithm::CoordinateDescent),
        ("Simultaneous Optimization", HybridOptimizationAlgorithm::SimultaneousOptimization),
        ("Hierarchical Optimization", HybridOptimizationAlgorithm::HierarchicalOptimization),
        ("Adaptive Optimization", HybridOptimizationAlgorithm::AdaptiveOptimization),
    ];
    ...
    for (name, algorithm) in algorithms {
        ...
        println!("    Initial learning rate: {}", optimizer.learning_rate_schedule.initial_rate);
    }
    ...
    println!("Quantum parallelism: {} circuits", optimizer.parallelization.quantum_parallelism);
    println!("Classical parallelism: {} threads", optimizer.parallelization.classical_parallelism);
    println!("Asynchronous execution: {}", optimizer.parallelization.asynchronous);
    println!("Load balancing: {:?}", optimizer.parallelization.load_balancing);
    ...
    let data_types = vec![
        ("Quantum measurements", "Raw measurement outcomes from quantum circuits"),
        ("Probability distributions", "Estimated probability distributions from measurements"),
        ("Classical matrices", "Processed data in matrix form for linear algebra"),
        ("Scalar values", "Single numerical values like energies or costs"),
        ("Parameter vectors", "Optimization parameters for both quantum and classical components"),
        ("Control signals", "Boolean flags for adaptive algorithm control"),
    ];
