...
use quantrs2_sim::{
    automatic_parallelization::{
        AutoParallelEngine, AutoParallelConfig, ParallelizationStrategy,
...
fn create_demo_circuits() -> Result<Vec<Circuit<16>>, Box<dyn std::error::Error>> {
    ...
    println!("- Gates: {}, Expected parallelism: High", circuit1.num_gates());
    ...
    println!("- Gates: {}, Expected parallelism: Low", circuit2.num_gates());
    ...
    println!("- Gates: {}, Expected parallelism: Medium", circuit3.num_gates());
    ...
    println!("- Gates: {}, Expected parallelism: Complex", circuit4.num_gates());
...
/// Test different parallelization strategies
fn test_parallelization_strategies(circuits: &[Circuit<16>]) -> Result<(), Box<dyn std::error::Error>> {
    println!("\n🧪 Testing Parallelization Strategies");
    println!("====================================\n");
    
    let strategies = vec![
        ("Dependency Analysis", ParallelizationStrategy::DependencyAnalysis),
        ("Layer-Based", ParallelizationStrategy::LayerBased),
        ("Qubit Partitioning", ParallelizationStrategy::QubitPartitioning),
        ...
        for (strategy_name, strategy) in &strategies {
            ...
            match engine.analyze_circuit(circuit) {
                Ok(analysis) => {
                    ...
                    println!("    • Critical path: {} layers", analysis.critical_path_length);
...
fn benchmark_parallelization(circuits: &[Circuit<16>]) -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("Average efficiency: {:.2}%", results.average_efficiency * 100.0);
    println!("Average max parallelism: {}", results.average_parallelism);
    
    println!("\nPer-Circuit Results:");
    for (i, result) in results.circuit_results.iter().enumerate() {
        println!("  Circuit {}: {} gates, {} qubits", 
                 i + 1, result.circuit_size, result.num_qubits);
...
fn demonstrate_parallel_execution() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("- CPU utilization: {:?}", analysis.resource_utilization.cpu_utilization);
    println!("- Load balance score: {:.2}", analysis.resource_utilization.load_balance_score);
    println!("- Communication overhead: {:.2}%", 
...
fn display_recommendations(analysis: &quantrs2_sim::automatic_parallelization::ParallelizationAnalysis) {
    if !analysis.recommendations.is_empty() {
...
fn compare_execution_performance() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for size in circuit_sizes {
        ...
        println!("{:11} | {:9.1}ms | {:7.1}ms | {:5.1}x", 
