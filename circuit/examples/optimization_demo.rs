...
fn demo_hardware_optimization(circuit: &Circuit<4>) {
    ...
    for backend in backends {
        ...
        match optimizer.optimize(circuit) {
            Ok(report) => {
                println!("  Initial cost: {:.2}", report.initial_metrics.execution_time);
...
fn demo_gate_properties() {
    ...
    for (g1, g2) in gate_pairs {
        println!("  {} ↔ {}: {}", g1, g2, 
            if comm_table.commutes(g1, g2) { "✓" } else { "✗" });
    }
    println!();
}
...
fn benchmark_optimization_passes(circuit: &Circuit<4>) {
    ...
    let passes: Vec<(&str, Box<dyn OptimizationPass>)> = vec![
        ...
        ("Two-Qubit Opt", Box::new(TwoQubitOptimization::new(false, true))),
    ];
    ...
    match optimizer.optimize(circuit) {
        Ok(report) => {
            ...
            println!("  Gates reduced: {} → {}", 
                report.initial_metrics.gate_count,
                report.final_metrics.gate_count);
