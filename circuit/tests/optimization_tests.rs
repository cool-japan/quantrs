...
fn test_commutation_table() {
    ...
    let z1 = single::PauliZ { target: QubitId::new(0) };
    let z2 = single::PauliZ { target: QubitId::new(1) };
    assert!(comm_table.gates_commute(&z1, &z2)); // Different qubits
    
    let x1 = single::PauliX { target: QubitId::new(0) };
    let z1_same = single::PauliZ { target: QubitId::new(0) };
    assert!(!comm_table.gates_commute(&x1, &z1_same)); // Same qubit, non-commuting
}

#[test]
fn test_cost_models() {
    let h_gate = single::Hadamard { target: QubitId::new(0) };
...
fn test_circuit_analyzer() {
    ...
    let gates: Vec<Box<dyn GateOp>> = vec![
        ...
        Box::new(single::PauliX { target: QubitId::new(1) }),
    ];
...
fn test_optimization_utils() {
    ...
    let h1 = single::Hadamard { target: QubitId::new(0) };
    let h2 = single::Hadamard { target: QubitId::new(0) };
    assert!(utils::gates_cancel(&h1, &h2));
    
    let x1 = single::PauliX { target: QubitId::new(0) };
    let x2 = single::PauliX { target: QubitId::new(0) };
    assert!(utils::gates_cancel(&x1, &x2));
    
    // Different qubits should not cancel
    let h3 = single::Hadamard { target: QubitId::new(1) };
    assert!(!utils::gates_cancel(&h1, &h3));
    
    // Test identity gate detection
    let rz_zero = single::RotationZ { target: QubitId::new(0), theta: 0.0 };
...
fn test_metric_improvement() {
    ...
    assert!((improvement.two_qubit_gates - 33.333333333333336).abs() < 0.00001); // ~33% reduction
}
