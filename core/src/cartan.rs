...
use crate::{
    ...
    synthesis::{SingleQubitDecomposition, decompose_single_qubit_zyz},
};
use ndarray::{Array2, Array1, s, ArrayView2};
...
impl CartanCoefficients {
    ...
    pub fn is_identity(&self, tolerance: f64) -> bool {
        self.xx.abs() < tolerance && 
        self.yy.abs() < tolerance && 
        self.zz.abs() < tolerance
    }
...
impl CartanDecomposer {
    ...
    pub fn decompose(&mut self, unitary: &Array2<Complex<f64>>) -> QuantRS2Result<CartanDecomposition> {
        // Validate input
        ...
        let (left_gates, right_gates) = self.compute_local_gates(
            unitary, &u_magic, &p, &coeffs
        )?;
    ...
    fn get_magic_basis(&self) -> Array2<Complex<f64>> {
        ...
        Array2::from_shape_vec((4, 4), vec![
            ...
            Complex::new(0.0, 0.0), Complex::new(1.0, 0.0), Complex::new(-1.0, 0.0), Complex::new(0.0, 0.0),
            Complex::new(1.0, 0.0), Complex::new(0.0, 0.0), Complex::new(0.0, 0.0), Complex::new(-1.0, 0.0),
        ]).unwrap() / Complex::new(sqrt2, 0.0)
    }
    
    /// Transform matrix to magic basis
    fn to_magic_basis(&self, u: &Array2<Complex<f64>>, magic: &Array2<Complex<f64>>) -> Array2<Complex<f64>> {
        let magic_dag = magic.mapv(|z| z.conj()).t().to_owned();
        magic_dag.dot(u).dot(magic)
    }
    
    /// Diagonalize a symmetric complex matrix
    fn diagonalize_symmetric(&self, m: &Array2<Complex<f64>>) -> QuantRS2Result<(Array1<f64>, Array2<Complex<f64>>)> {
    ...
    fn extract_coefficients(&self, eigenvalues: &Array1<f64>) -> CartanCoefficients {
        ...
        let phases: Vec<f64> = eigenvalues.iter()
    ...
    pub fn to_gates(&self, decomp: &CartanDecomposition, qubit_ids: &[QubitId]) -> QuantRS2Result<Vec<Box<dyn GateOp>>> {
        if qubit_ids.len() != 2 {
            return Err(QuantRS2Error::InvalidInput(
    ...
    fn single_qubit_to_gates(&self, decomp: &SingleQubitDecomposition, qubit: QubitId) -> Vec<Box<dyn GateOp>> {
    ...
    fn canonical_to_gates(&self, coeffs: &CartanCoefficients, q0: QubitId, q1: QubitId) -> QuantRS2Result<Vec<Box<dyn GateOp>>> {
        let mut gates: Vec<Box<dyn GateOp>> = Vec::new();
        ...
        match cnots {
            ...
            1 => {
                ...
                gates.push(Box::new(CNOT { control: q0, target: q1 }));
            }
            2 => {
                // Can be done with 2 CNOTs
                // Add rotations
                if coeffs.xx.abs() > self.tolerance {
                    gates.push(Box::new(RotationX { target: q0, theta: coeffs.xx * 2.0 }));
                }
                
                gates.push(Box::new(CNOT { control: q0, target: q1 }));
                
                if coeffs.zz.abs() > self.tolerance {
                    gates.push(Box::new(RotationZ { target: q1, theta: coeffs.zz * 2.0 }));
                }
            ...
            3 => {
                ...
                gates.push(Box::new(RotationZ { target: q0, theta: coeffs.xx * 2.0 }));
                gates.push(Box::new(RotationZ { target: q1, theta: coeffs.yy * 2.0 }));
                
                gates.push(Box::new(CNOT { control: q1, target: q0 }));
                
                gates.push(Box::new(RotationZ { target: q0, theta: coeffs.zz * 2.0 }));
...
impl OptimizedCartanDecomposer {
    ...
    fn check_special_cases(&self, unitary: &Array2<Complex<f64>>) -> QuantRS2Result<Option<CartanDecomposition>> {
        // Check for CNOT
    ...
    fn is_cz(&self, u: &Array2<Complex<f64>>) -> bool {
        let cz = Array2::from_shape_vec((4, 4), vec![
            Complex::new(1.0, 0.0), Complex::new(0.0, 0.0), Complex::new(0.0, 0.0), Complex::new(0.0, 0.0),
            Complex::new(0.0, 0.0), Complex::new(1.0, 0.0), Complex::new(0.0, 0.0), Complex::new(0.0, 0.0),
            Complex::new(0.0, 0.0), Complex::new(0.0, 0.0), Complex::new(1.0, 0.0), Complex::new(0.0, 0.0),
            Complex::new(0.0, 0.0), Complex::new(0.0, 0.0), Complex::new(0.0, 0.0), Complex::new(-1.0, 0.0),
    ...
    fn is_swap(&self, u: &Array2<Complex<f64>>) -> bool {
        let swap = Array2::from_shape_vec((4, 4), vec![
            Complex::new(1.0, 0.0), Complex::new(0.0, 0.0), Complex::new(0.0, 0.0), Complex::new(0.0, 0.0),
            Complex::new(0.0, 0.0), Complex::new(0.0, 0.0), Complex::new(1.0, 0.0), Complex::new(0.0, 0.0),
            Complex::new(0.0, 0.0), Complex::new(1.0, 0.0), Complex::new(0.0, 0.0), Complex::new(0.0, 0.0),
            Complex::new(0.0, 0.0), Complex::new(0.0, 0.0), Complex::new(0.0, 0.0), Complex::new(1.0, 0.0),
