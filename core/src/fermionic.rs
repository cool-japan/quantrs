...
impl FermionOperator {
    ...
    pub fn new(op_type: FermionOperatorType, mode: usize, coefficient: Complex64) -> Self {
        Self { op_type, mode, coefficient }
    }
    
    /// Create a creation operator
    pub fn creation(mode: usize) -> Self {
        Self::new(FermionOperatorType::Creation, mode, Complex64::new(1.0, 0.0))
    }
    
    /// Create an annihilation operator
    pub fn annihilation(mode: usize) -> Self {
        Self::new(FermionOperatorType::Annihilation, mode, Complex64::new(1.0, 0.0))
    }
    ...
    pub fn dagger(&self) -> Self {
        ...
        match self.op_type {
            FermionOperatorType::Creation => Self::new(FermionOperatorType::Annihilation, self.mode, conj_coeff),
            FermionOperatorType::Annihilation => Self::new(FermionOperatorType::Creation, self.mode, conj_coeff),
            FermionOperatorType::Number => Self::new(FermionOperatorType::Number, self.mode, conj_coeff),
            FermionOperatorType::Identity => Self::new(FermionOperatorType::Identity, self.mode, conj_coeff),
        }
    }
}
...
impl FermionTerm {
    ...
    fn should_swap(&self, idx: usize) -> bool {
        ...
        match (op1.op_type, op2.op_type) {
            ...
            (FermionOperatorType::Creation, FermionOperatorType::Creation) => {
                op1.mode > op2.mode
            }
...
impl FermionHamiltonian {
    ...
    pub fn add_chemical_potential(&mut self, i: usize, mu: f64) {
        let term = FermionTerm::new(
            vec![FermionOperator::number(i)],
            Complex64::new(mu, 0.0),
        );
        self.add_term(term);
    }
...
impl JordanWigner {
    ...
    pub fn transform_operator(&self, op: &FermionOperator) -> QuantRS2Result<Vec<QubitOperator>> {
        match op.op_type {
            ...
            FermionOperatorType::Annihilation => self.transform_annihilation(op.mode, op.coefficient),
            FermionOperatorType::Number => self.transform_number(op.mode, op.coefficient),
            FermionOperatorType::Identity => Ok(vec![QubitOperator::identity(self.n_modes, op.coefficient)]),
        }
    }
    
    /// Transform creation operator a†_j = (X_j - iY_j)/2 ⊗ Z_{<j}
    fn transform_creation(&self, mode: usize, coeff: Complex64) -> QuantRS2Result<Vec<QubitOperator>> {
        if mode >= self.n_modes {
            return Err(QuantRS2Error::InvalidInput(format!("Mode {} out of bounds", mode)));
        }
        ...
        let z_string: Vec<_> = (0..mode)
            .map(|i| (i, PauliOperator::Z))
            .collect();
    ...
    fn transform_annihilation(&self, mode: usize, coeff: Complex64) -> QuantRS2Result<Vec<QubitOperator>> {
        if mode >= self.n_modes {
            return Err(QuantRS2Error::InvalidInput(format!("Mode {} out of bounds", mode)));
        }
    ...
    fn transform_number(&self, mode: usize, coeff: Complex64) -> QuantRS2Result<Vec<QubitOperator>> {
        if mode >= self.n_modes {
            return Err(QuantRS2Error::InvalidInput(format!("Mode {} out of bounds", mode)));
        }
    ...
    pub fn transform_hamiltonian(&self, hamiltonian: &FermionHamiltonian) -> QuantRS2Result<QubitOperator> {
...
impl PauliOperator {
    ...
    pub fn matrix(&self) -> Array2<Complex64> {
        match self {
            PauliOperator::I => Array2::from_shape_vec((2, 2), vec![
                Complex64::new(1.0, 0.0), Complex64::new(0.0, 0.0),
                Complex64::new(0.0, 0.0), Complex64::new(1.0, 0.0),
            ]).unwrap(),
            PauliOperator::X => Array2::from_shape_vec((2, 2), vec![
                Complex64::new(0.0, 0.0), Complex64::new(1.0, 0.0),
                Complex64::new(1.0, 0.0), Complex64::new(0.0, 0.0),
            ]).unwrap(),
            PauliOperator::Y => Array2::from_shape_vec((2, 2), vec![
                Complex64::new(0.0, 0.0), Complex64::new(0.0, -1.0),
                Complex64::new(0.0, 1.0), Complex64::new(0.0, 0.0),
            ]).unwrap(),
            PauliOperator::Z => Array2::from_shape_vec((2, 2), vec![
                Complex64::new(1.0, 0.0), Complex64::new(0.0, 0.0),
                Complex64::new(0.0, 0.0), Complex64::new(-1.0, 0.0),
            ]).unwrap(),
            PauliOperator::Plus => Array2::from_shape_vec((2, 2), vec![
                Complex64::new(0.0, 0.0), Complex64::new(1.0, 0.0),
                Complex64::new(0.0, 0.0), Complex64::new(0.0, 0.0),
            ]).unwrap(),
            PauliOperator::Minus => Array2::from_shape_vec((2, 2), vec![
                Complex64::new(0.0, 0.0), Complex64::new(0.0, 0.0),
                Complex64::new(1.0, 0.0), Complex64::new(0.0, 0.0),
            ]).unwrap(),
        }
...
impl QubitOperator {
    ...
    pub fn add(&self, other: &Self) -> QuantRS2Result<Self> {
        if self.n_qubits != other.n_qubits {
            return Err(QuantRS2Error::InvalidInput("Operators must have same number of qubits".into()));
        }
    ...
    for term in &op.terms {
        ...
        for (qubit, pauli) in &term.operators {
            let gate: Box<dyn GateOp> = match pauli {
                ...
                PauliOperator::X => Box::new(single::PauliX { target: QubitId(*qubit as u32) }),
                PauliOperator::Y => Box::new(single::PauliY { target: QubitId(*qubit as u32) }),
                PauliOperator::Z => Box::new(single::PauliZ { target: QubitId(*qubit as u32) }),
...
impl BravyiKitaev {
    ...
    pub fn transform_operator(&self, op: &FermionOperator) -> QuantRS2Result<Vec<QubitOperator>> {
        ...
        Err(QuantRS2Error::UnsupportedOperation("Bravyi-Kitaev transformation not yet implemented".into()))
    }
}
