...
use crate::{
    ...
    variational::{VariationalCircuit, VariationalGate, DiffMode},
    gate::GateOp,
    qubit::QubitId,
    register::Register,
};
...
use scirs2_optimize::{
    unconstrained::{minimize, Method, Options, OptimizeResult},
};
...
impl VariationalQuantumOptimizer {
    ...
    pub fn optimize(
        ...
        match &self.method {
            ...
            OptimizationMethod::Powell => {
                self.optimize_with_scirs2(circuit, cost_fn)
            },
            _ => {
                self.optimize_custom(circuit, cost_fn)
            }
        }
    }
    ...
    fn optimize_with_scirs2(
        ...
        let initial_params: Vec<f64> = param_names.iter()
        ...
        let method = match &self.method {
            ...
            OptimizationMethod::LBFGS { memory_size: _ } => {
                Method::LBFGS
            },
    ...
    fn optimize_custom(
        ...
        for iter in 0..self.config.max_iterations {
            ...
            // Update history
            let current_params: Vec<f64> = param_names.iter()
                .map(|name| circuit.get_parameters().get(name).copied().unwrap_or(0.0))
                .collect();
            ...
            self.history.iteration_times.push(iter_start.elapsed().as_secs_f64() * 1000.0);
            self.history.total_iterations = iter + 1;
            
            // Callback
            if let Some(callback) = &self.config.callback {
                let params: Vec<f64> = param_names.iter()
                    .map(|name| circuit.get_parameters().get(name).copied().unwrap_or(0.0))
                    .collect();
    ...
    fn parameter_shift_gradient(
        ...
        let current_value = *current_params.get(param_name)
            .ok_or_else(|| QuantRS2Error::InvalidInput(format!("Parameter {} not found", param_name)))?;
        
        // Shift parameter by +π/2
        let mut circuit_plus = circuit.clone();
        let mut params_plus = current_params.clone();
        params_plus.insert(param_name.to_string(), current_value + std::f64::consts::PI / 2.0);
        ...
        params_minus.insert(param_name.to_string(), current_value - std::f64::consts::PI / 2.0);
    ...
    fn clip_gradients(&self, mut gradients: FxHashMap<String, f64>, max_norm: f64) -> FxHashMap<String, f64> {
    ...
    fn update_parameters(
        ...
        match &self.method {
            ...
            OptimizationMethod::Momentum { learning_rate, momentum } => {
                // Momentum-based gradient descent
                for (param_name, &grad) in gradients {
            ...
            OptimizationMethod::Adam { learning_rate, beta1, beta2, epsilon } => {
                // Adam optimizer
            ...
            OptimizationMethod::RMSprop { learning_rate, decay_rate, epsilon } => {
                // RMSprop optimizer
                for (param_name, &grad) in gradients {
            ...
            OptimizationMethod::NaturalGradient { learning_rate, regularization } => {
                // Natural gradient descent
                let fisher_inv = self.compute_fisher_inverse(circuit, gradients, *regularization)?;
            ...
            OptimizationMethod::SPSA { a, alpha, gamma, .. } => {
            ...
            OptimizationMethod::QNSPSA { learning_rate, regularization, .. } => {
                // Quantum Natural SPSA
                let fisher_inv = self.compute_fisher_inverse(circuit, gradients, *regularization)?;
                let natural_grad = self.apply_fisher_inverse(&fisher_inv, gradients);
            ...
            _ => {
                ...
                return Err(QuantRS2Error::InvalidInput("Invalid optimization method".to_string()));
            }
        }
    ...
    fn compute_fisher_inverse(
        ...
        if let Some(cache) = &self.fisher_cache {
            if let Some(cached_matrix) = cache.matrix.lock().unwrap().as_ref() {
                if let Some(cached_params) = cache.params.lock().unwrap().as_ref() {
                    ...
                    let diff_norm: f64 = current_params.iter()
        ...
        if n == 1 {
            ...
            if det.abs() < 1e-10 {
                return Err(QuantRS2Error::InvalidInput("Fisher matrix is singular".to_string()));
            }
    ...
    fn apply_fisher_inverse(
        ...
        let param_names: Vec<_> = gradients.keys().cloned().collect();
        let grad_vec: Vec<f64> = param_names.iter()
            .map(|name| gradients[name])
            .collect();
...
mod tests {
    use super::*;
    use crate::variational::{VariationalGate, DiffMode};
    ...
    fn test_gradient_descent_optimizer() {
        ...
        let cost_fn = |circuit: &VariationalCircuit| -> QuantRS2Result<f64> {
            let theta = circuit.get_parameters().get("theta").copied().unwrap_or(0.0);
            Ok((theta - 1.0).powi(2))
        };
    ...
    fn test_constrained_optimization() {
        ...
        constrained_opt.add_inequality_constraint(
            |params| 1.0 - params.get("x").copied().unwrap_or(0.0),
            0.0,
        );
