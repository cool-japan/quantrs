...
impl CliffordGate {
    ...
    pub fn matrix(&self) -> Array2<Complex64> {
        ...
        match self {
            CliffordGate::Hadamard => Array2::from_shape_vec(
                (2, 2),
                vec![h_val, h_val, h_val, mh_val],
            ).unwrap(),
            CliffordGate::Phase => Array2::from_shape_vec(
                (2, 2),
                vec![c1, c0, c0, ci],
            ).unwrap(),
            CliffordGate::PhaseDagger => Array2::from_shape_vec(
                (2, 2),
                vec![c1, c0, c0, cmi],
            ).unwrap(),
            CliffordGate::PauliX => Array2::from_shape_vec(
                (2, 2),
                vec![c0, c1, c1, c0],
            ).unwrap(),
            CliffordGate::PauliY => Array2::from_shape_vec(
                (2, 2),
                vec![c0, cmi, ci, c0],
            ).unwrap(),
            CliffordGate::PauliZ => Array2::from_shape_vec(
                (2, 2),
                vec![c1, c0, c0, cm1],
            ).unwrap(),
        }
...
impl CliffordTDecomposer {
    ...
    pub fn decompose(&mut self, unitary: &ArrayView2<Complex64>) -> QuantRS2Result<CliffordTSequence> {
    ...
    fn exact_synthesis(&mut self, unitary: &ArrayView2<Complex64>) -> QuantRS2Result<Option<CliffordTSequence>> {
    ...
    fn is_clifford(&self, unitary: &ArrayView2<Complex64>) -> QuantRS2Result<Option<CliffordTSequence>> {
        let clifford_gates = vec![
            (CliffordGate::Hadamard, CliffordGate::Hadamard.matrix()),
            (CliffordGate::Phase, CliffordGate::Phase.matrix()),
            (CliffordGate::PhaseDagger, CliffordGate::PhaseDagger.matrix()),
    ...
    fn try_clifford_t_clifford(&self, unitary: &ArrayView2<Complex64>) -> QuantRS2Result<Option<CliffordTSequence>> {
    ...
    fn approximate_synthesis(&self, unitary: &ArrayView2<Complex64>) -> QuantRS2Result<CliffordTSequence> {
        ...
        best_sequence.ok_or_else(|| {
            QuantRS2Error::ComputationError("No grid points available".to_string())
        })
    }
    
    /// Decompose with T-count optimization
    pub fn decompose_optimal(&mut self, unitary: &ArrayView2<Complex64>, max_t_count: Option<usize>) -> QuantRS2Result<CliffordTSequence> {
    ...
    fn optimize_t_count(&self, unitary: &ArrayView2<Complex64>, max_t_count: usize) -> QuantRS2Result<CliffordTSequence> {
        ...
        Err(QuantRS2Error::ComputationError(
            format!("Cannot achieve T-count <= {}", max_t_count),
        ))
    }
}
...
pub fn count_t_gates_in_sequence(gates: &[Box<dyn GateOp>]) -> usize {
    gates.iter().filter(|g| g.name() == "T" || g.name() == "T†").count()
}
...
mod tests {
    ...
    fn test_exact_clifford_detection() {
        ...
        assert_eq!(result.t_count, 0);
        assert_eq!(result.gates.len(), 1);
        assert!(matches!(result.gates[0], CliffordTGate::Clifford(CliffordGate::Phase)));
