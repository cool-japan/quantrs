...
use ndarray::{Array, Array2, Array3, Array4, ArrayD, IxDyn, Axis};
...
impl Tensor {
    ...
    pub fn new(id: usize, data: ArrayD<Complex64>, indices: Vec<String>) -> Self {
        ...
        Self { id, data, indices, shape }
    }
    
    /// Create a tensor from a 2D array (matrix)
    pub fn from_matrix(id: usize, matrix: Array2<Complex64>, in_idx: String, out_idx: String) -> Self {
    ...
    pub fn contract(&self, other: &Tensor, self_idx: &str, other_idx: &str) -> QuantRS2Result<Tensor> {
        ...
        let self_pos = self.indices.iter().position(|s| s == self_idx)
            .ok_or_else(|| QuantRS2Error::InvalidInput(format!("Index {} not found in tensor", self_idx)))?;
        let other_pos = other.indices.iter().position(|s| s == other_idx)
            .ok_or_else(|| QuantRS2Error::InvalidInput(format!("Index {} not found in tensor", other_idx)))?;
        
        // Check dimensions match
        if self.shape[self_pos] != other.shape[other_pos] {
            return Err(QuantRS2Error::InvalidInput(
                format!("Cannot contract indices with different dimensions: {} vs {}", 
    ...
    fn contract_indices(&self, other: &Tensor, self_idx: usize, other_idx: usize) -> QuantRS2Result<ArrayD<Complex64>> {
                ...
                for k in 0..other_left_dims {
                    for l in 0..other_right_dims {
                        ...
                        for c in 0..contract_dim {
                            let self_idx = i * contract_dim * self_right_dims + c * self_right_dims + j;
                            let other_idx = k * contract_dim * other_right_dims + c * other_right_dims + l;
    ...
    pub fn svd_decompose(&self, idx: usize, max_rank: Option<usize>) -> QuantRS2Result<(Tensor, Tensor)> {
        if idx >= self.rank() {
            return Err(QuantRS2Error::InvalidInput(
                format!("Index {} out of bounds for tensor with rank {}", idx, self.rank())
            ));
        }
        ...
        let (u, s, vt) = svd(&real_matrix.view(), false).map_err(|e| QuantRS2Error::ComputationError(format!("SVD failed: {:?}", e)))?;
...
impl TensorNetwork {
    ...
    pub fn connect(&mut self, tensor1: usize, index1: String, tensor2: usize, index2: String) -> QuantRS2Result<()> {
        // Verify tensors exist
        if !self.tensors.contains_key(&tensor1) {
            return Err(QuantRS2Error::InvalidInput(format!("Tensor {} not found", tensor1)));
        }
        if !self.tensors.contains_key(&tensor2) {
            return Err(QuantRS2Error::InvalidInput(format!("Tensor {} not found", tensor2)));
        }
        ...
        let idx1_pos = t1.indices.iter().position(|s| s == &index1)
            .ok_or_else(|| QuantRS2Error::InvalidInput(format!("Index {} not found in tensor {}", index1, tensor1)))?;
        let idx2_pos = t2.indices.iter().position(|s| s == &index2)
            .ok_or_else(|| QuantRS2Error::InvalidInput(format!("Index {} not found in tensor {}", index2, tensor2)))?;
        
        if t1.shape[idx1_pos] != t2.shape[idx2_pos] {
            return Err(QuantRS2Error::InvalidInput(
                format!("Connected indices must have same dimension: {} vs {}", 
    ...
    pub fn find_contraction_order(&self) -> Vec<(usize, usize)> {
        ...
        for edge in &self.edges {
            adjacency.entry(edge.tensor1).or_insert_with(Vec::new).push(edge.tensor2);
            adjacency.entry(edge.tensor2).or_insert_with(Vec::new).push(edge.tensor1);
        }
        ...
        while remaining_tensors.len() > 1 {
            ...
            if let Some((t1, t2)) = best_pair {
                ...
                if let Some(n1) = adjacency.get(&t1) {
                    virtual_neighbors.extend(n1.iter().filter(|&&n| n != t2 && remaining_tensors.contains(&n)));
                }
                if let Some(n2) = adjacency.get(&t2) {
                    virtual_neighbors.extend(n2.iter().filter(|&&n| n != t1 && remaining_tensors.contains(&n)));
                }
    ...
    pub fn contract_all(&mut self) -> QuantRS2Result<Tensor> {
        if self.tensors.is_empty() {
            return Err(QuantRS2Error::InvalidInput("Cannot contract empty tensor network".into()));
        }
...
impl TensorNetworkBuilder {
    ...
    pub fn apply_single_qubit_gate(&mut self, gate: &dyn GateOp, qubit: usize) -> QuantRS2Result<()> {
        ...
        if let Some(prev_tensor) = self.find_tensor_with_index(&in_idx) {
            self.network.connect(prev_tensor, in_idx.clone(), gate_id, in_idx)?;
        }
    ...
    pub fn apply_two_qubit_gate(&mut self, gate: &dyn GateOp, qubit1: usize, qubit2: usize) -> QuantRS2Result<()> {
        let matrix_vec = gate.matrix()?;
        let matrix = Array2::from_shape_vec((4, 4), matrix_vec)
            .map_err(|e| QuantRS2Error::InvalidInput(format!("Shape error: {}", e)))?;
        
        // Reshape to rank-4 tensor
        let tensor_data = matrix.into_shape((2, 2, 2, 2))
            .map_err(|e| QuantRS2Error::InvalidInput(format!("Shape error: {}", e)))?
            .into_dyn();
        ...
        let gate_tensor = Tensor::new(
            self.network.next_id,
            tensor_data,
            vec![in1_idx.clone(), in2_idx.clone(), out1_idx.clone(), out2_idx.clone()],
        );
        ...
        if let Some(prev1) = self.find_tensor_with_index(&in1_idx) {
            self.network.connect(prev1, in1_idx.clone(), gate_id, in1_idx)?;
        }
        if let Some(prev2) = self.find_tensor_with_index(&in2_idx) {
            self.network.connect(prev2, in2_idx.clone(), gate_id, in2_idx)?;
        }
...
impl TensorNetworkSimulator {
    ...
    pub fn simulate<const N: usize>(&self, gates: &[Box<dyn GateOp>]) -> QuantRS2Result<Register<N>> {
        ...
        for gate in gates {
            ...
            match qubits.len() {
                ...
                2 => builder.apply_two_qubit_gate(gate.as_ref(), qubits[0].0 as usize, qubits[1].0 as usize)?,
                _ => return Err(QuantRS2Error::UnsupportedOperation(
                    format!("Gates with {} qubits not supported in tensor network", qubits.len())
                )),
            }
        }
...
pub mod contraction_optimization {
    ...
    impl DynamicProgrammingOptimizer {
        ...
        fn find_optimal_order(&mut self, tensors: &[usize], network: &TensorNetwork) -> (usize, Vec<(usize, usize)>) {
            if tensors.len() <= 1 {
                return (0, vec![]);
            }
        ...
        fn are_connected(&self, t1: usize, t2: usize, network: &TensorNetwork) -> bool {
            network.edges.iter().any(|e| 
                (e.tensor1 == t1 && e.tensor2 == t2) || 
                (e.tensor1 == t2 && e.tensor2 == t1)
            )
        }
    }
}
...
mod tests {
    ...
    fn test_network_connection() {
        ...
        assert!(network.connect(id1, "bond".to_string(), id2, "bond".to_string()).is_err());
    }
}