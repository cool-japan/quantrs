...
impl FusionTree {
    ...
    pub fn get_fusion_outcome(&self) -> Option<AnyonType> {
        if self.external.len() == 2 && self.internal.is_empty() && !self.structure.is_empty() {
            ...
            Some(AnyonType::new(charge_id, match charge_id {
                0 => "1",
                1 => "σ", 
                2 => "ψ",
                _ => "τ",
...
impl TopologicalGate {
    ...
    pub fn cnot() -> Self {
        ...
        let braids = vec![
            BraidingOperation { anyon1: 0, anyon2: 1, over: true },
            BraidingOperation { anyon1: 2, anyon2: 3, over: true },
            BraidingOperation { anyon1: 1, anyon2: 2, over: false },
        ];
...
mod tests {
    ...
    fn test_fibonacci_model() {
        ...
        // Test fusion rules
        assert_eq!(model.fusion_multiplicity(
            AnyonType::VACUUM,
            AnyonType::new(1, "τ"),
            AnyonType::new(1, "τ")
        ), 1);
    ...
    fn test_braiding_operation() {
        ...
        assert!((initial_norm - 1.0).abs() < 1e-10, "Initial state not normalized: {}", initial_norm);
        ...
        assert!((norm - 1.0).abs() < 1e-10, "Final state not normalized: {}", norm);
    }
}