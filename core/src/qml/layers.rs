...
use super::{QMLLayer, EntanglementPattern, create_entangling_gates};
use crate::{
    error::{QuantRS2Error, QuantRS2Result},
    gate::GateOp,
    qubit::QubitId,
    parametric::{ParametricRotationX, ParametricRotationY, ParametricRotationZ, ParametricGate, SymbolicParameter},
};
    ...
    pub fn new(num_qubits: usize, axes: Vec<char>) -> QuantRS2Result<Self> {
        if axes.len() != num_qubits {
            return Err(QuantRS2Error::InvalidInput(
                format!("Expected {} axes, got {}", num_qubits, axes.len())
            ));
        }
        ...
        for &axis in &axes {
            if !['X', 'Y', 'Z'].contains(&axis) {
                return Err(QuantRS2Error::InvalidInput(
                    format!("Invalid rotation axis: {}", axis)
                ));
            }
        }
        ...
        if self.parameterized {
            ...
            pairs.iter()
                ...
                .map(|((ctrl, tgt), param)| {
                    ...
                    Box::new(CNOT { control: *ctrl, target: *tgt }) as Box<dyn GateOp>
                })
                .collect()
        } else {
...
impl StronglyEntanglingLayer {
    ...
    pub fn new(num_qubits: usize, pattern: EntanglementPattern) -> QuantRS2Result<Self> {
        ...
        let total_parameters = rotation_layers.iter()
            .map(|layer| layer.parameters().len())
            .sum::<usize>() + entangling_layer.parameters().len();
...
impl QMLLayer for StronglyEntanglingLayer {
    ...
    fn set_parameters(&mut self, values: &[f64]) -> QuantRS2Result<()> {
        if values.len() != self.total_parameters {
            return Err(QuantRS2Error::InvalidInput(
                format!("Expected {} parameters, got {}", self.total_parameters, values.len())
            ));
        }
    ...
    fn compute_gradients(
        ...
        if self.entangling_layer.parameterized {
            gradients.extend(self.entangling_layer.compute_gradients(state, loss_gradient)?);
        }
