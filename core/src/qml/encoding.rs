...
impl DataEncoder {
    ...
    pub fn encode(&self, data: &[f64]) -> QuantRS2Result<Vec<Box<dyn GateOp>>> {
        if data.len() != self.num_features {
            return Err(QuantRS2Error::InvalidInput(
                format!("Expected {} features, got {}", self.num_features, data.len())
            ));
        }
    ...
    fn amplitude_encoding(&self, data: &[f64]) -> QuantRS2Result<Vec<Box<dyn GateOp>>> {
        ...
        for i in 0..self.num_qubits {
            gates.push(Box::new(Hadamard { target: QubitId(i as u32) }));
        }
        ...
        for i in 0..self.num_qubits {
            for j in i + 1..self.num_qubits {
                if idx < data.len() {
                    ...
                    gates.push(Box::new(ParametricRotationZ::new(QubitId(i as u32), angle / 2.0)));
                    gates.push(Box::new(ParametricRotationZ::new(QubitId(j as u32), angle / 2.0)));
                    idx += 1;
                }
            }
        }
    ...
    fn basis_encoding(&self, data: &[f64]) -> QuantRS2Result<Vec<Box<dyn GateOp>>> {
        ...
        // Encode each bit
        for (i, &value) in data.iter().enumerate() {
            if value.abs() > 0.5 {  // Threshold at 0.5
                gates.push(Box::new(PauliX { target: QubitId(i as u32) }));
            }
        }
...
impl FeatureMap {
    ...
    fn pauli_feature_map(&self, features: &[f64]) -> QuantRS2Result<Vec<Box<dyn GateOp>>> {
        ...
        for (i, &feature) in features.iter().enumerate() {
            gates.push(Box::new(ParametricRotationZ::new(QubitId(i as u32), 2.0 * feature)));
        }
    ...
    fn zz_feature_map(&self, features: &[f64]) -> QuantRS2Result<Vec<Box<dyn GateOp>>> {
        ...
        for i in 0..self.num_qubits - 1 {
            ...
            gates.push(Box::new(ParametricRotationZ::new(QubitId((i + 1) as u32), angle)));
