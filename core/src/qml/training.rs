...
use super::{QMLCircuit, quantum_fisher_information, natural_gradient};
use crate::{
    error::{QuantRS2Error, QuantRS2Result},
    gpu::{GpuStateVector, GpuBackendFactory},
};
...
impl QMLTrainer {
    ...
    pub fn train(
        ...
        for epoch in 0..self.config.max_epochs {
            ...
            // Check convergence
            if epoch > 0 {
                let loss_change = (self.metrics.loss_history[epoch] - 
                                  self.metrics.loss_history[epoch - 1]).abs();
    ...
    fn train_epoch(
        ...
        for batch_idx in 0..num_batches {
            ...
            let grad_norm = clipped_gradients.iter()
                .map(|g| g * g)
                .sum::<f64>()
                .sqrt();
    ...
    fn update_parameters(&mut self, gradients: &[f64]) -> QuantRS2Result<()> {
        ...
        let new_params = match &mut self.optimizer {
            ...
            Optimizer::Adam { learning_rate, beta1, beta2, epsilon } => {
                if let Some(state) = &mut self.adam_state {
                    ...
                    for i in 0..current_params.len() {
                        ...
                        // Update parameters
                        new_params[i] = current_params[i] - *learning_rate * m_hat / (v_hat.sqrt() + *epsilon);
                    }
                    new_params
                } else {
                    current_params
            ...
            Optimizer::QuantumNatural { learning_rate, regularization } => {
