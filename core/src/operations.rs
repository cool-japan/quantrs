...
pub trait QuantumOperation: Debug + Send + Sync {
    ...
    fn apply_to_density_matrix(&self, rho: &ArrayView2<Complex64>) -> QuantRS2Result<Array2<Complex64>>;
...
impl ProjectiveMeasurement {
    ...
    fn project_onto_outcome(&self, state: &ArrayView1<Complex64>, outcome: usize) -> QuantRS2Result<(f64, Array1<Complex64>)> {
        let n_qubits = (state.len() as f64).log2() as usize;
...
impl ProjectiveMeasurement {
    ...
    fn create_projector(&self, outcome: usize, total_qubits: usize) -> QuantRS2Result<Array2<Complex64>> {
...
impl POVMMeasurement {
    ...
    pub fn get_probabilities(&self, state: &ArrayView1<Complex64>) -> QuantRS2Result<Vec<f64>> {
        ...
        for element in &self.elements {
            ...
            let prob = state.iter().zip(temp.iter())
