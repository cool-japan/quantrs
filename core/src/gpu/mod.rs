    ...
    fn apply_gate(
        ...
        match qubits.len() {
            1 => {
                ...
                let gate_array: [Complex64; 4] = [
                    matrix[0], matrix[1], matrix[2], matrix[3]
                ];
                self.kernel().apply_single_qubit_gate(
                    state,
                    &gate_array,
                    qubits[0],
                    n_qubits,
            ...
            _ => {
                ...
                self.kernel().apply_multi_qubit_gate(
                    state,
                    &matrix,
                    qubits,
                    n_qubits,
...
impl GpuStateVector {
    ...
    pub fn measure(&mut self, qubit: QubitId) -> QuantRS2Result<bool> {
        self.backend.measure(self.buffer.as_mut(), qubit, self.n_qubits)
    }
