    ...
    fn upload(&mut self, data: &[Complex64]) -> QuantRS2Result<()> {
        ...
        if buffer.len() != data.len() {
            return Err(QuantRS2Error::InvalidInput(
                format!("Buffer size mismatch: {} != {}", buffer.len(), data.len())
            ));
        }
...
impl GpuKernel for CpuKernel {
    fn apply_single_qubit_gate(
        ...
        let cpu_buffer = state.as_any_mut()
    ...
    fn apply_two_qubit_gate(
        ...
        for block in 0..num_blocks {
            ...
            for i in 0..(block_size / 4) {
                ...
                let base = block_start + (i & ((1 << low_idx) - 1)) + 
                          ((i >> low_idx) << (low_idx + 1)) +
    ...
    fn apply_multi_qubit_gate(
        ...
        if gate_matrix.dim() != (gate_dim, gate_dim) {
            return Err(QuantRS2Error::InvalidInput(
                format!("Gate matrix dimension mismatch: {:?} != ({}, {})", 
                    gate_matrix.dim(), gate_dim, gate_dim)
            ));
        }
