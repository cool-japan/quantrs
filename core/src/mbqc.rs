...
impl ClusterState {
    ...
    fn apply_hadamard(state: &Array1<Complex64>, qubit: usize, n: usize) -> QuantRS2Result<Array1<Complex64>> {
        let dim = 1 << n;
    ...
    fn apply_cz(state: &Array1<Complex64>, q1: usize, q2: usize, n: usize) -> QuantRS2Result<Array1<Complex64>> {
        let dim = 1 << n;
    ...
    pub fn measure(&mut self, qubit: usize, basis: MeasurementBasis) -> QuantRS2Result<bool> {
        ...
        let state = match basis {
            ...
            MeasurementBasis::X => Self::apply_hadamard(&self.state, qubit, self.graph.num_vertices)?,
            MeasurementBasis::Y => {
                // Apply S† then H
        ...
        // Project state
        let norm = if outcome { prob_1.sqrt() } else { prob_0.sqrt() };
    ...
    pub fn get_output_state(&self, output_qubits: &[usize]) -> QuantRS2Result<Array1<Complex64>> {
        ...
        // Normalize
        let norm = output_state.iter().map(|c: &Complex64| c.norm_sqr()).sum::<f64>().sqrt();
        if norm > 0.0 {
...
impl MBQCComputation {
    ...
    pub fn step(&mut self) -> QuantRS2Result<Option<(usize, bool)>> {
        ...
        if self.pattern.outputs.contains(&qubit) && 
           self.current_step == self.pattern.order.len() {
            return self.step();
        }
        
        // Get measurement basis
        let basis = self.pattern.measurements
            .get(&qubit)
            .copied()
        ...
        // Apply corrections
        self.cluster.apply_corrections(
            &self.pattern.x_corrections,
            &self.pattern.z_corrections,
        )?;
