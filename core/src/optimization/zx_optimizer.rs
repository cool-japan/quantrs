...
impl OptimizationPass for ZXOptimizationPass {
    fn optimize(&self, gates: Vec<Box<dyn GateOp>>) -> QuantRS2Result<Vec<Box<dyn GateOp>>> {
        ...
        if self.verbose {
            ...
            println!("ZX-Calculus: Processing circuit with {} gates, {} T-gates", 
                gates.len(), original_t);
        }
        ...
        if self.verbose {
            ...
            println!("ZX-Calculus: Optimized to {} gates, {} T-gates ({}% reduction)", 
                optimized.len(), 
                optimized_t,
                if original_t > 0 {
...
mod tests {
    ...
    fn test_zx_optimization_pass() {
        let gates: Vec<Box<dyn GateOp>> = vec![
            ...
            Box::new(RotationZ { target: QubitId(0), theta: std::f64::consts::PI / 4.0 }),
            Box::new(RotationZ { target: QubitId(0), theta: std::f64::consts::PI / 4.0 }),
            Box::new(Hadamard { target: QubitId(0) }),
        ];
