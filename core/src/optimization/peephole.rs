...
use super::{OptimizationPass, gates_can_commute};
...
impl PeepholeOptimizer {
    ...
    fn is_zero_rotation(&self, angle: f64) -> bool {
        ...
        normalized.abs() < self.zero_tolerance || (normalized - 2.0 * PI).abs() < self.zero_tolerance
    }
    
    /// Try to simplify a window of gates
    fn simplify_window(&self, window: &[Box<dyn GateOp>]) -> QuantRS2Result<Option<Vec<Box<dyn GateOp>>>> {
        match window.len() {
    ...
    fn simplify_pair(&self, gate1: &Box<dyn GateOp>, gate2: &Box<dyn GateOp>) -> QuantRS2Result<Option<Vec<Box<dyn GateOp>>>> {
        ...
        if gate1.qubits() == gate2.qubits() {
            match (gate1.name(), gate2.name()) {
                ...
                ("T", "T") => {
                    // T² = S
                    return Ok(Some(vec![Box::new(Phase { target: gate1.qubits()[0] })]));
                }
                ("T†", "T†") => {
                    // T†² = S†
                    return Ok(Some(vec![Box::new(PhaseDagger { target: gate1.qubits()[0] })]));
                }
    ...
    fn simplify_triple(&self, gate1: &Box<dyn GateOp>, gate2: &Box<dyn GateOp>, gate3: &Box<dyn GateOp>) -> QuantRS2Result<Option<Vec<Box<dyn GateOp>>>> {
        ...
        if gate1.name() == "H" && gate2.name() == "X" && gate3.name() == "H" {
            if gate1.qubits() == gate2.qubits() && gate2.qubits() == gate3.qubits() {
                return Ok(Some(vec![Box::new(PauliZ { target: gate1.qubits()[0] })]));
            }
        }
        ...
        if gate1.name() == "H" && gate2.name() == "Z" && gate3.name() == "H" {
            if gate1.qubits() == gate2.qubits() && gate2.qubits() == gate3.qubits() {
                return Ok(Some(vec![Box::new(PauliX { target: gate1.qubits()[0] })]));
            }
        }
    ...
    fn optimize(&self, gates: Vec<Box<dyn GateOp>>) -> QuantRS2Result<Vec<Box<dyn GateOp>>> {
        ...
        while changed && iterations < max_iterations {
            ...
            while i < current.len() {
                ...
                if i + 2 < current.len() {
                    if let Some(simplified) = self.simplify_triple(&current[i], &current[i+1], &current[i+2])? {
...
impl TCountOptimizer {
    ...
    fn reduce_t_count(&self, gates: &[Box<dyn GateOp>]) -> QuantRS2Result<Option<Vec<Box<dyn GateOp>>>> {
        // Pattern: T-S-T = S-T-S (both have T-count 2, but might enable other optimizations)
        if gates.len() >= 3 {
            for i in 0..gates.len() - 2 {
                if gates[i].name() == "T" && gates[i+1].name() == "S" && gates[i+2].name() == "T" {
                    if gates[i].qubits() == gates[i+1].qubits() && gates[i+1].qubits() == gates[i+2].qubits() {
...
mod tests {
    ...
    fn test_rotation_merging() {
        ...
        let gates: Vec<Box<dyn GateOp>> = vec![
            Box::new(RotationZ { target: qubit, theta: PI / 4.0 }),
            Box::new(RotationZ { target: qubit, theta: PI / 4.0 }),
        ];
    ...
    fn test_zero_rotation_removal() {
        ...
        let gates: Vec<Box<dyn GateOp>> = vec![
            Box::new(RotationX { target: qubit, theta: PI }),
            Box::new(RotationX { target: qubit, theta: PI }),
        ];
    ...
    fn test_cnot_rz_pattern() {
        ...
        let gates: Vec<Box<dyn GateOp>> = vec![
            ...
            Box::new(RotationZ { target: q1, theta: PI / 4.0 }),
            Box::new(CNOT { control: q0, target: q1 }),
        ];
