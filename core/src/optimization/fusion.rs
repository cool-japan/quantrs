...
use crate::gate::{single::*, multi::*, GateOp};
use crate::matrix_ops::{DenseMatrix, QuantumMatrix};
use crate::qubit::QubitId;
use crate::synthesis::{synthesize_unitary, identify_gate};
use ndarray::{Array2, Array1};
use num_complex::Complex64;
use std::collections::HashMap;

use super::{OptimizationPass, gates_are_disjoint};
...
impl GateFusion {
    ...
    fn fuse_single_qubit_gates(&self, gates: &[Box<dyn GateOp>]) -> QuantRS2Result<Option<Box<dyn GateOp>>> {
        if gates.is_empty() {
            return Ok(None);
        }
        ...
        if !gates.iter().all(|g| g.qubits().len() == 1 && g.qubits()[0] == target_qubit) {
            return Ok(None);
        }
        ...
        if let Some(gate_name) = identify_gate(&combined.view(), self.tolerance) {
            ...
            let identified_gate = match gate_name.as_str() {
                ...
                "X" => Some(Box::new(PauliX { target: target_qubit }) as Box<dyn GateOp>),
                "Y" => Some(Box::new(PauliY { target: target_qubit }) as Box<dyn GateOp>),
                "Z" => Some(Box::new(PauliZ { target: target_qubit }) as Box<dyn GateOp>),
                "H" => Some(Box::new(Hadamard { target: target_qubit }) as Box<dyn GateOp>),
                "S" => Some(Box::new(Phase { target: target_qubit }) as Box<dyn GateOp>),
                "S†" => Some(Box::new(PhaseDagger { target: target_qubit }) as Box<dyn GateOp>),
                "T" => Some(Box::new(T { target: target_qubit }) as Box<dyn GateOp>),
                "T†" => Some(Box::new(TDagger { target: target_qubit }) as Box<dyn GateOp>),
                _ => None,
            };
    ...
    fn fuse_cnot_gates(&self, gates: &[Box<dyn GateOp>]) -> QuantRS2Result<Option<Vec<Box<dyn GateOp>>>> {
        if gates.len() < 2 {
            return Ok(None);
        }
    ...
    fn fuse_rotation_gates(&self, gates: &[Box<dyn GateOp>]) -> QuantRS2Result<Option<Box<dyn GateOp>>> {
        if gates.len() < 2 {
            return Ok(None);
        }
...
impl CliffordFusion {
    ...
    fn fuse_clifford_pair(&self, gate1: &dyn GateOp, gate2: &dyn GateOp) -> QuantRS2Result<Option<Box<dyn GateOp>>> {
...
impl OptimizationPass for CliffordFusion {
    fn optimize(&self, gates: Vec<Box<dyn GateOp>>) -> QuantRS2Result<Vec<Box<dyn GateOp>>> {
        ...
        while i < gates.len() {
            if i + 1 < gates.len() {
                if let Some(fused) = self.fuse_clifford_pair(gates[i].as_ref(), gates[i + 1].as_ref())? {
...
mod tests {
    use super::*;
    use crate::gate::single::{Hadamard, Phase, PhaseDagger, T, TDagger};
    ...
    fn test_rotation_fusion() {
        ...
        let gates: Vec<Box<dyn GateOp>> = vec![
            Box::new(RotationZ { target: qubit, theta: 0.5 }),
            Box::new(RotationZ { target: qubit, theta: 0.3 }),
            Box::new(RotationZ { target: qubit, theta: 0.2 }),
        ];
    ...
    fn test_cnot_cancellation() {
        ...
        let gates: Vec<Box<dyn GateOp>> = vec![
            Box::new(CNOT { control: q0, target: q1 }),
            Box::new(CNOT { control: q0, target: q1 }),
        ];
    ...
    fn test_cnot_to_swap() {
        ...
        let gates: Vec<Box<dyn GateOp>> = vec![
            Box::new(CNOT { control: q0, target: q1 }),
            Box::new(CNOT { control: q1, target: q0 }),
        ];
    ...
    fn test_full_optimization() {
        ...
        let gates: Vec<Box<dyn GateOp>> = vec![
            ...
            Box::new(CNOT { control: q0, target: q1 }),
            Box::new(CNOT { control: q0, target: q1 }),
            Box::new(RotationZ { target: q1, theta: 0.5 }),
            Box::new(RotationZ { target: q1, theta: 0.5 }),
        ];
