...
use crate::gate::{single::*, multi::CNOT, GateOp};
...
pub fn decompose_single_qubit_zyz(unitary: &ArrayView2<Complex64>) -> QuantRS2Result<SingleQubitDecomposition> {
    ...
    let (theta1, theta2) = if phi.abs() < 1e-10 {
        ...
        let phase = if a.norm() > 0.5 { a.arg() * 2.0 } else { d.arg() * 2.0 };
        (0.0, phase)
...
pub fn decompose_single_qubit_xyx(unitary: &ArrayView2<Complex64>) -> QuantRS2Result<SingleQubitDecomposition> {
...
pub fn synthesize_unitary(
    ...
    if qubits.len() != num_qubits {
        return Err(QuantRS2Error::InvalidInput(
            format!("Need {} qubits, got {}", num_qubits, qubits.len()),
        ));
    }
    ...
    match num_qubits {
        ...
        _ => {
            ...
            Err(QuantRS2Error::UnsupportedOperation(
                format!("Synthesis for {}-qubit gates not yet implemented", num_qubits),
            ))
        }
    }
}
    ...
    match n {
        2 => {
            ...
            let gates = vec![
                ...
                ("H", Array2::from_shape_vec((2, 2), vec![
...
mod tests {
    ...
    fn test_single_qubit_decomposition() {
        ...
        assert!(matrices_approx_equal(&h.view(), &reconstructed.view(), 1e-10));
    }
