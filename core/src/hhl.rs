...
impl HHLAlgorithm {
    ...
    pub fn new(
        ...
        if n != 1 << params.n_qubits {
            return Err(format!(
                ...
                n, params.n_qubits, 1 << params.n_qubits
            ));
        }
    ...
    pub fn prepare_input_state(&self, state: &mut Vec<Complex64>) {
        ...
        // Normalize vector b
        let norm: f64 = self.vector_b.iter()
            .map(|c| c.norm_sqr())
            .sum::<f64>()
            .sqrt();
    ...
    pub fn apply_phase_estimation(&self, state: &mut Vec<Complex64>) {
    ...
    pub fn apply_eigenvalue_inversion(&self, state: &mut Vec<Complex64>) {
        let clock_size = 1 << self.params.clock_qubits;
        let n = 1 << self.params.n_qubits;
        
        // For each eigenvalue encoded in the clock register,
        // apply rotation on ancilla qubit proportional to 1/eigenvalue
        for clock_idx in 1..clock_size { // Skip 0 to avoid division by zero
            let eigenvalue = (clock_idx as f64) / (clock_size as f64) * self.params.eigenvalue_scale;
            ...
            for input_idx in 0..n {
                let idx0 = 0 + 2 * (input_idx + n * clock_idx); // ancilla = 0
    ...
    pub fn apply_inverse_phase_estimation(&self, state: &mut Vec<Complex64>) {
    ...
    pub fn postselect_ancilla(&self, state: &mut Vec<Complex64>) -> f64 {
        ...
        for i in 0..total_size / 2 {
            let idx1 = 2 * i + 1; // ancilla = 1
            new_state[i] = state[idx1];
...
mod tests {
    ...
    fn test_hermitian_check() {
        ...
        let nh = Array2::from_shape_vec((2, 2), vec![
