...
use crate::{
    ...
    qubit::QubitId,
    eigensolve::{eigen_decompose_unitary, EigenDecomposition as EigenDecomp},
};
...
impl GateCharacterizer {
    ...
    fn decompose_single_qubit(
        ...
        Ok(vec![
            Box::new(RotationZ { target: QubitId(0), theta: alpha }),
            Box::new(RotationY { target: QubitId(0), theta: beta }),
            Box::new(RotationZ { target: QubitId(0), theta: gamma }),
        ])
    }
    ...
    pub fn global_phase(&self, gate: &dyn GateOp) -> QuantRS2Result<f64> {
        ...
        let det = eigen.eigenvalues.iter().fold(Complex::new(1.0, 0.0), |acc, &lambda| acc * lambda);
...
mod tests {
    ...
    fn test_pauli_identification() {
        ...
        assert_eq!(
            characterizer.identify_gate_type(&PauliX { target: QubitId(0) }).unwrap(),
            GateType::PauliX
        );
        assert_eq!(
            characterizer.identify_gate_type(&PauliY { target: QubitId(0) }).unwrap(),
            GateType::PauliY
        );
        assert_eq!(
            characterizer.identify_gate_type(&PauliZ { target: QubitId(0) }).unwrap(),
            GateType::PauliZ
        );
    }
    ...
    fn test_eigenphases() {
        let characterizer = GateCharacterizer::new(1e-10);
        let rz = RotationZ { target: QubitId(0), theta: PI / 2.0 };
    ...
    fn test_closest_clifford() {
        ...
        let t_like = RotationZ { target: QubitId(0), theta: PI / 4.0 };
        let closest = characterizer.find_closest_clifford(&t_like).unwrap();
        
        // Should find S gate (Phase) as closest
        let s_distance = characterizer.gate_distance(&t_like, &Phase { target: QubitId(0) }).unwrap();
        let actual_distance = characterizer.gate_distance(&t_like, closest.as_ref()).unwrap();
    ...
    fn test_identity_check() {
        ...
        let identity_gate = RotationZ { target: QubitId(0), theta: 0.0 };
    ...
    fn test_global_phase() {
        ...
        let z_phase = characterizer.global_phase(&PauliZ { target: QubitId(0) }).unwrap();
