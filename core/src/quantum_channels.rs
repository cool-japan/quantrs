...
use crate::{
    ...
    matrix_ops::{DenseMatrix, QuantumMatrix, tensor_product_many, partial_trace},
    qubit::QubitId,
};
...
impl QuantumChannel {
    ...
    pub fn from_kraus(operators: Vec<Array2<Complex<f64>>>) -> QuantRS2Result<Self> {
        ...
        for (i, op) in operators.iter().enumerate() {
            if op.shape() != shape {
                return Err(QuantRS2Error::InvalidInput(
                    format!("Kraus operator {} has inconsistent dimensions", i)
                ));
            }
        }
    ...
    fn kraus_to_choi(&self, operators: &[Array2<Complex<f64>>]) -> QuantRS2Result<ChoiRepresentation> {
    ...
    fn kraus_to_stinespring(&self, operators: &[Array2<Complex<f64>>]) -> QuantRS2Result<StinespringRepresentation> {
    ...
    fn stinespring_to_kraus(&self, isometry: &Array2<Complex<f64>>, env_dim: usize) -> QuantRS2Result<KrausRepresentation> {
        let d_out = self.output_dim;
        let mut operators = Vec::new();
...
impl QuantumChannels {
    ...
    pub fn depolarizing(p: f64) -> QuantRS2Result<QuantumChannel> {
        ...
        let operators = vec![
            ...
            Array2::from_shape_vec((2, 2), vec![
                Complex::new(sqrt_1_minus_3p_4, 0.0), Complex::new(0.0, 0.0),
                Complex::new(0.0, 0.0), Complex::new(sqrt_1_minus_3p_4, 0.0),
            ]).unwrap(),
            // √(p/4) * X
            Array2::from_shape_vec((2, 2), vec![
                Complex::new(0.0, 0.0), Complex::new(sqrt_p_4, 0.0),
                Complex::new(sqrt_p_4, 0.0), Complex::new(0.0, 0.0),
            ]).unwrap(),
            // √(p/4) * Y
            Array2::from_shape_vec((2, 2), vec![
                Complex::new(0.0, 0.0), Complex::new(0.0, -sqrt_p_4),
                Complex::new(0.0, sqrt_p_4), Complex::new(0.0, 0.0),
            ]).unwrap(),
            // √(p/4) * Z
            Array2::from_shape_vec((2, 2), vec![
                Complex::new(sqrt_p_4, 0.0), Complex::new(0.0, 0.0),
    ...
    pub fn amplitude_damping(gamma: f64) -> QuantRS2Result<QuantumChannel> {
        ...
        let operators = vec![
            ...
            Array2::from_shape_vec((2, 2), vec![
                ...
                Complex::new(0.0, 0.0), Complex::new(sqrt_1_minus_gamma, 0.0),
            ]).unwrap(),
            // K₁ = √γ|0⟩⟨1|
            Array2::from_shape_vec((2, 2), vec![
                Complex::new(0.0, 0.0), Complex::new(sqrt_gamma, 0.0),
    ...
    pub fn phase_damping(gamma: f64) -> QuantRS2Result<QuantumChannel> {
        ...
        let operators = vec![
            // K₀ = √(1-γ) * I
            Array2::from_shape_vec((2, 2), vec![
                Complex::new(sqrt_1_minus_gamma, 0.0), Complex::new(0.0, 0.0),
                Complex::new(0.0, 0.0), Complex::new(sqrt_1_minus_gamma, 0.0),
            ]).unwrap(),
            // K₁ = √γ * Z
            Array2::from_shape_vec((2, 2), vec![
                Complex::new(sqrt_gamma, 0.0), Complex::new(0.0, 0.0),
                Complex::new(0.0, 0.0), Complex::new(-sqrt_gamma, 0.0),
            ]).unwrap(),
        ];
    ...
    pub fn bit_flip(p: f64) -> QuantRS2Result<QuantumChannel> {
        ...
        let operators = vec![
            // K₀ = √(1-p) * I
            Array2::from_shape_vec((2, 2), vec![
                Complex::new(sqrt_1_minus_p, 0.0), Complex::new(0.0, 0.0),
                Complex::new(0.0, 0.0), Complex::new(sqrt_1_minus_p, 0.0),
            ]).unwrap(),
            // K₁ = √p * X
            Array2::from_shape_vec((2, 2), vec![
                Complex::new(0.0, 0.0), Complex::new(sqrt_p, 0.0),
                Complex::new(sqrt_p, 0.0), Complex::new(0.0, 0.0),
            ]).unwrap(),
        ];
...
mod tests {
    ...
    fn test_unitary_channel() {
        ...
        let h = Array2::from_shape_vec((2, 2), vec![
            ...
            Complex::new(1.0, 0.0), Complex::new(-1.0, 0.0),
        ]).unwrap() / Complex::new(2.0_f64.sqrt(), 0.0);
    ...
    fn test_stinespring_conversion() {
        ...
        let kraus_decomposer = QuantumChannel::from_kraus(vec![Array2::eye(2).mapv(|x| Complex::new(x, 0.0))]).unwrap();
        let kraus = kraus_decomposer.stinespring_to_kraus(&stinespring.isometry, stinespring.env_dim).unwrap();
