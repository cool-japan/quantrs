...
use crate::{
    ...
    qubit::QubitId,
    zx_calculus::{ZXDiagram, Spider, SpiderType, EdgeType, CircuitToZX, ZXOptimizer},
};
...
impl ZXExtractor {
    ...
    fn topological_sort(&self, inputs: &[usize], outputs: &[usize]) -> QuantRS2Result<Vec<usize>> {
        ...
        for (&spider_id, _) in &self.diagram.spiders {
    ...
    fn is_forward_edge(&self, source: usize, target: usize, inputs: &[usize], outputs: &[usize]) -> bool {
                        ...
                        if let Some(neighbor) = self.diagram.spiders.get(&neighbor_id) {
                                ...
                                && edge_type == EdgeType::Regular 
                            {
                                ...
                                if let (Some(control_qubit), Some(target_qubit)) = 
                                    (self.get_spider_qubit(spider_id), self.get_spider_qubit(neighbor_id)) 
                                {
                                    ...
                                    let layer = self.spider_positions.get(&spider_id)
                                        .map(|(l, _)| *l)
                                        .unwrap_or(0);
    ...
    fn get_edge_type(&self, spider1: usize, spider2: usize) -> Option<EdgeType> {
        self.diagram.neighbors(spider1)
            .iter()
...
impl ZXPipeline {
    ...
    pub fn compare_t_count(&self, original: &[Box<dyn GateOp>], optimized: &[Box<dyn GateOp>]) -> (usize, usize) {
        let count_t = |gates: &[Box<dyn GateOp>]| {
            gates.iter()
                .filter(|g| g.name() == "T" || (g.name() == "RZ" && {
...
mod tests {
    ...
    fn test_t_count_reduction() {
        ...
        let gates: Vec<Box<dyn GateOp>> = vec![
            Box::new(RotationZ { target: QubitId(0), theta: PI / 4.0 }), // T gate
            Box::new(RotationZ { target: QubitId(0), theta: PI / 4.0 }), // T gate
        ];
