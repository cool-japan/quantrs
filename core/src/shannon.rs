...
use crate::{
    ...
    controlled::{ControlledGate, make_controlled},
...
impl ShannonDecomposer {
    ...
    pub fn decompose(&mut self, unitary: &Array2<Complex<f64>>, qubit_ids: &[QubitId]) -> QuantRS2Result<ShannonDecomposition> {
        let n = qubit_ids.len();
        let size = 1 << n;
        ...
        if unitary.shape() != [size, size] {
            return Err(QuantRS2Error::InvalidInput(
                format!("Unitary size {} doesn't match {} qubits", unitary.shape()[0], n)
            ));
        }
    ...
    ) -> QuantRS2Result<(Array2<Complex<f64>>, Array2<Complex<f64>>, Array2<Complex<f64>>)> {
        let size = a.shape()[0];
...
impl OptimizedShannonDecomposer {
    ...
    fn apply_peephole_optimization(&self, mut decomp: ShannonDecomposition) -> QuantRS2Result<ShannonDecomposition> {
        ...
        while i < decomp.gates.len() {
            if i + 1 < decomp.gates.len() {
                ...
                if self.gates_cancel(&decomp.gates[i], &decomp.gates[i + 1]) {
                    // Skip both gates
                    i += 2;
                    decomp.cnot_count = decomp.cnot_count.saturating_sub(
                        if decomp.gates[i - 2].name() == "CNOT" { 2 } else { 0 }
                    );
                ...
                if let Some(merged) = self.try_merge_rotations(&decomp.gates[i], &decomp.gates[i + 1]) {
    ...
    fn apply_commutation_optimization(&self, mut decomp: ShannonDecomposition) -> QuantRS2Result<ShannonDecomposition> {
    ...
    fn try_merge_rotations(&self, gate1: &Box<dyn GateOp>, gate2: &Box<dyn GateOp>) -> Option<Box<dyn GateOp>> {
...
pub fn shannon_decompose(unitary: &Array2<Complex<f64>>, qubit_ids: &[QubitId]) -> QuantRS2Result<Vec<Box<dyn GateOp>>> {
