...
impl PyQuantumComplexityAnalyzer {
    ...
    fn analyze_circuit(&mut self, gates: Vec<(String, Vec<u32>, Option<Vec<f64>>)>, 
                      algorithm_type: String, input_size: usize) -> PyResult<()> {
        let mut gate_count = HashMap::new();
        let mut qubit_usage = std::collections::HashSet::new();
        let mut max_parallel_depth = 0;
        ...
        let (time_complexity, space_complexity) = Self::analyze_complexity_class(&algorithm_type, input_size);
        ...
        let entanglement_entropy = Some(Self::estimate_entanglement_entropy(qubit_count, &gate_count));
    ...
    fn get_analysis_report(&self) -> String {
        ...
        let mut report = format!("# Quantum Algorithm Complexity Analysis Report\n");
        report.push_str(&format!("**Algorithm:** {}\n\n", self.algorithm_name));
        
        for (i, result) in self.analysis_results.iter().enumerate() {
            report.push_str(&format!("## Analysis {} - {} Algorithm\n", i + 1, result.algorithm_type));
            ...
            report.push_str(&format!("- **Space Complexity:** {}\n", result.space_complexity));
            report.push_str(&format!("- **Classical Complexity:** {}\n", result.classical_complexity));
            
            if let Some(advantage) = result.quantum_advantage {
                report.push_str(&format!("- **Quantum Advantage:** {:.2e}x speedup\n", advantage));
            }
            ...
            if let Some(entropy) = result.entanglement_entropy {
                report.push_str(&format!("- **Entanglement Entropy:** {:.3} bits\n", entropy));
            }
        ...
        report.push_str(&format!("- **Single-Qubit Gates:** {}\n", self.circuit_metrics.single_qubit_gates));
        report.push_str(&format!("- **Two-Qubit Gates:** {}\n", self.circuit_metrics.two_qubit_gates));
        report.push_str(&format!("- **Parallel Depth:** {}\n", self.circuit_metrics.parallel_depth));
        report.push_str(&format!("- **Gate Density:** {:.3}\n", self.circuit_metrics.gate_density));
        
        report
    }

    /// Get complexity scaling predictions
    fn predict_scaling(&self, target_input_sizes: Vec<usize>) -> HashMap<String, Vec<(usize, f64)>> {
        ...
        if let Some(latest_result) = self.analysis_results.last() {
            ...
            for &target_size in &target_input_sizes {
                ...
                let (gate_scaling, depth_scaling, qubit_scaling) = match latest_result.algorithm_type.as_str() {
                    "Shor" => (scaling_factor.powf(3.0), scaling_factor.powf(2.0), scaling_factor.log2()),
                    "Grover" => (scaling_factor.sqrt(), scaling_factor.sqrt(), scaling_factor.log2()),
                ...
                gate_predictions.push((target_size, self.circuit_metrics.total_gates as f64 * gate_scaling));
    ...
    fn analyze_error_correction_overhead(&self, target_logical_error_rate: f64) -> HashMap<String, f64> {
        ...
        if let Some(latest_result) = self.analysis_results.last() {
            ...
            if physical_error_rate < threshold {
                let code_distance = (target_logical_error_rate.log10() / physical_error_rate.log10()).ceil() as f64;
                ...
                overhead.insert("physical_qubits_per_logical".to_string(), physical_qubits_per_logical);
                overhead.insert("total_physical_qubits".to_string(), 
                ...
                overhead.insert("logical_gate_time_overhead".to_string(), 
    ...
    fn quantum_advantage_analysis(&self) -> HashMap<String, String> {
        ...
        if let Some(latest_result) = self.analysis_results.last() {
            ...
            analysis.insert("computational_advantage".to_string(), 
                          Self::analyze_computational_advantage(&latest_result.algorithm_type, latest_result.input_size));
            ...
            analysis.insert("sampling_advantage".to_string(),
                          Self::analyze_sampling_advantage(&latest_result.algorithm_type));
            
            analysis.insert("optimization_advantage".to_string(),
    ...
    fn __repr__(&self) -> String {
        format!(
            ...
            self.algorithm_name, self.analysis_results.len()
        )
    }
}
...
impl PyQuantumComplexityAnalyzer {
    ...
    fn estimate_quantum_advantage(algorithm_type: &str, input_size: usize) -> Option<f64> {
        match algorithm_type {
            "Shor" => {
                ...
                let classical_complexity = 2.0_f64.powf((input_size as f64).powf(1.0/3.0) * (input_size as f64).log2().powf(2.0/3.0));
    ...
    fn analyze_complexity_class(algorithm_type: &str, input_size: usize) -> (String, String) {
        match algorithm_type {
    ...
    fn estimate_circuit_fidelity(gate_count: &HashMap<String, usize>, circuit_depth: usize) -> Option<f64> {
        ...
        let total_two_qubit = *gate_count.get("CNOT").unwrap_or(&0) + 
                              *gate_count.get("CZ").unwrap_or(&0);
    ...
    fn estimate_entanglement_entropy(qubit_count: usize, gate_count: &HashMap<String, usize>) -> f64 {
        // Simplified entanglement entropy estimation
        let entangling_gates = *gate_count.get("CNOT").unwrap_or(&0) + 
                              *gate_count.get("CZ").unwrap_or(&0);
    ...
    fn get_classical_complexity(algorithm_type: &str, input_size: usize) -> String {
        match algorithm_type {
            "Shor" => format!("O(exp(n^(1/3) * log²n)) - Sub-exponential"),
            "Grover" => format!("O(N) - Linear search"),
            "VQE" => format!("O(exp(n)) - Exponential for general Hamiltonians"),
            "QAOA" => format!("O(exp(n)) - Exponential for general optimization"),
            "HHL" => format!("O(Ns κ log κ / ε) - Classical iterative methods"),
            _ => format!("Problem-dependent"),
        }
    }
    
    fn analyze_computational_advantage(algorithm_type: &str, input_size: usize) -> String {
        match algorithm_type {
    ...
    fn update_circuit_metrics(&mut self, gates: &[(String, Vec<u32>, Option<Vec<f64>>)]) {
        ...
        self.circuit_metrics.gate_density = gates.len() as f64 / 
            (self.circuit_metrics.single_qubit_gates + self.circuit_metrics.two_qubit_gates).max(1) as f64;
    }
}

/// Module-level functions for complexity analysis
#[pyfunction]
pub fn analyze_algorithm_complexity(algorithm_type: String, input_size: usize, 
                               gates: Vec<(String, Vec<u32>, Option<Vec<f64>>)>) -> PyResult<String> {
...
#[pyfunction]
pub fn compare_quantum_classical_complexity(algorithm_type: String, input_sizes: Vec<usize>) -> HashMap<String, Vec<(usize, String)>> {
    ...
    for size in input_sizes {
        let (quantum_time, _) = PyQuantumComplexityAnalyzer::analyze_complexity_class(&algorithm_type, size);
        let classical_time = PyQuantumComplexityAnalyzer::get_classical_complexity(&algorithm_type, size);
