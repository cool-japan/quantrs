...
// use scirs2_linalg::matrix_functions::expm; // Not available for complex
use std::fmt;
...
impl BosonOperator {
    ...
    pub fn new(op_type: BosonOperatorType, mode: usize, coefficient: Complex64, truncation: usize) -> Self {
        Self { op_type, mode, coefficient, truncation }
    }
    
    /// Create a creation operator
    pub fn creation(mode: usize, truncation: usize) -> Self {
        Self::new(BosonOperatorType::Creation, mode, Complex64::new(1.0, 0.0), truncation)
    }
    
    /// Create an annihilation operator
    pub fn annihilation(mode: usize, truncation: usize) -> Self {
        Self::new(BosonOperatorType::Annihilation, mode, Complex64::new(1.0, 0.0), truncation)
    }
    
    /// Create a number operator
    pub fn number(mode: usize, truncation: usize) -> Self {
        Self::new(BosonOperatorType::Number, mode, Complex64::new(1.0, 0.0), truncation)
    }
    
    /// Create a position operator
    pub fn position(mode: usize, truncation: usize) -> Self {
        Self::new(BosonOperatorType::Position, mode, Complex64::new(1.0, 0.0), truncation)
    }
    
    /// Create a momentum operator
    pub fn momentum(mode: usize, truncation: usize) -> Self {
        Self::new(BosonOperatorType::Momentum, mode, Complex64::new(1.0, 0.0), truncation)
    }
    ...
    pub fn dagger(&self) -> Self {
        ...
        match self.op_type {
            BosonOperatorType::Creation => Self::new(BosonOperatorType::Annihilation, self.mode, conj_coeff, self.truncation),
            BosonOperatorType::Annihilation => Self::new(BosonOperatorType::Creation, self.mode, conj_coeff, self.truncation),
            BosonOperatorType::Number => Self::new(BosonOperatorType::Number, self.mode, conj_coeff, self.truncation),
            BosonOperatorType::Position => Self::new(BosonOperatorType::Position, self.mode, conj_coeff, self.truncation),
            BosonOperatorType::Momentum => Self::new(BosonOperatorType::Momentum, self.mode, -conj_coeff, self.truncation), // p† = -p
            BosonOperatorType::Identity => Self::new(BosonOperatorType::Identity, self.mode, conj_coeff, self.truncation),
            BosonOperatorType::Displacement => Self::new(BosonOperatorType::Displacement, self.mode, conj_coeff, self.truncation),
            BosonOperatorType::Squeeze => Self::new(BosonOperatorType::Squeeze, self.mode, conj_coeff, self.truncation),
        }
    }
}
...
impl BosonTerm {
    ...
    pub fn to_matrix(&self, n_modes: usize) -> QuantRS2Result<Array2<Complex64>> {
        if self.operators.is_empty() {
            ...
            let total_dim = self.operators.first()
        ...
        for mode in 0..n_modes {
            ...
            let mode_op = self.operators.iter()
        ...
        let mut final_result = result.ok_or_else(|| QuantRS2Error::InvalidInput("No operators in term".into()))?;
...
impl GaussianState {
    ...
    pub fn apply_symplectic(&mut self, s: &Array2<f64>) -> QuantRS2Result<()> {
        if s.shape() != [2 * self.n_modes, 2 * self.n_modes] {
            return Err(QuantRS2Error::InvalidInput("Symplectic matrix has wrong dimensions".into()));
        }
        
        // Transform displacement: d' = S d
        let d = Array2::from_shape_vec((2 * self.n_modes, 1), self.displacement.clone())
            .map_err(|e| QuantRS2Error::LinalgError(format!("Failed to create displacement vector: {}", e)))?;
        let d_prime = s.dot(&d);
...
pub fn boson_to_qubit_encoding(op: &BosonOperator, encoding: &str) -> QuantRS2Result<Vec<Box<dyn GateOp>>> {
    match encoding {
        "binary" => {
            ...
            Err(QuantRS2Error::UnsupportedOperation("Binary encoding not yet implemented".into()))
        }
        "unary" => {
            // Unary encoding: |n⟩ → |0...010...0⟩ (n-th position is 1)
            // Number of qubits = truncation
            Err(QuantRS2Error::UnsupportedOperation("Unary encoding not yet implemented".into()))
        }
        "gray" => {
            // Gray code encoding
            Err(QuantRS2Error::UnsupportedOperation("Gray code encoding not yet implemented".into()))
        }
        _ => Err(QuantRS2Error::InvalidInput(format!("Unknown encoding: {}", encoding))),
    }
}
...
fn matrix_exponential_complex(a: &Array2<Complex64>) -> QuantRS2Result<Array2<Complex64>> {
    ...
    let c = vec![1.0, 1.0/2.0, 1.0/6.0, 1.0/24.0, 1.0/120.0, 1.0/720.0, 1.0/5040.0];
    
    u = &u * c[0] + &a_scaled * c[1] + &a2 * c[2] + &a_scaled.dot(&a2) * c[3] 
        + &a4 * c[4] + &a_scaled.dot(&a4) * c[5] + &a6 * c[6];
    
    v = &v * c[0] - &a_scaled * c[1] + &a2 * c[2] - &a_scaled.dot(&a2) * c[3] 
        + &a4 * c[4] - &a_scaled.dot(&a4) * c[5] + &a6 * c[6];
...
fn solve_complex(a: &Array2<Complex64>, b: &Array2<Complex64>) -> QuantRS2Result<Array2<Complex64>> {
    let n = a.shape()[0];
    if a.shape()[1] != n || b.shape()[0] != n {
        return Err(QuantRS2Error::InvalidInput("Invalid dimensions for solve".into()));
    }
...
impl SparseBosonOperator {
    ...
    pub fn from_triplets(rows: Vec<usize>, cols: Vec<usize>, data: Vec<Complex64>, shape: (usize, usize)) -> Self {
        Self { rows, cols, data, shape }
    }
...
mod tests {
    ...
    fn test_gaussian_state() {
        ...
        assert_relative_eq!(coherent.displacement[0], alpha.re * 2.0_f64.sqrt(), epsilon = 1e-12);
        assert_relative_eq!(coherent.displacement[1], alpha.im * 2.0_f64.sqrt(), epsilon = 1e-12);
