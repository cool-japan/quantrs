...
impl<const N: usize> Register<N> {
    ...
    pub fn with_amplitudes(amplitudes: Vec<Complex64>) -> QuantrsResult<Self> {
        ...
        if amplitudes.len() != expected_dim {
            return Err(QuantrsError::CircuitValidationFailed(
                format!("Amplitudes vector has incorrect dimension. Expected {}, got {}",
        ...
        if (norm_squared - 1.0).abs() > 1e-10 {
            return Err(QuantrsError::CircuitValidationFailed(
                format!("Amplitudes vector is not properly normalized. Norm^2 = {}", norm_squared)
            ));
        }
    ...
    pub fn amplitude(&self, bits: &[u8]) -> QuantrsResult<Complex64> {
        if bits.len() != N {
            return Err(QuantrsError::CircuitValidationFailed(
                format!("Bits slice has incorrect length. Expected {}, got {}", N, bits.len())
            ));
        }
        ...
        for &bit in bits {
            if bit > 1 {
                return Err(QuantrsError::CircuitValidationFailed(
                    format!("Invalid bit value {}. Must be 0 or 1", bit)
                ));
            }
        }
