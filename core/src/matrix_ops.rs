...
impl DenseMatrix {
    ...
    pub fn from_vec(data: Vec<Complex64>, dim: usize) -> QuantRS2Result<Self> {
        if data.len() != dim * dim {
            return Err(QuantRS2Error::InvalidInput(
                format!("Expected {} elements, got {}", dim * dim, data.len()),
            ));
        }
...
impl QuantumMatrix for DenseMatrix {
    ...
    fn apply(&self, state: &ArrayView1<Complex64>) -> QuantRS2Result<Array1<Complex64>> {
        if state.len() != self.dim() {
            return Err(QuantRS2Error::InvalidInput(
                format!("State dimension {} doesn't match matrix dimension {}", 
                        state.len(), self.dim()),
            ));
        }
...
pub fn partial_trace(
    ...
    if matrix.nrows() != full_dim || matrix.ncols() != full_dim {
        return Err(QuantRS2Error::InvalidInput(
            format!("Matrix dimension {} doesn't match {} qubits", 
                    matrix.nrows(), total_qubits),
        ));
    }
...
mod tests {
    ...
    fn test_tensor_product() {
        // Identity ⊗ Pauli-X
        let id = DenseMatrix::new(Array2::from_shape_vec(
            (2, 2),
            vec![
                Complex64::new(1.0, 0.0),
                Complex64::new(0.0, 0.0),
        ...
        let x = DenseMatrix::new(Array2::from_shape_vec(
            (2, 2),
            vec![
                Complex64::new(0.0, 0.0),
                Complex64::new(1.0, 0.0),
