...
impl PauliString {
    ...
    pub fn get_operator(&self, qubit: QubitId) -> PauliOperator {
        self.operators.get(&qubit).copied().unwrap_or(PauliOperator::I)
    }
...
impl fmt::Display for SymbolicHamiltonianTerm {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        if self.pauli_string.is_identity() {
            ...
            write!(f, "{} * {}", self.coefficient.to_symbolic_expression(), self.pauli_string)
        }
    }
}
...
impl SymbolicHamiltonian {
    ...
    pub fn add_term(&mut self, coefficient: Parameter, pauli_string: PauliString) {
        if pauli_string.n_qubits != self.n_qubits {
            panic!("Pauli string size mismatch");
        }
        self.terms.push(SymbolicHamiltonianTerm::new(coefficient, pauli_string));
    }
    ...
    pub fn add_x(&mut self, qubit: QubitId, coefficient: Parameter) {
        let pauli_string = PauliString::new(self.n_qubits)
            .with_operator(qubit, PauliOperator::X);
    ...
    pub fn add_y(&mut self, qubit: QubitId, coefficient: Parameter) {
        let pauli_string = PauliString::new(self.n_qubits)
            .with_operator(qubit, PauliOperator::Y);
        self.add_term(coefficient, pauli_string);
    }
    ...
    pub fn add_z(&mut self, qubit: QubitId, coefficient: Parameter) {
        let pauli_string = PauliString::new(self.n_qubits)
            .with_operator(qubit, PauliOperator::Z);
        self.add_term(coefficient, pauli_string);
    }
    ...
    pub fn evaluate(&self, variables: &HashMap<String, f64>) -> QuantRS2Result<Vec<(f64, PauliString)>> {
        let mut result = Vec::new();
        for term in &self.terms {
            let coeff = term.evaluate_coefficient(variables)?;
            if coeff.abs() > 1e-12 {  // Filter out nearly zero terms
    ...
    pub fn commutator(&self, other: &SymbolicHamiltonian) -> SymbolicHamiltonian {
        ...
        for term1 in &self.terms {
            for term2 in &other.terms {
                ...
                let coeff = term1.coefficient.clone() * term2.coefficient.clone() * 
        ...
        for term2 in &other.terms {
            for term1 in &self.terms {
                ...
                let coeff = Parameter::constant(-1.0) * term2.coefficient.clone() * 
                           term1.coefficient.clone() * Parameter::complex_constant(phase);
                result.add_term(coeff, pauli_product);
            }
        }
    ...
    pub fn simplify(&self) -> Self {
        ...
        for term in &self.terms {
            grouped_terms.entry(term.pauli_string.clone())
    ...
    pub fn expectation_value(&self, state_vector: &[Complex64]) -> QuantRS2Result<SymbolicExpression> {
    ...
    pub fn gradients(&self, parameter_names: &[String]) -> QuantRS2Result<HashMap<String, SymbolicHamiltonian>> {
        use crate::symbolic::calculus;
        
        let mut gradients = HashMap::new();
        
        for param_name in parameter_names {
            let mut grad_hamiltonian = SymbolicHamiltonian::new(self.n_qubits);
...
impl fmt::Display for SymbolicHamiltonian {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        let terms_str: Vec<String> = self.terms.iter()
    ...
    pub fn number_partitioning(coefficients: &[f64]) -> SymbolicHamiltonian {
        ...
        for i in 0..n_qubits {
            for j in i+1..n_qubits {
                ...
                hamiltonian.add_zz(QubitId::from(i), QubitId::from(j), Parameter::constant(coeff));
            }
        }
...
fn multiply_pauli_operators(op1: PauliOperator, op2: PauliOperator) -> (Complex64, PauliOperator) {
    use PauliOperator::*;
    match (op1, op2) {
...
fn pauli_operators_commute(op1: PauliOperator, op2: PauliOperator) -> bool {
    use PauliOperator::*;
    match (op1, op2) {
        (I, _) | (_, I) => true,
        (X, X) | (Y, Y) | (Z, Z) => true,
...
fn compute_pauli_expectation(pauli_string: &PauliString, state_vector: &[Complex64]) -> QuantRS2Result<Complex64> {
...
fn compute_pauli_matrix_element(pauli_string: &PauliString, i: usize, j: usize) -> Complex64 {
    ...
    for qubit in 0..pauli_string.n_qubits {
        ...
        let local_element = match op {
            PauliOperator::I => {
                if bit_i == bit_j { Complex64::new(1.0, 0.0) } 
            ...
            PauliOperator::Y => {
                if bit_i != bit_j {
                    if bit_j == 1 { Complex64::new(0.0, 1.0) } 
                    else { Complex64::new(0.0, -1.0) }
                } else { Complex64::new(0.0, 0.0) }
            },
            PauliOperator::Z => {
                if bit_i == bit_j {
                    if bit_i == 0 { Complex64::new(1.0, 0.0) } 
...
mod tests {
    ...
    fn test_pauli_string_creation() {
        ...
        assert_eq!(pauli_string.get_operator(QubitId::from(0)), PauliOperator::X);
        assert_eq!(pauli_string.get_operator(QubitId::from(1)), PauliOperator::I);
        assert_eq!(pauli_string.get_operator(QubitId::from(2)), PauliOperator::Z);
    ...
    fn test_hamiltonian_scaling() {
        ...
        let evaluated = scaled.evaluate(&vars).unwrap();
