...
use crate::parametric::{Parameter, ParametricGate};
...
impl std::fmt::Debug for HamiltonianExpectation {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("HamiltonianExpectation")
            ...
            .field("state_prep", &self.state_prep.as_ref().map(|_| "<function>"))
            .finish()
    }
}
...
impl SymbolicObjective for HamiltonianExpectation {
    ...
    fn gradients(&self, parameters: &HashMap<String, f64>) -> QuantRS2Result<HashMap<String, f64>> {
        ...
        #[cfg(feature = "symbolic")]
        {
            ...
            if self.hamiltonian.variables().iter().all(|v| parameters.contains_key(v)) {
        ...
        let step = 1e-8;
        let base_value = self.evaluate(parameters)?;
...
impl SymbolicObjective for QAOACostFunction {
    ...
    fn parameter_bounds(&self) -> HashMap<String, (Option<f64>, Option<f64>)> {
        ...
        for layer in 0..self.p_layers {
            ...
            bounds.insert(format!("gamma_{}", layer), (Some(0.0), Some(2.0 * std::f64::consts::PI)));
            bounds.insert(format!("beta_{}", layer), (Some(0.0), Some(std::f64::consts::PI)));
        }
        bounds
    }
}
        ...
        for iteration in 0..self.config.max_iterations {
            ...
            for param_name in objective.parameter_names() {
                if let Some(gradient) = gradients.get(&param_name) {
                    ...
                    let bounded_value = if let Some((lower, upper)) = objective.parameter_bounds().get(&param_name) {
            ...
            if iteration % 100 == 0 {
                println!("Iteration {}: objective = {:.6e}, max_grad = {:.6e}", 
...
pub mod circuit_optimization {
    ...
    pub fn apply_optimized_parameters(
        ...
        for gate in gates {
            let param_assignments: Vec<(String, f64)> = gate.parameter_names()
                .into_iter()
...
mod tests {
    ...
    fn test_qaoa_cost_function() {
        ...
        let mixer_h = hamiltonians::transverse_field_ising(2, Parameter::constant(0.0), Parameter::constant(1.0));
