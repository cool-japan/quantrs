...
use crate::{
    ...
    cartan::{CartanDecomposer, CartanDecomposition, CartanCoefficients},
    shannon::{ShannonDecomposer},
};
use ndarray::{Array2, Array1, s, ArrayView2, Axis};
...
impl MultiQubitKAKDecomposer {
    ...
    fn choose_decomposition_method(&self, unitary: &Array2<Complex<f64>>, n: usize) -> DecompositionMethod {
    ...
    fn decompose_block_diagonal(
        ...
        for i in 0..num_blocks {
            ...
            let (block_tree, block_gates) = self.decompose_recursive(&block, block_qubits, depth + 1)?;
            ...
            DecompositionTree::Leaf { qubits, gate_type } => {
                ...
                match gate_type {
                    LeafType::SingleQubit(_) => {
                        *self.stats.method_counts.entry("single_qubit".to_string()).or_insert(0) += 1;
                    }
                    LeafType::TwoQubit(cartan) => {
                        *self.stats.method_counts.entry("two_qubit".to_string()).or_insert(0) += 1;
                        let cnots = cartan.interaction.cnot_count(1e-10);
                        *self.stats.cnot_distribution.entry(cnots).or_insert(0) += 1;
                    }
                }
            }
            DecompositionTree::Node { method, children, .. } => {
                ...
                *self.stats.method_counts.entry(method_name.to_string()).or_insert(0) += 1;
...
mod tests {
    ...
    fn test_multiqubit_kak_single() {
        ...
        match &decomp.tree {
            DecompositionTree::Leaf { gate_type: LeafType::SingleQubit(_), .. } => {},
    ...
    fn test_multiqubit_kak_two() {
        ...
        // Check tree structure
        match &decomp.tree {
            DecompositionTree::Leaf { gate_type: LeafType::TwoQubit(_), .. } => {},
