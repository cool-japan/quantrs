...
use super::{BatchStateVector, BatchMeasurementResult};
use crate::{
...
pub fn measure_batch(
    ...
    if config.parallel && batch_size > 16 {
        ...
        for i in 0..batch_size {
            let state = batch.states.row(i);
            let (outcome, probs) = measure_single_state(&state.to_owned(), qubits_to_measure, &config);
...
fn measure_qubit(
    ...
    let probability = if outcome == 0 { prob_zero } else { 1.0 - prob_zero };
...
fn get_computational_measurements(qubits: &[QubitId]) -> Vec<(String, Vec<(QubitId, Array2<Complex64>)>)> {
    use ndarray::array;
...
mod tests {
    ...
    fn test_expectation_measurement() {
        ...
        let expectations = measure_expectation_batch(
            &batch,
            &[(QubitId(0), pauli_z)],
        ).unwrap();
