...
use super::{BatchStateVector, BatchGateOp};
use crate::{
...
pub fn apply_single_qubit_gate_batch(
    ...
    if batch_size > 32 {
        batch.states.axis_iter_mut(Axis(0))
            .into_par_iter()
            .try_for_each(|mut state_row| -> QuantRS2Result<()> {
                apply_single_qubit_to_state(&mut state_row.to_owned(), gate_matrix, target_idx, n_qubits)?;
                Ok(())
            })?;
    } else {
...
pub fn apply_two_qubit_gate_batch(
    ...
    if control_idx >= n_qubits || target_idx >= n_qubits {
        return Err(QuantRS2Error::InvalidQubitId(
            if control_idx >= n_qubits { control.0 } else { target.0 }
        ));
    }
    ...
    if batch_size > 16 {
        batch.states.axis_iter_mut(Axis(0))
            .into_par_iter()
            .try_for_each(|mut state_row| -> QuantRS2Result<()> {
                apply_two_qubit_to_state(&mut state_row.to_owned(), gate_matrix, 
                                       control_idx, target_idx, n_qubits)?;
                Ok(())
            })?;
    } else {
        // Sequential for small batches
...
fn apply_two_qubit_to_state(
    ...
    for i in 0..state_size {
        if (i & control_mask == 0) && (i & target_mask == 0) {
            ...
            state[i00] = gate_matrix[0] * a00 + gate_matrix[1] * a01 + 
                        gate_matrix[2] * a10 + gate_matrix[3] * a11;
            state[i01] = gate_matrix[4] * a00 + gate_matrix[5] * a01 + 
                        gate_matrix[6] * a10 + gate_matrix[7] * a11;
            state[i10] = gate_matrix[8] * a00 + gate_matrix[9] * a01 + 
                        gate_matrix[10] * a10 + gate_matrix[11] * a11;
            state[i11] = gate_matrix[12] * a00 + gate_matrix[13] * a01 + 
                        gate_matrix[14] * a10 + gate_matrix[15] * a11;
        }
    }
    ...
    for (gate, qubits) in gates {
        ...
        // TODO: Add batch-optimized gate detection
        {
            ...
            match qubits.len() {
                ...
                _ => {
                    return Err(QuantRS2Error::InvalidInput(
                        "Batch operations for gates with more than 2 qubits not yet supported".to_string()
                    ));
                }
            }
        }
...
pub fn batch_state_matrix_multiply(
    ...
    if num_matrices != batch_size {
        return Err(QuantRS2Error::InvalidInput(
            format!("Number of matrices {} doesn't match batch size {}", 
    ...
    if cols != batch.states.ncols() {
        return Err(QuantRS2Error::InvalidInput(
            format!("Matrix columns {} don't match state size {}", 
                   cols, batch.states.ncols())
        ));
    }
...
pub fn compute_expectation_values_batch(
    ...
    if batch_size > 16 {
        ...
        for i in 0..batch_size {
            let state = batch.states.row(i);
            expectations.push(compute_expectation_value(&state.to_owned(), observable_matrix));
        }
        Ok(expectations)
    }
}
