...
use super::execution::{BatchCircuitExecutor, BatchCircuit};
use crate::{
...
impl BatchParameterOptimizer {
    ...
    pub fn optimize_batch(
        ...
        match result {
            ...
            Err(e) => Err(QuantRS2Error::InvalidInput(format!("Optimization failed: {:?}", e))),
        }
    }
    ...
    pub fn compute_gradients_batch(
        ...
        if let Some(cache) = &self.gradient_cache {
            for (i, cached_params) in cache.parameters.iter().enumerate() {
                if params.iter().zip(cached_params).all(|(a, b)| (a - b).abs() < 1e-10) {
...
impl BatchVQE {
    ...
    pub fn optimize(
        ...
        // Run optimization
        let result = self.optimizer.optimize_batch(
            ansatz_fn,
            initial_params,
            cost_fn,
            &batch,
    ...
    pub fn optimize(
        ...
        if initial_params.len() != 2 * self.p {
            return Err(QuantRS2Error::InvalidInput(
                format!("Expected {} parameters, got {}", 2 * self.p, initial_params.len())
            ));
        }
...
mod tests {
    ...
    fn test_gradient_computation() {
        ...
        let cost_fn = |_states: &BatchStateVector| -> f64 {
            1.0
        };
        ...
        let gradients = optimizer.compute_gradients_batch(
            circuit_fn,
            &params,
            cost_fn,
            &batch,
