...
impl BatchStateVector {
        ...
        if let Some(limit) = config.memory_limit {
            ...
            if required_memory > limit {
                return Err(QuantRS2Error::InvalidInput(
                    format!("Batch requires {} bytes, limit is {}", required_memory, limit)
                ));
            }
        }
    ...
    pub fn get_state(&self, index: usize) -> QuantRS2Result<Array1<Complex64>> {
        if index >= self.batch_size() {
            return Err(QuantRS2Error::InvalidInput(
                format!("Index {} out of bounds for batch size {}", index, self.batch_size())
            ));
        }
    ...
    pub fn set_state(&mut self, index: usize, state: &Array1<Complex64>) -> QuantRS2Result<()> {
        ...
        if state.len() != self.states.ncols() {
            return Err(QuantRS2Error::InvalidInput(
                format!("State size {} doesn't match expected {}", state.len(), self.states.ncols())
            ));
        }
    ...
    I: IntoIterator<Item = Array1<Complex64>>,
{
    ...
    if states_vec.is_empty() {
        return Err(QuantRS2Error::InvalidInput("Cannot create empty batch".to_string()));
    }
    ...
    for (i, state) in states_vec.iter().enumerate() {
        if state.len() != state_size {
            return Err(QuantRS2Error::InvalidInput(
                format!("State {} has size {}, expected {}", i, state.len(), state_size)
            ));
        }
    }
...
pub fn merge_batches(
    ...
    if batches.is_empty() {
        return Err(QuantRS2Error::InvalidInput("Cannot merge empty batches".to_string()));
    }
    ...
    for (i, batch) in batches.iter().enumerate() {
        if batch.n_qubits != n_qubits {
            return Err(QuantRS2Error::InvalidInput(
                format!("Batch {} has {} qubits, expected {}", i, batch.n_qubits, n_qubits)
            ));
        }
    }
