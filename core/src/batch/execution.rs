...
use super::{BatchStateVector, BatchExecutionResult, BatchConfig};
use crate::{
...
use scirs2_core::parallel::{WorkStealingScheduler, SchedulerConfig};
...
impl BatchCircuitExecutor {
    ...
    pub fn execute_batch(
        ...
        if batch.n_qubits != circuit.n_qubits {
            return Err(QuantRS2Error::InvalidInput(
                format!("Circuit has {} qubits but batch has {}", circuit.n_qubits, batch.n_qubits)
            ));
        }
    ...
    fn execute_parallel(
        ...
        if let Some(scheduler) = &self.scheduler {
            ...
            batch.states.axis_iter_mut(ndarray::Axis(0))
                .into_par_iter()
    ...
    fn execute_with_scheduler(
        ...
        let results: Vec<Array1<Complex64>> = (0..batch_size)
            ...
            .map(|i| {
                ...
                apply_gates_to_state(&mut state, gates, n_qubits)
                    .map(|_| state)
            })
            .collect::<QuantRS2Result<Vec<_>>>()?;
    ...
    pub fn execute_parameterized_batch(
        ...
        let results: Vec<_> = parameter_sets
            ...
            .map(|(i, params)| {
                ...
                apply_gates_to_state(&mut state, &circuit.gate_sequence().collect::<Vec<_>>(), circuit.n_qubits)?;
                Ok(state)
            })
            .collect::<QuantRS2Result<Vec<_>>>()?;
...
fn apply_gates_to_state(
    ...
    for gate in gates {
        ...
        match qubits.len() {
            ...
            _ => {
                return Err(QuantRS2Error::InvalidInput(
                    format!("Gates with {} qubits not yet supported", qubits.len())
                ));
            }
        }
    }
...
fn apply_two_qubit_gate(
    ...
    for i in 0..state_size {
        if (i & control_mask == 0) && (i & target_mask == 0) {
            ...
            state[i00] = matrix[0] * a00 + matrix[1] * a01 + 
                        matrix[2] * a10 + matrix[3] * a11;
            state[i01] = matrix[4] * a00 + matrix[5] * a01 + 
                        matrix[6] * a10 + matrix[7] * a11;
            state[i10] = matrix[8] * a00 + matrix[9] * a01 + 
                        matrix[10] * a10 + matrix[11] * a11;
            state[i11] = matrix[12] * a00 + matrix[13] * a01 + 
                        matrix[14] * a10 + matrix[15] * a11;
        }
    }
...
mod tests {
    ...
    fn test_batch_circuit_execution() {
        ...
        circuit.add_gate(Box::new(Hadamard { target: QubitId(0) })).unwrap();
        circuit.add_gate(Box::new(Hadamard { target: QubitId(1) })).unwrap();
    ...
    fn test_parallel_circuit_execution() {
        ...
        let results = executor.execute_multiple_circuits(&circuits, &batch).unwrap();
