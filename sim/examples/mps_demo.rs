...
#[cfg(feature = "mps")]
fn demo_bell_state() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("\n   Sampling 10 measurements:");
    let mut mps_copy = bell_mps.clone();
    for i in 0..10 {
        let outcome = mps_copy.sample();
        println!("   Sample {}: |{}{}>", 
            i + 1,
            if outcome[0] { 1 } else { 0 },
            if outcome[1] { 1 } else { 0 }
        );
    }
...
#[cfg(feature = "mps")]
fn demo_ghz_state() -> Result<(), Box<dyn std::error::Error>> {
    println!("2. GHZ State with Variable Size");
    
    for n in [3, 5, 10] {
        println!("\n   Creating {}-qubit GHZ state: (|0...0⟩ + |1...1⟩)/√2", n);
...
#[cfg(feature = "mps")]
fn demo_linear_circuit() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let bitstring: String = outcome.iter()
        .map(|&b| if b { '1' } else { '0' })
        .collect();
...
#[cfg(feature = "mps")]
fn demo_entanglement_entropy() -> Result<(), Box<dyn std::error::Error>> {
...
fn demo_performance_comparison() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let configs = vec![
        ("Low bond (χ=4)", MPSConfig { 
            max_bond_dim: 4, 
            ..Default::default() 
        }),
        ("Medium bond (χ=16)", MPSConfig { 
            max_bond_dim: 16, 
            ..Default::default() 
        }),
        ("High bond (χ=64)", MPSConfig { 
            max_bond_dim: 64, 
            ..Default::default() 
        }),
    ];
