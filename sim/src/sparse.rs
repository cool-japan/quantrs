...
impl CSRMatrix {
    ...
    pub fn matvec(&self, vec: &Array1<Complex64>) -> Result<Array1<Complex64>> {
        if vec.len() != self.num_cols {
            return Err(SimulatorError::DimensionMismatch(
                format!("Vector length {} doesn't match matrix columns {}", vec.len(), self.num_cols)
            ));
        }
    ...
    pub fn matmul(&self, other: &CSRMatrix) -> Result<CSRMatrix> {
        ...
        Ok(CSRMatrix::new(values, col_indices, row_ptr, self.num_rows, other.num_cols))
    }
...
impl SparseGates {
    ...
    pub fn rotation(axis: &str, angle: f64) -> Result<CSRMatrix> {
        ...
        match axis {
            ...
            _ => return Err(SimulatorError::InvalidConfiguration(
                format!("Unknown rotation axis: {}", axis)
            )),
        }
...
pub fn apply_sparse_gate(
    ...
    if gate.num_rows != gate_dim || gate.num_cols != gate_dim {
        return Err(SimulatorError::DimensionMismatch(
            format!("Gate dimension {} doesn't match qubit count {}", gate.num_rows, gate_qubits)
        ));
    }
...
pub fn optimize_sparse_gates(gates: Vec<CSRMatrix>) -> Result<CSRMatrix> {
    if gates.is_empty() {
        return Err(SimulatorError::InvalidInput("Empty gate sequence".to_string()));
    }
