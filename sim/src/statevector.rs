...
use crate::utils::{gate_vec_to_array2, flip_bit};
...
impl StateVectorSimulator {
        ...
        if self.parallel {
            ...
            for i in 0..dim {
                ...
                if bit_val == 0 {
                    ...
                    new_state[paired_idx] = matrix[[1, 0]] * state[i] + matrix[[1, 1]] * state[paired_idx];
                }
            }
    ...
    fn apply_two_qubit_gate<const N: usize>(
        ...
        if control_idx >= N || target_idx >= N {
            return Err(QuantrsError::InvalidQubitId(
                if control_idx >= N { control.id() } else { target.id() }
            ));
        }
        ...
        if self.parallel {
            ...
            state.par_iter_mut().enumerate().for_each(|(idx, amp)| {
                ...
                *amp = match idx {
                    i if i == idx00 => 
                        matrix[[0, 0]] * val00 + matrix[[0, 1]] * val01 +
                        matrix[[0, 2]] * val10 + matrix[[0, 3]] * val11,
                    i if i == idx01 => 
                        matrix[[1, 0]] * val00 + matrix[[1, 1]] * val01 +
                        matrix[[1, 2]] * val10 + matrix[[1, 3]] * val11,
                    i if i == idx10 => 
                        matrix[[2, 0]] * val00 + matrix[[2, 1]] * val01 +
                        matrix[[2, 2]] * val10 + matrix[[2, 3]] * val11,
                    i if i == idx11 => 
                        matrix[[3, 0]] * val00 + matrix[[3, 1]] * val01 +
                        matrix[[3, 2]] * val10 + matrix[[3, 3]] * val11,
                    _ => unreachable!(),
                };
            });
        } else {
    ...
    fn apply_swap<const N: usize>(
        ...
        if q1_idx >= N || q2_idx >= N {
            return Err(QuantrsError::InvalidQubitId(
                if q1_idx >= N { qubit1.id() } else { qubit2.id() }
            ));
        }
    ...
    fn run(&self, circuit: &Circuit<N>) -> QuantrsResult<Register<N>> {
        ...
        for gate in circuit.gates() {
            match gate.name() {
                ...
                // Three-qubit gates
                "Toffoli" => {
                    if let Some(_) = gate.as_any().downcast_ref::<multi::Toffoli>() {
                        ...
                        return Err(QuantrsError::UnsupportedOperation(
                            "Direct Toffoli gate not yet implemented. Use gate decomposition.".into()
                        ));
                    }
                },
                "Fredkin" => {
                    if let Some(_) = gate.as_any().downcast_ref::<multi::Fredkin>() {
                        // Implement Fredkin as a sequence of simpler gates
                        // (This is a placeholder for a more efficient implementation)
                        return Err(QuantrsError::UnsupportedOperation(
                            "Direct Fredkin gate not yet implemented. Use gate decomposition.".into()
                        ));
                    }
                },
                ...
                _ => {
                    return Err(QuantrsError::UnsupportedOperation(
                        format!("Gate {} not supported", gate.name())
                    ));
                },
            }
        }
