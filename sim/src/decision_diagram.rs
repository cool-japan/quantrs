...
impl DecisionDiagram {
    ...
    fn get_or_create_node(&mut self, variable: usize, high: Edge, low: Edge) -> Edge {
        ...
        if let Some(&existing_id) = self.unique_table.get(&key) {
            self.ref_counts.entry(existing_id).and_modify(|c| *c += 1).or_insert(1);
            return Edge {
                target: existing_id,
                weight: Complex64::new(1.0, 0.0),
            };
    ...
    fn increment_ref_count(&mut self, node_id: NodeId) {
        self.ref_counts.entry(node_id).and_modify(|c| *c += 1).or_insert(1);
    }
    ...
    fn decrement_ref_count(&mut self, node_id: NodeId) {
        if let Some(count) = self.ref_counts.get_mut(&node_id) {
            *count -= 1;
            if *count == 0 && node_id > 1 { // Don't garbage collect terminals
    ...
    pub fn apply_single_qubit_gate(&mut self, gate_matrix: &Array2<Complex64>, target: usize) -> Result<()> {
        ...
        let new_root = self.apply_gate_recursive(
            &self.root.clone(),
            gate_matrix,
            target,
            0,
    ...
    fn apply_gate_recursive(
        ...
        if current_var == target {
            ...
            let high_result = self.apply_gate_recursive(&node.high, gate_matrix, target, current_var + 1)?;
            let low_result = self.apply_gate_recursive(&node.low, gate_matrix, target, current_var + 1)?;
            
            // Apply gate transformation
            let new_high = Edge {
                target: high_result.target,
                weight: gate_matrix[[1, 1]] * high_result.weight + gate_matrix[[1, 0]] * low_result.weight,
            };
            
            let new_low = Edge {
                target: low_result.target,
                weight: gate_matrix[[0, 0]] * low_result.weight + gate_matrix[[0, 1]] * high_result.weight,
            };
    ...
    pub fn apply_cnot(&mut self, control: usize, target: usize) -> Result<()> {
        let new_root = self.apply_cnot_recursive(
            &self.root.clone(),
            control,
            target,
            0,
    ...
    fn apply_cnot_recursive(
        ...
        if current_var == control.min(target) {
            ...
            if control < target {
                // Control is first
                let high_result = self.apply_cnot_recursive(&node.high, control, target, current_var + 1)?;
                let low_result = self.apply_cnot_recursive(&node.low, control, target, current_var + 1)?;
    ...
    fn apply_conditional_x(&mut self, edge: Edge, target: usize, current_var: usize) -> Result<Edge> {
    ...
    fn extract_amplitudes(
        ...
        if let Some(node) = self.nodes.get(&edge.target) {
            ...
            self.extract_amplitudes(&node.high, current_var + 1, high_basis, current_amplitude, state);
            
            // Low edge (bit = 0)
            self.extract_amplitudes(&node.low, current_var + 1, basis_state, current_amplitude, state);
        }
    }
    ...
    pub fn garbage_collect(&mut self) {
        ...
        for (&node_id, &ref_count) in &self.ref_counts {
            if ref_count == 0 && node_id > 1 { // Don't remove terminals
    ...
    fn inner_product_recursive(&self, edge1: &Edge, edge2: &Edge, var: usize) -> Complex64 {
        ...
        let (node1, node2) = (
            self.nodes.get(&edge1.target),
            self.nodes.get(&edge2.target)
        );
...
impl DDSimulator {
    ...
    pub fn is_classical_state(&self) -> bool {
        let state = self.get_state_vector();
        state.iter().all(|amp| amp.im.abs() < 1e-10 && amp.re >= 0.0)
    }
...
mod tests {
    ...
    fn test_dd_creation() {
        ...
        assert_eq!(dd.node_count(), 2); // Just terminals initially
    }

    #[test]
    fn test_computational_basis_state() {
        let mut dd = DecisionDiagram::new(2);
        let edge = dd.create_computational_basis_state(&[true, false]); // |10⟩
        
        let state = dd.to_state_vector();
        assert!((state[2].re - 1.0).abs() < 1e-10); // |10⟩ = index 2
        assert!(state.iter().enumerate().all(|(i, &amp)| 
            if i == 2 { amp.norm() > 0.9 } else { amp.norm() < 1e-10 }
        ));
    }
    ...
    fn test_bell_state() {
        ...
        assert!((state[0].re - expected_amp).abs() < 1e-10); // |00⟩
        assert!((state[3].re - expected_amp).abs() < 1e-10); // |11⟩
    ...
    #[test]
    fn test_compression() {
        let mut sim = DDSimulator::new(4).unwrap();
        
        // Create a highly entangled state
        for i in 0..4 {
            sim.apply_hadamard(i).unwrap();
        }
