...
impl SciRS2QFT {
    ...
    pub fn apply_qft(&mut self, state: &mut Array1<Complex64>) -> Result<()> {
        ...
        if state.len() != 1 << self.num_qubits {
            return Err(SimulatorError::DimensionMismatch(
                format!("State vector length {} doesn't match 2^{} qubits", 
                        state.len(), self.num_qubits)
            ));
        }
    ...
    fn apply_scirs2_approximate_inverse_qft(&mut self, state: &mut Array1<Complex64>) -> Result<()> {
        if let Some(_backend) = &mut self.backend {
            let mut complex_data: Vec<Complex64> = state.to_vec();
    ...
    fn apply_qft_approximation(&self, data: &mut [Complex64]) -> Result<()> {
        ...
        let threshold = self.config.precision_threshold * 10.0_f64.powi(self.config.approximation_level as i32);
...
impl QFTUtils {
    ...
    pub fn create_test_state(num_qubits: usize, pattern: &str) -> Result<Array1<Complex64>> {
        ...
        match pattern {
            ...
            _ => {
                return Err(SimulatorError::InvalidInput(
                    format!("Unknown test pattern: {}", pattern)
                ));
            }
        }
    ...
    pub fn verify_qft_roundtrip(
        ...
        // Check fidelity with initial state
        let fidelity = initial_state.iter().zip(state.iter())
            .map(|(a, b)| (a.conj() * b).re)
            .sum::<f64>();
        
        Ok((1.0 - fidelity.abs()).abs() < tolerance)
    }
...
pub fn benchmark_qft_methods(num_qubits: usize) -> Result<HashMap<String, QFTStats>> {
    ...
    for (name, method) in methods {
        let config = QFTConfig {
            method,
            approximation_level: if method == QFTMethod::SciRS2Approximate { 1 } else { 0 },
            bit_reversal: true,
            parallel: true,
            precision_threshold: 1e-10,
        };
        ...
        let mut qft = if method == QFTMethod::SciRS2Exact || method == QFTMethod::SciRS2Approximate {
            SciRS2QFT::new(num_qubits, config.clone())?.with_backend().unwrap_or_else(|_| {
                SciRS2QFT::new(num_qubits, config).unwrap()
            })
        } else {
...
pub fn compare_qft_accuracy(num_qubits: usize) -> Result<HashMap<String, f64>> {
    ...
    for (name, method) in methods {
        ...
        let error = reference_result.iter().zip(state.iter())
            .map(|(ref_val, qft_val)| (ref_val - qft_val).norm())
            .sum::<f64>() / reference_result.len() as f64;
...
mod tests {
    ...
    fn test_qft_roundtrip() {
        ...
        let initial_state = QFTUtils::create_test_state(3, "random").unwrap();
        
        let is_correct = QFTUtils::verify_qft_roundtrip(&mut qft, &initial_state, 1e-10).unwrap();
        assert!(is_correct);
    }
