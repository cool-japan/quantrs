...
use ndarray::{Array2, Array3, Array1, ArrayView2, s, Axis};
...
impl MPS {
    ...
    fn left_canonicalize_up_to(&mut self, position: usize) -> QuantRS2Result<()> {
        for i in 0..position {
            ...
            let (left_dim, phys_dim, right_dim) = 
                (tensor.left_dim, 2, tensor.right_dim);
            ...
            if i + 1 < self.num_qubits {
                ...
                let next_matrix = next.data.view()
    ...
    fn move_center_left(&mut self, position: usize) -> QuantRS2Result<()> {
        ...
        if position > 0 {
            ...
            let prev_matrix = prev.data.view()
...
fn qr_decomposition(matrix: &ArrayView2<Complex64>) -> QuantRS2Result<(Array2<Complex64>, Array2<Complex64>)> {
...
mod tests {
    ...
    fn test_single_qubit_gate() {
        ...
        let h = Hadamard { target: QubitId::new(0) };
