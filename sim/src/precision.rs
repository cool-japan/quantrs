...
impl AdaptiveStateVector {
    ...
    pub fn new(num_qubits: usize, precision: Precision) -> Result<Self> {
        ...
        match precision {
            Precision::Half => {
                let mut state = Array1::from_elem(size, ComplexF16 {
                    re: f16::from_f64(0.0),
                    im: f16::from_f64(0.0),
                });
                state[0] = ComplexF16 {
            ...
            AdaptiveStateVector::Half(v) => {
                if v.len() != data.len() {
                    return Err(SimulatorError::DimensionMismatch(
                        format!("Size mismatch: {} vs {}", v.len(), data.len())
                    ));
                }
    ...
    pub fn needs_precision_upgrade(&self, threshold: f64) -> bool {
        ...
        let min_amplitude = match self {
            AdaptiveStateVector::Half(v) => {
                v.iter().map(|c| c.norm_sqr()).filter(|&n| n > 0.0).fold(None, |acc, x| {
                    match acc {
                        None => Some(x),
            ...
            AdaptiveStateVector::Single(v) => {
                v.iter().map(|c| c.norm_sqr() as f64).filter(|&n| n > 0.0).fold(None, |acc, x| {
                    match acc {
                        None => Some(x),
                        Some(y) => Some(if x < y { x } else { y })
                    }
