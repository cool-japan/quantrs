...
impl ComplexSimdVector {
    ...
    /// Detect optimal SIMD width for current hardware
    pub fn detect_simd_width() -> usize {
        #[cfg(target_arch = "x86_64")]
        {
            if is_x86_feature_detected!("avx512f") {
                8 // 8 double precision floats in 512-bit register
            } else if is_x86_feature_detected!("avx2") {
                4 // 4 double precision floats in 256-bit register  
            } else if is_x86_feature_detected!("sse2") {
                2 // 2 double precision floats in 128-bit register
            } else {
                1 // Scalar fallback
            }
        }
        ...
        {
            2 // NEON supports 2 double precision floats
        }
        #[cfg(not(any(target_arch = "x86_64", target_arch = "aarch64")))]
        {
            1 // Scalar fallback for other architectures
        }
    }
