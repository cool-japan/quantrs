...
pub enum BreakCondition {
    ...
    FidelityThreshold { target_state: Vec<Complex64>, threshold: f64 },
    /// Break when a Pauli observable expectation value crosses threshold
    ObservableThreshold { observable: String, threshold: f64, direction: ThresholdDirection },
...
impl<const N: usize> QuantumDebugger<N> {
    ...
    pub fn new(config: DebugConfig) -> Result<Self> {
        ...
        let mps_simulator = if config.use_mps {
            Some(EnhancedMPS::new(N, config.mps_config.clone().unwrap_or_default()))
        } else {
            None
        };
    ...
    pub fn remove_watchpoint(&mut self, id: &str) -> Result<()> {
        if self.watchpoints.remove(id).is_none() {
            return Err(SimulatorError::InvalidInput(format!("Watchpoint '{}' not found", id)));
        }
        Ok(())
    }
    ...
    pub fn step(&mut self) -> Result<StepResult> {
        ...
        // Update metrics
        *self.metrics.gate_times.entry(gate_name.clone()).or_insert(Duration::new(0, 0)) += gate_time;
        *self.metrics.gate_counts.entry(gate_name).or_insert(0) += 1;
        ...
        if let Some(reason) = self.check_breakpoints()? {
            self.execution_state = ExecutionState::Paused { reason: reason.clone() };
    ...
    pub fn get_pauli_expectation(&self, pauli_string: &str) -> Result<Complex64> {
        #[cfg(feature = "mps")]
        if let Some(ref mps) = self.mps_simulator {
            return mps.expectation_value_pauli(pauli_string);
        }
            ...
            match breakpoint {
                ...
                BreakCondition::EntanglementThreshold { cut, threshold } => {
                    ...
                    if entropy > *threshold {
                        return Ok(Some(format!("Entanglement entropy {:.4} > {:.4} at cut {}", entropy, threshold, cut)));
                    }
                },
                BreakCondition::ObservableThreshold { observable, threshold, direction } => {
                    ...
                    if hit {
                        return Ok(Some(format!("Observable {} = {:.4} crossed threshold {:.4}", observable, expectation, threshold)));
                    }
                },
                _ => {
    ...
    fn generate_recommendations(&self) -> Vec<String> {
        ...
        if self.metrics.max_entanglement > 3.0 {
            recommendations.push("High entanglement detected. Consider using MPS simulation for better scaling.".to_string());
        }
        
        if self.metrics.gate_counts.get("CNOT").unwrap_or(&0) > &50 {
            recommendations.push("Many CNOT gates detected. Consider gate optimization.".to_string());
        }
...
/// Compute entanglement entropy from state vector
fn compute_entanglement_entropy(state: &Array1<Complex64>, cut: usize, num_qubits: usize) -> Result<f64> {
    if cut >= num_qubits - 1 {
    ...
    let state_matrix = Array2::from_shape_vec((left_dim, right_dim), state.to_vec())
        .map_err(|_| SimulatorError::DimensionMismatch("Invalid state vector dimension".to_string()))?;
