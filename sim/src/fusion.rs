...
impl SciRS2MatrixMultiplier {
    fn multiply_sparse(a: &CSRMatrix, b: &CSRMatrix) -> Result<CSRMatrix> {
        ...
        if a.num_cols != b.num_rows {
            return Err(SimulatorError::DimensionMismatch(
                format!("Cannot multiply {}x{} with {}x{}", a.num_rows, a.num_cols, b.num_rows, b.num_cols)
            ));
        }
    ...
    fn multiply_dense(a: &Array2<Complex64>, b: &Array2<Complex64>) -> Result<Array2<Complex64>> {
        ...
        if a.ncols() != b.nrows() {
            return Err(SimulatorError::DimensionMismatch(
                format!("Cannot multiply {}x{} with {}x{}", a.nrows(), a.ncols(), b.nrows(), b.ncols())
            ));
        }
...
impl GateFusion {
    ...
    fn can_fuse_with_group(&self, group: &GateGroup, gate: &dyn GateOp) -> bool {
        ...
        match self.strategy {
            ...
            FusionStrategy::DepthOptimized => {
                ...
                let combined_qubits: HashSet<_> = gate_qubits.union(&group_qubits).cloned().collect();
    ...
    pub fn fuse_group(
        ...
        for &gate_idx in &group.gate_indices {
            ...
            let expanded = self.expand_gate_matrix(
                &gate_matrix,
                &gate_qubits,
                &qubit_map,
                group_size,
...
pub fn benchmark_fusion_strategies(
    ...
    for strategy in [
        ...
        println!("  Fusion ratio: {:.2}%", optimized.stats.fusion_ratio * 100.0);
        println!("  Groups fused: {}/{}", 
            optimized.stats.groups_fused, 
            optimized.stats.groups_analyzed
        );
        println!("  Memory usage: {:.2} MB", optimized.memory_usage() as f64 / 1e6);
...
mod tests {
    ...
    fn test_fusion_cost() {
        ...
        let gates: Vec<Box<dyn GateOp>> = vec![
            ...
            Box::new(CNOT { control: QubitId::new(0), target: QubitId::new(1) }),
        ];
