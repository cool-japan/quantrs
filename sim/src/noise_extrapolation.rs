...
use crate::open_quantum_systems::{QuantumChannel, NoiseModelBuilder};
...
impl ZeroNoiseExtrapolator {
        ...
        F: Fn(f64) -> Result<f64> + Sync + Send,
    {
        ...
        let measurements: Result<Vec<(f64, f64, f64)>> = self.noise_factors
            .par_iter()
            .map(|&noise_factor| {
        ...
        let (zero_noise_value, error_estimate, fit_stats) = match self.extrapolation_method {
            ExtrapolationMethod::Linear => self.linear_extrapolation(&noise_factors, &measured_values, &uncertainties)?,
            ExtrapolationMethod::Exponential => self.exponential_extrapolation(&noise_factors, &measured_values, &uncertainties)?,
            ExtrapolationMethod::Polynomial(order) => self.polynomial_extrapolation(&noise_factors, &measured_values, &uncertainties, order)?,
            ExtrapolationMethod::Richardson => self.richardson_extrapolation(&noise_factors, &measured_values)?,
            ExtrapolationMethod::Adaptive => self.adaptive_extrapolation(&noise_factors, &measured_values, &uncertainties)?,
        };
    ...
    fn linear_extrapolation(
        ...
        let (a, b, fit_stats) = self.weighted_linear_fit(noise_factors, measured_values, uncertainties)?;
        
        // Extrapolate to zero noise (x=0)
        let zero_noise_value = a;
        
        // Error estimate from fit uncertainty
        let error_estimate = fit_stats.residuals.iter().map(|r| r.abs()).sum::<f64>() / fit_stats.residuals.len() as f64;
    ...
    fn exponential_extrapolation(
        ...
        let log_values: Vec<f64> = measured_values.iter()
        ...
        let (ln_a, b, mut fit_stats) = self.weighted_linear_fit(noise_factors, &log_values, &log_uncertainties)?;
        ...
        // Error propagation
        let error_estimate = a * fit_stats.residuals.iter().map(|r| r.abs()).sum::<f64>() / fit_stats.residuals.len() as f64;
        
        // Transform residuals back to original space
        fit_stats.residuals = fit_stats.residuals.iter().zip(noise_factors.iter())
            .map(|(&res, &x)| a * (b * x).exp() - measured_values[noise_factors.iter().position(|&nx| nx == x).unwrap()])
            .collect();
    ...
    fn polynomial_extrapolation(
        ...
        if noise_factors.len() <= order {
            return Err(SimulatorError::InvalidInput(
                format!("Need more than {} data points for order {} polynomial", order, order)
            ));
        }
        ...
        let coefficients = self.solve_weighted_least_squares(&design_matrix, measured_values, uncertainties)?;
        ...
        for (i, &x) in noise_factors.iter().enumerate() {
            let predicted: f64 = coefficients.iter().enumerate()
                .map(|(j, &coeff)| coeff * x.powi(j as i32))
                .sum();
            residuals.push(measured_values[i] - predicted);
        }
        
        let error_estimate = residuals.iter().map(|r| r.abs()).sum::<f64>() / residuals.len() as f64;
    ...
    fn richardson_extrapolation(
        ...
        let a = (y1 * x2 * x3 * (x2 - x3) + y2 * x1 * x3 * (x3 - x1) + y3 * x1 * x2 * (x1 - x2)) / denominator;
    ...
    fn adaptive_extrapolation(
        ...
        best_result.ok_or_else(|| SimulatorError::ComputationError(
            "No extrapolation method succeeded".to_string()
        ))
    }
    ...
    fn weighted_linear_fit(
        ...
        let n = x.len();
        if n < 2 {
            return Err(SimulatorError::InvalidInput("Need at least 2 points for linear fit".to_string()));
        }
        
        // Convert uncertainties to weights
        let w: Vec<f64> = weights.iter().map(|&sigma| if sigma > 0.0 { 1.0 / (sigma * sigma) } else { 1.0 }).collect();
        ...
        let sum_wxy: f64 = w.iter().zip(x.iter()).zip(y.iter()).map(|((&wi, &xi), &yi)| wi * xi * yi).sum();
        
        let delta = sum_w * sum_wxx - sum_wx * sum_wx;
        if delta.abs() < 1e-12 {
            return Err(SimulatorError::NumericalError("Singular matrix in linear fit".to_string()));
        }
...
impl VirtualDistillation {
    ...
    pub fn distill<F>(&self, noisy_executor: F, observable: &str) -> Result<VirtualDistillationResult>
    where
        F: Fn() -> Result<f64>,
    {
        // Execute circuit multiple times
        let measurements: Result<Vec<f64>> = (0..self.num_copies)
            .map(|_| noisy_executor())
            .collect();
...
impl SymmetryVerification {
    ...
    pub fn verify_and_correct<F>(&self, executor: F, observable: &str) -> Result<SymmetryVerificationResult>
    where
        F: Fn(&str) -> Result<f64>,
    {
        ...
        let post_selection_prob = valid_measurements.len() as f64 / (self.symmetries.len() + 1) as f64;
...
mod tests {
    ...
    fn test_zne_linear_extrapolation() {
        ...
        let (zero_noise, _error, _stats) = zne.linear_extrapolation(&noise_factors, &measured_values, &uncertainties).unwrap();
    ...
    fn test_richardson_extrapolation() {
        ...
        let (zero_noise, _error, _stats) = zne.richardson_extrapolation(&noise_factors, &measured_values).unwrap();
