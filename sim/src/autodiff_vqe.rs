...
impl AutoDiffContext {
    ...
    pub fn set_parameter(&mut self, name: &str, value: f64) -> Result<()> {
        if let Some(i) = self.parameter_names.iter().position(|n| n == name) {
            ...
            Err(SimulatorError::InvalidInput(format!("Parameter '{}' not found", name)))
        }
    }
}
...
impl ParametricGate for ParametricRX {
    ...
    fn qubits(&self) -> Vec<usize> { vec![self.qubit] }
    
    fn parameter_indices(&self) -> Vec<usize> { vec![self.param_idx] }
    ...
    fn gradient(&self, params: &[f64], param_idx: usize) -> Result<Array2<Complex64>> {
        ...
        Ok(ndarray::array![
            [Complex64::new(-sin_half / 2.0, 0.), Complex64::new(0., -cos_half / 2.0)],
            [Complex64::new(0., -cos_half / 2.0), Complex64::new(-sin_half / 2.0, 0.)]
        ])
    }
}
...
impl ParametricGate for ParametricRY {
    ...
    fn gradient(&self, params: &[f64], param_idx: usize) -> Result<Array2<Complex64>> {
        ...
        Ok(ndarray::array![
            [Complex64::new(-sin_half / 2.0, 0.), Complex64::new(-cos_half / 2.0, 0.)],
            [Complex64::new(cos_half / 2.0, 0.), Complex64::new(-sin_half / 2.0, 0.)]
        ])
    }
}
    ...
    pub fn evaluate(&self, params: &[f64]) -> Result<Array1<Complex64>> {
        if params.len() != self.num_parameters {
            return Err(SimulatorError::InvalidInput(
                format!("Expected {} parameters, got {}", self.num_parameters, params.len())
            ));
        }
    ...
    pub fn gradient_expectation(
        ...
        match method {
            GradientMethod::ParameterShift => {
                self.parameter_shift_gradient(observable, params)
            },
            GradientMethod::FiniteDifference { step_size } => {
                self.finite_difference_gradient(observable, params, step_size)
            },
            GradientMethod::SPSA { step_size } => {
                self.spsa_gradient(observable, params, step_size)
            },
        }
    }
...
impl VQEWithAutodiff {
    ...
    fn is_converged(&self) -> Result<bool> {
        ...
        let energy_converged = (current.energy - previous.energy).abs() < self.convergence.energy_tolerance;
...
pub mod ansatze {
    ...
    pub fn qaoa_maxcut(num_qubits: usize, num_layers: usize, edges: &[(usize, usize)]) -> ParametricCircuit {
        let mut circuit = ParametricCircuit::new(num_qubits);
        let mut param_idx = 0;
...
mod tests {
    ...
    fn test_parametric_rx_matrix() {
        let rx_gate = ParametricRX { qubit: 0, param_idx: 0 };
