    ...
    fn apply_specialized(
        ...
        if parallel {
            ...
            state.par_iter_mut().enumerate().for_each(|(idx, amp)| {
                ...
                let val0 = if bit_val == 0 { state_copy[idx] } else { state_copy[paired_idx] };
                let val1 = if bit_val == 0 { state_copy[paired_idx] } else { state_copy[idx] };
                
                *amp = sqrt2_inv * if bit_val == 0 {
                    val0 + val1
                } else {
                    val0 - val1
                };
    ...
    fn apply_specialized(
        ...
        if control_idx >= n_qubits || target_idx >= n_qubits {
            return Err(QuantRS2Error::InvalidQubitId(
                if control_idx >= n_qubits { self.control.id() } else { self.target.id() }
            ));
        }
...
impl SpecializedGate for SWAPSpecialized {
    fn apply_specialized(
        ...
        if idx1 >= n_qubits || idx2 >= n_qubits {
            return Err(QuantRS2Error::InvalidQubitId(
                if idx1 >= n_qubits { self.qubit1.id() } else { self.qubit2.id() }
            ));
        }
        ...
        if parallel {
            ...
            state.par_iter_mut().enumerate().for_each(|(idx, amp)| {
                ...
                if bit1 != bit2 {
                    let swapped_idx = (idx & !(1 << idx1) & !(1 << idx2))
                        | (bit2 << idx1)
                        | (bit1 << idx2);
...
impl SpecializedGate for ToffoliSpecialized {
    fn apply_specialized(
        ...
        if ctrl1_idx >= n_qubits || ctrl2_idx >= n_qubits || target_idx >= n_qubits {
            return Err(QuantRS2Error::InvalidQubitId(
                if ctrl1_idx >= n_qubits { self.control1.id() }
                else if ctrl2_idx >= n_qubits { self.control2.id() }
                else { self.target.id() }
            ));
        }
...
impl SpecializedGate for FredkinSpecialized {
    fn apply_specialized(
        ...
        if ctrl_idx >= n_qubits || tgt1_idx >= n_qubits || tgt2_idx >= n_qubits {
            return Err(QuantRS2Error::InvalidQubitId(
                if ctrl_idx >= n_qubits { self.control.id() }
                else if tgt1_idx >= n_qubits { self.target1.id() }
                else { self.target2.id() }
            ));
        }
...
pub fn specialize_gate(gate: &dyn GateOp) -> Option<Box<dyn SpecializedGate>> {
    ...
    if let Some(rx) = gate.as_any().downcast_ref::<RotationX>() {
        return Some(Box::new(RXSpecialized { target: rx.target, theta: rx.theta }));
    }
    if let Some(ry) = gate.as_any().downcast_ref::<RotationY>() {
        return Some(Box::new(RYSpecialized { target: ry.target, theta: ry.theta }));
    }
    if let Some(rz) = gate.as_any().downcast_ref::<RotationZ>() {
        return Some(Box::new(RZSpecialized { target: rz.target, theta: rz.theta }));
    }
    ...
    if let Some(cnot) = gate.as_any().downcast_ref::<CNOT>() {
        return Some(Box::new(CNOTSpecialized { control: cnot.control, target: cnot.target }));
    }
    if let Some(cz) = gate.as_any().downcast_ref::<CZ>() {
        return Some(Box::new(CZSpecialized { control: cz.control, target: cz.target }));
    }
    if let Some(swap) = gate.as_any().downcast_ref::<SWAP>() {
        return Some(Box::new(SWAPSpecialized { qubit1: swap.qubit1, qubit2: swap.qubit2 }));
    }
...
impl_gate_op_for_specialized!(HadamardSpecialized, "H", 
...
impl_gate_op_for_specialized!(PauliXSpecialized, "X",
...
impl_gate_op_for_specialized!(PauliYSpecialized, "Y",
...
impl_gate_op_for_specialized!(PauliZSpecialized, "Z",
...
impl_gate_op_for_specialized!(CNOTSpecialized, "CNOT",
...
impl_gate_op_for_specialized!(PhaseSpecialized, "Phase",
    |g: &PhaseSpecialized| vec![g.target],
    |g: &PhaseSpecialized| Ok(vec![
        Complex64::new(1.0, 0.0), Complex64::new(0.0, 0.0),
        Complex64::new(0.0, 0.0), Complex64::from_polar(1.0, g.phase),
    ])
);

impl_gate_op_for_specialized!(SGateSpecialized, "S",
...
impl_gate_op_for_specialized!(TGateSpecialized, "T",
...
impl_gate_op_for_specialized!(RXSpecialized, "RX",
...
impl_gate_op_for_specialized!(RYSpecialized, "RY",
...
impl_gate_op_for_specialized!(RZSpecialized, "RZ",
...
impl_gate_op_for_specialized!(CZSpecialized, "CZ",
...
impl_gate_op_for_specialized!(SWAPSpecialized, "SWAP",
...
impl_gate_op_for_specialized!(CPhaseSpecialized, "CPhase",
...
impl_gate_op_for_specialized!(ToffoliSpecialized, "Toffoli",
...
impl_gate_op_for_specialized!(FredkinSpecialized, "Fredkin",
...
mod tests {
    ...
    fn test_cnot_specialized() {
        ...
        let gate = CNOTSpecialized { control: QubitId(0), target: QubitId(1) };
