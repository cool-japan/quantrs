...
impl PhotonicSimulator {
        ...
        match self.config.method {
            PhotonicMethod::FockBasis | PhotonicMethod::TruncatedFock => {
                ...
                for (i, fock_state) in fock_states.iter().enumerate() {
                    ...
                    let amplitude = displacement_factor * alpha.powu(n as u32) / 
                                  Self::factorial(n).sqrt();
        ...
        match self.config.method {
            PhotonicMethod::FockBasis | PhotonicMethod::TruncatedFock => {
                ...
                for (i, fock_state) in fock_states.iter().enumerate() {
                    ...
                    let n = fock_state.photons_in_mode(mode);
                    if n % 2 == 0 { // Only even photon numbers for squeezed vacuum
                ...
                // Normalize
                let norm = state_vector.iter().map(|x| x.norm_sqr()).sum::<f64>().sqrt();
                if norm > 1e-15 {
        ...
        match &mut self.state {
            PhotonicState::Fock(state_vector) => {
                ...
                for (i, amplitude) in state_vector.iter().enumerate() {
                    if amplitude.norm() < 1e-15 { continue; }
                    
                    let mut new_fock = fock_states[i].clone();
                    new_fock.photon_numbers[mode] += 1;
                    
                    if let Some(&j) = self.get_state_index(&new_fock, &fock_states) {
            ...
            PhotonicState::Fock(state_vector) => {
                ...
                for (i, amplitude) in state_vector.iter().enumerate() {
                    ...
                    if current_fock.photon_numbers[mode] > 0 {
                        ...
                        if let Some(&j) = self.get_state_index(&new_fock, &fock_states) {
                            let annihilation_factor = (current_fock.photon_numbers[mode] as f64).sqrt();
    ...
    /// Apply beam splitter
    fn apply_beam_splitter(&mut self, mode1: usize, mode2: usize, transmittance: f64) -> Result<()> {
        if mode1 == mode2 {
    ...
    pub fn wigner_function(&self) -> Result<Array2<f64>> {
        match &self.state {
            PhotonicState::Wigner { function_values, .. } => {
    ...
    fn calculate_fock_dimension(&self) -> Result<usize> {
        ...
        // Multinomial coefficient: (n + m - 1) choose (m - 1)
        let dim = Self::binomial_coefficient(n + m - 1, m - 1);
    ...
    fn get_state_index(&self, state: &FockState, fock_states: &[FockState]) -> Option<&usize> {
        self.basis_cache.get(state).or_else(|| {
            fock_states.iter().position(|s| s == state)
        })
    }
    ...
    fn calculate_squeezed_amplitude(&self, n: usize, r: f64, phi: f64) -> Complex64 {
        ...
        let amplitude = sech_r.sqrt() *
                       ...
                       Self::double_factorial(2 * m - 1) / Self::factorial(m) *
                       tanh_r.powf(m as f64);
        
        amplitude
    }
    
    fn build_displacement_matrix(&self, mode: usize, alpha: Complex64) -> Result<Array2<Complex64>> {
        let dim = self.calculate_fock_dimension()?;
    ...
    fn build_beam_splitter_matrix(&self, mode1: usize, mode2: usize, theta: f64) -> Result<Array2<Complex64>> {
        let dim = self.calculate_fock_dimension()?;
        let mut matrix = Array2::eye(dim);
    ...
    fn apply_displacement_to_state(&self, state: &Array1<Complex64>, alpha: Complex64) -> Result<Array1<Complex64>> {
...
impl PhotonicUtils {
    ...
    pub fn cat_state(alpha: Complex64, phase: f64) -> impl Fn(&mut PhotonicSimulator) -> Result<()> {
    ...
    pub fn fidelity(state1: &PhotonicState, state2: &PhotonicState) -> Result<f64> {
        match (state1, state2) {
            (PhotonicState::Fock(s1), PhotonicState::Fock(s2)) => {
                ...
                let overlap: Complex64 = s1.iter().zip(s2.iter())
                    .map(|(a, b)| a.conj() * b)
                    .sum();
    ...
    pub fn von_neumann_entropy(state: &PhotonicState) -> Result<f64> {
        match state {
            PhotonicState::Fock(state_vector) => {
                let probabilities: Vec<f64> = state_vector.iter()
...
mod tests {
    ...
    fn test_creation_operator() {
        ...
        simulator.apply_operator(PhotonicOperator::Creation(0)).unwrap();
    ...
    fn test_photon_number_measurement() {
        ...
        // Create two-photon state
        simulator.apply_operator(PhotonicOperator::Creation(0)).unwrap();
        simulator.apply_operator(PhotonicOperator::Creation(0)).unwrap();
        
        let photon_number = simulator.measure_photon_number(0).unwrap();
        assert_abs_diff_eq!(photon_number, 2.0, epsilon = 1e-10);
    }
