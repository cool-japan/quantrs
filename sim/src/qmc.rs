        ...
        match self {
            WaveFunction::Product(amps) => {
                ...
                for (i, &bit) in config.iter().enumerate() {
                    if i < amps.len() {
                        result *= if bit { amps[i] } else { Complex64::new(1.0, 0.0) - amps[i] };
                    }
                }
                result
            }
...
impl VMC {
    ...
    pub fn run(
        ...
        for step in 0..optimization_steps {
            ...
            for _ in 0..num_samples {
                ...
                if let WaveFunction::Jastrow { .. } = &self.wave_function {
                    for p in 0..2 {
                        ...
                        gradients[p] += (e_loc.re - local_energies.iter().map(|e| e.re).sum::<f64>() / local_energies.len() as f64) * deriv;
                    }
                }
            }
            ...
            let variance = local_energies.iter()
            ...
            if let WaveFunction::Jastrow { ref mut alpha, ref mut beta } = &mut self.wave_function {
            ...
            // Print progress
            if step % 10 == 0 {
                println!("VMC Step {}: E = {:.6} ± {:.6}", step, mean_energy, variance.sqrt());
            }
        }
...
impl DMC {
    ...
    pub fn run(&mut self, num_blocks: usize, steps_per_block: usize) -> Result<DMCResult> {
        ...
        for block in 0..num_blocks {
            ...
            self.reference_energy = avg_energy - (walkers.len() as f64 - self.target_walkers as f64).ln() / self.tau;
            
            if block % 10 == 0 {
                println!("DMC Block {}: E = {:.6}, Walkers = {}", block, avg_energy, walkers.len());
            }
        }
    ...
    fn population_control(&self, walkers: &mut Vec<Walker>) -> Result<()> {
        ...
        if current_size == 0 {
            return Err(SimulatorError::ComputationError("All walkers died".to_string()));
        }
...
mod tests {
    ...
    fn test_vmc_ising() {
        ...
        let wf = WaveFunction::Jastrow { alpha: 0.5, beta: 0.1 };
