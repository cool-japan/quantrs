...
fn test_ghz_state_all_simulators() {
    ...
    circuit
        .h(QubitId::new(0)).unwrap()
        .cnot(QubitId::new(0), QubitId::new(1)).unwrap()
        .cnot(QubitId::new(1), QubitId::new(2)).unwrap();
...
fn test_qft_like_circuit() {
    ...
    assert_state_vector_close(&standard_result.amplitudes(), &simple_opt_result.amplitudes(), 1e-10);
    assert_state_vector_close(&standard_result.amplitudes(), &chunked_opt_result.amplitudes(), 1e-10);
    assert_state_vector_close(&standard_result.amplitudes(), &full_opt_result.amplitudes(), 1e-10);
}
...
fn test_larger_circuit() {
    ...
    for i in 0..(QUBITS - 1) {
        circuit.cnot(QubitId::new(i as u32), QubitId::new((i + 1) as u32)).unwrap();
    }
    
    // Apply some rotations
    for i in 0..QUBITS {
        circuit.rz(QubitId::new(i as u32), std::f64::consts::PI / (i + 1) as f64).unwrap();
    }
    ...
    assert_state_vector_close(&simple_opt_result.amplitudes(), &full_opt_result.amplitudes(), 1e-10);
}
...
fn create_simple_qft_circuit<const N: usize>() -> Circuit<N> {
    ...
    for i in 0..(N-1) {
        circuit.cz(QubitId::new(i as u32), QubitId::new((i+1) as u32)).unwrap();
    }
    
    // Apply some Z rotations
    for i in 0..N {
        circuit.rz(QubitId::new(i as u32), std::f64::consts::PI / 4.0).unwrap();
    }
    
    // Apply SWAP operations at the end
    for i in 0..(N / 2) {
        circuit.swap(QubitId::new(i as u32), QubitId::new((N - i - 1) as u32)).unwrap();
    }
...
fn assert_state_vector_close(actual: &[Complex64], expected: &[Complex64], epsilon: f64) {
    assert_eq!(actual.len(), expected.len(), "State vectors must have the same length");
