...
use crate::specialized_gates::{SpecializedGate, specialize_gate};
    ...
    fn apply_multi_qubit_generic(
        ...
        if matrix.len() != gate_dim * gate_dim {
            return Err(QuantRS2Error::InvalidInput(
                format!("Invalid matrix size for {}-qubit gate", gate_qubits)
            ));
        }
...
pub fn benchmark_specialization(n_qubits: usize, n_gates: usize) -> (f64, f64, SpecializationStats) {
    ...
    for _ in 0..n_gates {
        ...
        match gate_type {
            0 => { circuit.h(qubit); }
            1 => { circuit.x(qubit); }
            2 => { circuit.ry(qubit, rng.gen_range(0.0..std::f64::consts::TAU)); }
            3 => if n_qubits > 1 {
...
mod tests {
    ...
    fn test_benchmark() {
        ...
        println!("Specialized: {:.3}ms, Base: {:.3}ms", spec_time * 1000.0, base_time * 1000.0);
        println!("Stats: {:?}", stats);
