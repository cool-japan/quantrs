...
impl QuantumSampler {
    ...
    pub fn sample_state(&mut self, state: &Array1<Complex64>) -> Result<ShotResult> {
        ...
        if (total_prob - 1.0).abs() > 1e-10 {
            return Err(SimulatorError::InvalidInput(
                format!("State vector not normalized: total probability = {}", total_prob)
            ));
        }
    ...
    fn measure_pauli_basis(&mut self, _state: &Array1<Complex64>, _pauli_string: &PauliString) -> Result<f64> {
    ...
    fn compute_statistics(&self, outcomes: &[BitString]) -> Result<MeasurementStatistics> {
        ...
        for (outcome, &count) in &counts {
            ...
            confidence_intervals.insert(outcome.clone(), (
                (prob - margin).max(0.0),
                (prob + margin).min(1.0),
            ));
        }
        ...
        // Compute entropy
        let entropy = probabilities.values()
            .filter(|&&p| p > 0.0)
            .map(|&p| -p * p.ln())
            .sum::<f64>();
        ...
        let probability_variance = probabilities.values()
    ...
    pub fn estimate_convergence(
        ...
        while shots_taken < self.config.max_shots_for_convergence && !converged {
            ...
            let batch_shots = self.config.convergence_check_interval.min(
                self.config.max_shots_for_convergence - shots_taken
            );
            ...
            if expectation_history.len() >= 3 {
                ...
                let max_diff = recent_values.iter()
        ...
        let expectation_std = if expectation_history.len() > 1 {
            ...
            (expectation_history.iter()
                .map(|x| (x - mean).powi(2))
                .sum::<f64>() / (expectation_history.len() - 1) as f64).sqrt()
        } else {
            0.0
        };
...
pub mod analysis {
    ...
    pub fn required_shots_for_precision(
        desired_error: f64,
    ...
    pub fn compare_shot_results(
        ...
        ComparisonResult {
            chi_square,
            degrees_of_freedom,
            p_value: if chi_square > critical_value { 0.01 } else { 0.1 }, // Rough
