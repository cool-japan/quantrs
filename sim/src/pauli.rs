...
use crate::trotter::{DynamicCircuit, Hamiltonian, HamiltonianTerm, TrotterDecomposer, TrotterMethod};
...
impl PauliOperator {
    ...
    pub fn from_str(s: &str) -> Result<Self> {
        match s.to_uppercase().as_str() {
            ...
            _ => Err(SimulatorError::InvalidInput(format!("Invalid Pauli operator: {}", s))),
        }
    }
    ...
    pub fn multiply(&self, other: &PauliOperator) -> (PauliOperator, Complex64) {
        match (self, other) {
            ...
            (PauliOperator::X, PauliOperator::X) | (PauliOperator::Y, PauliOperator::Y) | (PauliOperator::Z, PauliOperator::Z) => {
                (PauliOperator::I, Complex64::new(1.0, 0.0))
            },
...
impl PauliString {
    ...
    pub fn from_string(pauli_str: &str, coefficient: Complex64) -> Result<Self> {
        let operators: Result<Vec<_>> = pauli_str.chars()
    ...
    pub fn from_ops(num_qubits: usize, ops: &[(usize, PauliOperator)], coefficient: Complex64) -> Result<Self> {
    ...
    pub fn multiply(&self, other: &PauliString) -> Result<PauliString> {
        if self.num_qubits != other.num_qubits {
            return Err(SimulatorError::DimensionMismatch(
                format!("Pauli strings have different lengths: {} vs {}", self.num_qubits, other.num_qubits)
            ));
        }
    ...
    pub fn weight(&self) -> usize {
        self.operators.iter().filter(|&&op| op != PauliOperator::I).count()
    }
        ...
        if non_identity.len() == 1 {
            ...
            match op {
                PauliOperator::X => circuit.add_gate(Box::new(RotationX { 
                    target: QubitId::new(qubit as u32), theta: angle 
                }))?,
            ...
            for &(qubit, op) in &non_identity {
                match op {
                    ...
                    PauliOperator::Y => {
                        circuit.add_gate(Box::new(Hadamard { target: QubitId::new(qubit as u32) }))?;
                        circuit.add_gate(Box::new(Phase { target: QubitId::new(qubit as u32) }))?;
                    },
            ...
            for &(qubit, op) in non_identity.iter().rev() {
                match op {
                    ...
                    PauliOperator::Y => {
                        circuit.add_gate(Box::new(PhaseDagger { target: QubitId::new(qubit as u32) }))?;
                        circuit.add_gate(Box::new(Hadamard { target: QubitId::new(qubit as u32) }))?;
                    },
                    PauliOperator::Z => {}, // No basis change needed
                    PauliOperator::I => {}, // Should not happen
...
impl fmt::Display for PauliString {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "({}) {}", self.coefficient, self.to_string())
    }
}
...
impl PauliOperatorSum {
    ...
    pub fn add_term(&mut self, pauli_string: PauliString) -> Result<()> {
        if pauli_string.num_qubits != self.num_qubits {
            return Err(SimulatorError::DimensionMismatch(
                format!("Pauli string has {} qubits, expected {}", pauli_string.num_qubits, self.num_qubits)
            ));
        }
    ...
    pub fn simplify(&mut self) {
        ...
        for term in &self.terms {
            ...
            *simplified_terms.entry(key).or_insert(Complex64::new(0.0, 0.0)) += term.coefficient;
        }
        ...
        for term in &self.terms {
            ...
            match non_identity.len() {
                ...
                _ => {
                    ...
                    let paulis: Vec<String> = non_identity.iter().map(|&(_, op)| {
                        match op {
...
impl PauliUtils {
    ...
    pub fn random(num_qubits: usize, weight: usize, coeff: Complex64) -> Result<PauliString> {
        if weight > num_qubits {
            return Err(SimulatorError::InvalidInput("Weight cannot exceed number of qubits".to_string()));
        }
...
mod tests {
    ...
    fn test_utils_single_qubit() {
        let pauli = PauliUtils::single_qubit(3, 1, PauliOperator::X, Complex64::new(1.0, 0.0)).unwrap();
