...
use crate::pauli::{PauliOperator, PauliString, PauliOperatorSum};
    ...
    pub fn ordering_key(&self) -> (usize, usize) {
        match self {
            ...
            FermionicOperator::Interaction { sites } => {
                ...
                sorted_sites.sort();
                (4, sorted_sites[0] * 1000000 + sorted_sites[1] * 10000 + sorted_sites[2] * 100 + sorted_sites[3])
            },
        }
    }
}
...
impl FermionicString {
    ...
    pub fn new(operators: Vec<FermionicOperator>, coefficient: Complex64, num_modes: usize) -> Self {
        Self {
            operators,
            coefficient,
            num_modes,
    ...
    pub fn single_operator(op: FermionicOperator, coefficient: Complex64, num_modes: usize) -> Self {
    ...
    pub fn annihilation(site: usize, coefficient: Complex64, num_modes: usize) -> Self {
        Self::single_operator(FermionicOperator::Annihilation(site), coefficient, num_modes)
    }
    ...
    fn canonicalize_operators(&self, ops: &[FermionicOperator]) -> Result<(Vec<FermionicOperator>, Complex64)> {
    ...
    fn apply_fermionic_algebra(&self, ops: &[FermionicOperator]) -> Result<Vec<FermionicOperator>> {
        ...
        while i < ops.len() {
            if i + 1 < ops.len() {
                match (&ops[i], &ops[i + 1]) {
                    // c†_i c_i = n_i
                    (FermionicOperator::Creation(a), FermionicOperator::Annihilation(b)) if a == b => {
                        result.push(FermionicOperator::Number(*a));
                        i += 2;
                        continue;
                    },
                    // c_i c_i = 0 (skip both)
                    (FermionicOperator::Annihilation(a), FermionicOperator::Annihilation(b)) if a == b => {
    ...
    pub fn hermitian_conjugate(&self) -> FermionicString {
        ...
        for op in self.operators.iter().rev() {
            let conjugate_op = match op {
                ...
                FermionicOperator::Hopping { from, to } => FermionicOperator::Hopping { from: *to, to: *from },
                FermionicOperator::Interaction { sites } => {
...
impl FermionicHamiltonian {
    ...
    pub fn hubbard_model(
        ...
        for i in 0..num_modes {
            let mu_term = FermionicString::number(
                i,
                Complex64::new(-chemical_potential, 0.0),
                num_modes,
            );
...
impl JordanWignerTransform {
    ...
    fn creation_to_pauli(&self, site: usize) -> Result<PauliString> {
        ...
        let ops: Vec<(usize, PauliOperator)> = paulis.iter().enumerate()
...
impl FermionicSimulator {
    ...
    fn evolve_pauli_hamiltonian(&mut self, _hamiltonian: &PauliOperatorSum, _time: f64) -> Result<()> {
...
mod tests {
    ...
    fn test_fermionic_simulator() {
        ...
        simulator.set_initial_state(&[true, false, false, false]).unwrap();
