...
impl Hamiltonian {
    ...
    pub fn add_two_pauli(
        ...
        if qubit1 >= self.num_qubits || qubit2 >= self.num_qubits {
            return Err(SimulatorError::InvalidInput("Qubit index out of bounds".to_string()));
        }
    ...
    fn term_to_circuit(&self, term: &HamiltonianTerm, time: f64) -> Result<DynamicCircuit> {
        ...
        match term {
            HamiltonianTerm::SinglePauli { qubit, pauli, coefficient } => {
                let angle = -2.0 * coefficient * time;
                match pauli.as_str() {
                    "X" => circuit.add_gate(Box::new(RotationX { target: QubitId::new(*qubit as u32), theta: angle }))?,
                    "Y" => circuit.add_gate(Box::new(RotationY { target: QubitId::new(*qubit as u32), theta: angle }))?,
                    "Z" => circuit.add_gate(Box::new(RotationZ { target: QubitId::new(*qubit as u32), theta: angle }))?,
                    _ => return Err(SimulatorError::InvalidInput(
                        format!("Unknown Pauli operator: {}", pauli)
                    )),
                }
            }
            HamiltonianTerm::TwoPauli { qubit1, qubit2, pauli1, pauli2, coefficient } => {
                ...
                // ZZ rotation
                circuit.add_gate(Box::new(CNOT { control: QubitId::new(*qubit1 as u32), target: QubitId::new(*qubit2 as u32) }))?;
                circuit.add_gate(Box::new(RotationZ { target: QubitId::new(*qubit2 as u32), theta: angle }))?;
                circuit.add_gate(Box::new(CNOT { control: QubitId::new(*qubit1 as u32), target: QubitId::new(*qubit2 as u32) }))?;
            ...
            HamiltonianTerm::PauliString { qubits, paulis, coefficient } => {
                let angle = -2.0 * coefficient * time;
                ...
                circuit.add_gate(Box::new(RotationZ { target: QubitId::new(qubits[qubits.len() - 1] as u32), theta: angle }))?;
...
impl TrotterDecomposer {
    ...
    pub fn error_bound(
        ...
        let coeff_sum: f64 = hamiltonian.terms.iter()
            .map(|term| match term {
...
mod tests {
    ...
    fn test_hamiltonian_construction() {
        ...
        ham.add_pauli_string(vec![0, 1, 2], vec!["X".to_string(), "Y".to_string(), "Z".to_string()], 0.25).unwrap();
