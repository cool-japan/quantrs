...
fn config_demo() -> Result<()> {
    ...
    println!("   Large model: {} params, {} heads, {} layers",
    ...
    println!("   Custom model: {} dim, {} qubits, {:?} attention",
    ...
    println!("   Created transformer with {} total parameters", transformer.num_parameters());
...
fn attention_mechanisms_demo() -> Result<()> {
    ...
    let attention_types = vec![
        ...
        ("Hybrid Quantum-Classical", QuantumAttentionType::HybridQuantumClassical),
        ("Variational Quantum", QuantumAttentionType::VariationalQuantum),
        ("Quantum Enhanced Multi-Head", QuantumAttentionType::QuantumEnhancedMultiHead),
        ("Quantum Self-Attention", QuantumAttentionType::QuantumSelfAttention),
    ];
    ...
    for (name, attention_type) in attention_types {
        ...
        println!("   Created attention module: {} heads, {} model dim", 
            attention.num_heads, attention.model_dim);
        ...
        println!("   Attention output shape: {:?}", attention_output.output.dim());
        println!("   Attention weights shape: {:?}", attention_output.attention_weights.dim());
        ...
        let max_coherence = quantum_info.coherence_scores.iter()
            .cloned()
        ...
        let max_attention = attention_weights.iter()
            .cloned()
            .fold(f64::NEG_INFINITY, f64::max);
        ...
        let sparsity = attention_weights.iter()
            .filter(|&&x| x < 0.01)
            .count() as f64 / attention_weights.len() as f64;
...
fn position_encoding_demo() -> Result<()> {
    ...
    for (name, encoding_type) in encoding_types {
        ...
        let pos_enc = QuantumPositionEncoding::new(
            encoding_type, model_dim, max_seq_len, num_qubits
        )?;
        ...
        println!("   Distance between first and last position: {:.4}", position_distance);
        ...
        if name == "Sinusoidal" {
            ...
            if !periodicities.is_empty() {
                let avg_period = periodicities.iter().sum::<usize>() as f64 / periodicities.len() as f64;
...
fn transformer_forward_demo() -> Result<()> {
    ...
    println!("   Created transformer: {} layers, {} parameters", 
        config.num_layers, transformer.num_parameters());
    ...
    for (batch_size, seq_len, input_dim) in test_sequences {
        ...
        println!("   Output statistics: mean={:.4}, std={:.4}, range={:.4}", 
        ...
        let memory_per_token = (transformer.num_parameters() * 8 + 
                               output.len() * 8) as f64 / (batch_size * seq_len) as f64;
...
fn language_modeling_demo() -> Result<()> {
    ...
    let input_tokens = Array3::from_shape_fn((batch_size, seq_len, config.model_dim), |(b, s, d)| {
    ...
    println!("   Processing {} sequences of length {}", batch_size, seq_len);
    ...
    for batch_idx in 0..batch_size {
        ...
        for pos in 0..seq_len-1 {
            ...
            let max_logit = current_logits.iter().cloned().fold(f64::NEG_INFINITY, f64::max);
    ...
    if !perplexities.is_empty() {
        ...
        println!("   - {} layers provide hierarchical representation", config.num_layers);
        
        // Information flow analysis
        let first_layer_norm = logits.slice(ndarray::s![0, .., ..]).var(0.0).sqrt();
        println!("   - Output layer standard deviation: {:.4}", first_layer_norm);
        
        // Quantum coherence in language representation
        let quantum_coherence = analyze_quantum_language_coherence(&logits)?;
        println!("   - Quantum coherence in representations: {:.4}", quantum_coherence);
    }
...
fn seq2seq_demo() -> Result<()> {
    ...
    println!("\n   Processing translation: {} -> {} tokens", src_len, tgt_len);
    ...
    println!("   Cross-attention shape: {:?}", cross_attention_scores.dim());
    
    // Analyze attention alignment
    let max_alignment = cross_attention_scores.iter()
        .cloned()
        .fold(f64::NEG_INFINITY, f64::max);
    ...
    let cross_lingual_entanglement = analyze_cross_lingual_entanglement(&encoder_output, &decoder_output)?;
    println!("   Cross-lingual quantum entanglement: {:.4}", cross_lingual_entanglement);
...
fn quantum_data_demo() -> Result<()> {
    ...
    let quantum_data = Array3::from_shape_fn((batch_size, seq_len, model_dim), |(b, t, d)| {
        ...
        let quantum_amplitude = decoherence_factor * 
            (2.0 * std::f64::consts::PI * t as f64 / 8.0 + b as f64).sin();
    ...
    println!("   Processing {} quantum sequences of {} measurements each", 
        batch_size, seq_len);
    ...
    println!("   Coherence preservation: {:.1}%", coherence_preservation * 100.0);
    
    // Quantum information extraction
    let quantum_features = extract_quantum_features(&output)?;
    println!("   Extracted quantum features:");
    println!("   - Entanglement signature: {:.4}", quantum_features.entanglement);
    println!("   - Phase coherence: {:.4}", quantum_features.phase_coherence);
    println!("   - Amplitude stability: {:.4}", quantum_features.amplitude_stability);
    ...
    println!("   - Time constant: {:.2} steps", decoherence_pattern.time_constant);
    ...
    for (i, classification) in state_classifications.iter().enumerate() {
        println!("   - Sequence {}: {:.1}% entangled, {:.1}% coherent", 
            i, classification.entangled_prob * 100.0, classification.coherent_prob * 100.0);
    }
...
fn multiscale_demo() -> Result<()> {
    ...
    let scales = vec![
        ("Fine-scale", QuantumTransformerConfig {
            model_dim: 128,
            num_heads: 4,
            ff_dim: 512,
            num_layers: 2,
        ...
        ("Medium-scale", QuantumTransformerConfig {
            model_dim: 256,
            num_heads: 8,
            ff_dim: 1024,
            num_layers: 4,
        ...
        ("Coarse-scale", QuantumTransformerConfig {
            model_dim: 512,
            num_heads: 16,
            ff_dim: 2048,
            num_layers: 6,
    ...
    for (scale_name, transformer) in &transformers {
        ...
        println!("   - Local patterns: {:.4}", pattern_analysis.local_strength);
        println!("   - Global patterns: {:.4}", pattern_analysis.global_strength);
        println!("   - Cross-scale coherence: {:.4}", pattern_analysis.coherence);
    }
    
    // Multi-scale fusion
    println!("\n   Multi-scale fusion analysis:");
    let fusion_result = fuse_multiscale_outputs(&scale_outputs)?;
    println!("   Fused representation dimensions: {} features", fusion_result.len());
    
    let fusion_quality = evaluate_fusion_quality(&fusion_result)?;
    println!("   Fusion quality metrics:");
    println!("   - Information preservation: {:.1}%", fusion_quality.info_preservation * 100.0);
    println!("   - Scale consistency: {:.1}%", fusion_quality.scale_consistency * 100.0);
    println!("   - Quantum coherence: {:.4}", fusion_quality.quantum_coherence);
...
fn extract_quantum_features(data: &Array3<f64>) -> Result<QuantumFeatures> {
    ...
    let phase_coherence = 1.0 - data.mapv(|x| (x * std::f64::consts::PI).sin().abs()).mean().unwrap_or(0.0);
...
fn detect_decoherence_pattern(data: &Array3<f64>) -> Result<DecoherencePattern> {
    ...
    for t in 0..seq_len {
        let slice_norm = data.slice(ndarray::s![.., t, ..]).mapv(|x| x * x).sum().sqrt();
...
fn classify_quantum_states(data: &Array3<f64>) -> Result<Vec<StateClassification>> {
    ...
    for b in 0..batch_size {
        ...
        let entanglement_measure = sequence.var(0.0) / (sequence.mean().unwrap_or(1.0).abs() + 1e-10);
        let entangled_prob = (1.0 / (1.0 + (-5.0 * entanglement_measure).exp())).min(1.0);
        
        let coherence_measure = 1.0 - sequence.mapv(|x| (x * std::f64::consts::PI).sin().abs()).mean().unwrap_or(0.0);
...
fn adapt_input_for_scale(input: &Array3<f64>, config: &QuantumTransformerConfig) -> Result<Array3<f64>> {
...
fn analyze_scale_patterns(data: &Array3<f64>) -> Result<PatternAnalysis> {
    ...
    for s in 0..seq_len-1 {
        ...
        let correlation = current.dot(&next) / (model_dim as f64).sqrt();
    ...
    for s in 0..seq_len-step {
        ...
        let correlation = current.dot(&distant) / (model_dim as f64).sqrt();
...
fn fuse_multiscale_outputs(outputs: &[(&str, Array3<f64>)]) -> Result<Array1<f64>> {
    ...
    for (_, output) in outputs {
        ...
        let feature_vector = output.mean_axis(Axis(0)).unwrap().mean_axis(Axis(0)).unwrap();
...
fn evaluate_fusion_quality(fused: &Array1<f64>) -> Result<FusionQuality> {
    ...
    let quantum_coherence = fused.mapv(|x| (x * std::f64::consts::PI).cos().abs()).mean().unwrap_or(0.0);
