...
fn compare_noise_schedules() -> Result<()> {
    ...
    let schedules = vec![
        ("Linear", NoiseSchedule::Linear { beta_start: 0.0001, beta_end: 0.02 }),
        ("Cosine", NoiseSchedule::Cosine { s: 0.008 }),
        ("Quadratic", NoiseSchedule::Quadratic { beta_start: 0.0001, beta_end: 0.02 }),
        ("Sigmoid", NoiseSchedule::Sigmoid { beta_start: 0.0001, beta_end: 0.02 }),
    ];
...
fn score_diffusion_demo() -> Result<()> {
    ...
    println!("   - After {} steps: [{:.3}, {:.3}]", num_steps, sample[0], sample[1]);
    println!("   - Distance moved: {:.3}", 
...
fn advanced_diffusion_demo() -> Result<()> {
    ...
    println!("   Generated {} conditional samples", conditional_samples.nrows());
    ...
    println!("   - Latent log_var: [{:.3}, {:.3}]", log_var[0], log_var[1]);
