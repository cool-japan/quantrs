...
fn ewc_demo() -> Result<()> {
    ...
    for (i, task) in tasks.iter().enumerate() {
        ...
        if i > 0 {
            ...
            let avg_prev_accuracy = all_accuracies.iter()
                .take(i)
                .map(|(_, &acc)| acc)
                .sum::<f64>() / i as f64;
            
            println!("   - Average accuracy on previous tasks: {:.3}", avg_prev_accuracy);
        }
    }
    ...
    println!("\n   EWC Results:");
    println!("   - Average accuracy: {:.3}", forgetting_metrics.average_accuracy);
    println!("   - Forgetting measure: {:.3}", forgetting_metrics.forgetting_measure);
    println!("   - Continual learning score: {:.3}", forgetting_metrics.continual_learning_score);
...
fn experience_replay_demo() -> Result<()> {
    ...
    println!("   - Final average accuracy: {:.3}", final_metrics.average_accuracy);
    println!("   - Forgetting reduction: {:.3}", 1.0 - final_metrics.forgetting_measure);
...
fn lwf_demo() -> Result<()> {
    ...
    for (i, task) in tasks.iter().enumerate() {
        ...
        if i > 0 {
            ...
            let all_accuracies = learner.evaluate_all_tasks()?;
            let stability = all_accuracies.values()
                .map(|&acc| if acc > 0.6 { 1.0 } else { 0.0 })
                .sum::<f64>() / all_accuracies.len() as f64;
...
fn parameter_isolation_demo() -> Result<()> {
    ...
    for (i, task) in tasks.iter().enumerate() {
        ...
        println!("   - Parameter efficiency: {:.1}%", used_params as f64 / 16.0 * 100.0);
        
        if i > 0 {
            let all_accuracies = learner.evaluate_all_tasks()?;
            let interference = 1.0 - all_accuracies.values()
                .take(i)
...
fn task_sequence_demo() -> Result<()> {
    ...
    let strategies = vec![
        ("EWC", ContinualLearningStrategy::ElasticWeightConsolidation {
            importance_weight: 500.0,
            fisher_samples: 100,
        }),
        ("Experience Replay", ContinualLearningStrategy::ExperienceReplay {
            buffer_size: 300,
            replay_ratio: 0.2,
            memory_selection: MemorySelectionStrategy::Random,
        }),
        ("Quantum Regularization", ContinualLearningStrategy::QuantumRegularization {
    ...
    println!("\n   Comparing strategies on {} challenging tasks:", tasks.len());
    ...
    for (strategy_name, strategy) in strategies {
        ...
        let final_metrics = learner.get_forgetting_metrics();
        println!("   - Average accuracy: {:.3}", final_metrics.average_accuracy);
        println!("   - Forgetting measure: {:.3}", final_metrics.forgetting_measure);
        println!("   - CL score: {:.3}", final_metrics.continual_learning_score);
    }
...
fn forgetting_analysis_demo() -> Result<()> {
    ...
    for (i, task) in tasks.iter().enumerate() {
        println!("   \n   Learning {} (difficulty level {})...", task.task_id, i + 1);
        ...
        accuracy_matrix.push(current_row);
    ...
    if let Some(final_row) = accuracy_matrix.last() {
        ...
        println!("   - Continual learning effectiveness: {:.1}%", 
...
fn generate_diverse_tasks(num_tasks: usize, samples_per_task: usize, feature_dim: usize) -> Vec<ContinualTask> {
    ...
    for i in 0..num_tasks {
        ...
        let labels = Array1::from_shape_fn(samples_per_task, |row| {
            ...
            if features_sum > feature_dim as f64 * 0.5 { 1 } else { 0 }
        });
...
/// Generate related tasks for transfer learning
fn generate_related_tasks(num_tasks: usize, samples_per_task: usize, feature_dim: usize) -> Vec<ContinualTask> {
    let mut tasks = Vec::new();
    ...
    for i in 0..num_tasks {
        ...
        let labels = Array1::from_shape_fn(samples_per_task, |row| {
            let correlation = data.row(row).iter().zip(base_pattern.iter())
                .map(|(&x, &y)| x * y)
                .sum::<f64>();
            if correlation > 0.5 { 1 } else { 0 }
        });
...
fn generate_varying_complexity_tasks(num_tasks: usize, samples_per_task: usize, feature_dim: usize) -> Vec<ContinualTask> {
    let mut tasks = Vec::new();
    ...
    for i in 0..num_tasks {
        ...
        let data = Array2::from_shape_fn((samples_per_task, feature_dim), |(row, col)| {
            ...
            let value = match i {
                0 => if x > 0.5 { 1.0 } else { 0.0 }, // Simple linear
                1 => if x * x + y * y > 0.25 { 1.0 } else { 0.0 }, // Circular
                2 => if (x * 4.0).sin() * (y * 4.0).cos() > 0.0 { 1.0 } else { 0.0 }, // Sinusoidal
                _ => {
                    // Very complex pattern
                    let pattern = (x * 8.0).sin() * (y * 8.0).cos() + (x * y * 16.0).sin();
                    if pattern > 0.0 { 1.0 } else { 0.0 }
                }
            };
        ...
        let labels = Array1::from_shape_fn(samples_per_task, |row| {
            ...
            let decision_value = features.iter()
                .enumerate()
                .map(|(j, &x)| x * (1.0 + j as f64 * complexity * 0.1))
                .sum::<f64>();
            
            if decision_value > feature_dim as f64 * 0.5 { 1 } else { 0 }
        });
...
/// Generate challenging task sequence
fn generate_challenging_sequence(num_tasks: usize, samples_per_task: usize, feature_dim: usize) -> Vec<ContinualTask> {
    let mut tasks = Vec::new();
    ...
    for i in 0..num_tasks {
        ...
        let labels = Array1::from_shape_fn(samples_per_task, |row| {
            let features = data.row(row);
            match challenge_type {
                0 => if features.sum() > feature_dim as f64 * 0.5 { 1 } else { 0 },
                1 => if features[0] > 0.5 { 1 } else { 0 },
                2 => if features.iter().enumerate().map(|(j, &x)| x * (j as f64 + 1.0)).sum::<f64>() > 2.0 { 1 } else { 0 },
                _ => if features.iter().filter(|&&x| x > 0.5).count() > feature_dim / 2 { 1 } else { 0 },
            }
        });
...
fn generate_increasing_difficulty_tasks(num_tasks: usize, samples_per_task: usize, feature_dim: usize) -> Vec<ContinualTask> {
    let mut tasks = Vec::new();
    ...
    for i in 0..num_tasks {
        ...
        let data = Array2::from_shape_fn((samples_per_task, feature_dim), |(row, col)| {
            ...
            let base_pattern = (x * pattern_complexity * 3.14159).sin() * 
                             (y * pattern_complexity * 3.14159).cos();
        ...
        let labels = Array1::from_shape_fn(samples_per_task, |row| {
            ...
            if decision_value > threshold { 1 } else { 0 }
        });
