...
fn test_pendulum_dynamics() -> Result<()> {
    ...
    for (i, action) in actions.iter().enumerate() {
        ...
        println!("     State: [θ_cos={:.3}, θ_sin={:.3}, θ_dot={:.3}]", 
            state[0], state[1], state[2]);
        println!("     Next: [θ_cos={:.3}, θ_sin={:.3}, θ_dot={:.3}]", 
...
fn test_trained_agent(
    ...
    for episode in 0..test_episodes {
        ...
        println!("   Test episode {}: Reward = {:.2}, Steps = {}", 
            episode + 1, episode_reward, steps);
    }
...
fn custom_environment_demo() -> Result<()> {
    ...
    impl ContinuousEnvironment for Navigation2D {
        ...
        fn step(&mut self, action: Array1<f64>) -> Result<(Array1<f64>, f64, bool)> {
            ...
            let distance = ((self.position[0] - self.goal[0]).powi(2) + 
                          (self.position[1] - self.goal[1]).powi(2)).sqrt();
    ...
    for i in 0..3 {
        ...
        println!("   Step {}: action=[{:.2}, {:.2}], pos=[{:.2}, {:.2}], reward={:.2}, done={}",
            i + 1, action[0], action[1], next_state[0], next_state[1], reward, done);
    }
