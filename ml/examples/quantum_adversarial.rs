...
fn adversarial_attack_demo() -> Result<()> {
    ...
    println!("   Created quantum model with {} parameters", trainer.model.parameters.len());
    ...
    let fgsm_success_rate = fgsm_examples.iter()
        .map(|ex| if ex.attack_success { 1.0 } else { 0.0 })
        .sum::<f64>() / fgsm_examples.len() as f64;
    
    println!("     Success rate: {:.2}%", fgsm_success_rate * 100.0);
    
    if let Some(example) = fgsm_examples.first() {
        println!("     Average perturbation: {:.4}", example.perturbation_norm);
    }
    ...
    let pgd_examples = trainer.generate_adversarial_examples(
        &test_data,
        &test_labels,
        QuantumAttackType::PGD { epsilon: 0.1, alpha: 0.01, num_steps: 10 },
    )?;
    
    let pgd_success_rate = pgd_examples.iter()
        .map(|ex| if ex.attack_success { 1.0 } else { 0.0 })
        .sum::<f64>() / pgd_examples.len() as f64;
    ...
    let param_success_rate = param_examples.iter()
        .map(|ex| if ex.attack_success { 1.0 } else { 0.0 })
        .sum::<f64>() / param_examples.len() as f64;
    ...
    let state_success_rate = state_examples.iter()
        .map(|ex| if ex.attack_success { 1.0 } else { 0.0 })
        .sum::<f64>() / state_examples.len() as f64;
...
fn defense_mechanisms_demo() -> Result<()> {
    ...
    let model = QuantumNeuralNetwork::new(layers, 4, 4, 2)?;
    let config = create_default_adversarial_config();
    let trainer = QuantumAdversarialTrainer::new(model, preprocessing_defense, config);
...
fn adversarial_training_demo() -> Result<()> {
    ...
    let defense = QuantumDefenseStrategy::AdversarialTraining {
        attack_types: vec![
            ...
            QuantumAttackType::PGD { epsilon: 0.08, alpha: 0.01, num_steps: 7 },
        ],
    ...
    println!("   - Attack success rate: {:.3}", metrics.attack_success_rate);
...
fn robustness_evaluation_demo() -> Result<()> {
    ...
    for &epsilon in &epsilons {
        ...
        let success_rate = attack_examples.iter()
            .map(|ex| if ex.attack_success { 1.0 } else { 0.0 })
            .sum::<f64>() / attack_examples.len() as f64;
        
        let avg_perturbation = attack_examples.iter()
            .map(|ex| ex.perturbation_norm)
            .sum::<f64>() / attack_examples.len() as f64;
        
        println!("   ε = {:.2}: Attack success = {:.1}%, Avg perturbation = {:.4}",
            epsilon, success_rate * 100.0, avg_perturbation);
    }
    ...
    let attack_types = vec![
        ...
        ("PGD", QuantumAttackType::PGD { epsilon: 0.1, alpha: 0.01, num_steps: 10 }),
        ("Parameter Shift", QuantumAttackType::ParameterShift { 
            shift_magnitude: 0.05, target_parameters: None 
        }),
        ("State Perturbation", QuantumAttackType::StatePerturbation { 
            perturbation_strength: 0.1, basis: "pauli_z".to_string() 
        }),
    ];
...
fn attack_comparison_demo() -> Result<()> {
    ...
    let attack_configs = vec![
        ...
        ("PGD-5", QuantumAttackType::PGD { epsilon: 0.1, alpha: 0.02, num_steps: 5 }),
        ("PGD-10", QuantumAttackType::PGD { epsilon: 0.1, alpha: 0.01, num_steps: 10 }),
        ("Parameter Shift", QuantumAttackType::ParameterShift { 
            shift_magnitude: 0.1, target_parameters: None 
        }),
        ("Circuit Manipulation", QuantumAttackType::CircuitManipulation { 
            gate_error_rate: 0.01, coherence_time: 100.0 
        }),
    ];
    
    println!("\n   Attack effectiveness comparison:");
    println!("   {:20} {:>12} {:>15} {:>15}", "Attack Type", "Success Rate", "Avg Perturbation", "Effectiveness");
    ...
    for (name, attack_type) in attack_configs {
        ...
        let avg_perturbation = examples.iter()
            .map(|ex| ex.perturbation_norm)
            .sum::<f64>() / examples.len() as f64;
        ...
        println!("   {:20} {:>11.1}% {:>14.4} {:>14.2}",
            name, success_rate * 100.0, avg_perturbation, effectiveness);
    }
...
fn ensemble_defense_demo() -> Result<()> {
    ...
    println!("   - Single model attack success: {}", 
        if single_model_attack.attack_success { "Yes" } else { "No" });
    println!("   - Perturbation magnitude: {:.4}", single_model_attack.perturbation_norm);
