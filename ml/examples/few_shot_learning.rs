...
fn main() -> Result<()> {
    ...
    for class_id in 0..num_classes {
        for sample_idx in 0..num_samples_per_class {
            ...
            for feat in 0..num_features {
                data[[idx, feat]] = (class_id as f64 * 0.5 + feat as f64 * 0.3 + 
                                   sample_idx as f64 * 0.1).sin() + 
    ...
    println!("   Dataset created: {} samples, {} features, {} classes",
...
fn test_prototypical_networks(
    ...
    println!("   - Final accuracy: {:.2}%", accuracies.last().unwrap() * 100.0);
...
fn test_maml(
    ...
    for _ in 0..num_tasks {
        let task = FewShotLearner::generate_episode(
            data,
            labels,
            3,  // 3-way (fewer classes for MAML)
            5,  // 5-shot
            5,  // 5 query examples
        )?;
        tasks.push(task);
