...
fn feature_attribution_demo() -> Result<()> {
    ...
    println!("   Created quantum model with {} parameters", model.parameters.len());
    
    // Test different attribution methods
    let attribution_methods = vec![
        ("Integrated Gradients", ExplanationMethod::QuantumFeatureAttribution {
            method: AttributionMethod::IntegratedGradients,
            num_samples: 50,
            baseline: Some(Array1::zeros(4)),
        }),
        ("Gradient × Input", ExplanationMethod::QuantumFeatureAttribution {
            method: AttributionMethod::GradientInput,
            num_samples: 1,
            baseline: None,
        }),
        ("Gradient SHAP", ExplanationMethod::QuantumFeatureAttribution {
            method: AttributionMethod::GradientSHAP,
            num_samples: 30,
            baseline: None,
        }),
        ("Quantum Attribution", ExplanationMethod::QuantumFeatureAttribution {
        ...
        if let Some(ref attributions) = explanation.feature_attributions {
            ...
            for (i, &attr) in attributions.iter().enumerate() {
                println!("     Feature {}: {:+.4} {}", 
                    i, attr,
                    if attr.abs() > 0.1 {
                        if attr > 0.0 { "(strong positive)" } else { "(strong negative)" }
                    } else {
                        "(weak influence)"
                    });
            }
            ...
            let max_idx = attributions.iter()
                .enumerate()
                .max_by(|a, b| a.1.abs().partial_cmp(&b.1.abs()).unwrap())
                .map(|(i, _)| i)
                .unwrap_or(0);
            
            println!("     → Most important feature: Feature {} ({:.4})", 
    ...
    if let Some(ref circuit) = explanation.circuit_explanation {
        ...
        for (i, &importance) in circuit.parameter_importance.iter().enumerate() {
            if importance > 0.5 {
                ...
                println!("     Parameter {}: {:.3} (medium importance)", i, importance);
            }
        }
        ...
        for (i, layer_analysis) in circuit.layer_analysis.iter().enumerate() {
            println!("     Layer {}: {}", i, format_layer_type(&layer_analysis.layer_type));
            println!("       Information gain: {:.3}", layer_analysis.information_gain);
            println!("       Entanglement generated: {:.3}", layer_analysis.entanglement_generated);
...
fn quantum_state_demo() -> Result<()> {
    ...
    for (i, input) in test_inputs.iter().enumerate() {
        ...
        if let Some(ref state) = explanation.state_properties {
            ...
            println!("     - Entanglement entropy: {:.3}", state.entanglement_entropy);
            ...
            let max_component = state.superposition_components.iter()
                .cloned()
            ...
            let most_likely = state.measurement_probabilities.iter()
                .enumerate()
                .max_by(|a, b| a.1.partial_cmp(b.1).unwrap())
                .map(|(idx, &prob)| (idx, prob))
                .unwrap_or((0, 0.0));
            
            println!("     - Most likely outcome: state {} with prob {:.3}", 
                most_likely.0, most_likely.1);
            
            // State fidelities
            if let Some(highest_fidelity) = state.state_fidelities.values()
                .cloned()
                .fold(None, |acc, x| Some(acc.map_or(x, |y| f64::max(x, y)))) {
                println!("     - Highest basis state fidelity: {:.3}", highest_fidelity);
            }
...
fn saliency_mapping_demo() -> Result<()> {
    ...
    let perturbation_methods = vec![
        ("Gaussian Noise", PerturbationMethod::Gaussian { sigma: 0.1 }),
        ("Quantum Phase", PerturbationMethod::QuantumPhase { magnitude: 0.2 }),
        ("Feature Masking", PerturbationMethod::FeatureMasking),
        ("Parameter Perturbation", PerturbationMethod::ParameterPerturbation { strength: 0.1 }),
    ];
    ...
    for (method_name, perturbation_method) in perturbation_methods {
        ...
        if let Some(ref saliency) = explanation.saliency_map {
            ...
            for output_idx in 0..saliency.ncols() {
                ...
                for input_idx in 0..saliency.nrows() {
                    ...
                    if saliency_score > 0.1 {
                        println!("       Feature {} → Output {}: {:.3} (important)", 
                            input_idx, output_idx, saliency_score);
                    } else if saliency_score > 0.05 {
                        println!("       Feature {} → Output {}: {:.3} (moderate)", 
                            input_idx, output_idx, saliency_score);
                    }
                }
            }
            ...
            println!("     → Most salient: Feature {} → Output {} ({:.3})", 
...
fn quantum_lime_demo() -> Result<()> {
    ...
    for (model_name, local_model) in local_models {
        ...
        if let Some(ref attributions) = explanation.feature_attributions {
            ...
            println!("     → Local explanation complexity: {:.3}", local_complexity);
        }
    }
...
fn quantum_shap_demo() -> Result<()> {
    ...
    for (i, input) in test_inputs.iter().enumerate() {
        ...
        if let Some(ref shap_values) = explanation.feature_attributions {
            ...
            // Feature ranking
            let mut indexed_shap: Vec<(usize, f64)> = shap_values.iter()
                .enumerate()
            ...
            for (rank, (feature_idx, abs_value)) in indexed_shap.iter().enumerate() {
                ...
                println!("     {}. Feature {}: {:.4} (|{:.4}|)", 
                    rank + 1, feature_idx, original_value, abs_value);
            }
            
            // SHAP properties
            println!("     → SHAP values satisfy efficiency property (sum to prediction difference)");
...
fn quantum_lrp_demo() -> Result<()> {
    ...
    let lrp_rules = vec![
        ...
        ("Alpha-Beta Rule", LRPRule::AlphaBeta { alpha: 2.0, beta: 1.0 }),
    ...
    for (rule_name, lrp_rule) in lrp_rules {
        ...
        if let Some(ref relevance) = explanation.feature_attributions {
            ...
            for (i, &rel) in relevance.iter().enumerate() {
                ...
                println!("     Feature {}: {:.4} ({:.1}% of total relevance)", 
                    i, rel, percentage);
            }
...
fn comprehensive_explanation_demo() -> Result<()> {
    ...
    // Add concept vectors
    xai.add_concept("pattern_A".to_string(), Array1::from_vec(vec![1.0, 0.0, 1.0, 0.0]));
    xai.add_concept("pattern_B".to_string(), Array1::from_vec(vec![0.0, 1.0, 0.0, 1.0]));
    
    // Set background data
    let background_data = Array2::from_shape_fn((30, 4), |(i, j)| {
        0.3 + 0.4 * ((i * j) as f64 * 0.15).sin()
    });
    xai.set_background_data(background_data);
    ...
    if let Some(ref circuit) = explanation.circuit_explanation {
        ...
        println!("   - Number of analyzed layers: {}", circuit.layer_analysis.len());
    ...
    if let Some(ref state) = explanation.state_properties {
        println!("\n   Quantum State Properties:");
        println!("   - Entanglement entropy: {:.3}", state.entanglement_entropy);
        println!("   - Coherence measures: {} types", state.coherence_measures.len());
        
        let max_measurement_prob = state.measurement_probabilities.iter()
            .cloned()
            .fold(f64::NEG_INFINITY, f64::max);
        println!("   - Max measurement probability: {:.3}", max_measurement_prob);
    }
    ...
    if let Some(ref concepts) = explanation.concept_activations {
        ...
        for (concept, &activation) in concepts {
            ...
            println!("   - {}: {:.3} ({} similarity)", concept, activation, similarity);
        }
    }
    ...
    if let Some(ref attributions) = explanation.feature_attributions {
        let max_attr_idx = attributions.iter()
            .enumerate()
            .max_by(|a, b| a.1.abs().partial_cmp(&b.1.abs()).unwrap())
            .map(|(i, _)| i)
            .unwrap_or(0);
        
        println!("   • Most influential feature: Feature {} ({:.3})", 
    ...
    if let Some(ref state) = explanation.state_properties {
        ...
        let coherence_level = state.coherence_measures.values()
            .cloned()
            .fold(0.0, f64::max);
    ...
    if let Some(ref concepts) = explanation.concept_activations {
        if let Some((&max_activation, best_concept)) = concepts.iter()
            .max_by(|a, b| a.1.partial_cmp(b.1).unwrap()) {
