    ...
    for (i, task) in tasks.iter().take(3).enumerate() {
        if let Some(amplitude) = task.metadata.get("amplitude") {
            if let Some(phase) = task.metadata.get("phase") {
                println!("   Task {}: amplitude={:.2}, phase={:.2}", i, amplitude, phase);
            }
        }
    }
...
fn metasgd_demo() -> Result<()> {
    ...
    let algorithm = MetaLearningAlgorithm::MetaSGD {
        inner_steps: 3,
    };
...
fn task_distribution_demo() -> Result<()> {
    ...
    let angles: Vec<f64> = rotation_tasks.iter()
    ...
    let amplitudes: Vec<f64> = sinusoid_tasks.iter()
    ...
    println!("   - Rotation tasks: {} training samples each", rotation_tasks[0].train_data.len());
    println!("   - Sinusoid tasks: {} training samples each", sinusoid_tasks[0].train_data.len());
