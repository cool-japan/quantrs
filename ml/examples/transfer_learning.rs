...
fn main() -> Result<()> {
    ...
    println!("   - Original accuracy: {:.2}%", 
        pretrained.performance_metrics.get("accuracy").unwrap_or(&0.0) * 100.0);
    ...
    println!("   - MAE: {:.4}", 
        chemistry_model.performance_metrics.get("mae").unwrap_or(&0.0));
    println!("   - R² score: {:.4}", 
        chemistry_model.performance_metrics.get("r2_score").unwrap_or(&0.0));
...
fn print_layer_configs(configs: &[LayerConfig]) {
    for (i, config) in configs.iter().enumerate() {
        println!("   Layer {}: frozen={}, lr_multiplier={:.2}", 
