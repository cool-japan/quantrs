...
fn evolutionary_search_demo() -> Result<()> {
    ...
    let eval_data = Array2::from_shape_fn((100, 4), |(i, j)| {
    ...
    println!("   - Best architectures found: {}", best_architectures.len());
    ...
    println!("   - Total architectures evaluated: {}", summary.total_architectures_evaluated);
...
fn multi_objective_demo() -> Result<()> {
    ...
    for (i, arch) in pareto_front.iter().take(3).enumerate() {
        println!("   Architecture {}: {} params, {:.3} accuracy", 
            i + 1, arch.metrics.parameter_count, 
...
fn create_custom_search_space() -> SearchSpace {
    SearchSpace {
        ...
        param_ranges: vec![
            ("variational_params".to_string(), (3, 12)),
        ].into_iter().collect(),
...
fn create_darts_search_space() -> SearchSpace {
    SearchSpace {
        ...
        param_ranges: vec![
            ("variational_params".to_string(), (6, 9)),
        ].into_iter().collect(),
        connectivity_patterns: vec![
            "full".to_string(),
        ],
