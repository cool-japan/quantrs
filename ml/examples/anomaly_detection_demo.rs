...
fn main() -> quantrs2_ml::Result<()> {
    ...
    println!("Generated {} normal samples with {} features", n_samples, n_features);
    ...
    println!("Generated {} test samples (80 normal, 20 anomalous)", n_test);
    ...
    println!("\nDetection Results:");
    println!("==================");
    println!("Anomaly scores range: [{:.3}, {:.3}]", 
             result.anomaly_scores.fold(f64::INFINITY, |a, &b| a.min(b)),
             result.anomaly_scores.fold(f64::NEG_INFINITY, |a, &b| a.max(b)));
    
    let anomaly_count = result.anomaly_labels.iter().sum::<i32>();
    println!("Detected {} anomalies out of {} samples", anomaly_count, n_test);
    ...
    println!("\nProcessing Statistics:");
    println!("======================");
    println!("Total time: {:.3} seconds", result.processing_stats.total_time);
    println!("Quantum time: {:.3} seconds", result.processing_stats.quantum_time);
    println!("Classical time: {:.3} seconds", result.processing_stats.classical_time);
    println!("Memory usage: {:.1} MB", result.processing_stats.memory_usage);
    println!("Quantum executions: {}", result.processing_stats.quantum_executions);
    ...
    println!("Detected anomalies: {}", network_result.anomaly_labels.iter().sum::<i32>());
    ...
    println!("Detected anomalies: {}", fraud_result.anomaly_labels.iter().sum::<i32>());
    ...
    for i in 0..10 {
        ...
        let anomaly_score = streaming_detector.detect_stream(&sample)?;
        let is_anomaly = if anomaly_score > 0.5 { "ANOMALY" } else { "normal" };
        println!("Sample {}: score = {:.3} -> {}", i + 1, anomaly_score, is_anomaly);
    }
