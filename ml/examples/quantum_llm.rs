...
fn model_configurations_demo() -> Result<()> {
    ...
    println!("   - Model dimension: {}", small_config.transformer_config.model_dim);
    println!("   - Number of heads: {}", small_config.transformer_config.num_heads);
    println!("   - Number of layers: {}", small_config.transformer_config.num_layers);
    println!("   - Quantum qubits: {}", small_config.transformer_config.num_qubits);
    println!("   - Memory layers: {}", small_config.quantum_memory_layers);
    
    let small_model = QuantumLLM::new(small_config)?;
    println!("   Small model parameters: {:.1}M", 
    ...
    println!("   - Model dimension: {}", medium_config.transformer_config.model_dim);
    println!("   - Number of layers: {}", medium_config.transformer_config.num_layers);
    println!("   - Quantum qubits: {}", medium_config.transformer_config.num_qubits);
    println!("   - Max context length: {}", medium_config.max_context_length);
    
    let medium_model = QuantumLLM::new(medium_config)?;
    println!("   Medium model parameters: {:.1}M", 
    ...
    println!("   - Model dimension: {}", large_config.transformer_config.model_dim);
    println!("   - Number of layers: {}", large_config.transformer_config.num_layers);
    println!("   - Quantum qubits: {}", large_config.transformer_config.num_qubits);
    println!("   - Max context length: {}", large_config.max_context_length);
    println!("   - Reasoning steps: {}", large_config.reasoning_config.reasoning_steps);
    
    let large_model = QuantumLLM::new(large_config)?;
    println!("   Large model parameters: {:.1}B", 
    ...
    let quantum_efficiency = calculate_quantum_efficiency(&small_model, &medium_model, &large_model)?;
    println!("   - Quantum parameter efficiency: {:.2}x classical equivalent", quantum_efficiency);
...
fn quantum_reasoning_demo() -> Result<()> {
    ...
    for (name, config) in reasoning_configs {
        ...
        let reasoning_enhancement = analyze_reasoning_enhancement(&hidden_states, &reasoned_output)?;
        println!("   Reasoning enhancement metrics:");
        println!("   - Pattern amplification: {:.3}", reasoning_enhancement.pattern_amplification);
        println!("   - Logical consistency: {:.3}", reasoning_enhancement.logical_consistency);
        println!("   - Causal coherence: {:.3}", reasoning_enhancement.causal_coherence);
        ...
        let enhancement_effect = (&enhanced_logits - &sample_logits).mapv(|x| x.abs()).mean().unwrap_or(0.0);
...
fn text_generation_demo() -> Result<()> {
    ...
    for (config_name, gen_config) in generation_configs {
        ...
        println!("   - Quantum reasoning: {}", gen_config.use_quantum_reasoning);
...
fn chain_of_thought_demo() -> Result<()> {
    ...
    for (problem_type, prompt) in reasoning_problems {
        ...
        println!("   - Quantum enhancement: {:.3}", reasoning_analysis.quantum_enhancement);
...
fn multimodal_demo() -> Result<()> {
    ...
    let multimodal_tasks = vec![
        ("Text + Quantum Data", "Analyze this quantum measurement sequence"),
        ("Text + Mathematical", "Solve this quantum mechanics equation"),
        ("Text + Logical", "Apply quantum logic to this proposition"),
        ("Text + Memory", "Recall information about quantum algorithms"),
    ];
    ...
    for (modality, task_description) in multimodal_tasks {
        ...
        let text_input = Array2::from_shape_vec((1, 8), vec![100, 200, 300, 400, 500, 600, 700, 800])?;
        ...
        println!("   - Cross-modal coherence: {:.3}", integration_quality.coherence);
        println!("   - Information fusion: {:.3}", integration_quality.fusion_quality);
        println!("   - Quantum entanglement: {:.3}", integration_quality.quantum_entanglement);
        
        // Test specific capabilities based on modality
        match modality {
            "Text + Quantum Data" => {
                let quantum_analysis = analyze_quantum_data_processing(&output)?;
                println!("   - Quantum state recognition: {:.3}", quantum_analysis.state_recognition);
                println!("   - Measurement prediction: {:.3}", quantum_analysis.measurement_prediction);
            }
            "Text + Mathematical" => {
                let math_analysis = analyze_mathematical_reasoning(&output)?;
                println!("   - Equation understanding: {:.3}", math_analysis.equation_understanding);
                println!("   - Symbol manipulation: {:.3}", math_analysis.symbol_manipulation);
            }
            "Text + Logical" => {
                let logic_analysis = analyze_logical_processing(&output)?;
                println!("   - Logical validity: {:.3}", logic_analysis.validity);
                println!("   - Inference quality: {:.3}", logic_analysis.inference_quality);
            }
            "Text + Memory" => {
                let memory_analysis = analyze_memory_retrieval(&output)?;
                println!("   - Memory accuracy: {:.3}", memory_analysis.accuracy);
                println!("   - Retrieval efficiency: {:.3}", memory_analysis.efficiency);
            }
            _ => {}
        }
    }
...
fn performance_analysis_demo() -> Result<()> {
    ...
    for (name, model) in &models {
        ...
        println!("   - Model dimension: {}", config.transformer_config.model_dim);
        println!("   - Quantum qubits: {}", config.transformer_config.num_qubits);
        println!("   - Memory size: {}", config.memory_config.memory_size);
        println!("   - Reasoning steps: {}", config.reasoning_config.reasoning_steps);
        
        // Estimate quantum advantage
        let quantum_advantage = estimate_quantum_advantage(model)?;
        println!("   - Quantum advantage: {:.2}x", quantum_advantage.speedup);
        println!("   - Memory efficiency: {:.2}x", quantum_advantage.memory_efficiency);
        println!("   - Reasoning enhancement: {:.2}x", quantum_advantage.reasoning_enhancement);
    }
    ...
    for (task_name, benchmark_fn) in benchmark_tasks {
        ...
        for (model_name, model) in &models {
            ...
            println!("   {} Model: {:.2} ops/sec, {:.1} MB memory", 
    ...
    println!("   - Parameter scaling: {:.2} (vs {:.2} classical)", 
        scaling_analysis.quantum_scaling, scaling_analysis.classical_scaling);
    println!("   - Performance scaling: {:.2}", scaling_analysis.performance_scaling);
    println!("   - Quantum efficiency: {:.1}%", scaling_analysis.efficiency * 100.0);
    
    // Future projections
    println!("\n   Future Projections:");
    println!("   - 100B parameter QLLM estimated efficiency: {:.2}x classical", 
        project_future_efficiency(100_000_000_000));
    println!("   - Quantum coherence preservation: {:.1}%", 
        project_coherence_preservation() * 100.0);
    println!("   - Reasoning capability enhancement: {:.2}x", 
...
fn calculate_quantum_efficiency(
    ...
    let avg_efficiency = (small_qubits.powi(2) / small_params + 
                         ...
                         large_qubits.powi(2) / large_params) / 3.0;
...
fn analyze_generation_quality(
    ...
    let quantum_advantage = if config.use_quantum_reasoning { 0.3 } else { 0.1 };
...
fn analyze_cot_quality(generated_text: &str) -> Result<ChainOfThoughtAnalysis> {
    ...
    let quantum_enhancement = if generated_text.contains("quantum") { 0.6 } else { 0.3 };
...
fn estimate_quantum_advantage(model: &QuantumLLM) -> Result<QuantumAdvantage> {
    ...
    let reasoning_enhancement = if config.reasoning_config.logical_reasoning { 2.5 } else { 1.2 };
