        ...
        let layers = model_config
            ...
            .map(|(layer_type, size)| {
                match layer_type.as_str() {
                    "encoding" => crate::qnn::QNNLayerType::EncodingLayer { num_features: *size },
...
impl QuantumFLServer {
    ...
    fn secure_multiparty_aggregation(
        ...
        for (_, params, _) in &client_updates {
            for (param_name, param_value) in params {
                shares.entry(param_name.clone()).or_insert(Vec::new()).push(*param_value);
            }
        }
    ...
    fn quantum_secret_sharing_aggregation(
        ...
        for (client_id, params, _) in &client_updates {
            for (param_name, param_value) in params {
                ...
                quantum_shares.entry(param_name.clone()).or_insert(Vec::new()).push(share);
            }
        }
...
impl DistributedQuantumLearning {
    ...
    pub fn new(
        ...
        for i in 0..num_clients {
            ...
            let client = QuantumFLClient::new(
                client_id.clone(),
                &model_config,
                dataset_size,
                epsilon,
    ...
    fn compute_convergence(
        ...
        Ok(if count > 0 { diff_sum / count as f64 } else { 0.0 })
    }
}
...
mod tests {
    ...
    fn test_quantum_fl_client() {
        ...
        let mut client = QuantumFLClient::new(
            "client_1".to_string(),
            &config,
            100,
            1.0,
    ...
    fn test_federated_averaging() {
        ...
        let mut server = QuantumFLServer::new(
            config,
            SecureAggregationProtocol::FederatedAveraging,
            0.2,
        );
