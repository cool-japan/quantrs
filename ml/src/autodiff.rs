...
impl AutoDiff {
    ...
    fn evaluate_node(&mut self, node: &ComputationNode) -> Result<f64> {
        match node {
            ...
            ComputationNode::Expectation { circuit_params, observable } => {
...
enum Operation {
    ...
    /// Multiplication
    Mul { result: String, left: String, right: String },
    /// Quantum operation
...
impl GradientTape {
    ...
    pub fn gradient(&self, output: &str, inputs: &[&str]) -> HashMap<String, f64> {
        ...
        for op in self.operations.iter().rev() {
            match op {
                Operation::Add { result, left, right } => {
                ...
                Operation::Mul { result, left, right } => {
                    if let Some(&grad) = gradients.get(result) {
                        let left_val = self.variables[left];
                        let right_val = self.variables[right];
        ...
        inputs
            .iter()
            .map(|&input| (input.to_string(), gradients.get(input).copied().unwrap_or(0.0)))
            .collect()
    }
}
...
mod tests {
    ...
    fn test_parameter_shift() {
        let executor = |params: &[f64]| -> f64 {
            params[0].cos() + params[1].sin()
        };
