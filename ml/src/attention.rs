            ...
            for h in 0..self.num_heads {
                for i in 0..seq_len {
                    for j in 0..seq_len {
                        ...
                        let score = self.quantum_inner_product(&q_vec.to_owned(), &k_vec.to_owned())?;
    ...
    fn quantum_inner_product(&self, vec1: &Array1<f64>, vec2: &Array1<f64>) -> Result<f64> {
        ...
        for i in 0..self.qubits_per_head {
            ...
            if i < vec2.len() {
                ...
                circuit.add_gate("RY", vec![i + self.qubits_per_head], vec![angle2.to_string()]);
            }
        }
        ...
        for i in 0..self.qubits_per_head {
            circuit.add_gate("CSWAP", vec![num_qubits - 1, i, i + self.qubits_per_head], vec![]);
        }
