...
impl QuantumMetaLearner {
    ...
    pub fn new(
        ...
        let per_param_lr = match algorithm {
            MetaLearningAlgorithm::MetaSGD { .. } => {
                Some(Array1::from_elem(num_params, 0.01))
            }
            _ => None,
        };
        ...
        for epoch in 0..meta_epochs {
            ...
            if epoch % 10 == 0 {
                println!("Epoch {}: Loss = {:.4}, Accuracy = {:.2}%", 
                    epoch, epoch_loss, epoch_acc * 100.0);
            }
        }
    ...
    fn maml_update(
        ...
        let (inner_steps, inner_lr, first_order) = match self.algorithm {
            MetaLearningAlgorithm::MAML { inner_steps, inner_lr, first_order } => {
        ...
        Ok((total_loss / tasks.len() as f64, total_acc / tasks.len() as f64))
    }
    ...
    fn reptile_update(
        ...
        let (inner_steps, inner_lr) = match self.algorithm {
            MetaLearningAlgorithm::Reptile { inner_steps, inner_lr } => {
    ...
    fn protomaml_update(
        ...
        let (inner_steps, inner_lr, proto_weight) = match self.algorithm {
            MetaLearningAlgorithm::ProtoMAML { inner_steps, inner_lr, proto_weight } => {
        ...
        for task in tasks {
            ...
            for _ in 0..inner_steps {
                let grad = self.compute_task_gradient(&task.train_data, &task_params)?;
                let proto_reg = self.prototype_regularization(&task.train_data, &prototypes, &task_params)?;
                task_params = task_params - inner_lr * (&grad + proto_weight * &proto_reg);
            }
            
            // Evaluate with prototypical classification
            let (loss, acc) = self.evaluate_with_prototypes(&task.test_data, &prototypes, &task_params)?;
            total_loss += loss;
            total_acc += acc;
        }
    ...
    fn anil_update(
        ...
        let (inner_steps, inner_lr) = match self.algorithm {
            MetaLearningAlgorithm::ANIL { inner_steps, inner_lr } => {
                (inner_steps, inner_lr)
            }
            _ => unreachable!(),
        };
    ...
    pub fn adapt_to_task(&mut self, task: &MetaTask) -> Result<Array1<f64>> {
        let adapted_params = match self.algorithm {
            MetaLearningAlgorithm::MAML { inner_steps, inner_lr, .. } |
            MetaLearningAlgorithm::Reptile { inner_steps, inner_lr } |
            MetaLearningAlgorithm::ProtoMAML { inner_steps, inner_lr, .. } |
            MetaLearningAlgorithm::ANIL { inner_steps, inner_lr } => {
    ...
    pub fn generate_rotation_task(&self, num_samples: usize) -> MetaTask {
        ...
        for i in 0..num_samples {
            ...
            for j in 0..self.feature_dim {
                features[j] = if j % self.num_classes == label { 1.0 } else { 0.0 };
        ...
        MetaTask {
            ...
            metadata: vec![
                ("rotation_angle".to_string(), angle),
            ].into_iter().collect(),
        }
    }
}
