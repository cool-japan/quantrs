...
impl QuantumDiffusionModel {
    ...
    pub fn new(
        ...
        let layers = vec![
            QNNLayerType::EncodingLayer { num_features: data_dim + 1 }, // +1 for timestep
            QNNLayerType::VariationalLayer { num_params: num_qubits * 3 },
        ...
        let (betas, alphas, alphas_cumprod) = Self::compute_schedule(num_timesteps, &noise_schedule);
        ...
        match schedule {
            NoiseSchedule::Linear { beta_start, beta_end } => {
                for t in 0..num_timesteps {
                    betas[t] = beta_start + (beta_end - beta_start) * t as f64 / (num_timesteps - 1) as f64;
                }
            }
            NoiseSchedule::Cosine { s } => {
                for t in 0..num_timesteps {
                    let f_t = ((t as f64 / num_timesteps as f64 + s) / (1.0 + s) * PI / 2.0).cos().powi(2);
                    let f_t_prev = if t == 0 {
                        1.0
                    } else {
                        (((t - 1) as f64 / num_timesteps as f64 + s) / (1.0 + s) * PI / 2.0).cos().powi(2)
                    };
            ...
            NoiseSchedule::Sigmoid { beta_start, beta_end } => {
                for t in 0..num_timesteps {
    ...
    pub fn reverse_diffusion_step(
        ...
        let mean = sqrt_recip_alpha * (xt - beta_t / sqrt_one_minus_alpha_cumprod * &predicted_noise);
...
impl QuantumScoreDiffusion {
    ...
    pub fn new(
        ...
        let layers = vec![
            QNNLayerType::EncodingLayer { num_features: data_dim + 1 }, // +1 for noise level
            QNNLayerType::VariationalLayer { num_params: num_qubits * 4 },
            QNNLayerType::EntanglementLayer { connectivity: "full".to_string() },
            QNNLayerType::VariationalLayer { num_params: num_qubits * 4 },
            QNNLayerType::MeasurementLayer { measurement_basis: "Pauli-XYZ".to_string() },
        ];
    ...
    pub fn estimate_score(
        ...
        let score = Array1::from_shape_fn(self.data_dim, |_| {
            2.0 * rand::random::<f64>() - 1.0
        });
...
impl QuantumVariationalDiffusion {
    ...
    pub fn new(
        ...
        let encoder_layers = vec![
            QNNLayerType::EncodingLayer { num_features: data_dim },
            QNNLayerType::VariationalLayer { num_params: num_qubits * 3 },
            QNNLayerType::EntanglementLayer { connectivity: "circular".to_string() },
        ...
        let decoder_layers = vec![
            QNNLayerType::EncodingLayer { num_features: latent_dim },
            QNNLayerType::VariationalLayer { num_params: num_qubits * 3 },
            QNNLayerType::EntanglementLayer { connectivity: "circular".to_string() },
            QNNLayerType::MeasurementLayer { measurement_basis: "Pauli-Z".to_string() },
        ];
