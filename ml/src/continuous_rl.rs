...
impl QuantumActor {
    ...
    pub fn new(
        ...
        let layers = vec![
            QNNLayerType::EncodingLayer { num_features: state_dim },
...
impl QuantumDDPG {
    ...
    fn update_critic(&mut self, batch: &[Experience], optimizer: &mut dyn Optimizer) -> Result<()> {
        ...
        for exp in batch {
            ...
            let target_q = self.target_critic.get_q_value(&exp.next_state, &target_action)?;
    ...
    fn soft_update(&mut self) -> Result<()> {
        ...
        for i in 0..self.actor.qnn.parameters.len() {
            self.target_actor.qnn.parameters[i] = 
                self.tau * self.actor.qnn.parameters[i] + 
                (1.0 - self.tau) * self.target_actor.qnn.parameters[i];
        }
        
        // Update target critic parameters
        for i in 0..self.critic.qnn.parameters.len() {
            self.target_critic.qnn.parameters[i] = 
                self.tau * self.critic.qnn.parameters[i] + 
