...
impl QuantumNAS {
    ...
    pub fn search(&mut self, max_iterations: usize) -> Result<Vec<ArchitectureCandidate>> {
        ...
        match self.strategy {
            SearchStrategy::Evolutionary { .. } => {
                self.evolutionary_search(max_iterations)
            }
            SearchStrategy::ReinforcementLearning { .. } => {
                self.rl_search(max_iterations)
            }
            SearchStrategy::Random { .. } => {
                self.random_search(max_iterations)
            }
            SearchStrategy::BayesianOptimization { .. } => {
                self.bayesian_search(max_iterations)
            }
            SearchStrategy::DARTS { .. } => {
                self.darts_search(max_iterations)
            }
        }
    }
    
    /// Evolutionary algorithm search
    fn evolutionary_search(&mut self, max_generations: usize) -> Result<Vec<ArchitectureCandidate>> {
        let (population_size, mutation_rate, crossover_rate, elitism_ratio) = match self.strategy {
            SearchStrategy::Evolutionary { population_size, mutation_rate, crossover_rate, elitism_ratio } => {
                (population_size, mutation_rate, crossover_rate, elitism_ratio)
            }
            _ => unreachable!(),
        };
        ...
        for generation in 0..max_generations {
            ...
            println!("Generation {}: Best fitness = {:.4}", 
                generation, self.compute_fitness(&population[0]));
    ...
    fn rl_search(&mut self, max_episodes: usize) -> Result<Vec<ArchitectureCandidate>> {
        let (agent_type, exploration_rate, learning_rate) = match self.strategy {
            SearchStrategy::ReinforcementLearning { agent_type, exploration_rate, learning_rate } => {
        ...
        for episode in 0..max_episodes {
            ...
            loop {
                ...
                // Apply action
                let (next_architecture, reward, done) = self.apply_action(&current_architecture, &action)?;
    ...
    fn bayesian_search(&mut self, max_iterations: usize) -> Result<Vec<ArchitectureCandidate>> {
        let (acquisition_fn, num_initial) = match self.strategy {
            SearchStrategy::BayesianOptimization { acquisition_function, num_initial_points } => {
    ...
    fn darts_search(&mut self, max_epochs: usize) -> Result<Vec<ArchitectureCandidate>> {
        let (learning_rate, weight_decay) = match self.strategy {
            SearchStrategy::DARTS { learning_rate, weight_decay } => {
    ...
    fn sample_random_architecture(&self) -> Result<ArchitectureCandidate> {
        let depth = fastrand::usize(
            self.search_space.depth_range.0..=self.search_space.depth_range.1
        );
        let num_qubits = fastrand::usize(
            self.search_space.qubit_constraints.min_qubits..=self.search_space.qubit_constraints.max_qubits
        );
    ...
    fn mutate(&self, candidate: &mut ArchitectureCandidate) -> Result<()> {
        ...
        match mutation_type {
            ...
            1 => {
                // Remove layer
                if candidate.layers.len() > 3 { // Keep encoding and measurement
            ...
            2 => {
                ...
                if candidate.layers.len() > 2 {
                    ...
                    candidate.layers[layer_idx] = self.search_space.layer_types[new_layer_idx].clone();
                }
            }
            3 => {
                // Change qubit count
    ...
    fn update_best_architectures(&mut self, candidates: &[ArchitectureCandidate]) {
        ...
        let mut fitness_scores: Vec<(usize, f64)> = self.best_architectures
            .iter()
            .enumerate()
    ...
    fn update_pareto_front(&mut self, candidates: &[ArchitectureCandidate]) {
        for candidate in candidates {
            let is_dominated = self.pareto_front.iter().any(|other| {
    ...
    fn estimate_circuit_depth(&self, layers: &[QNNLayerType]) -> usize {
        layers.iter().map(|layer| match layer {
    ...
    fn compute_architecture_properties(&self, candidate: &mut ArchitectureCandidate) -> Result<()> {
        ...
        let expressivity = (candidate.metrics.parameter_count as f64).ln() * 
                          (candidate.metrics.circuit_depth as f64).sqrt() / 100.0;
        candidate.properties.expressivity = Some(expressivity.min(1.0));
        
        // Estimate entanglement capability
        let entanglement_layers = candidate.layers.iter()
            .filter(|layer| matches!(layer, QNNLayerType::EntanglementLayer { .. }))
            .count();
        candidate.properties.entanglement_capability = Some(
            (entanglement_layers as f64 / candidate.layers.len() as f64).min(1.0)
        );
    ...
    fn sample_random_action(&self, arch: &ArchitectureCandidate) -> Result<ArchitectureAction> {
        ...
        match action_type {
            0 => {
                let layer_idx = fastrand::usize(0..self.search_space.layer_types.len());
                Ok(ArchitectureAction::AddLayer(self.search_space.layer_types[layer_idx].clone()))
            }
            1 => {
    ...
    fn derive_architecture_from_weights(&self, alpha: &Array2<f64>) -> Result<ArchitectureCandidate> {
...
impl fmt::Display for ArchitectureCandidate {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "Architecture {} (layers: {}, qubits: {}, accuracy: {:.3})",
            self.id,
            self.layers.len(),
            self.num_qubits,
...
mod tests {
    ...
    fn test_search_space_creation() {
        ...
        assert!(search_space.qubit_constraints.min_qubits <= search_space.qubit_constraints.max_qubits);
    }
