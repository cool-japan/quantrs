...
impl QuantumMultiHeadAttention {
    ...
    pub fn new(
        ...
        for _ in 0..num_heads {
            let q_layers = vec![
                QNNLayerType::EncodingLayer { num_features: model_dim },
                QNNLayerType::VariationalLayer { num_params: head_dim * 2 },
                QNNLayerType::MeasurementLayer { measurement_basis: "computational".to_string() },
            ];
            query_layers.push(QuantumNeuralNetwork::new(q_layers, num_qubits, model_dim, head_dim)?);
            ...
            key_layers.push(QuantumNeuralNetwork::new(k_layers, num_qubits, model_dim, head_dim)?);
            ...
            value_layers.push(QuantumNeuralNetwork::new(v_layers, num_qubits, model_dim, head_dim)?);
        }
        
        // Output projection layer
        let out_layers = vec![
            QNNLayerType::EncodingLayer { num_features: model_dim },
            QNNLayerType::VariationalLayer { num_params: model_dim },
            QNNLayerType::MeasurementLayer { measurement_basis: "computational".to_string() },
        ];
        let output_projection = QuantumNeuralNetwork::new(out_layers, num_qubits, model_dim, model_dim)?;
    ...
    fn create_attention_circuit(num_qubits: usize, attention_type: &QuantumAttentionType) -> Result<Circuit<16>> {
    ...
    pub fn forward(
        ...
        if model_dim != self.model_dim {
            return Err(MLError::DimensionMismatch(
                format!("Expected model_dim {}, got {}", self.model_dim, model_dim)
            ));
        }
        ...
        for head_idx in 0..self.num_heads {
            let head_output = self.compute_head_attention(
                query, key, value, head_idx, attention_mask, &mut quantum_info
            )?;
        ...
        for batch_idx in 0..batch_size {
            for seq_idx in 0..seq_len {
                ...
                final_output.slice_mut(s![batch_idx, seq_idx, ..]).assign(&output);
            }
        }
    ...
    fn compute_head_attention(
        ...
        let attention_scores = self.compute_quantum_attention_scores(
            &q_proj, &k_proj, quantum_info
        )?;
        ...
        match self.attention_type {
            ...
            _ => {
                ...
                for batch_idx in 0..batch_size {
                    ...
                    attention_scores.slice_mut(s![batch_idx, .., ..]).assign(&scores);
                }
            }
        }
    ...
    fn quantum_softmax(&self, scores: &Array3<f64>) -> Result<Array3<f64>> {
        ...
        for batch_idx in 0..scores.dim().0 {
            for seq_idx in 0..scores.dim().1 {
                ...
                weights.slice_mut(s![batch_idx, seq_idx, ..]).assign(&exp_scores);
            }
        }
    ...
    fn apply_attention_to_values(
        ...
        for batch_idx in 0..batch_size {
            ...
            output.slice_mut(s![batch_idx, .., ..]).assign(&attended_values);
        }
        ...
        for batch_idx in 0..batch_size {
            for seq_idx in 0..seq_len {
                ...
                for (head_idx, head_output) in head_outputs.iter().enumerate() {
                    ...
                    concat_vec.slice_mut(s![start_idx..end_idx])
                        .assign(&head_output.slice(s![batch_idx, seq_idx, ..]));
                }
                
                concatenated.slice_mut(s![batch_idx, seq_idx, ..]).assign(&concat_vec);
            }
        }
    ...
    fn compute_coherence(&self, state: &[f64]) -> Result<f64> {
        ...
        for (i, &amplitude) in state.iter().enumerate() {
            if i > 0 { // Exclude diagonal elements (|0><0|, |1><1|, etc.)
...
impl QuantumPositionEncoding {
    ...
    fn generate_sinusoidal_encoding(&self, encodings: &mut Array3<f64>, seq_len: usize) -> Result<()> {
        for pos in 0..seq_len {
            for i in 0..self.model_dim {
                let angle = pos as f64 / 10000_f64.powf(2.0 * (i / 2) as f64 / self.model_dim as f64);
                
                let encoding_value = if i % 2 == 0 {
                    angle.sin()
                } else {
                    angle.cos()
                };
    ...
    fn generate_quantum_phase_encoding(&self, encodings: &mut Array3<f64>, seq_len: usize) -> Result<()> {
        let simulator = StateVectorSimulator::new();
    ...
    fn generate_learnable_quantum_encoding(&self, encodings: &mut Array3<f64>, seq_len: usize) -> Result<()> {
    ...
    fn generate_relative_encoding(&self, encodings: &mut Array3<f64>, seq_len: usize) -> Result<()> {
...
impl QuantumFeedForward {
    ...
    pub fn new(
        ...
        let layer1_structure = vec![
            ...
            QNNLayerType::VariationalLayer { num_params: hidden_dim * 2 },
            QNNLayerType::EntanglementLayer { connectivity: "circular".to_string() },
            QNNLayerType::VariationalLayer { num_params: hidden_dim },
            QNNLayerType::MeasurementLayer { measurement_basis: "computational".to_string() },
        ];
        let layer1 = QuantumNeuralNetwork::new(layer1_structure, num_qubits, input_dim, hidden_dim)?;
        
        // Second layer: hidden_dim -> output_dim
        let layer2_structure = vec![
            QNNLayerType::EncodingLayer { num_features: hidden_dim },
            QNNLayerType::VariationalLayer { num_params: output_dim * 2 },
            QNNLayerType::MeasurementLayer { measurement_basis: "computational".to_string() },
        ];
        let layer2 = QuantumNeuralNetwork::new(layer2_structure, num_qubits, hidden_dim, output_dim)?;
    ...
    pub fn forward(&self, input: &Array2<f64>) -> Result<Array2<f64>> {
        ...
        for batch_idx in 0..batch_size {
            for seq_idx in 0..seq_len {
                ...
                output.slice_mut(s![batch_idx, out_start..out_end]).assign(&output_slice);
            }
        }
...
impl QuantumTransformerLayer {
    ...
    pub fn new(
        ...
        let attention = QuantumMultiHeadAttention::new(
            num_heads, model_dim, attention_type, num_qubits
        )?;
        
        let feedforward = QuantumFeedForward::new(
            model_dim, ff_dim, model_dim, num_qubits,
            ActivationType::QuantumGELU, dropout_rate
        )?;
    ...
    fn layer_norm(
        ...
        for batch_idx in 0..batch_size {
            for seq_idx in 0..seq_len {
                ...
                let variance = input_slice.mapv(|x| (x - mean).powi(2)).mean().unwrap_or(1.0);
    ...
    fn reshape_for_feedforward(&self, input: &Array3<f64>) -> Result<Array2<f64>> {
        ...
        for batch_idx in 0..batch_size {
            for seq_idx in 0..seq_len {
                ...
                output.slice_mut(s![batch_idx, start_idx..end_idx])
...
impl QuantumTransformer {
    ...
    pub fn new(config: QuantumTransformerConfig) -> Result<Self> {
        ...
        let embedding_layers = vec![
            QNNLayerType::EncodingLayer { num_features: config.model_dim },
            QNNLayerType::VariationalLayer { num_params: config.model_dim },
            QNNLayerType::MeasurementLayer { measurement_basis: "computational".to_string() },
        ];
        let input_embedding = QuantumNeuralNetwork::new(
            embedding_layers, config.num_qubits, config.model_dim, config.model_dim
        )?;
        ...
        let output_projection = QuantumNeuralNetwork::new(
            output_layers, config.num_qubits, config.model_dim, config.model_dim
        )?;
    ...
    pub fn forward(
        ...
        for batch_idx in 0..batch_size {
            for seq_idx in 0..seq_len {
                ...
                padded_input.slice_mut(s![..copy_len])
                    .assign(&input_vec.slice(s![..copy_len]));
                
                let embedding_output = self.input_embedding.forward(&padded_input)?;
                embedded.slice_mut(s![batch_idx, seq_idx, ..]).assign(&embedding_output);
            }
        }
        ...
        for batch_idx in 0..batch_size {
            for seq_idx in 0..seq_len {
                ...
                output.slice_mut(s![batch_idx, seq_idx, ..]).assign(&projected_output);
            }
        }
    ...
    pub fn num_parameters(&self) -> usize {
        ...
        for layer in &self.layers {
            total += layer.attention.query_layers.iter().map(|l| l.parameters.len()).sum::<usize>();
            total += layer.attention.key_layers.iter().map(|l| l.parameters.len()).sum::<usize>();
            total += layer.attention.value_layers.iter().map(|l| l.parameters.len()).sum::<usize>();
...
mod tests {
    ...
    fn test_quantum_multi_head_attention_creation() {
        let attention = QuantumMultiHeadAttention::new(
            8, 512, QuantumAttentionType::HybridQuantumClassical, 10
        );
    ...
    fn test_quantum_position_encoding() {
        let pos_enc = QuantumPositionEncoding::new(
            PositionEncodingType::Sinusoidal, 256, 512, 8
        );
    ...
    fn test_quantum_feedforward() {
        let ff = QuantumFeedForward::new(
            256, 1024, 256, 8, ActivationType::QuantumGELU, 0.1
        );
