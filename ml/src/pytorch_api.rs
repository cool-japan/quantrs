...
use crate::simulator_backends::{SimulatorBackend, Observable};
...
impl QuantumLinear {
    ...
    pub fn new(in_features: usize, out_features: usize) -> Result<Self> {
        ...
        let weights = Parameter::new(
            SciRS2Array::with_grad(weight_data),
            "weight",
        );
    ...
    pub fn with_bias(mut self) -> Result<Self> {
        ...
        self.bias = Some(Parameter::new(
            SciRS2Array::with_grad(bias_data),
            "bias",
        ));
        Ok(self)
    }
...
impl QuantumLoss for QuantumCrossEntropyLoss {
    fn forward(&self, predictions: &SciRS2Array, targets: &SciRS2Array) -> Result<SciRS2Array> {
        ...
        let max_val = predictions.data.iter().fold(f64::NEG_INFINITY, |a, &b| a.max(b));
...
pub mod quantum_nn {
    ...
    pub fn create_feedforward(
        ...
        for &hidden_size in hidden_sizes {
            model = model.add(Box::new(
                QuantumLinear::new(prev_size, hidden_size)?
                    .with_bias()?
            ));
        ...
        // Output layer
        model = model.add(Box::new(
            QuantumLinear::new(prev_size, output_size)?
                .with_bias()?
        ));
    ...
    pub fn create_cnn(
        ...
        let model = QuantumSequential::new()
            .add(Box::new(
                QuantumConv2d::new(input_channels, 32, (3, 3))?
                    .with_bias()?
            ))
            .add(Box::new(QuantumActivation::relu()))
            .add(Box::new(
                QuantumConv2d::new(32, 64, (3, 3))?
                    .with_bias()?
            ))
            .add(Box::new(QuantumActivation::relu()))
            .add(Box::new(
                QuantumLinear::new(64, num_classes)?
                    .with_bias()?
            ));
