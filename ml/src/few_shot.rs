...
impl QuantumPrototypicalNetwork {
    ...
    pub fn train_episode(
        ...
        for (data, label) in &episode.support_set {
            class_examples.entry(*label).or_insert(Vec::new()).push(data.clone());
        }
...
impl QuantumMAML {
    ...
    pub fn predict_adapted(
        &self,
        query: &Array1<f64>,
        task_id: &str,
    ) -> Result<usize> {
        let params = self.task_params.get(task_id)
...
impl FewShotLearner {
    ...
    pub fn generate_episode(
        ...
        for class_id in selected_classes {
            ...
            let class_indices: Vec<usize> = labels.iter()
                .enumerate()
            ...
            if class_indices.len() < k_shot + query_per_class {
                return Err(MLError::ModelCreationError(
                    format!("Not enough examples for class {}", class_id)
                ));
            }
    ...
    pub fn train(
        ...
        match self.method {
            ...
            FewShotMethod::MAML { inner_steps, inner_lr } => {
                let mut maml = QuantumMAML::new(
                    self.model.clone(),
                    inner_lr,
                    inner_steps,
                );
...
mod tests {
    ...
    fn test_prototypical_network() {
        ...
        let proto_net = QuantumPrototypicalNetwork::new(
            qnn,
            8,
            DistanceMetric::Euclidean,
        );
