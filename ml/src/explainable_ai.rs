...
use quantrs2_core::gate::{GateOp, single::{RotationX, RotationY, RotationZ}};
...
impl QuantumExplainableAI {
    ...
    pub fn explain(&mut self, input: &Array1<f64>) -> Result<ExplanationResult> {
        ...
        for method in &self.methods.clone() {
            match method {
                ExplanationMethod::QuantumFeatureAttribution { method: attr_method, num_samples, baseline } => {
                    let attributions = self.compute_feature_attributions(input, attr_method, *num_samples, baseline.as_ref())?;
                    result.feature_attributions = Some(attributions);
                }
                
                ExplanationMethod::CircuitVisualization { include_measurements, parameter_sensitivity } => {
                    let circuit_explanation = self.analyze_circuit(input, *include_measurements, *parameter_sensitivity)?;
                    result.circuit_explanation = Some(circuit_explanation);
                }
                
                ExplanationMethod::StateAnalysis { entanglement_measures, coherence_analysis, superposition_analysis } => {
                    let state_props = self.analyze_quantum_state(input, *entanglement_measures, *coherence_analysis, *superposition_analysis)?;
                    result.state_properties = Some(state_props);
                }
                
                ExplanationMethod::SaliencyMapping { perturbation_method, aggregation } => {
                    let saliency = self.compute_saliency_map(input, perturbation_method, aggregation)?;
                    result.saliency_map = Some(saliency);
                }
                
                ExplanationMethod::QuantumLIME { num_perturbations, kernel_width, local_model } => {
                    let lime_attributions = self.explain_with_lime(input, *num_perturbations, *kernel_width, local_model)?;
                    result.feature_attributions = Some(lime_attributions);
                }
                
                ExplanationMethod::QuantumSHAP { num_coalitions, background_samples } => {
                    let shap_values = self.compute_shap_values(input, *num_coalitions, *background_samples)?;
                    result.feature_attributions = Some(shap_values);
                }
                
                ExplanationMethod::QuantumLRP { propagation_rule, epsilon } => {
                    let lrp_scores = self.layer_wise_relevance_propagation(input, propagation_rule, *epsilon)?;
                    result.feature_attributions = Some(lrp_scores);
                }
                
                ExplanationMethod::ConceptActivation { concept_datasets, activation_threshold } => {
                    let concept_activations = self.compute_concept_activations(input, concept_datasets, *activation_threshold)?;
    ...
    fn compute_feature_attributions(
        ...
        match method {
            ...
            AttributionMethod::GradientSHAP => {
                self.gradient_shap(input, num_samples)
            }
            
            AttributionMethod::QuantumAttribution => {
                self.quantum_specific_attribution(input)
            }
        }
    }
    ...
    fn compute_shap_values(
        ...
        if let Some(ref background) = self.background_data {
            ...
            for _ in 0..num_coalitions {
                ...
                let coalition: Vec<bool> = (0..input.len())
                    .map(|_| fastrand::bool())
                    .collect();
                
                for i in 0..input.len() {
                    // Compute marginal contribution of feature i
                    let with_i = self.compute_coalition_value(input, &coalition, Some(i), &bg_indices)?;
                    let without_i = self.compute_coalition_value(input, &coalition, None, &bg_indices)?;
    ...
    fn compute_concept_activations(
        ...
        for concept_name in concept_datasets {
            if let Some(concept_vector) = self.concept_vectors.get(concept_name) {
                ...
                let activation = internal_repr.iter()
    ...
    fn generate_textual_explanation(&self, result: &ExplanationResult) -> Result<String> {
            ...
            for (i, &attr) in attributions.iter().enumerate() {
                if attr.abs() > 0.1 {
                    explanation.push_str(&format!(
                        ...
                        i,
                        attr,
                        if attr > 0.0 { "positive influence" } else { "negative influence" }
                    ));
                }
            }
            explanation.push('\n');
        ...
        if let Some(ref circuit) = result.circuit_explanation {
            ...
            let max_importance = circuit.parameter_importance.iter()
                .cloned()
        ...
        if let Some(ref state) = result.state_properties {
            ...
            let max_prob = state.measurement_probabilities.iter()
                .cloned()
                .fold(f64::NEG_INFINITY, f64::max);
    ...
    fn compute_confidence_scores(&self, result: &ExplanationResult) -> Result<HashMap<String, f64>> {
        let mut confidence = HashMap::new();
        
        // Feature attribution confidence
        if let Some(ref attributions) = result.feature_attributions {
            let total_magnitude = attributions.iter().map(|x| x.abs()).sum::<f64>();
            let max_magnitude = attributions.iter().cloned().fold(f64::NEG_INFINITY, f64::max);
    ...
    fn compute_quantum_fisher_information(&self, input: &Array1<f64>, feature_idx: usize) -> Result<f64> {
    ...
    /// Compute coherence measures
    fn compute_coherence_measures(&self, state_vector: &Array1<f64>) -> Result<HashMap<String, f64>> {
        ...
        let rel_entropy = state_vector.iter()
            .map(|&p| if p > 1e-10 { p * (p / uniform_state).ln() } else { 0.0 })
            .sum::<f64>();
    ...
    fn compute_state_fidelities(&self, state_vector: &Array1<f64>) -> Result<HashMap<String, f64>> {
        ...
        for i in 0..state_vector.len().min(8) { // Limit to first 8 basis states
    ...
    fn apply_perturbation(&self, input: &Array1<f64>, method: &PerturbationMethod) -> Result<Array1<f64>> {
        match method {
            PerturbationMethod::Gaussian { sigma } => {
                let noise = Array1::from_shape_fn(input.len(), |_| {
                    sigma * (fastrand::f64() - 0.5) * 2.0
                });
                Ok(input + &noise)
            }
    ...
    fn analyze_layer(&self, input: &Array1<f64>, layer: &QNNLayerType, layer_idx: usize) -> Result<LayerAnalysis> {
        ...
        let feature_transformations = Array2::from_shape_fn((feature_dim, feature_dim), |(i, j)| {
            if i == j { 1.0 } else { 0.1 * fastrand::f64() }
        });
    ...
    fn analyze_gates(&self, input: &Array1<f64>) -> Result<Vec<GateContribution>> {
        ...
        for i in 0..10 { // Assume 10 gates for demo
            let contribution = GateContribution {
                gate_index: i,
                gate_type: if i % 3 == 0 { "RX".to_string() } else if i % 3 == 1 { "RY".to_string() } else { "CNOT".to_string() },
                contribution: 0.1 + 0.8 * fastrand::f64(),
                qubits: vec![i % self.model.num_qubits, (i + 1) % self.model.num_qubits],
                parameters: if i % 3 != 2 { Some(Array1::from_vec(vec![PI * fastrand::f64()])) } else { None },
            };
    ...
    fn find_critical_path(&self, param_importance: &Array1<f64>) -> Result<Vec<usize>> {
        ...
        let mut indexed_importance: Vec<(usize, f64)> = param_importance.iter()
            .enumerate()
            .map(|(i, &val)| (i, val))
            .collect();
        ...
        Ok(indexed_importance.into_iter().take(5).map(|(i, _)| i).collect())
    }
    ...
    fn generate_lime_perturbation(&self, input: &Array1<f64>) -> Result<Array1<f64>> {
        ...
        for i in 0..input.len() {
            if fastrand::f64() < 0.3 { // 30% chance to mask
                perturbed[i] = 0.0;
            }
        }
    ...
    fn fit_linear_model(
        ...
        Ok(Array1::from_shape_fn(feature_dim, |i| 0.1 + 0.8 * fastrand::f64()))
    }
    ...
    fn fit_decision_tree(
        ...
        let feature_dim = perturbations[0].len();
        Ok(Array1::from_shape_fn(feature_dim, |i| if i % 2 == 0 { 0.8 } else { 0.2 }))
    }
    ...
    fn fit_quantum_linear_model(
        ...
        let feature_dim = perturbations[0].len();
        Ok(Array1::from_shape_fn(feature_dim, |i| (i as f64 * 0.3).sin().abs()))
    }
    ...
    fn compute_coherence_weight(&self, input: &Array1<f64>) -> Result<f64> {
        ...
        let coherence = state_props.coherence_measures.get("l1_coherence").unwrap_or(&1.0);
        Ok(*coherence)
    }
}
