...
impl QuantumAdversarialTrainer {
    ...
    pub fn train(
        ...
        for epoch in 0..self.config.epochs {
            ...
            let num_batches = (train_data.nrows() + self.config.batch_size - 1) / self.config.batch_size;
            ...
            if epoch % self.config.eval_interval == 0 {
                ...
                println!("Epoch {}: Loss = {:.4}, Clean Acc = {:.3}, Robust Acc = {:.3}",
                    epoch, epoch_loss, 
    ...
    fn generate_single_adversarial_example(
        ...
        let adversarial_input = match attack_type {
            QuantumAttackType::FGSM { epsilon } => {
                self.fgsm_attack(input, true_label, epsilon)?
            }
            QuantumAttackType::PGD { epsilon, alpha, num_steps } => {
                self.pgd_attack(input, true_label, epsilon, alpha, num_steps)?
            }
            QuantumAttackType::ParameterShift { shift_magnitude, target_parameters } => {
                self.parameter_shift_attack(input, shift_magnitude, target_parameters)?
            }
            QuantumAttackType::StatePerturbation { perturbation_strength, ref basis } => {
                self.state_perturbation_attack(input, perturbation_strength, basis)?
            }
            QuantumAttackType::CircuitManipulation { gate_error_rate, coherence_time } => {
                self.circuit_manipulation_attack(input, gate_error_rate, coherence_time)?
            }
            QuantumAttackType::UniversalPerturbation { perturbation_budget, success_rate_threshold } => {
        ...
        let adversarial_class = adversarial_prediction.iter()
            .enumerate()
            .max_by(|a, b| a.1.partial_cmp(b.1).unwrap())
            .map(|(i, _)| i)
            .unwrap_or(0);
        ...
        match basis {
            "pauli_x" => {
                ...
                for i in 0..adversarial_input.len() {
                    ...
                    let perturbed_angle = angle + perturbation_strength * (2.0 * rand::random::<f64>() - 1.0);
            ...
            "pauli_y" => {
                ...
                for i in 0..adversarial_input.len() {
                    adversarial_input[i] += perturbation_strength * (2.0 * rand::random::<f64>() - 1.0);
                }
            }
            "pauli_z" | _ => {
                ...
                for i in 0..adversarial_input.len() {
                    ...
                    adversarial_input[i] = (adversarial_input[i] + phase_shift / (2.0 * PI)).fract();
                }
            }
        }
    ...
    fn universal_perturbation_attack(
        ...
        for i in 0..adversarial_input.len() {
            let universal_component = perturbation_budget * (2.0 * PI * i as f64 / adversarial_input.len() as f64).sin();
    ...
    fn compute_input_gradient(&self, input: &Array1<f64>, true_label: usize) -> Result<Array1<f64>> {
    ...
    fn generate_adversarial_batch(
        ...
        match &self.defense_strategy {
            QuantumDefenseStrategy::AdversarialTraining { attack_types, adversarial_ratio } => {
                ...
                for i in 0..num_adversarial {
                    ...
                    combined_data.row_mut(idx).assign(&adversarial_example.adversarial_input);
                }
    ...
    fn evaluate_robustness(
        ...
        for (input, &label) in val_data.outer_iter().zip(val_labels.iter()) {
            ...
            let clean_pred = clean_output.iter()
                .enumerate()
                .max_by(|a, b| a.1.partial_cmp(b.1).unwrap())
                .map(|(i, _)| i)
                .unwrap_or(0);
        ...
        self.robustness_metrics.avg_perturbation_norm = total_perturbation / num_attack_tests as f64;
        self.robustness_metrics.attack_success_rate = successful_attacks as f64 / num_attack_tests as f64;
    ...
    pub fn apply_defense(&self, input: &Array1<f64>) -> Result<Array1<f64>> {
        match &self.defense_strategy {
            QuantumDefenseStrategy::InputPreprocessing { noise_addition, feature_squeezing } => {
            ...
            QuantumDefenseStrategy::RandomizedCircuit { randomization_strength, .. } => {
                let mut defended_input = input.clone();
...
pub fn create_comprehensive_defense() -> QuantumDefenseStrategy {
    QuantumDefenseStrategy::AdversarialTraining {
        attack_types: vec![
            QuantumAttackType::FGSM { epsilon: 0.1 },
            QuantumAttackType::PGD { epsilon: 0.1, alpha: 0.01, num_steps: 7 },
            QuantumAttackType::ParameterShift { shift_magnitude: 0.05, target_parameters: None },
        ],
