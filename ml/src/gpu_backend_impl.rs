...
#[cfg(all(feature = "gpu", not(target_os = "macos")))]
use quantrs2_core::prelude::{Gate, GateOp, QubitId};
...
impl GPUBackend {
    ...
    pub fn new(device_id: usize, max_qubits: usize) -> Result<Self> {
        ...
        if !SciRS2GpuStateVectorSimulator::is_available() {
            return Err(MLError::BackendError(
    ...
    fn execute_circuit_typed<const N: usize>(
        ...
        for gate in param_circuit.gates_mut() {
        ...
        let mut gpu_sim = simulator.as_ref().clone();
        gpu_sim.initialize();
        ...
        let measurements = if let Some(n_shots) = shots {
            Some(self.sample_measurements(&probabilities, n_shots))
        } else {
            None
        };
    ...
    fn sample_measurements(&self, probabilities: &[f64], n_shots: usize) -> Vec<Vec<u8>> {
        ...
        (0..n_shots)
            .map(|_| {
                ...
                let num_qubits = (probabilities.len() as f64).log2() as usize;
                (0..num_qubits)
                    .map(|i| ((outcome >> i) & 1) as u8)
                    .collect()
            })
            .collect()
    }
...
impl SimulatorBackend for GPUBackend {
    ...
    fn expectation_value(
        ...
        match observable {
            Observable::PauliString(pauli) => {
                Ok(self.compute_pauli_expectation(&state, &pauli.string, &pauli.qubits))
            }
            Observable::PauliZ(qubits) => {
                let pauli_string = "Z".repeat(qubits.len());
                Ok(self.compute_pauli_expectation(&state, &pauli_string, qubits))
            }
            Observable::Hamiltonian(terms) => {
                let mut total = 0.0;
                for (coeff, pauli) in terms {
                    let exp = self.compute_pauli_expectation(&state, &pauli.string, &pauli.qubits);
                    total += coeff * exp;
                }
                Ok(total)
            }
    ...
    fn compute_gradients(
        ...
        match gradient_method {
            ...
            GradientMethod::FiniteDiff(epsilon) => {
                let mut gradients = Array1::zeros(parameters.len());
