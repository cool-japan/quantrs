...
use crate::simulator_backends::{SimulatorBackend, BackendCapabilities};
...
pub trait Benchmark: Send + Sync {
    ...
    fn run(&mut self, backend: &dyn SimulatorBackend) -> Result<BenchmarkRunResult>;
...
impl BenchmarkFramework {
    ...
    pub fn with_config(mut self, config: BenchmarkConfig) -> Self {
        self.config = config;
        self.results.metadata.config = config.clone();
        self
    }
        ...
        for backend in backends {
            ...
            for run_idx in 0..self.config.repetitions {
                ...
                if start_time.elapsed().as_secs_f64() > self.config.max_time_per_benchmark {
                    println!("Benchmark {} timed out after {} runs", backend_name, run_idx + 1);
                    break;
                }
            }
    ...
    fn compute_summary(&self, runs: &[BenchmarkRunResult]) -> Result<BenchmarkSummary> {
        if runs.is_empty() {
            return Err(MLError::InvalidConfiguration("No benchmark runs".to_string()));
        }
...
pub mod algorithm_benchmarks {
    ...
    impl Benchmark for VQEBenchmark {
        ...
        fn setup(&mut self) -> Result<()> {
            // Setup VQE circuit
            self.circuit = Some(VariationalCircuit::new(self.num_qubits));
            Ok(())
        }
        
        fn run(&mut self, backend: &dyn SimulatorBackend) -> Result<BenchmarkRunResult> {
            let start = Instant::now();
            let parameters = Array1::zeros(self.num_parameters);
    ...
    impl Benchmark for QAOABenchmark {
        ...
        fn run(&mut self, _backend: &dyn SimulatorBackend) -> Result<BenchmarkRunResult> {
            let start = Instant::now();
...
impl BenchmarkReport {
    ...
    fn analyze_scaling(&mut self) {
        ...
        for benchmark_name in self.summaries.keys() {
            ...
            self.scaling_analysis.insert(benchmark_name.clone(), analysis);
        }
    }
    ...
    fn generate_recommendations(&mut self) {
        ...
        self.recommendations.push(
            "Use statevector backend for small circuits (<20 qubits)".to_string(),
        );
        self.recommendations.push(
            "Use MPS backend for large circuits with limited entanglement".to_string(),
        );
        self.recommendations.push(
            "Consider GPU acceleration for repeated circuit evaluations".to_string(),
        );
    }
...
pub mod benchmark_utils {
    use super::*;
    use crate::simulator_backends::{StatevectorBackend, MPSBackend};
    ...
    pub fn create_benchmark_backends() -> Vec<Box<dyn SimulatorBackend>> {
        let mut backends: Vec<Box<dyn SimulatorBackend>> = Vec::new();
        
        // Statevector backend
        backends.push(Box::new(StatevectorBackend::new(15)));
        
        // MPS backend
        backends.push(Box::new(MPSBackend::new(64, 50)));
        ...
        #[cfg(feature = "gpu")]
        {
            ...
            if let Ok(gpu_backend) = GPUBackend::new(0, 20) {
                backends.push(Box::new(gpu_backend));
            }
        }
...
mod tests {
    ...
    fn test_vqe_benchmark() {
        ...
        let run_result = benchmark.run(&backend);
