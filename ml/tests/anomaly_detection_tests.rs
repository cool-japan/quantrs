...
fn test_streaming_detection() -> Result<()> {
    ...
    for i in 0..10 {
        ...
        let score = detector.detect_stream(&sample)?;
        assert!(score >= 0.0);
    }
...
fn test_isolation_forest() -> Result<()> {
    ...
    if let Some(MethodSpecificResult::IsolationForest { path_lengths, tree_depths }) = 
...
fn test_autoencoder() -> Result<()> {
    ...
    if let Some(MethodSpecificResult::Autoencoder { reconstruction_errors, latent_representations }) = 
