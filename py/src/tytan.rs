...
#[cfg(feature = "tytan")]
use quantrs2_tytan::{
    sampler::{SampleResult, SASampler, Sampler},
    analysis::visualization::{
        prepare_energy_landscape, analyze_solution_distribution, analyze_convergence,
        extract_tsp_tour, extract_graph_coloring, spring_layout,
        EnergyLandscapeConfig, SolutionDistributionConfig,
        export_to_csv, export_solution_matrix,
    },
};
...
impl PyEnergyLandscapeVisualizer {
    ...
    fn prepare_landscape(
        ...
        {
            Err(PyValueError::new_err("Tytan features not enabled. Install with 'pip install quantrs2[tytan]'"))
        }
    }
    ...
    fn export_csv(data: &Bound<'_, PyDict>, output_path: &str) -> PyResult<()> {
        #[cfg(feature = "tytan")]
        {
            ...
            Err(PyValueError::new_err("Use pandas DataFrame.to_csv() on the returned data"))
        }
...
impl PySolutionAnalyzer {
    ...
    fn analyze_distribution(
        ...
        #[cfg(feature = "tytan")]
        {
            ...
            let dist_data = analyze_solution_distribution(&rust_results, Some(config))
                .map_err(|e| PyValueError::new_err(format!("Failed to analyze distribution: {}", e)))?;
...
impl PyProblemVisualizer {
    ...
    fn spring_layout(
        ...
        #[cfg(feature = "tytan")]
        {
            ...
            let flat_coords: Vec<f64> = positions.into_iter()
...
impl PyConvergenceAnalyzer {
    ...
    fn analyze_convergence(
        ...
        #[cfg(feature = "tytan")]
        {
            ...
            let conv_data = analyze_convergence(&rust_iterations, ma_window)
                .map_err(|e| PyValueError::new_err(format!("Failed to analyze convergence: {}", e)))?;
