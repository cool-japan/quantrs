...
impl PyGrover {
    ...
    fn apply_diffusion(&self, circuit: &mut PyCircuit) -> PyResult<()> {
        ...
        if self.n_qubits == 2 {
            ...
            return Err(PyRuntimeError::new_err("Diffusion for >3 qubits not implemented"));
        }
...
impl PyQPE {
    ...
    fn circuit(
        ...
        PyQFT::apply_to_circuit(
            &mut circuit,
            (0..n_counting_qubits).collect(),
            Some(true),
        )?;
...
#[pyfunction]
fn create_ising_hamiltonian(py: Python, n_qubits: usize, j_coupling: f64, h_field: f64) -> PyResult<Py<PyArray2<Complex64>>> {
    let dim = 1 << n_qubits;
