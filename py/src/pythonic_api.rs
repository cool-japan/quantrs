...
impl PyQuantumCircuit {
    ...
    fn new(n_qubits: &Bound<'_, PyAny>, n_clbits: Option<usize>, name: Option<String>) -> PyResult<Self> {
        let num_qubits = if let Ok(n) = n_qubits.extract::<usize>() {
            n
        } else if let Ok(qreg) = n_qubits.extract::<PyRef<PyQuantumRegister>>() {
            qreg.size
        } else {
            return Err(PyValueError::new_err("n_qubits must be int or QuantumRegister"));
        };
    ...
    fn add_register(&mut self, register: &PyQuantumRegister) -> PyResult<()> {
        if self.registers.contains_key(&register.name) {
            return Err(PyValueError::new_err(format!("Register {} already exists", register.name)));
        }
        
        let start_idx = self.registers.values().flatten().max().map(|&x| x + 1).unwrap_or(0);
    ...
    fn ccx(&mut self, control1: &Bound<'_, PyAny>, control2: &Bound<'_, PyAny>, target: &Bound<'_, PyAny>) -> PyResult<()> {
    ...
    fn toffoli(&mut self, control1: &Bound<'_, PyAny>, control2: &Bound<'_, PyAny>, target: &Bound<'_, PyAny>) -> PyResult<()> {
    ...
    fn execute(&self, py: Python, backend: Option<&str>, shots: Option<usize>) -> PyResult<Py<PySimulationResult>> {
    ...
    fn parse_qubit(&self, qubit: &Bound<'_, PyAny>) -> PyResult<usize> {
        if let Ok(q) = qubit.extract::<usize>() {
            ...
            self.registers.get(&reg_name)
                ...
                .copied()
                .ok_or_else(|| PyValueError::new_err(format!("Invalid register access: {}[{}]", reg_name, idx)))
        } else {
            Err(PyValueError::new_err("Qubit must be int, Qubit, or (register, index)"))
        }
    }
}
