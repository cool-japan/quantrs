...
impl OptimizationGate {
    fn qubits(&self) -> Vec<usize> {
        match self {
            OptimizationGate::H(q) | OptimizationGate::X(q) | OptimizationGate::Y(q) |
            OptimizationGate::Z(q) | OptimizationGate::S(q) | OptimizationGate::SDag(q) |
            OptimizationGate::T(q) | OptimizationGate::TDag(q) | OptimizationGate::RX(q, _) |
            OptimizationGate::RY(q, _) | OptimizationGate::RZ(q, _) | OptimizationGate::U1(q, _) |
            OptimizationGate::U2(q, _, _) | OptimizationGate::U3(q, _, _, _) => vec![*q],
            
            OptimizationGate::CNOT(c, t) | OptimizationGate::CY(c, t) | OptimizationGate::CZ(c, t) |
            OptimizationGate::CRX(c, t, _) | OptimizationGate::CRY(c, t, _) | OptimizationGate::CRZ(c, t, _) => vec![*c, *t],
    ...
    fn is_inverse_of(&self, other: &OptimizationGate) -> bool {
        match (self, other) {
            ...
            (OptimizationGate::CNOT(c1, t1), OptimizationGate::CNOT(c2, t2)) => c1 == c2 && t1 == t2,
...
impl PyCircuitTranspiler {
    ...
    fn new(optimization_level: u8, basis_gates: Option<Vec<String>>) -> Self {
        let default_basis = vec![
            ...
            "cnot".to_string(), "cy".to_string(), "cz".to_string(),
        ];
...
impl PyDeviceRouter {
    ...
    fn new(coupling_map: Vec<(usize, usize)>) -> PyResult<Self> {
        let n_physical_qubits = coupling_map.iter()
    ...
    fn route(&self, py: Python, circuit: &Bound<'_, PyList>, initial_layout: Option<Vec<usize>>) -> PyResult<(PyObject, Vec<usize>)> {
        ...
        for gate_info in circuit {
            ...
            if physical_qubits.len() == 2 {
                ...
                let connected = self.coupling_map.contains(&(p0, p1)) || 
                               self.coupling_map.contains(&(p1, p0));
                
                if !connected {
                    // Need to insert SWAPs (simplified - just fail for now)
                    return Err(PyValueError::new_err(format!(
                        "No connection between physical qubits {} and {}", p0, p1
                    )));
                }
            }
