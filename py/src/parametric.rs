...
impl PyParametricCircuit {
    ...
    pub fn set_parameter(&mut self, name: &str, value: f64) -> PyResult<()> {
        if self.parameters.contains_key(name) {
            ...
            Err(PyValueError::new_err(format!("Parameter {} not found", name)))
        }
    }
    ...
    pub fn rx(&mut self, _qubit: usize, param_name: &str, initial_value: Option<f64>) -> PyResult<()> {
        let value = initial_value.unwrap_or(0.0);
        self.add_parameter(param_name.to_string(), value)
    }
    
    pub fn ry(&mut self, _qubit: usize, param_name: &str, initial_value: Option<f64>) -> PyResult<()> {
        let value = initial_value.unwrap_or(0.0);
        self.add_parameter(param_name.to_string(), value)
    }
    
    pub fn rz(&mut self, _qubit: usize, param_name: &str, initial_value: Option<f64>) -> PyResult<()> {
        let value = initial_value.unwrap_or(0.0);
        self.add_parameter(param_name.to_string(), value)
    }
