...
use numpy::{PyArray1, PyArray2, PyArrayDyn, IntoPyArray, PyReadonlyArray1, PyReadonlyArray2};
...
impl PySciRS2LinAlg {
    ...
    fn svd<'py>(&self, py: Python<'py>, matrix: PyReadonlyArray2<'py, f64>) -> PyResult<(Py<PyArray2<f64>>, Py<PyArray1<f64>>, Py<PyArray2<f64>>)> {
    ...
    #[pyo3(text_signature = "(matrix, /)")]
    fn eig<'py>(&self, py: Python<'py>, matrix: PyReadonlyArray2<'py, Complex64>) -> PyResult<(Py<PyArray1<Complex64>>, Py<PyArray2<Complex64>>)> {
        let mat = matrix.as_array();
    ...
    #[pyo3(text_signature = "(matrix, /)")]
    fn qr<'py>(&self, py: Python<'py>, matrix: PyReadonlyArray2<'py, f64>) -> PyResult<(Py<PyArray2<f64>>, Py<PyArray2<f64>>)> {
        let mat = matrix.as_array();
        let (q, r) = SciRS2LinearAlgebra::qr(&mat.to_owned());
        
        Ok((
            q.into_pyarray(py).into(),
            r.into_pyarray(py).into(),
        ))
    }
    
    /// Matrix multiplication with optimized backend
    #[pyo3(text_signature = "(a, b, /)")]
    fn matmul<'py>(&self, py: Python<'py>, a: PyReadonlyArray2<'py, f64>, b: PyReadonlyArray2<'py, f64>) -> PyResult<Py<PyArray2<f64>>> {
        ...
        if a_arr.ncols() != b_arr.nrows() {
            return Err(PyValueError::new_err(format!(
                ...
                a_arr.nrows(), a_arr.ncols(),
                b_arr.nrows(), b_arr.ncols()
            )));
        }
    ...
    #[pyo3(text_signature = "(a, b, /)")]
    fn solve<'py>(&self, py: Python<'py>, a: PyReadonlyArray2<'py, f64>, b: PyReadonlyArray1<'py, f64>) -> PyResult<Py<PyArray1<f64>>> {
        let a_arr = a.as_array();
        let b_arr = b.as_array();
...
impl PySciRS2Stats {
    ...
    fn correlation<'py>(&self, py: Python<'py>, data: PyReadonlyArray2<'py, f64>) -> PyResult<Py<PyArray2<f64>>> {
        ...
        for i in 0..n_features {
            for j in i+1..n_features {
                ...
                let cov: f64 = col_i.iter()
                    .zip(col_j.iter())
                    .map(|(a, b)| (a - means[i]) * (b - means[j]))
                    .sum::<f64>() / (arr.nrows() - 1) as f64;
                
                let std_i = ((col_i.iter().map(|a| (a - means[i]).powi(2)).sum::<f64>()) / (arr.nrows() - 1) as f64).sqrt();
                let std_j = ((col_j.iter().map(|b| (b - means[j]).powi(2)).sum::<f64>()) / (arr.nrows() - 1) as f64).sqrt();
    ...
    fn pca<'py>(&self, py: Python<'py>, data: PyReadonlyArray2<'py, f64>, n_components: Option<usize>) -> PyResult<(Py<PyArray2<f64>>, Py<PyArray1<f64>>)> {
        let arr = data.as_array();
...
impl PySciRS2FFT {
    ...
    fn fft<'py>(&self, py: Python<'py>, signal: PyReadonlyArray1<'py, Complex64>) -> PyResult<Py<PyArray1<Complex64>>> {
    ...
    fn ifft<'py>(&self, py: Python<'py>, spectrum: PyReadonlyArray1<'py, Complex64>) -> PyResult<Py<PyArray1<Complex64>>> {
    ...
    fn fft2<'py>(&self, py: Python<'py>, image: PyReadonlyArray2<'py, Complex64>) -> PyResult<Py<PyArray2<Complex64>>> {
...
impl PyQuantumNumerics {
    ...
    fn fidelity<'py>(&self, state1: PyReadonlyArray1<'py, Complex64>, state2: PyReadonlyArray1<'py, Complex64>) -> PyResult<f64> {
        ...
        let inner_product: Complex64 = s1.iter()
            .zip(s2.iter())
            .map(|(a, b)| a.conj() * b)
            .sum();
    ...
    fn entanglement_entropy<'py>(&self, state: PyReadonlyArray1<'py, Complex64>, partition: &Bound<'py, PyList>) -> PyResult<f64> {
