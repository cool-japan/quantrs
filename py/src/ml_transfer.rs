...
#[cfg(feature = "ml")]
use quantrs2_ml::transfer::{
    TransferStrategy, PretrainedModel, QuantumTransferLearning,
};
...
impl From<PyTransferStrategy> for TransferStrategy {
    fn from(py_strategy: PyTransferStrategy) -> Self {
        match py_strategy {
            PyTransferStrategy::FineTuning { num_trainable_layers } => {
                TransferStrategy::FineTuning { num_trainable_layers }
            }
...
impl PyPretrainedModel {
    ...
    fn n_qubits(&self) -> PyResult<usize> {
        ...
        #[cfg(not(feature = "ml"))]
        {
            Err(PyValueError::new_err("ML features not enabled. Install with 'pip install quantrs2[ml]'"))
        }
    }
...
impl PyQuantumTransferLearning {
    #[new]
    pub fn new(pretrained_model: &PyPretrainedModel, strategy: PyTransferStrategy) -> PyResult<Self> {
        #[cfg(feature = "ml")]
        {
    ...
    fn qaoa_classifier(n_qubits: usize, n_layers: usize) -> PyResult<PyPretrainedModel> {
        #[cfg(feature = "ml")]
        {
            ...
            Ok(PyPretrainedModel {
                ...
                description: format!("Pretrained QAOA model with {} qubits and {} layers", n_qubits, n_layers),
            })
        }
    ...
    fn quantum_autoencoder(n_qubits: usize, latent_dim: usize) -> PyResult<PyPretrainedModel> {
        #[cfg(feature = "ml")]
        {
            ...
            Ok(PyPretrainedModel {
                ...
                description: format!("Pretrained autoencoder with {} qubits and {} latent dimensions", n_qubits, latent_dim),
            })
        }
