...
impl ParametricUGate {
    ...
    fn new(
        ...
        let extract_param = |param: Option<&Bound<'_, PyAny>>, default_name: &str| -> PyResult<Parameter> {
            if let Some(p) = param {
                if let Ok(val) = p.extract::<f64>() {
                    ...
                    Err(PyTypeError::new_err(
                        "Parameter must be a float or string",
                    ))
                }
            } else {
...
impl CustomGate {
    #[new]
    fn new(name: String, qubits: Vec<usize>, matrix: PyReadonlyArray2<Complex64>) -> PyResult<(Self, Gate)> {
        ...
        if matrix.shape()[0] != expected_size || matrix.shape()[1] != expected_size {
            return Err(PyValueError::new_err(format!(
                ...
                matrix.shape()[0], matrix.shape()[1], expected_size, expected_size, qubits.len()
            )));
        }
        ...
        Ok((custom, Gate { gate: Box::new(gate_impl) }))
    }
}
