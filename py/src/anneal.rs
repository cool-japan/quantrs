...
use quantrs2_anneal::{
    ...
    penalty_optimization::{PenaltyOptimizer, PenaltyConfig},
...
impl PyQuboModel {
    ...
    pub fn new(n_vars: usize) -> Self {
        ...
        {
            Self {
                inner: Some(IsingQuboModel::new(n_vars)),
                n_vars,
            }
        }
    ...
    fn add_linear(&mut self, var: usize, coeff: f64) -> PyResult<()> {
        ...
        #[cfg(not(feature = "anneal"))]
        {
            Err(PyValueError::new_err("Anneal features not enabled. Install with 'pip install quantrs2[anneal]'"))
        }
    }
    ...
    fn add_quadratic(&mut self, var1: usize, var2: usize, coeff: f64) -> PyResult<()> {
        ...
        {
            if let Some(model) = &mut self.inner {
                model.add_quadratic(var1, var2, coeff);
                Ok(())
            } else {
                Err(PyValueError::new_err("Model not initialized"))
            }
    ...
    fn update_penalties(
        ...
        #[cfg(feature = "anneal")]
        {
            if let Some(optimizer) = &mut self.inner {
                let penalties = optimizer.update_penalties(&chain_breaks, constraint_violations);
                Ok(penalties)
            } else {
    ...
    pub fn new(
        ...
        #[cfg(feature = "anneal")]
        {
            let config = LayoutConfig {
                use_coordinates: use_coordinates.unwrap_or(true),
                chain_strength_factor: chain_strength_factor.unwrap_or(1.2),
                target_topology: target_topology,
            };
            
            let embedder = LayoutAwareEmbedder::new(config)
                .map_err(|e| PyValueError::new_err(format!("Failed to create embedder: {}", e)))?;
    ...
    fn find_embedding(
        ...
        #[cfg(feature = "anneal")]
        {
            if let Some(embedder) = &mut self.inner {
                let embedding = embedder.find_embedding(&source_edges, &target_graph, initial_chains)
