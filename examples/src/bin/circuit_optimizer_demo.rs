...
fn optimize_complex_circuit() {
    ...
    let optimizer = CircuitOptimizer::<6>::new()
...
fn optimize_with_custom_passes() {
    ...
    circuit
        ...
        .cnot(QubitId::new(2), QubitId::new(3)).unwrap()
        .cnot(QubitId::new(3), QubitId::new(4)).unwrap()
        .h(QubitId::new(4)).unwrap();
    
    println!("Circuit created for custom optimization");
    
    // Create optimizer with only specific passes
    use quantrs2_circuit::optimizer::{
        OptimizationContext, PassResult, PeepholeOptimizer, CommutationOptimizer
    };
...
fn optimize_for_hardware() {
    ...
    let optimizer = CircuitOptimizer::<5>::new()
        .add_pass(OptimizationPassType::Hardware(hw_optimizer));
