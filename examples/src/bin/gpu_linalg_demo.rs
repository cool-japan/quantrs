...
async fn demo_matrix_multiplication(gpu: &GpuLinearAlgebra) -> Result<(), Box<dyn std::error::Error>> {
...
async fn demo_quantum_gates(gpu: &GpuLinearAlgebra) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let diff: f64 = cnot_squared.iter()
        .zip(identity_4.iter())
        .map(|(a, b)| (a - Complex64::new(*b, 0.0)).norm())
        .sum();
    println!("Distance from identity: {:.2e}", diff);
...
async fn compare_performance() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for size in [16, 32, 64, 128, 256] {
        ...
        if gpu_time < cpu_time {
            ...
            println!("  CPU is faster by: {:.2}x", gpu_time.as_secs_f64() / cpu_time.as_secs_f64());
        }
        println!();
    }
