...
fn main() {
    ...
    let (qubo_model, var_map) = formulate_maxcut_qubo(&graph);
    println!("\nFormulated MaxCut problem as QUBO with {} variables", qubo_model.num_variables);
        ...
        Ok(simulator) => {
            match simulator.solve(&ising_model) {
                Ok(result) => {
                    ...
                    println!("Cut size: {} out of {} total edges", cut_size, graph.edges.len());
                    println!("Partition A: {:?}", graph.vertices.iter()
                        .enumerate()
                        .filter(|&(i, _)| binary_solution[i])
                        .map(|(_, v)| v)
                        .collect::<Vec<_>>());
                    println!("Partition B: {:?}", graph.vertices.iter()
                        .enumerate()
                        .filter(|&(i, _)| !binary_solution[i])
                        .map(|(_, v)| v)
                        .collect::<Vec<_>>());
...
fn formulate_maxcut_qubo(graph: &Graph) -> (QuboModel, std::collections::HashMap<String, usize>) {
    ...
    for &(i, j, weight) in &graph.edges {
        ...
        builder.minimize_quadratic(&variables[i], &variables[j], 2.0 * weight).unwrap();
    }
