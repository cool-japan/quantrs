...
use quantrs_tytan::{symbols_list, Compile, SASampler, Auto_array};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (i, r) in result.iter().enumerate() {
        println!("Solution {}: Energy = {}, Occurrences = {}", i+1, r.energy, r.occurrences);
    ...
    if !result.is_empty() {
        ...
        let (arr, subs) = Auto_array::new(best).get_ndarray("q{}_{}").expect("Failed to convert to array");
