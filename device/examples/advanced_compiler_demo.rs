...
async fn demo_advanced_optimization() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("📊 Original circuit: {} gates, estimated depth: {}", 
             circuit.gates().len(), 
    ...
    println!("  - Gate count: {} → {} ({}% reduction)", 
             result.optimization_stats.original_gate_count,
             result.optimization_stats.optimized_gate_count,
             ((result.optimization_stats.original_gate_count - result.optimization_stats.optimized_gate_count) as f64 / 
              result.optimization_stats.original_gate_count as f64 * 100.0));
    
    println!("  - Circuit depth: {} → {} ({}% reduction)",
             result.optimization_stats.original_depth,
             result.optimization_stats.optimized_depth,
             ((result.optimization_stats.original_depth - result.optimization_stats.optimized_depth) as f64 / 
              result.optimization_stats.original_depth as f64 * 100.0));
...
async fn demo_adaptive_compilation() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (i, circuit) in circuits.into_iter().enumerate() {
        ...
        println!("  - Optimization iterations: {}", 
...
fn create_aws_config() -> CompilerConfig {
    ...
    config.target = CompilationTarget::AWSBraket {
        ...
        supported_gates: ["x", "y", "z", "h", "cnot", "swap"].iter().map(|s| s.to_string()).collect(),
        max_shots: 1000,
...
fn create_azure_config() -> CompilerConfig {
    ...
    config.target = CompilationTarget::AzureQuantum {
        ...
        supported_operations: ["x", "y", "z", "h", "cnot", "swap"].iter().map(|s| s.to_string()).collect(),
...
fn print_compilation_summary(platform: &str, result: &CompilationResult) {
    println!("  ✓ {} compilation completed in {:.2}ms", 
             platform, 
             result.compilation_time.as_millis());
