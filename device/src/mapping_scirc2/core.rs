...
impl SciRS2QubitMapper {
    ...
    pub fn map_circuit<const N: usize>(
        ...
        let community_analysis = self.perform_community_analysis(&logical_graph, &physical_graph)?;
                    ...
                    for gate in circuit.gates() {
                        ...
                        if qubits.len() == 1 {
                            ...
                            if let Some(&physical_q) = mapping.get(&q) {
                                total_fidelity += calibration.single_qubit_fidelity(physical_q).unwrap_or(0.99);
                                gate_count += 1;
                            }
                            ...
                            if let (Some(&pq1), Some(&pq2)) = (mapping.get(&q1), mapping.get(&q2)) {
                                total_fidelity += calibration.gate_fidelity(pq1, pq2).unwrap_or(0.95);
                                gate_count += 1;
                            }
                        }
                    }
