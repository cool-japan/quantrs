...
use crate::calibration::{DeviceCalibration, CalibrationManager};
...
impl CalibrationOptimizer {
    ...
    pub fn optimize_circuit(
        ...
        if !self.calibration_manager.is_calibration_valid(device_id) {
            return Err(QuantRS2Error::InvalidInput(
                format!("No valid calibration for device {}", device_id)
            ));
        }
        
        let calibration = self.calibration_manager.get_calibration(device_id)
    ...
    fn rank_qubits_by_quality(&self, calibration: &DeviceCalibration) -> Vec<(QubitId, f64)> {
        let mut qubit_scores: Vec<(QubitId, f64)> = calibration.qubit_calibrations
            .iter()
            .map(|(id, cal)| {
    ...
    fn estimate_circuit_fidelity(
        ...
        for gate in circuit.gates() {
            ...
            let fidelity = self.calibration_manager
    ...
    fn estimate_circuit_duration(
        ...
        for gate in circuit.gates() {
            let qubits = gate.qubits();
            let duration = self.calibration_manager
...
impl FidelityEstimator {
    ...
    pub fn estimate_process_fidelity(&self, circuit: &Circuit) -> QuantRS2Result<f64> {
        ...
        for gate in circuit.gates() {
            ...
            let error_rate = match qubits.len() {
                1 => {
                    self.calibration.single_qubit_gates
                        .get(gate.name())
                ...
                2 => {
                    self.calibration.two_qubit_gates
        ...
        let avg_readout_error: f64 = self.calibration.readout_calibration
            .qubit_readout
            .values()
            .map(|r| 1.0 - (r.p0_given_0 + r.p1_given_1) / 2.0)
            .sum::<f64>() / self.calibration.readout_calibration.qubit_readout.len() as f64;
        
        total_infidelity += avg_readout_error * circuit.qubit_count() as f64;
    ...
    pub fn estimate_state_fidelity(
        &self,
        circuit: &Circuit,
        ...
        if include_decoherence {
            ...
            let circuit_duration = circuit.gates().iter()
                .map(|gate| {
                    let qubits = gate.qubits();
                    match qubits.len() {
                        1 => self.calibration.single_qubit_gates
                            .get(gate.name())
                            .and_then(|g| g.qubit_data.get(&qubits[0]))
                            .map(|d| d.duration)
                            .unwrap_or(20.0),
                        2 => self.calibration.two_qubit_gates
                            .get(&(qubits[0], qubits[1]))
                            .map(|g| g.duration)
                            .unwrap_or(200.0),
                        _ => 500.0,
            ...
            // Average T1 and T2
            let avg_t1 = self.calibration.qubit_calibrations.values()
                .map(|q| q.t1)
                .sum::<f64>() / self.calibration.qubit_calibrations.len() as f64;
            
            let avg_t2 = self.calibration.qubit_calibrations.values()
                .map(|q| q.t2)
                .sum::<f64>() / self.calibration.qubit_calibrations.len() as f64;
