...
impl std::fmt::Debug for TranslationMethod {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::ParameterizedDecomposition(_) => write!(f, "ParameterizedDecomposition(<function>)"),
    ...
    fn clone(&self) -> Self {
        match self {
            ...
            Self::ParameterizedDecomposition(_) => {
                panic!("Cannot clone ParameterizedDecomposition - use Arc<TranslationMethod> instead")
            }
...
impl GateTranslator {
    ...
    fn init_ibm_gates(&mut self) {
        ...
        self.native_gates.insert(HardwareBackend::IBMQuantum, gate_set);
    ...
    fn init_google_gates(&mut self) {
        ...
        self.native_gates.insert(HardwareBackend::GoogleSycamore, gate_set);
    ...
    fn init_amazon_gates(&mut self) {
        ...
        self.native_gates.insert(HardwareBackend::AmazonBraket, gate_set);
    }
    ...
    fn init_azure_gates(&mut self) {
        ...
        self.native_gates.insert(HardwareBackend::AzureQuantum, gate_set);
    }
    ...
    fn init_honeywell_gates(&mut self) {
        ...
        self.native_gates.insert(HardwareBackend::Honeywell, gate_set);
    }
    ...
    pub fn translate_gate(
        ...
        if let Some(rule) = self.translation_rules.get(&(backend, gate_name.to_string())) {
            let decomposition = match &rule.method {
                ...
                TranslationMethod::Custom(func) => {
                    func(gate)?
                }
            };
            
            // Cache the result
            self.decomposition_cache.insert(cache_key, decomposition.clone());
        ...
        match gate.num_qubits() {
            ...
            _ => Err(QuantRS2Error::InvalidInput(
                format!("No translation available for {}-qubit gate {}", gate.num_qubits(), gate_name)
            ))
        }
    }
        ...
        match method {
            ...
            _ => {
                Err(QuantRS2Error::InvalidInput(
                    format!("Synthesis method {:?} not yet implemented", method)
                ))
            }
        }
    }
    ...
    fn add_decomposed_gate<const N: usize>(
        ...
        match gate.native_gate.as_str() {
            ...
            "x" => { circuit.x(gate.qubits[0]); }
            "sx" => { circuit.sx(gate.qubits[0]); }
            "rz" => { circuit.rz(gate.qubits[0], gate.parameters[0]); }
            "cx" => { circuit.cnot(gate.qubits[0], gate.qubits[1]); }
            
            // IonQ gates
            "rx" => { circuit.rx(gate.qubits[0], gate.parameters[0]); }
            "ry" => { circuit.ry(gate.qubits[0], gate.parameters[0]); }
            "xx" => {
            ...
            // Common gates
            "h" => { circuit.h(gate.qubits[0]); }
            "y" => { circuit.y(gate.qubits[0]); }
            "z" => { circuit.z(gate.qubits[0]); }
            "s" => { circuit.s(gate.qubits[0]); }
            "t" => { circuit.t(gate.qubits[0]); }
            "cnot" => { circuit.cnot(gate.qubits[0], gate.qubits[1]); }
            "cz" => { circuit.cz(gate.qubits[0], gate.qubits[1]); }
            "swap" => { circuit.swap(gate.qubits[0], gate.qubits[1]); }
            "ccnot" => { circuit.toffoli(gate.qubits[0], gate.qubits[1], gate.qubits[2]); }
            
            _ => {
                return Err(QuantRS2Error::InvalidInput(
                    format!("Unknown native gate: {}", gate.native_gate)
                ));
            }
        }
...
mod tests {
    ...
    fn test_gate_translator_creation() {
        ...
        assert!(translator.get_native_gates(HardwareBackend::IBMQuantum).is_some());
        assert!(translator.get_native_gates(HardwareBackend::GoogleSycamore).is_some());
    ...
    fn test_hadamard_translation_ibm() {
        ...
        let decomposed = translator.translate_gate(&h_gate, HardwareBackend::IBMQuantum).unwrap();
    ...
    fn test_circuit_translation() {
        ...
        let translated = translator.translate_circuit(&circuit, HardwareBackend::IBMQuantum).unwrap();
