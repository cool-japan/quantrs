...
use crate::topology_analysis::{TopologyAnalyzer, AllocationStrategy};
...
impl AdvancedQubitRouter {
    ...
    fn find_optimal_initial_mapping(
        ...
        let physical_qubits = self.analyzer.allocate_qubits(
            num_qubits,
    ...
    fn calculate_mapping_score(
        ...
        if let Some(log_node) = logical_graph.node_indices()
    ...
    fn are_connected(&self, q1: usize, q2: usize) -> bool {
        self.topology.gate_properties.contains_key(&(q1 as u32, q2 as u32)) ||
        self.topology.gate_properties.contains_key(&(q2 as u32, q1 as u32))
    }
    ...
    fn get_distance(&self, q1: usize, q2: usize) -> usize {
        ...
        if let (Some(n1), Some(n2)) = (
            self.topology.connectivity.node_indices()
    ...
    fn update_front_layer(
        ...
        for (gate_id, dep) in dependencies.iter().enumerate() {
            if !dep.scheduled {
                ...
                if dep.predecessors.iter().all(|&pred| state.scheduled_gates.contains(&pred)) {
    ...
    fn sabre_heuristic(
        ...
        let executable_count = front_layer.iter()
    ...
    fn route_astar(
        ...
        while let Some(current) = open_set.pop() {
            ...
            // Generate neighbors
            let neighbors = self.generate_astar_neighbors(
                &current.routing_state,
                &dependencies,
                &mut metrics,
            );
        ...
        Err(DeviceError::RoutingError("A* search failed to find solution".to_string()))
    }
    ...
    fn find_swap_path(&self, start: usize, target: usize) -> DeviceResult<Vec<usize>> {
        ...
        let start_node = self.topology.connectivity.node_indices()
            .find(|&n| self.topology.connectivity[n] == start as u32)
            .ok_or(DeviceError::RoutingError("Start qubit not found".to_string()))?;
            
        let target_node = self.topology.connectivity.node_indices()
            .find(|&n| self.topology.connectivity[n] == target as u32)
            .ok_or(DeviceError::RoutingError("Target qubit not found".to_string()))?;
        ...
        if let Some((_, path)) = result {
            ...
            Err(DeviceError::RoutingError("No path found between qubits".to_string()))
        }
    }
    ...
    fn route_hybrid(
        ...
        let strategies = [
            (AdvancedRoutingStrategy::SABRE { heuristic_weight: 0.5 }, 0.4),
            (AdvancedRoutingStrategy::AStarLookahead { lookahead_depth: 3 }, 0.4),
        ...
        for (strategy, weight) in strategies {
            let mut temp_router = AdvancedQubitRouter::new(
                self.topology.clone(),
                strategy,
                self.rng.gen(),
            );
            ...
            if let Ok(result) = match strategy {
                AdvancedRoutingStrategy::SABRE { heuristic_weight } => {
                    temp_router.route_sabre(dependencies.clone(), initial_mapping.clone(), heuristic_weight)
                }
                AdvancedRoutingStrategy::AStarLookahead { lookahead_depth } => {
                    temp_router.route_astar(dependencies.clone(), initial_mapping.clone(), lookahead_depth)
                }
                AdvancedRoutingStrategy::TokenSwapping => {
        ...
        best_result.ok_or(DeviceError::RoutingError("Hybrid routing failed".to_string()))
    }
}
...
mod tests {
    ...
    fn test_sabre_routing() {
        ...
        circuit.add_gate(quantrs2_core::gate::multi::CNOT {
    ...
    fn test_hybrid_routing() {
        ...
        let mut circuit = Circuit::<4>::new();
        circuit.add_gate(quantrs2_core::gate::single::Hadamard {
