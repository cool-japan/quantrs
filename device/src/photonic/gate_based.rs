...
impl PhotonicQubitState {
    ...
    pub fn hadamard(&mut self) {
        ...
        self.relative_phase = if old_amp_1 * old_phase.sin() >= 0.0 { 0.0 } else { PI };
    }
...
impl PhotonicGates {
    ...
    pub fn polarization_x() -> PhotonicGateImpl {
        PhotonicGateImpl {
            ...
            optical_elements: vec![
                OpticalElement::HalfWaveplate { angle: PI / 4.0 },
            ],
    ...
    pub fn polarization_z() -> PhotonicGateImpl {
        PhotonicGateImpl {
            ...
            encoding_required: PhotonicQubitEncoding::Polarization,
            optical_elements: vec![
                OpticalElement::PhaseShift { phase: PI },
            ],
            success_probability: 1.0,
            fidelity: 0.999,
        }
    ...
    pub fn polarization_hadamard() -> PhotonicGateImpl {
        PhotonicGateImpl {
            ...
            encoding_required: PhotonicQubitEncoding::Polarization,
            optical_elements: vec![
                OpticalElement::HalfWaveplate { angle: PI / 8.0 },
            ],
            success_probability: 1.0,
            fidelity: 0.995,
        }
    ...
    pub fn dual_rail_cnot(control_rails: (usize, usize), target_rails: (usize, usize)) -> PhotonicGateImpl {
        PhotonicGateImpl {
    ...
    pub fn path_cz(control_paths: (usize, usize), target_paths: (usize, usize)) -> PhotonicGateImpl {
        PhotonicGateImpl {
    ...
    pub fn arbitrary_rotation(axis: RotationAxis, angle: f64) -> PhotonicGateImpl {
        let elements = match axis {
            ...
            RotationAxis::Z => vec![
                OpticalElement::PhaseShift { phase: angle },
            ],
        };
...
impl PhotonicGateImpl {
    ...
    pub fn apply(&self, state: &mut PhotonicQubitState) -> PhotonicGateResult<bool> {
        ...
        if std::mem::discriminant(&state.encoding) != std::mem::discriminant(&self.encoding_required) {
            return Err(PhotonicGateError::InvalidEncoding(
                format!("Gate requires {:?}, but state uses {:?}", 
        ...
        match element {
            OpticalElement::HalfWaveplate { angle } => {
                ...
                match state.encoding {
                    ...
                    _ => return Err(PhotonicGateError::InvalidEncoding(
            ...
            OpticalElement::BeamSplitter { transmittance, phase, .. } => {
                ...
                state.amplitude_0 = (cos_theta * old_amp_0 + sin_theta * old_amp_1 * phase.cos()).abs();
                state.amplitude_1 = (sin_theta * old_amp_0 - cos_theta * old_amp_1 * phase.cos()).abs();
            }
...
impl PhotonicCircuitCompiler {
    ...
    pub fn compile_gate_sequence(
        ...
        for gate in gates {
            ...
            if total_requirements.waveplates + requirements.waveplates > self.hardware_constraints.available_waveplates {
                return Err(PhotonicGateError::InsufficientPhotons(
                    "Not enough waveplates available".to_string()
                ));
            }
            
            if total_requirements.beam_splitters + requirements.beam_splitters > self.hardware_constraints.available_beam_splitters {
                return Err(PhotonicGateError::InsufficientPhotons(
        ...
        for gate in &implementation.gates {
            ...
            if accumulated_phase.abs() > 1e-10 {
                optimized_gates.push(PhotonicGateImpl {
                    ...
                    encoding_required: PhotonicQubitEncoding::Polarization,
                    optical_elements: vec![OpticalElement::PhaseShift { phase: accumulated_phase }],
                    success_probability: 1.0,
                    fidelity: 0.999,
                });
