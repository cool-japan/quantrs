...
impl GaussianState {
    ...
    pub fn squeezed_vacuum(r: f64, phi: f64, mode: usize, num_modes: usize) -> CVResult<Self> {
        ...
        state.covariance[x_idx][x_idx] = 0.5 * (exp_2r + exp_neg_2r + (exp_2r - exp_neg_2r) * cos_2phi) / 2.0;
        state.covariance[p_idx][p_idx] = 0.5 * (exp_2r + exp_neg_2r - (exp_2r - exp_neg_2r) * cos_2phi) / 2.0;
    ...
    pub fn two_mode_squeeze(&mut self, r: f64, phi: f64, mode1: usize, mode2: usize) -> CVResult<()> {
        ...
        let orig_cov = [
            [self.covariance[x1_idx][x1_idx], self.covariance[x1_idx][p1_idx], 
             self.covariance[x1_idx][x2_idx], self.covariance[x1_idx][p2_idx]],
            [self.covariance[p1_idx][x1_idx], self.covariance[p1_idx][p1_idx], 
             self.covariance[p1_idx][x2_idx], self.covariance[p1_idx][p2_idx]],
            [self.covariance[x2_idx][x1_idx], self.covariance[x2_idx][p1_idx], 
             self.covariance[x2_idx][x2_idx], self.covariance[x2_idx][p2_idx]],
            [self.covariance[p2_idx][x1_idx], self.covariance[p2_idx][p1_idx], 
             self.covariance[p2_idx][x2_idx], self.covariance[p2_idx][p2_idx]],
        ];
    ...
    pub fn beamsplitter(&mut self, theta: f64, phi: f64, mode1: usize, mode2: usize) -> CVResult<()> {
        ...
        let orig_mean = [
            self.mean[indices[0]], self.mean[indices[1]], 
            self.mean[indices[2]], self.mean[indices[3]]
        ];
        ...
        let orig_cov = [
            [self.covariance[indices[0]][indices[0]], self.covariance[indices[0]][indices[1]], 
             self.covariance[indices[0]][indices[2]], self.covariance[indices[0]][indices[3]]],
            [self.covariance[indices[1]][indices[0]], self.covariance[indices[1]][indices[1]], 
             self.covariance[indices[1]][indices[2]], self.covariance[indices[1]][indices[3]]],
            [self.covariance[indices[2]][indices[0]], self.covariance[indices[2]][indices[1]], 
             self.covariance[indices[2]][indices[2]], self.covariance[indices[2]][indices[3]]],
            [self.covariance[indices[3]][indices[0]], self.covariance[indices[3]][indices[1]], 
             self.covariance[indices[3]][indices[2]], self.covariance[indices[3]][indices[3]]],
        ];
...
impl CVGateSet {
    ...
    pub fn displacement(alpha: Complex, mode: usize) -> impl Fn(&mut GaussianState) -> CVResult<()> {
    ...
    pub fn two_mode_squeezing(r: f64, phi: f64, mode1: usize, mode2: usize) -> impl Fn(&mut GaussianState) -> CVResult<()> {
        move |state: &mut GaussianState| state.two_mode_squeeze(r, phi, mode1, mode2)
    }

    /// Create beamsplitter operation
    pub fn beamsplitter(theta: f64, phi: f64, mode1: usize, mode2: usize) -> impl Fn(&mut GaussianState) -> CVResult<()> {
