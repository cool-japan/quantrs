...
impl ExtrapolationFitter {
    ...
    pub fn fit_and_extrapolate(
        ...
        match method {
            ...
            ExtrapolationMethod::Richardson => Self::richardson_extrapolation(scale_factors, values),
    ...
    fn richardson_extrapolation(x: &[f64], y: &[f64]) -> DeviceResult<ZNEResult> {
        ...
        let mut paired: Vec<(f64, f64)> = x.iter().zip(y.iter()).map(|(&xi, &yi)| (xi, yi)).collect();
        ...
        for j in 1..paired.len() {
            for i in 0..(paired.len() - j) {
                ...
                let factor = x_ij / x_i;
                let value = (factor * richardson_table[i + 1][j - 1] - richardson_table[i][j - 1]) / (factor - 1.0);
    ...
    fn adaptive_fit(x: &[f64], y: &[f64]) -> DeviceResult<ZNEResult> {
        ...
        best_result.ok_or(DeviceError::APIError(
            "Adaptive fitting failed".to_string(),
        ))
    }
