...
impl ParametricCircuit {
    ...
    fn extract_expr_parameter_names(&mut self, expr: &ParameterExpression) {
        match expr {
            ...
            ParameterExpression::Add(p1, p2) | ParameterExpression::Mul(p1, p2) | ParameterExpression::Div(p1, p2) => {
                self.extract_parameter_names(p1);
                self.extract_parameter_names(p2);
            }
            ParameterExpression::Sin(p) | ParameterExpression::Cos(p) | ParameterExpression::Pow(p, _) => {
    ...
    pub fn bind_parameters<const N: usize>(
        ...
        for gate in &self.gates {
            ...
            match gate.gate_type.as_str() {
                "H" => {
                    circuit.add_gate(Hadamard { target: QubitId(gate.qubits[0] as u32) })
                        .map_err(|e| DeviceError::APIError(e.to_string()))?;
                }
                "X" => {
                    circuit.add_gate(PauliX { target: QubitId(gate.qubits[0] as u32) })
                        .map_err(|e| DeviceError::APIError(e.to_string()))?;
                }
                "Y" => {
                    circuit.add_gate(PauliY { target: QubitId(gate.qubits[0] as u32) })
                        .map_err(|e| DeviceError::APIError(e.to_string()))?;
                }
                "Z" => {
                    circuit.add_gate(PauliZ { target: QubitId(gate.qubits[0] as u32) })
                        .map_err(|e| DeviceError::APIError(e.to_string()))?;
                }
                "RX" => {
                    let angle = self.evaluate_parameter(&gate.parameters[0], params)?;
                    circuit.add_gate(RotationX { target: QubitId(gate.qubits[0] as u32), theta: angle })
                        .map_err(|e| DeviceError::APIError(e.to_string()))?;
                }
                "RY" => {
                    let angle = self.evaluate_parameter(&gate.parameters[0], params)?;
                    circuit.add_gate(RotationY { target: QubitId(gate.qubits[0] as u32), theta: angle })
                        .map_err(|e| DeviceError::APIError(e.to_string()))?;
                }
                "RZ" => {
                    let angle = self.evaluate_parameter(&gate.parameters[0], params)?;
                    circuit.add_gate(RotationZ { target: QubitId(gate.qubits[0] as u32), theta: angle })
                        .map_err(|e| DeviceError::APIError(e.to_string()))?;
                }
                "CNOT" => {
                ...
                _ => {
                    return Err(DeviceError::UnsupportedOperation(
                        format!("Gate type {} not supported", gate.gate_type)
                    ));
                }
            }
        }
    ...
    pub fn bind_parameters_array<const N: usize>(
        ...
        let param_map: HashMap<String, f64> = self.parameter_names
            .iter()
            .zip(params.iter())
    ...
    fn evaluate_parameter(&self, param: &Parameter, values: &HashMap<String, f64>) -> DeviceResult<f64> {
        match param {
            Parameter::Fixed(val) => Ok(*val),
            Parameter::Named(name) => values.get(name)
                .copied()
    ...
    fn evaluate_expression(
        ...
        match expr {
            ParameterExpression::Param(name) => values.get(name)
                .copied()
                .ok_or_else(|| DeviceError::APIError(format!("Missing parameter: {}", name))),
    ...
    fn instantiate_gate(
        ...
        match gate.gate_type.as_str() {
            "H" => Ok(Box::new(Hadamard { target: QubitId(gate.qubits[0] as u32) })),
            "X" => Ok(Box::new(PauliX { target: QubitId(gate.qubits[0] as u32) })),
            "Y" => Ok(Box::new(PauliY { target: QubitId(gate.qubits[0] as u32) })),
            "Z" => Ok(Box::new(PauliZ { target: QubitId(gate.qubits[0] as u32) })),
            "RX" => {
                let angle = self.evaluate_parameter(&gate.parameters[0], values)?;
                Ok(Box::new(RotationX { target: QubitId(gate.qubits[0] as u32), theta: angle }))
            }
            "RY" => {
                let angle = self.evaluate_parameter(&gate.parameters[0], values)?;
                Ok(Box::new(RotationY { target: QubitId(gate.qubits[0] as u32), theta: angle }))
            }
            "RZ" => {
                let angle = self.evaluate_parameter(&gate.parameters[0], values)?;
                Ok(Box::new(RotationZ { target: QubitId(gate.qubits[0] as u32), theta: angle }))
            }
...
mod tests {
    ...
    fn test_parameter_expressions() {
        ...
        let circuit = ParametricCircuitBuilder::new(1)
            .rx(0, expr)
            .build();
