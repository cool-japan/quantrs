...
impl CalibrationManager {
    ...
    pub fn load_calibration(&mut self, path: &str) -> QuantRS2Result<()> {
        ...
        let calibration: DeviceCalibration = serde_json::from_str(&data)
            .map_err(|e| QuantRS2Error::InvalidInput(format!("Failed to parse calibration: {}", e)))?;
    ...
    pub fn save_calibration(&self, device_id: &str, path: &str) -> QuantRS2Result<()> {
        let calibration = self.get_calibration(device_id)
            .ok_or_else(|| QuantRS2Error::InvalidInput(format!("No calibration for device {}", device_id)))?;
        
        let data = serde_json::to_string_pretty(calibration)
            .map_err(|e| QuantRS2Error::InvalidInput(format!("Failed to serialize calibration: {}", e)))?;
        
        std::fs::write(path, data)
            .map_err(|e| QuantRS2Error::InvalidInput(format!("Failed to write calibration: {}", e)))?;
    ...
    pub fn update_calibration(&mut self, calibration: DeviceCalibration) {
        ...
        if let Some(old_cal) = self.calibrations.get(&device_id) {
            self.history.push((device_id.clone(), timestamp, old_cal.clone()));
...
impl CalibrationBuilder {
    ...
    pub fn add_qubit_calibration(mut self, calibration: QubitCalibration) -> Self {
        self.qubit_calibrations.insert(calibration.qubit_id, calibration);
        self
    }
    
    /// Add single-qubit gate calibration
    pub fn add_single_qubit_gate(mut self, gate_name: String, calibration: SingleQubitGateCalibration) -> Self {
        self.single_qubit_gates.insert(gate_name, calibration);
        self
    }
    
    /// Add two-qubit gate calibration
    pub fn add_two_qubit_gate(mut self, control: QubitId, target: QubitId, calibration: TwoQubitGateCalibration) -> Self {
    ...
    pub fn build(self) -> QuantRS2Result<DeviceCalibration> {
        let readout_calibration = self.readout_calibration
            .ok_or_else(|| QuantRS2Error::InvalidInput("Readout calibration required".into()))?;
        
        let crosstalk_matrix = self.crosstalk_matrix
...
pub fn create_ideal_calibration(device_id: String, num_qubits: usize) -> DeviceCalibration {
    ...
    for gate_name in ["X", "Y", "Z", "H", "S", "T", "RX", "RY", "RZ"] {
        ...
        for i in 0..num_qubits {
            qubit_data.insert(QubitId(i as u32), SingleQubitGateData {
                error_rate: 0.001,
                fidelity: 0.999,
                duration: 20.0, // 20 ns
                amplitude: 1.0,
    ...
    for i in 0..num_qubits {
        qubit_readout.insert(QubitId(i as u32), QubitReadoutData {
            p0_given_0: 0.999,
            p1_given_1: 0.999,
