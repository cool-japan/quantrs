    ...
    fn partition_circuit(
        ...
        if let Some((node_id, _)) = nodes.iter().next() {
            let partition = CircuitPartition {
                ...
                dependencies: vec![],
                input_qubits: (0..circuit.qubit_count)
                    .map(|i| QubitId {
                        node_id: node_id.clone(),
                        local_id: i,
                    ...
                    .collect(),
                output_qubits: (0..circuit.qubit_count)
                    .map(|i| QubitId {
                        node_id: node_id.clone(),
                        local_id: i,
                        global_id: Uuid::new_v4(),
                ...
                resource_requirements: ResourceRequirements {
                    qubits_needed: circuit.qubit_count,
                    gates_count: circuit.gates.len() as u32,
                    memory_mb: 100,
