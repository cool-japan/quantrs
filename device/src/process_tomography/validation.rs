...
impl SciRS2ProcessTomographer {
    ...
    fn perform_cross_validation(&self, experimental_data: &ExperimentalData) -> DeviceResult<CrossValidationResults> {
        ...
        for fold_idx in 0..num_folds {
            ...
            let validation_data = self.create_validation_fold(experimental_data, val_start, val_end)?;
    ...
    fn perform_bootstrap_validation(&self, experimental_data: &ExperimentalData) -> DeviceResult<BootstrapResults> {
        ...
        let confidence_intervals = self.calculate_bootstrap_confidence_intervals(&bootstrap_metrics)?;
    ...
    fn perform_benchmark_validation(&self, experimental_data: &ExperimentalData) -> DeviceResult<BenchmarkResults> {
        ...
        for (idx, benchmark_name) in benchmarks.iter().enumerate() {
            ...
            let fidelity = self.calculate_process_fidelity_between(&experimental_process, &benchmark_process)?;
    ...
    fn perform_model_selection(&self, experimental_data: &ExperimentalData) -> DeviceResult<ModelSelectionResults> {
    ...
    fn create_training_fold(
        ...
        for (i, (&result, &uncertainty)) in data.measurement_results
            .iter()
    ...
    fn evaluate_process_quality(
        ...
        for (observed, &uncertainty) in validation_data.measurement_results
            .iter()
    ...
    fn collect_bootstrap_metrics(
        ...
        bootstrap_metrics.entry("process_fidelity".to_string())
            .or_insert_with(Vec::new)
            .push(metrics.process_fidelity);
        
        bootstrap_metrics.entry("average_gate_fidelity".to_string())
            .or_insert_with(Vec::new)
            .push(metrics.average_gate_fidelity);
        
        bootstrap_metrics.entry("unitarity".to_string())
            .or_insert_with(Vec::new)
            .push(metrics.unitarity);
        
        bootstrap_metrics.entry("entangling_power".to_string())
            .or_insert_with(Vec::new)
    ...
    fn calculate_bootstrap_bias(&self, bootstrap_samples: &[ProcessMetrics]) -> DeviceResult<HashMap<String, f64>> {
        ...
        // Calculate means
        let mean_fidelity = bootstrap_samples.iter()
            .map(|m| m.process_fidelity)
            .sum::<f64>() / bootstrap_samples.len() as f64;
        
        let mean_unitarity = bootstrap_samples.iter()
            .map(|m| m.unitarity)
            .sum::<f64>() / bootstrap_samples.len() as f64;
