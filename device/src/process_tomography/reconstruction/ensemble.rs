...
use super::{linear_inversion, maximum_likelihood, compressed_sensing, bayesian};
...
pub fn reconstruct_ensemble(
    ...
    let (li_process, li_quality) = linear_inversion::reconstruct_linear_inversion(tomographer, experimental_data)?;
    let (mle_process, mle_quality) = maximum_likelihood::reconstruct_maximum_likelihood(tomographer, experimental_data)?;
    let (cs_process, cs_quality) = compressed_sensing::reconstruct_compressed_sensing(tomographer, experimental_data)?;
    let (bayes_process, bayes_quality) = bayesian::reconstruct_bayesian(tomographer, experimental_data)?;
    ...
    for i in 0..dim {
        for j in 0..dim {
            for k in 0..dim {
                for l in 0..dim {
                    ensemble_process[[i, j, k, l]] = 
                        (li_process[[i, j, k, l]] * li_weight +
                         mle_process[[i, j, k, l]] * mle_weight +
                         cs_process[[i, j, k, l]] * cs_weight +
                         bayes_process[[i, j, k, l]] * bayes_weight) / total_weight;
                }
            }
        }
    }
