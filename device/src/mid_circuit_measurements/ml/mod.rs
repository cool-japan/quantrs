...
impl MLOptimizer {
    ...
    fn extract_features(&self, measurement_history: &[MeasurementEvent]) -> DeviceResult<MLFeatures> {
        ...
        let statistical_features = StatisticalFeatures {
            ...
            std_confidence: {
                ...
                let variance = confidences.iter().map(|&x| (x - mean).powi(2)).sum::<f64>() / confidences.len() as f64;
                variance.sqrt()
            },
        ...
        let temporal_features = TemporalFeatures {
            measurement_rate: measurement_history.len() as f64 / (timestamps.last().unwrap_or(&1.0) - timestamps.first().unwrap_or(&0.0)),
    ...
    fn should_retrain(
        ...
        if let Some(ref model) = self.model_cache {
            ...
            if feature_drift > 0.2 { // 20% drift threshold
                return Ok(true);
            }
        }
    ...
    async fn train_model(
        ...
        let training_epoch = TrainingEpoch {
            ...
            learning_rate: match &self.config.training_config.learning_rate_schedule {
                crate::mid_circuit_measurements::config::LearningRateSchedule::Constant { rate } => *rate,
        ...
        let model = OptimizationModel {
            model_type: self.config.model_types.first()
                .map(|t| format!("{:?}", t))
    ...
    /// Generate optimization recommendations
    fn generate_recommendations(&self, features: &MLFeatures) -> DeviceResult<Vec<OptimizationRecommendation>> {
        let mut recommendations = Vec::new();

        // Latency optimization
        if features.statistical_features.mean_latency > 1000.0 { // > 1ms
            recommendations.push(OptimizationRecommendation {
                ...
                confidence: 0.85,
                rationale: "High average latency detected, reducing timeout may improve performance".to_string(),
            });
        }
        ...
        if features.statistical_features.mean_confidence < 0.95 {
            recommendations.push(OptimizationRecommendation {
                ...
                confidence: 0.75,
                rationale: "Low confidence detected, increasing repetitions may improve reliability".to_string(),
            });
        }

        // Timing optimization
        if features.temporal_features.measurement_rate < 100.0 { // < 100 Hz
            recommendations.push(OptimizationRecommendation {
                ...
                confidence: 0.65,
                rationale: "Low measurement rate, increasing frequency may improve throughput".to_string(),
            });
        }
    ...
    fn predict_improvements(
        ...
        let overall_improvement = recommendations.iter()
    ...
    fn get_model_version(&self) -> String {
        format!("v{}.{}", self.training_history.len() / 10, self.training_history.len() % 10)
    }
    ...
    fn calculate_kurtosis(&self, values: &[f64]) -> f64 {
        ...
        if std_dev > 1e-10 {
            let kurtosis = values.iter()
                ...
                .sum::<f64>() / values.len() as f64 - 3.0; // Excess kurtosis
            kurtosis
        } else {
            0.0
        }
    ...
    fn calculate_pattern_complexity(&self, values: &[f64]) -> f64 {
        ...
        for i in 1..(values.len() - 1) {
            ...
            if prev_diff * curr_diff < 0.0 { // Sign change
                changes += 1;
            }
        }
    ...
    fn calculate_feature_importance(&self) -> DeviceResult<Vec<FeatureImportance>> {
        ...
        Ok(vec![
            FeatureImportance { feature_name: "mean_latency".to_string(), importance: 0.3 },
            FeatureImportance { feature_name: "mean_confidence".to_string(), importance: 0.25 },
            FeatureImportance { feature_name: "temporal_autocorrelation".to_string(), importance: 0.2 },
            FeatureImportance { feature_name: "latency_confidence_correlation".to_string(), importance: 0.15 },
            FeatureImportance { feature_name: "measurement_rate".to_string(), importance: 0.1 },
        ])
    }

    fn detect_feature_drift(&self, current_features: &MLFeatures, training_features: &MLFeatures) -> DeviceResult<f64> {
