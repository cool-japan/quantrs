...
use crate::calibration::{
    DeviceCalibration, QubitCalibration, SingleQubitGateData,
    TwoQubitGateCalibration, QubitReadoutData,
};
...
impl CalibrationNoiseModel {
    ...
    pub fn from_calibration(calibration: &DeviceCalibration) -> Self {
        ...
        for (qubit_id, qubit_cal) in &calibration.qubit_calibrations {
            qubit_noise.insert(*qubit_id, QubitNoiseParams {
        ...
        for (qubit_id, readout_data) in &calibration.readout_calibration.qubit_readout {
            ...
            readout_noise.insert(*qubit_id, ReadoutNoiseParams {
                assignment_matrix: [
                    [p00, 1.0 - p11],
                    [1.0 - p00, p11],
                ],
        ...
        for (gate_name, gate_cal) in &calibration.single_qubit_gates {
            ...
            if count > 0 {
                ...
                gate_noise.insert(gate_name.clone(), GateNoiseParams {
        ...
        for ((control, target), gate_cal) in &calibration.two_qubit_gates {
            ...
            two_qubit_noise.insert((*control, *target), TwoQubitNoiseParams {
        ...
        // Get temperature
        let temperature = calibration.qubit_calibrations
            .values()
            .filter_map(|q| q.temperature)
            .sum::<f64>() / calibration.qubit_calibrations.len() as f64;
    ...
    pub fn apply_gate_noise(
        ...
        match qubits.len() {
            ...
            2 => self.apply_two_qubit_noise(state, gate_name, qubits[0], qubits[1], duration_ns, rng),
            _ => {
                // For multi-qubit gates, apply as decomposed single/two-qubit noise
                for &qubit in qubits {
                    self.apply_single_qubit_noise(state, gate_name, qubit, duration_ns / qubits.len() as f64, rng)?;
                }
                Ok(())
            }
        }
    ...
    fn apply_single_qubit_noise(
        ...
        if let Some(params) = gate_params {
            ...
            if params.amplitude_noise > 0.0 {
                let amplitude_error = 1.0 + rng.gen_range(-params.amplitude_noise..params.amplitude_noise);
    ...
    fn apply_rotation_error(&self, state: &mut Vec<Complex64>, qubit: QubitId, angle: f64) -> QuantRS2Result<()> {
    ...
    fn apply_amplitude_scaling(&self, state: &mut Vec<Complex64>, qubit: QubitId, scale: f64) -> QuantRS2Result<()> {
    ...
    fn apply_phase_error(&self, state: &mut Vec<Complex64>, qubit: QubitId, phase: f64) -> QuantRS2Result<()> {
        let qubit_idx = qubit.id() as usize;
    ...
    fn apply_amplitude_damping(&self, state: &mut Vec<Complex64>, qubit: QubitId, gamma: f64) -> QuantRS2Result<()> {
    ...
    fn apply_phase_damping(&self, state: &mut Vec<Complex64>, qubit: QubitId, gamma: f64) -> QuantRS2Result<()> {
    ...
    fn apply_crosstalk_error(&self, state: &mut Vec<Complex64>, qubit: QubitId, strength: f64) -> QuantRS2Result<()> {
