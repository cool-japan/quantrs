...
fn demonstrate_recovery_patterns() {
    ...
    match result {
        Ok(_) => println!("   ✓ Operation succeeded (or retry succeeded)"),
    ...
    for attempt in 1..=MAX_RETRIES {
        ...
        match result {
            ...
            Err(e) if e.is_recoverable() => {
                println!("   Attempt {}: Recoverable error, retrying...", attempt);
                continue;
            }
            Err(e) => {
                println!("   Attempt {}: Non-recoverable error", attempt);
                return Err(e);
            }
        }
    }
...
fn fallback_on_resource_error() -> QuantRS2Result<String> {
    ...
    match try_large_simulation(40) {
        Ok(_) => Ok("Used 40-qubit simulation".into()),
