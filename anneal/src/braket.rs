...
mod client {
    ...
    impl BraketClient {
        ...
        pub fn get_devices(&self) -> BraketResult<Vec<BraketDevice>> {
            let url = format!(
                "https://braket.{}.amazonaws.com/devices",
                self.region
            );
            ...
            self.runtime.block_on(async {
                ...
                let devices: Vec<BraketDevice> = serde_json::from_value(
                    devices_response["devices"].clone()
                )?;
                Ok(devices)
            })
        }
        ...
        pub fn select_device(&self, selector: Option<&DeviceSelector>) -> BraketResult<BraketDevice> {
            ...
            let filtered_devices: Vec<_> = devices
                ...
                .filter(|device| {
                    ...
                    let type_match = selector.device_type.as_ref()
                        .map(|dt| &device.device_type == dt)
                        .unwrap_or(true);

                    // Filter by provider
                    let provider_match = selector.provider.as_ref()
                ...
                match task_result.task_status {
                    ...
                    TaskStatus::Failed => {
                        return Err(BraketError::TaskError(format!(
                            "Task {} failed", task_arn
                        )));
                    }
                    TaskStatus::Cancelled => {
                        return Err(BraketError::TaskError(format!(
                            "Task {} was cancelled", task_arn
                        )));
                    }
                    TaskStatus::Running | TaskStatus::Queued | TaskStatus::Created => {
                        if start_time.elapsed() > self.task_timeout {
                            return Err(BraketError::TimeoutError(format!(
                                "Timeout waiting for task {} completion", task_arn
                            )));
                        }
        ...
        pub fn get_task_metrics(&self, task_arn: &str) -> BraketResult<TaskMetrics> {
            ...
            let queue_time = Duration::from_secs(
                metadata["queueTime"].as_u64().unwrap_or(0)
            );
            let execution_time = Duration::from_secs(
                metadata["executionTime"].as_u64().unwrap_or(0)
            );
            ...
            let (average_energy, best_energy, energy_std) = if let Some(measurements) = &task_result.measurements {
                let energies: Vec<f64> = measurements.iter()
        ...
        pub fn list_tasks(&self, limit: Option<usize>) -> BraketResult<Vec<TaskResult>> {
            let mut url = format!(
                "https://braket.{}.amazonaws.com/quantum-tasks",
                self.region
            );
            ...
            self.runtime.block_on(async {
                ...
                let tasks: Vec<TaskResult> = serde_json::from_value(
                    tasks_response["quantumTasks"].clone()
                )?;
                Ok(tasks)
            })
        }
        ...
        pub fn get_cost_summary(&self) -> BraketResult<serde_json::Value> {
            let url = format!(
                "https://braket.{}.amazonaws.com/usage",
                self.region
            );
        ...
        fn submit_annealing_task(
            ...
            if let Some(prog_therm) = params.programming_thermalization {
                task_spec["deviceParameters"]["programmingThermalization"] = serde_json::json!(prog_therm);
            }
            if let Some(readout_therm) = params.readout_thermalization {
                task_spec["deviceParameters"]["readoutThermalization"] = serde_json::json!(readout_therm);
            }
        ...
        async fn get_auth_header(&self) -> BraketResult<String> {
            ...
            Ok(format!("AWS4-HMAC-SHA256 Credential={}/...", self.credentials.0))
        }
    }
}
