...
use crate::simulator::{TemperatureSchedule, TransverseFieldSchedule, AnnealingParams};
...
impl ProblemSpecificScheduler {
    ...
    fn init_default_templates(&mut self) {
        ...
        self.templates.insert(ProblemType::TSP, ScheduleTemplate {
        ...
        self.templates.insert(ProblemType::MaxCut, ScheduleTemplate {
        ...
        self.templates.insert(ProblemType::GraphColoring, ScheduleTemplate {
        ...
        self.templates.insert(ProblemType::NumberPartitioning, ScheduleTemplate {
        ...
        self.templates.insert(ProblemType::Portfolio, ScheduleTemplate {
        ...
        self.templates.insert(ProblemType::SpinGlass, ScheduleTemplate {
            initial_temp_factor: 2.0,
            final_temp_factor: 0.001,
    ...
    pub fn create_schedule(
        ...
        let template = self.templates.get(&problem_type)
            .cloned()
    ...
    fn adapt_template(
        ...
        let energy_scale = stats.max_coupling.abs().max(stats.mean_coupling.abs() * 3.0);
    ...
    fn template_to_params(&self, template: &ScheduleTemplate, num_qubits: usize) -> AnnealingParams {
        let mut params = AnnealingParams::new();
        
        params.initial_temperature = template.initial_temp_factor;
...
impl ProblemAnalyzer {
    ...
    fn analyze(&mut self, model: &IsingModel) -> IsingResult<()> {
        ...
        if !couplings.is_empty() {
            let strengths: Vec<f64> = couplings.iter()
                .map(|c| c.strength)
                .collect();
            
            self.coupling_stats.mean_coupling = strengths.iter().sum::<f64>() / strengths.len() as f64;
            self.coupling_stats.max_coupling = strengths.iter().cloned().fold(f64::NEG_INFINITY, f64::max);
            self.coupling_stats.min_coupling = strengths.iter().cloned().fold(f64::INFINITY, f64::min);
        ...
        self.graph_properties.avg_degree = degrees.iter().sum::<usize>() as f64 / model.num_qubits as f64;
    ...
    fn check_bipartite(&self, model: &IsingModel, couplings: &[crate::ising::Coupling]) -> bool {
        ...
        for start in 0..model.num_qubits {
            ...
            while let Some(node) = queue.pop() {
                ...
                for coupling in couplings {
                    let neighbor = if coupling.i == node { coupling.j } else if coupling.j == node { coupling.i } else { continue };
...
impl AdaptiveScheduleOptimizer {
    ...
    pub fn suggest_schedule(
        ...
        if !similar_runs.is_empty() {
            ...
            params.initial_transverse_field = params.initial_transverse_field * (1.0 - self.learning_rate)
...
mod tests {
    ...
    fn test_schedule_creation() {
        ...
        let params = scheduler.create_schedule(&model, ProblemType::MaxCut).unwrap();
