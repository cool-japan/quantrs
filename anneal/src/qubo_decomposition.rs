...
use crate::partitioning::{SpectralPartitioner, Partition};
...
impl QuboDecomposer {
    ...
    pub fn solve(&self, qubo: &QuboModel) -> DecompositionResult<DecomposedSolution> {
        ...
        println!("Decomposed problem into {} sub-problems", sub_problems.len());
        ...
        for iteration in 0..self.config.refinement_iterations {
            ...
            let (new_solutions, improvement) = self.refine_solutions(
                qubo,
                &sub_problems,
                &sub_solutions,
            )?;
            ...
            println!("Refinement iteration {}: improvement = {:.6}", iteration + 1, improvement);
        }
        ...
        let subproblem_sizes: Vec<usize> = sub_problems.iter()
            .map(|sp| sp.variables.len())
            .collect();
        
        let stats = DecompositionStats {
            num_subproblems: sub_problems.len(),
            avg_subproblem_size: subproblem_sizes.iter().sum::<usize>() as f64 / subproblem_sizes.len() as f64,
    ...
    fn decompose_problem(&self, qubo: &QuboModel) -> DecompositionResult<Vec<SubProblem>> {
        match &self.config.strategy {
            DecompositionStrategy::Spectral { num_partitions, overlap_size } => {
                self.spectral_decomposition(qubo, *num_partitions, *overlap_size)
            }
            DecompositionStrategy::Block { block_size, overlap_size } => {
                self.block_decomposition(qubo, *block_size, *overlap_size)
            }
            DecompositionStrategy::Hierarchical { min_partition_size, max_depth, overlap_size } => {
                self.hierarchical_decomposition(qubo, *min_partition_size, *max_depth, *overlap_size)
            }
            DecompositionStrategy::Clustering { num_clusters, strength_threshold, overlap_size } => {
                self.clustering_decomposition(qubo, *num_clusters, *strength_threshold, *overlap_size)
            }
        }
    }
    ...
    fn spectral_decomposition(
        ...
        let edges: Vec<(usize, usize, f64)> = ising.couplings()
            .into_iter()
            .map(|coupling| (coupling.i, coupling.j, coupling.strength))
            .collect();
        let partition = partitioner.partition_graph(ising.num_qubits, &edges, num_partitions)
        ...
        for id in 0..num_partitions {
            ...
            let variable_mapping = variables.iter().enumerate()
                .map(|(i, &var)| (i, var))
                .collect();
    ...
    fn recursive_bisection(
        ...
        // Recurse
        self.recursive_bisection(
            qubo, left_with_overlap, depth + 1, max_depth, min_size, overlap_size,
            sub_problems, id
        )?;
        
        self.recursive_bisection(
            qubo, right_with_overlap, depth + 1, max_depth, min_size, overlap_size,
            sub_problems, id
        )?;
    ...
    fn add_overlap(&self, variables: &[usize], ising: &IsingModel, overlap_size: usize) -> Vec<usize> {
    ...
    fn add_overlap_to_vars(&self, variables: &[usize], all_vars: &[usize], overlap_size: usize) -> Vec<usize> {
        let mut result: HashSet<usize> = variables.iter().copied().collect();
    ...
    fn extract_subproblem(&self, qubo: &QuboModel, variables: &[usize]) -> DecompositionResult<QuboModel> {
        let mut sub_qubo = QuboModel::new(variables.len());
        
        // Create variable index mapping
        let var_map: HashMap<usize, usize> = variables.iter().enumerate()
            .map(|(i, &var)| (var, i))
            .collect();
    ...
    fn solve_subproblems(&self, sub_problems: &[SubProblem]) -> DecompositionResult<Vec<SubSolution>> {
        ...
        for sub_problem in sub_problems {
            ...
            let values: HashMap<usize, bool> = result.best_spins.iter().enumerate()
                .map(|(i, &spin)| {
...
mod tests {
    ...
    fn test_small_qubo_decomposition() {
        ...
        let vars: Vec<_> = (0..6).map(|i| builder.add_variable(format!("x{}", i)).unwrap()).collect();
        ...
        for i in 0..5 {
            builder.set_quadratic_term(&vars[i], &vars[i+1], -2.0).unwrap();
        }
