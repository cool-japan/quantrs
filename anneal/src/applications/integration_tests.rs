...
impl IntegrationTestSuite {
    ...
    pub fn run_all_tests(&mut self) -> ApplicationResult<()> {
        ...
        println!("Integration test suite completed in {:.2} seconds", total_time);
        ...
        for industry in &self.config.test_industries.clone() {
            for &size in &self.config.test_sizes {
                ...
                match self.test_industry_functionality(industry, size) {
                    ...
                    Err(e) => {
                        let execution_time = start_time.elapsed().as_secs_f64();
                        self.record_test_error(&test_id, ErrorCategory::ProblemConstruction, &e.to_string());
                        self.results.push(TestResult {
                            test_id,
                            category: TestCategory::Functionality,
            ...
            for industry2 in &self.config.test_industries.clone() {
                if industry1 != industry2 {
                    ...
                    match self.test_cross_industry_compatibility(&factory, industry1, industry2) {
                        ...
                        Err(e) => {
                            let execution_time = start_time.elapsed().as_secs_f64();
                            self.record_test_error(&test_id, ErrorCategory::SolverExecution, &e.to_string());
                            self.results.push(TestResult {
                                test_id,
                                category: TestCategory::CrossIndustry,
                                status: TestStatus::Failed,
    ...
    fn run_solver_integration_tests(&mut self) -> ApplicationResult<()> {
        ...
        for solver_type in &self.config.test_solvers.clone() {
            for industry in &self.config.test_industries.clone() {
                let test_id = format!("solver_{}_{}", format!("{:?}", solver_type).to_lowercase(), industry);
                let start_time = Instant::now();
        ...
        for industry in &self.config.test_industries.clone() {
            for &size in &test_sizes {
                ...
                match self.test_performance_scaling(&factory, industry, size) {
                    ...
                    Err(e) => {
                        let execution_time = start_time.elapsed().as_secs_f64();
                        self.record_test_error(&test_id, ErrorCategory::ResourceExhaustion, &e.to_string());
                        self.results.push(TestResult {
                            test_id,
                            category: TestCategory::Performance,
                            status: TestStatus::Failed,
    ...
    fn test_industry_functionality(&self, industry: &str, size: usize) -> ApplicationResult<TestResult> {
        let factory = UnifiedSolverFactory::new();
        ...
        let test_solution = vec![1; qubo.num_variables().min(20)];
        ...
        solution_metrics.insert("num_variables".to_string(), qubo.num_variables() as f64);
    ...
    fn test_solver_integration(
        ...
        let result = match solver_type {
            ...
            _ => return Err(ApplicationError::OptimizationError("Solver not implemented".to_string())),
        };
        
        let mut metrics = HashMap::new();
        metrics.insert("solution_quality".to_string(), 1.0 / (1.0 + result.best_energy.abs()));
        metrics.insert("convergence_time".to_string(), result.solve_time.unwrap_or(0.0));
        metrics.insert("energy_variance".to_string(), result.energy_variance.unwrap_or(0.0));
    ...
    fn test_performance_scaling(
        ...
        metrics.insert("num_variables".to_string(), qubo.num_variables() as f64);
    ...
    fn test_complete_workflow(
        ...
        if solution.binary_solution.is_empty() {
            return Err(ApplicationError::OptimizationError("Empty solution".to_string()));
        }
        
        let mut metrics = HashMap::new();
        metrics.insert("workflow_success".to_string(), 1.0);
        metrics.insert("solution_size".to_string(), solution.binary_solution.len() as f64);
        metrics.insert("objective_value".to_string(), solution.objective_value);
        metrics.insert("solve_time".to_string(), solution.solve_time_ms);
        metrics.insert("iterations".to_string(), solution.iterations as f64);
        
        Ok(metrics)
    }

    /// Create test problem configuration for a given industry
    fn create_test_problem_config(&self, industry: &str, size: usize) -> ApplicationResult<HashMap<String, serde_json::Value>> {
        let mut config = HashMap::new();
        
        match industry {
            "finance" => {
                config.insert("num_assets".to_string(), serde_json::Value::Number(serde_json::Number::from(size)));
                config.insert("budget".to_string(), serde_json::Value::Number(serde_json::Number::from_f64(100000.0).unwrap()));
    ...
    fn test_malformed_inputs(&mut self) -> ApplicationResult<()> {
        ...
        let mut config = HashMap::new();
        config.insert("num_assets".to_string(), serde_json::Value::Number(serde_json::Number::from(-5)));
    ...
    fn test_system_limits(&self, factory: &UnifiedSolverFactory, size: usize) -> ApplicationResult<()> {
    ...
    fn calculate_performance_metrics(&mut self) {
        ...
        self.performance_metrics.tests_passed = self.results.iter()
            .filter(|r| r.status == TestStatus::Passed)
            .count();
        self.performance_metrics.tests_failed = self.results.iter()
            .filter(|r| r.status == TestStatus::Failed)
            .count();
        
        if !self.results.is_empty() {
            self.performance_metrics.avg_execution_time = self.results.iter()
        ...
        for industry in &self.config.test_industries {
            ...
            if !industry_results.is_empty() {
                let success_rate = industry_results.iter()
                    .filter(|r| r.status == TestStatus::Passed)
                    .count() as f64 / industry_results.len() as f64;
                
                let avg_execution_time = industry_results.iter()
                    .map(|r| r.execution_time)
                    .sum::<f64>() / industry_results.len() as f64;
        ...
        for solver_type in &self.config.test_solvers {
            ...
            if !solver_results.is_empty() {
                let success_rate = solver_results.iter()
                    .filter(|r| r.status == TestStatus::Passed)
                    .count() as f64 / solver_results.len() as f64;
    ...
    fn generate_test_report(&self) -> ApplicationResult<String> {
        ...
        report.push_str(&format!("Success Rate: {:.1}%\n", 
            (self.performance_metrics.tests_passed as f64 / self.performance_metrics.total_tests as f64) * 100.0));
        report.push_str(&format!("Average Execution Time: {:.3}s\n\n", self.performance_metrics.avg_execution_time));
...
mod tests {
    ...
    fn test_problem_config_creation() {
        ...
        let finance_config = test_suite.create_test_problem_config("finance", 10).unwrap();
        assert!(finance_config.contains_key("num_assets"));
        
        let logistics_config = test_suite.create_test_problem_config("logistics", 8).unwrap();
