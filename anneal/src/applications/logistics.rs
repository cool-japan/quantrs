...
impl VehicleRoutingProblem {
    ...
    pub fn calculate_route_duration(&self, route: &[usize]) -> f64 {
        ...
        let travel_time: f64 = route.windows(2)
            .map(|pair| self.distance_matrix[pair[0]][pair[1]])
            .sum();
        
        let service_time: f64 = route.iter()
            .map(|&loc| self.service_times[loc])
            .sum();
...
impl OptimizationProblem for VehicleRoutingProblem {
    ...
    fn validate(&self) -> ApplicationResult<()> {
        ...
        for (i, &capacity) in self.vehicle_capacities.iter().enumerate() {
            if capacity <= 0.0 {
                return Err(ApplicationError::DataValidationError(
                    format!("Vehicle {} has non-positive capacity", i)
                ));
            }
        }
...
impl IndustrySolution for VehicleRoutingSolution {
    ...
    fn summary(&self) -> HashMap<String, String> {
        ...
        summary.insert("total_distance".to_string(), format!("{:.2}", self.total_distance));
        ...
        let total_customers: usize = self.route_stats.iter()
            .map(|stats| stats.customers_served)
            .sum();
        summary.insert("customers_served".to_string(), total_customers.to_string());
        
        let avg_utilization = self.route_stats.iter()
            .map(|stats| stats.capacity_utilization)
            .sum::<f64>() / self.route_stats.len() as f64;
        summary.insert("avg_capacity_utilization".to_string(), format!("{:.1}%", avg_utilization * 100.0));
    ...
    fn export_format(&self) -> ApplicationResult<String> {
        ...
        output.push_str(&format!("Active Vehicles: {}\n", 
            self.routes.iter().filter(|route| route.len() > 2).count()));
        
        output.push_str("\n## Route Details\n");
        for (i, route) in self.routes.iter().enumerate() {
            if route.len() > 2 { // Skip empty routes
                ...
                output.push_str(&format!("Capacity Utilization: {:.1}%\n", 
...
pub fn create_benchmark_problems(size: usize) -> ApplicationResult<Vec<Box<dyn OptimizationProblem<Solution = Vec<i8>, ObjectiveValue = f64>>>> {
    ...
    if size >= 8 {
        ...
        let large_vrp = VehicleRoutingProblem::new(3, large_capacities, large_locations, large_demands)?;
...
mod tests {
    ...
    fn test_time_windows() {
        ...
        vrp.set_time_windows(vec![(0.0, 100.0), (10.0, 20.0), (50.0, 60.0)]).unwrap();
