...
use crate::simulator::{AnnealingParams, ClassicalAnnealingSimulator, QuantumAnnealingSimulator, AnnealingResult};
use super::{
    ApplicationError, ApplicationResult, OptimizationProblem, IndustrySolution,
    ProblemCategory, IndustryConstraint, IndustryObjective,
    finance, logistics, energy, manufacturing, healthcare, telecommunications,
    unified::{UnifiedSolverFactory, UnifiedProblem, SolverType, ProblemComplexity},
    integration_tests::{TestConfiguration, IntegrationTestSuite}
};
...
impl PerformanceBenchmarkSuite {
    ...
    pub fn run_all_benchmarks(&mut self) -> ApplicationResult<()> {
        ...
        println!("Performance benchmark suite completed in {:.2} seconds", total_time);
            ...
            for &size in &self.config.problem_sizes {
                for solver_type in &self.config.solver_types.clone() {
                    for temp_profile in &self.config.temperature_profiles.clone() {
                        let test_id = format!("algorithm_{}_{}_{}_{:?}", 
                            industry, size, format!("{:?}", solver_type).to_lowercase(), temp_profile);

                        match self.run_single_algorithm_benchmark(&factory, industry, size, solver_type, &temp_profile) {
                            Ok(result) => self.results.push(result),
                            Err(e) => {
                                eprintln!("Benchmark {} failed: {}", test_id, e);
                                self.record_benchmark_error(&test_id, BenchmarkErrorType::SolverFailure, &e.to_string());
                            }
                        }
                    }
                }
        ...
        for industry in &self.config.benchmark_industries.clone() {
            ...
            for solver1 in &self.config.solver_types.clone() {
                for solver2 in &self.config.solver_types.clone() {
                    if solver1 != solver2 {
                        let test_id = format!("comparison_{}_{}_{:?}_vs_{:?}", 
                            industry, test_size, solver1, solver2);

                        match self.run_solver_comparison(&factory, industry, test_size, solver1, solver2) {
                            Ok(result) => self.results.push(result),
                            Err(e) => {
    ...
    fn run_parameter_sensitivity_benchmarks(&mut self) -> ApplicationResult<()> {
        ...
        for industry in &self.config.benchmark_industries.clone() {
            for &threshold in &self.config.convergence_thresholds {
                let test_id = format!("sensitivity_{}_{}_threshold_{}", industry, base_size, threshold);
        ...
        for &size in &stress_sizes {
            ...
            match self.run_stress_test(&factory, size) {
                ...
                Err(e) => {
                    ...
                    self.record_benchmark_error(&test_id, BenchmarkErrorType::MemoryExhaustion, &e.to_string());
                }
            }
        }
    ...
    fn run_single_algorithm_benchmark(
        ...
        let result = self.solve_with_solver(&ising, &solver_config)?;
        ...
        detailed_metrics.iterations_per_second = 
            result.iterations.unwrap_or(0) as f64 / (detailed_metrics.solving_time_ms / 1000.0);
        detailed_metrics.variables_per_second = 
            qubo.num_variables() as f64 / (detailed_metrics.solving_time_ms / 1000.0);
        ...
        let solution_quality = SolutionQualityMetrics {
            best_objective_value: result.best_energy,
            average_objective_value: result.best_energy,
            objective_value_std_dev: result.energy_variance.unwrap_or(0.0).sqrt(),
            success_rate: 1.0,
            optimality_gap: None,
        ...
        let convergence_analysis = ConvergenceAnalysis {
            ...
            energy_progression: vec![result.best_energy],
    ...
    fn create_benchmark_problem_config(&self, industry: &str, size: usize) -> ApplicationResult<HashMap<String, serde_json::Value>> {
        let mut config = HashMap::new();
        
        match industry {
            "finance" => {
                config.insert("num_assets".to_string(), serde_json::Value::Number(serde_json::Number::from(size)));
                config.insert("budget".to_string(), serde_json::Value::Number(serde_json::Number::from_f64(1000000.0).unwrap()));
                config.insert("risk_tolerance".to_string(), serde_json::Value::Number(serde_json::Number::from_f64(0.5).unwrap()));
            },
            "logistics" => {
                config.insert("num_vehicles".to_string(), serde_json::Value::Number(serde_json::Number::from(3)));
                config.insert("num_customers".to_string(), serde_json::Value::Number(serde_json::Number::from(size)));
            },
            "telecommunications" => {
                config.insert("num_nodes".to_string(), serde_json::Value::Number(serde_json::Number::from(size)));
            },
            _ => {
                config.insert("size".to_string(), serde_json::Value::Number(serde_json::Number::from(size)));
            }
        }
    ...
    fn apply_temperature_profile(&self, params: &mut AnnealingParams, profile: &TemperatureProfile) {
        match profile {
            TemperatureProfile::Linear => {
                params.temperature_schedule = crate::simulator::TemperatureSchedule::Linear;
            },
            TemperatureProfile::Exponential => {
                params.temperature_schedule = crate::simulator::TemperatureSchedule::Exponential;
            },
            TemperatureProfile::Logarithmic => {
                params.temperature_schedule = crate::simulator::TemperatureSchedule::Linear; // Fallback
            },
    ...
    fn analyze_solver_performance(&mut self) -> ApplicationResult<()> {
        ...
        for solver_type in &self.config.solver_types {
            ...
            if !solver_results.is_empty() {
                let avg_time: f64 = solver_results.iter()
                    .map(|r| r.performance_metrics.total_time_ms)
                    .sum::<f64>() / solver_results.len() as f64;

                let avg_quality: f64 = solver_results.iter()
    ...
    fn analyze_industry_performance(&mut self) -> ApplicationResult<()> {
        for industry in &self.config.benchmark_industries {
            let industry_results: Vec<_> = self.results.iter()
            ...
            if !industry_results.is_empty() {
                ...
                self.analysis.industry_analysis.insert(industry.clone(), analysis);
            }
        }
    ...
    fn calculate_average_metrics(&self, results: &[&BenchmarkTestResult]) -> DetailedPerformanceMetrics {
        let count = results.len() as f64;
        
        DetailedPerformanceMetrics {
            total_time_ms: results.iter().map(|r| r.performance_metrics.total_time_ms).sum::<f64>() / count,
            qubo_construction_time_ms: results.iter().map(|r| r.performance_metrics.qubo_construction_time_ms).sum::<f64>() / count,
            ising_conversion_time_ms: results.iter().map(|r| r.performance_metrics.ising_conversion_time_ms).sum::<f64>() / count,
            solver_init_time_ms: results.iter().map(|r| r.performance_metrics.solver_init_time_ms).sum::<f64>() / count,
            solving_time_ms: results.iter().map(|r| r.performance_metrics.solving_time_ms).sum::<f64>() / count,
            solution_interpretation_time_ms: results.iter().map(|r| r.performance_metrics.solution_interpretation_time_ms).sum::<f64>() / count,
            iterations_per_second: results.iter().map(|r| r.performance_metrics.iterations_per_second).sum::<f64>() / count,
            variables_per_second: results.iter().map(|r| r.performance_metrics.variables_per_second).sum::<f64>() / count,
            time_complexity_factor: 1.0,
            space_complexity_factor: 1.0,
        }
    }
    ...
    fn record_benchmark_error(&mut self, test_id: &str, error_type: BenchmarkErrorType, message: &str) {
    ...
    fn generate_performance_report(&self) -> ApplicationResult<String> {
        ...
        report.push_str(&format!("Industries Tested: {:?}\n", self.config.benchmark_industries));
        ...
        for (solver, ranking) in &self.analysis.solver_comparison.performance_rankings {
            ...
            report.push_str(&format!("- Performance Score: {:.3}\n", ranking.performance_score));
...
pub fn run_performance_benchmarks_with_config(config: BenchmarkConfiguration) -> ApplicationResult<()> {
    let mut benchmark_suite = PerformanceBenchmarkSuite::new(config);
    benchmark_suite.run_all_benchmarks()?;
    Ok(())
}
...
mod tests {
    ...
    fn test_benchmark_problem_config_creation() {
        ...
        let finance_config = suite.create_benchmark_problem_config("finance", 10).unwrap();
        assert!(finance_config.contains_key("num_assets"));
        
        let logistics_config = suite.create_benchmark_problem_config("logistics", 8).unwrap();
