...
impl SmartGridOptimization {
    ...
    pub fn add_transmission_line(
        ...
        self.transmission_capacities.insert((from_node, to_node), capacity);
    ...
    pub fn set_renewable_availability(&mut self, availability: Vec<Vec<f64>>) -> ApplicationResult<()> {
...
impl OptimizationProblem for SmartGridOptimization {
    ...
    fn size_metrics(&self) -> HashMap<String, usize> {
        ...
        metrics.insert("num_transmission_lines".to_string(), self.transmission_capacities.len());
        metrics.insert("num_storage_systems".to_string(), self.storage_systems.len());
        metrics
    }
    ...
    fn to_qubo(&self) -> ApplicationResult<QuboFormulation> {
        ...
        for g in 0..self.num_generators {
            for t in 0..self.num_time_periods {
                for level in 0..precision {
                    let generation = (level as f64) * self.generator_capacities[g] / (precision as f64);
    ...
    fn is_feasible(&self, solution: &Self::Solution) -> bool {
        ...
        for t in 0..self.num_time_periods {
            let total_generation: f64 = solution.generation_schedule.iter()
...
impl IndustrySolution for GridSolution {
    ...
    fn from_binary(problem: &Self::Problem, binary_solution: &[i8]) -> ApplicationResult<Self> {
        let precision = 10;
        let mut generation_schedule = vec![vec![0.0; problem.num_time_periods]; problem.num_generators];
        let mut var_idx = 0;
            ...
            for t in 0..problem.num_time_periods {
                for level in 0..precision {
                    if var_idx < binary_solution.len() && binary_solution[var_idx] == 1 {
                        generation_schedule[g][t] = (level as f64) * 
                            problem.generator_capacities[g] / (precision as f64);
                        break;
                    }
                    var_idx += 1;
                }
        ...
        let storage_operations = vec![vec![0.0; problem.num_time_periods]; problem.storage_systems.len()];
        
        // Calculate metrics
        let total_energy: f64 = generation_schedule.iter()
            .flat_map(|gen| gen.iter())
            .sum();
        ...
        let load_factor = if peak_demand > 0.0 { average_demand / peak_demand } else { 0.0 };
    ...
    fn summary(&self) -> HashMap<String, String> {
        ...
        summary.insert("peak_demand".to_string(), format!("{:.1} MW", self.grid_metrics.peak_demand));
        summary.insert("total_energy".to_string(), format!("{:.1} MWh", self.grid_metrics.total_energy));
        summary.insert("load_factor".to_string(), format!("{:.1}%", self.grid_metrics.load_factor * 100.0));
        summary.insert("efficiency".to_string(), format!("{:.1}%", self.grid_metrics.efficiency * 100.0));
        summary.insert("reliability".to_string(), format!("{:.2}%", self.grid_metrics.reliability_score * 100.0));
        summary
    }
    ...
    fn metrics(&self) -> HashMap<String, f64> {
        ...
        metrics.insert("renewable_penetration".to_string(), self.grid_metrics.renewable_penetration);
        metrics.insert("efficiency".to_string(), self.grid_metrics.efficiency);
        metrics.insert("reliability_score".to_string(), self.grid_metrics.reliability_score);
        
        // Calculate additional operational metrics
        let num_active_generators = self.generation_schedule.iter()
            .filter(|schedule| schedule.iter().any(|&gen| gen > 1e-6))
            .count();
        metrics.insert("active_generators".to_string(), num_active_generators as f64);
        
        let max_generation: f64 = self.generation_schedule.iter()
    ...
    fn export_format(&self) -> ApplicationResult<String> {
        ...
        output.push_str(&format!("Peak Demand: {:.1} MW\n", self.grid_metrics.peak_demand));
        output.push_str(&format!("Total Energy: {:.1} MWh\n", self.grid_metrics.total_energy));
        output.push_str(&format!("Load Factor: {:.1}%\n", self.grid_metrics.load_factor * 100.0));
        output.push_str(&format!("Grid Efficiency: {:.1}%\n", self.grid_metrics.efficiency * 100.0));
...
pub fn create_benchmark_problems(size: usize) -> ApplicationResult<Vec<Box<dyn OptimizationProblem<Solution = Vec<i8>, ObjectiveValue = f64>>>> {
    ...
    problems.push(Box::new(grid_problem) as Box<dyn OptimizationProblem<Solution = Vec<i8>, ObjectiveValue = f64>>);
    ...
    if size >= 5 {
        ...
        let large_demands = (0..3).map(|_| {
            (0..6).map(|t| 30.0 + t as f64 * 15.0).collect()
        }).collect();
        
        let mut large_grid = SmartGridOptimization::new(
            size,
            3,
            large_capacities,
            large_costs,
        ...
        problems.push(Box::new(large_grid) as Box<dyn OptimizationProblem<Solution = Vec<i8>, ObjectiveValue = f64>>);
    }
