...
use crate::simulator::{AnnealingParams, AnnealingSolution, TemperatureSchedule, TransverseFieldSchedule};
...
impl PopulationAnnealingSimulator {
    ...
    pub fn new(config: PopulationAnnealingConfig) -> PopulationAnnealingResult<Self> {
        ...
        let mpi_interface = config.mpi_config.as_ref()
    ...
    pub fn solve(&mut self, model: &IsingModel) -> PopulationAnnealingResult<PopulationAnnealingSolution> {
        ...
        for (step, &temperature) in temperatures.iter().enumerate() {
            ...
            let should_resample = (step + 1) % self.config.resampling_frequency == 0 
                || stats.effective_sample_size < self.config.ess_threshold * self.config.population_size as f64;
    ...
    fn initialize_population(&mut self, model: &IsingModel) -> PopulationAnnealingResult<Vec<PopulationMember>> {
    ...
    fn generate_temperature_schedule(&self) -> Vec<f64> {
        ...
        for i in 0..self.config.num_temperature_steps {
            ...
            let temperature = self.config.temperature_schedule.calculate(
                t,
                1.0,
    ...
    fn calculate_statistics(&self, population: &[PopulationMember], temperature: f64) -> EnergyStatistics {
    ...
    fn resample_population(
        ...
        let min_energy = population.iter().map(|m| m.energy).fold(f64::INFINITY, f64::min);
...
impl MpiInterface {
    ...
    fn exchange_populations(&mut self, _population: &mut Vec<PopulationMember>) -> PopulationAnnealingResult<()> {
