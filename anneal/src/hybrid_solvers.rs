...
use crate::ising::{IsingModel, IsingResult, IsingError, QuboModel};
use crate::qubo::QuboFormulation;
use crate::simulator::{ClassicalAnnealingSimulator, QuantumAnnealingSimulator, AnnealingParams, AnnealingSolution};
...
impl HybridQuantumClassicalSolver {
    ...
    pub fn new(config: HybridSolverConfig) -> IsingResult<Self> {
        ...
        Ok(Self {
            config,
            quantum_solver: QuantumAnnealingSimulator::new(quantum_params)
                .map_err(|e| IsingError::InvalidValue(format!("Failed to create quantum solver: {}", e)))?,
            classical_solver: ClassicalAnnealingSimulator::new(classical_params)
                .map_err(|e| IsingError::InvalidValue(format!("Failed to create classical solver: {}", e)))?,
        ...
        for iteration in 0..self.config.num_iterations {
            ...
            for partition in &partitions {
                ...
                let subsolution = if subproblem.num_qubits <= self.config.classical_threshold {
                    ...
                    self.classical_solver.solve(&subproblem)
                        .map_err(|e| IsingError::InvalidValue(format!("Classical solver failed: {}", e)))?
                } else {
                    // Use quantum solver for larger problems
                    result.quantum_calls += 1;
                    self.quantum_solver.solve(&subproblem)
                        .map_err(|e| IsingError::InvalidValue(format!("Quantum solver failed: {}", e)))?
                };
            ...
            let combined_solution = self.combine_solutions(&partition_solutions, model.num_qubits)?;
    ...
    fn partition_problem(&self, model: &IsingModel) -> IsingResult<Vec<Partition>> {
        ...
        let partition = self.partitioner.partition_graph(n, &edges, num_partitions.max(2))?;
    ...
    fn extract_subproblem(&self, model: &IsingModel, partition: &Partition) -> IsingResult<IsingModel> {
        ...
        let index_map: HashMap<usize, usize> = indices.iter()
            .enumerate()
        ...
        for coupling in model.couplings() {
            if let (Some(&new_i), Some(&new_j)) = (index_map.get(&coupling.i), index_map.get(&coupling.j)) {
    ...
    fn add_boundary_terms(
        ...
        let partition_nodes: std::collections::HashSet<usize> = partition.assignment.keys().cloned().collect();
    ...
    fn update_partitioning_strategy(&mut self, partition_solutions: &[(Partition, AnnealingSolution)]) {
        ...
        let avg_quality = partition_qualities.iter().sum::<f64>() / partition_qualities.len() as f64;
...
impl VariationalHybridSolver {
    ...
    fn generate_solution_with_params(&self, model: &IsingModel, parameters: &[f64]) -> IsingResult<Vec<i8>> {
        let n = model.num_qubits;
