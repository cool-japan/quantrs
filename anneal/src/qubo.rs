...
use crate::ising::{QuboModel, IsingError, IsingResult};
...
impl QuboBuilder {
    ...
    pub fn set_constraint_weight(&mut self, weight: f64) -> QuboResult<()> {
        if !weight.is_finite() || weight <= 0.0 {
            return Err(QuboError::ConstraintError(
                format!("Constraint weight must be positive and finite, got {}", weight)
            ));
        }
    ...
    pub fn add_variables(&mut self, names: impl IntoIterator<Item = impl Into<String>>) -> QuboResult<Vec<Variable>> {
    ...
    pub fn set_quadratic_term(&mut self, var1: &Variable, var2: &Variable, value: f64) -> QuboResult<()> {
        ...
        if var1.index == var2.index {
            return Err(QuboError::ConstraintError(
                format!("Cannot set quadratic term for the same variable: {}", var1.name)
            ));
        }
    ...
    pub fn set_offset(&mut self, offset: f64) -> QuboResult<()> {
        if !offset.is_finite() {
            return Err(QuboError::ConstraintError(
                format!("Offset must be finite, got {}", offset)
            ));
        }
    ...
    pub fn minimize_quadratic(&mut self, var1: &Variable, var2: &Variable, coeff: f64) -> QuboResult<()> {
    ...
    pub fn constrain_one_hot(&mut self, vars: &[Variable]) -> QuboResult<()> {
        if vars.is_empty() {
            return Err(QuboError::ConstraintError("Empty one-hot constraint".to_string()));
        }
    ...
    pub fn constrain_at_most_one(&mut self, vars: &[Variable]) -> QuboResult<()> {
        if vars.is_empty() {
            return Err(QuboError::ConstraintError("Empty at-most-one constraint".to_string()));
        }
    ...
    pub fn constrain_at_least_one(&mut self, vars: &[Variable]) -> QuboResult<()> {
        if vars.is_empty() {
            return Err(QuboError::ConstraintError("Empty at-least-one constraint".to_string()));
        }
...
mod tests {
    ...
    fn test_qubo_builder_one_hot() {
        ...
        builder.constrain_one_hot(&[x1.clone(), x2.clone(), x3.clone()]).unwrap();
