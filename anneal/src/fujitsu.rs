...
impl FujitsuClient {
    ...
    pub async fn solve_qubo(
        ...
        let best_solution = response.solutions
            .into_iter()
            .min_by(|a, b| a.energy.partial_cmp(&b.energy).unwrap())
            .ok_or_else(|| FujitsuError::RetrievalError("No solutions returned".to_string()))?;
            
        Ok(best_solution.configuration.into_iter().map(|x| x > 0).collect())
    }
    ...
    async fn submit_problem(&self, problem: FujitsuProblem) -> FujitsuResult<String> {
        ...
        match response.status() {
            ...
            StatusCode::UNAUTHORIZED => {
                Err(FujitsuError::AuthenticationError("Invalid API key".to_string()))
            }
            status => {
    ...
    fn convert_results(
        ...
        let spins: Vec<i8> = best_solution.configuration
            .into_iter()
...
pub fn is_available() -> bool {
    ...
    std::env::var("FUJITSU_DAU_API_KEY").is_ok() && 
    std::env::var("FUJITSU_DAU_ENDPOINT").is_ok()
}

/// Create a Fujitsu client from environment variables
pub fn from_env() -> FujitsuResult<FujitsuClient> {
    let api_key = std::env::var("FUJITSU_DAU_API_KEY")
        .map_err(|_| FujitsuError::AuthenticationError("FUJITSU_DAU_API_KEY not set".to_string()))?;
    let endpoint = std::env::var("FUJITSU_DAU_ENDPOINT")
        .map_err(|_| FujitsuError::AuthenticationError("FUJITSU_DAU_ENDPOINT not set".to_string()))?;
