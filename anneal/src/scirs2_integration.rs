...
use crate::ising::{IsingModel, QuboModel, IsingResult};
...
impl SciRS2QuboModel {
    ...
    pub fn set_linear(&mut self, var: usize, value: f64) -> ApplicationResult<()> {
        if var >= self.num_variables {
            return Err(ApplicationError::InvalidConfiguration(
                format!("Variable index {} out of range", var)
            ));
        }
    ...
    fn estimate_memory_usage(&self) -> usize {
        ...
        (std::mem::size_of::<(usize, usize)>() + std::mem::size_of::<f64>()) * self.quadratic_terms.len()
    }
}
...
impl SciRS2GraphAnalyzer {
    ...
    pub fn analyze_problem_graph(&mut self, qubo: &SciRS2QuboModel) -> ApplicationResult<GraphAnalysisResult> {
        ...
        let avg_degree = if num_nodes > 0 { 2.0 * num_edges as f64 / num_nodes as f64 } else { 0.0 };
...
impl SciRS2SolutionAnalyzer {
    ...
    pub fn analyze_solutions(&mut self, solutions: &[Vec<i8>], energies: &[f64]) -> ApplicationResult<SolutionAnalysisResult> {
    ...
    fn compute_solution_diversity(&self, solutions: &[Vec<i8>]) -> f64 {
        ...
        for i in 0..solutions.len() {
            for j in (i + 1)..solutions.len() {
                let hamming_distance = solutions[i].iter()
    ...
    fn generate_recommendations(&self) -> Vec<String> {
        ...
        if self.stats.diversity < 1.0 {
            recommendations.push("Increase temperature or annealing time to improve solution diversity".to_string());
        }

        if self.stats.std_energy > 1.0 {
            recommendations.push("Solutions show high energy variance - consider parameter tuning".to_string());
        }

        if self.stats.num_solutions < 100 {
            recommendations.push("Collect more samples for better statistical analysis".to_string());
        }
