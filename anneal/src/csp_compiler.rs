    ...
    pub fn values(&self) -> Vec<CspValue> {
        match self {
            ...
            Domain::IntegerRange { min, max } => {
                (*min..=*max).map(CspValue::Integer).collect()
            }
            Domain::Discrete(values) => values.iter().map(|&v| CspValue::Integer(v)).collect(),
            Domain::Categorical(labels) => labels.iter().map(|s| CspValue::String(s.clone())).collect(),
        }
    }
}
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            CspConstraint::AllDifferent { variables } => {
                f.debug_struct("AllDifferent")
                    .field("variables", variables)
                    .finish()
            }
            CspConstraint::Linear { terms, comparison, rhs } => {
            ...
            CspConstraint::AtMostOne { variables } => {
                f.debug_struct("AtMostOne")
                    .field("variables", variables)
                    .finish()
            }
            CspConstraint::Element { array_var, index_var, value_var } => {
            ...
            CspConstraint::GlobalCardinality { variables, values, min_counts, max_counts } => {
                f.debug_struct("GlobalCardinality")
                    .field("variables", variables)
                    .field("values", values)
            ...
            CspConstraint::Table { variables, tuples, allowed } => {
            ...
            CspConstraint::Custom { name, variables, .. } => {
...
impl CspProblem {
        ...
        for var_name in &var_names {
            if !self.variables.contains_key(var_name) {
                return Err(CspError::InvalidConstraint(format!(
                    "Unknown variable '{}' in constraint", var_name
                )));
            }
        }
    ...
    pub fn set_objective(&mut self, objective: CspObjective) -> CspResult<()> {
        ...
        for var_name in &var_names {
            if !self.variables.contains_key(var_name) {
                return Err(CspError::InvalidConstraint(format!(
                    "Unknown variable '{}' in objective", var_name
                )));
            }
        }
    ...
    pub fn compile_to_qubo(&self) -> CspResult<(QuboModel, CspCompilationInfo)> {
        ...
        builder.set_constraint_weight(self.compilation_params.constraint_penalty)
    ...
    fn create_variable_encoding(
        ...
        for (var_name, csp_var) in &self.variables {
            let encoding = match csp_var.domain {
                Domain::Boolean => {
                    let qubo_var = builder.add_variable(var_name.clone())
                        .map_err(|e| CspError::CompilationFailed(e.to_string()))?;
                ...
                _ => {
                    ...
                    if domain_size <= self.compilation_params.max_onehot_size && !self.compilation_params.use_log_encoding {
                        // One-hot encoding
                        let mut qubo_vars = Vec::new();
                        for (i, value) in csp_var.domain.values().iter().enumerate() {
                            let var_name_encoded = format!("{}_{}", var_name, i);
                            let qubo_var = builder.add_variable(var_name_encoded)
                                .map_err(|e| CspError::CompilationFailed(e.to_string()))?;
                        ...
                        builder.constrain_one_hot(&vars_only)
                            .map_err(|e| CspError::CompilationFailed(e.to_string()))?;
                        ...
                        for i in 0..num_bits {
                            ...
                            let qubo_var = builder.add_variable(var_name_bit)
                                .map_err(|e| CspError::CompilationFailed(e.to_string()))?;
    ...
    fn add_constraint_penalties(
        ...
        for (i, constraint) in self.constraints.iter().enumerate() {
            match constraint {
                ...
                CspConstraint::Linear { terms, comparison, rhs } => {
                    self.add_linear_constraint(builder, var_encoding, terms, comparison, *rhs, info)?;
                }
    ...
    fn add_not_equal_penalty(
        ...
        match (encoding1, encoding2) {
            (VariableEncoding::Direct(q1), VariableEncoding::Direct(q2)) => {
                ...
                builder.set_quadratic_term(q1, q2, self.compilation_params.constraint_penalty)
                    .map_err(|e| CspError::CompilationFailed(e.to_string()))?;
                // Penalty for both being 0: (1-x1) * (1-x2) = 1 - x1 - x2 + x1*x2
                // This becomes: 1 - x1 - x2 + 2*x1*x2 (since we already have x1*x2)
                builder.set_linear_term(q1, -self.compilation_params.constraint_penalty)
                    .map_err(|e| CspError::CompilationFailed(e.to_string()))?;
                builder.set_linear_term(q2, -self.compilation_params.constraint_penalty)
                    .map_err(|e| CspError::CompilationFailed(e.to_string()))?;
                builder.set_quadratic_term(q1, q2, self.compilation_params.constraint_penalty)
                    .map_err(|e| CspError::CompilationFailed(e.to_string()))?;
            }
            ...
            (VariableEncoding::OneHot(vars1), VariableEncoding::OneHot(vars2)) => {
                ...
                for (value, qubo_var1) in &values1 {
                    if let Some(qubo_var2) = values2.get(value) {
                        builder.set_quadratic_term(qubo_var1, qubo_var2, self.compilation_params.constraint_penalty)
                            .map_err(|e| CspError::CompilationFailed(e.to_string()))?;
                    }
                }
            }
    ...
    fn add_exactly_one_constraint(
        ...
        builder.constrain_one_hot(&qubo_vars)
            .map_err(|e| CspError::CompilationFailed(e.to_string()))?;
    ...
    fn add_at_most_one_constraint(
        ...
        for i in 0..qubo_vars.len() {
            for j in (i+1)..qubo_vars.len() {
                builder.set_quadratic_term(&qubo_vars[i], &qubo_vars[j], self.compilation_params.constraint_penalty)
                    .map_err(|e| CspError::CompilationFailed(e.to_string()))?;
            }
        }
    ...
    fn get_constraint_variables(&self, constraint: &CspConstraint) -> Vec<String> {
        match constraint {
            ...
            CspConstraint::Linear { terms, .. } => terms.iter().map(|(var, _)| var.clone()).collect(),
    ...
    fn get_objective_variables(&self, objective: &CspObjective) -> Vec<String> {
        match objective {
            CspObjective::Linear { terms, .. } => terms.iter().map(|(var, _)| var.clone()).collect(),
...
impl CspCompilationInfo {
    ...
    fn add_variable_info(&mut self, name: String, domain_size: usize) {
        ...
        self.variable_info.insert(name, VariableInfo {
            domain_size,
