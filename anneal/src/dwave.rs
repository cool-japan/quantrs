...
mod client {
        ...
        pub fn pause_and_ramp(annealing_time: f64, pause_start: f64, pause_duration: f64) -> Self {
            Self {
                schedule: vec![
                    ...
                    (pause_start + pause_duration, 1.0 - pause_start / annealing_time),
                    (annealing_time, 0.0),
                ],
            }
        }
    ...
    impl DWaveClient {
        ...
        pub fn with_config(
            ...
            let endpoint = endpoint.unwrap_or_else(|| "https://cloud.dwavesys.com/sapi/v2".to_string());
        ...
        pub fn select_solver(&self, selector: Option<&SolverSelector>) -> DWaveResult<LeapSolverInfo> {
            ...
            let filtered_solvers: Vec<_> = solvers
                ...
                .filter(|solver| {
                    ...
                    let category_match = match selector.category {
                        SolverCategory::QPU => matches!(solver.solver_type, SolverType::QuantumProcessor),
                        SolverCategory::Hybrid => matches!(solver.solver_type, SolverType::Hybrid),
                        SolverCategory::Software => matches!(solver.solver_type, SolverType::Software),
        ...
        pub fn submit_ising_with_embedding(
            ...
            let solver = if let Some(id) = solver_id {
                self.get_leap_solvers()?
                    .into_iter()
                    .find(|s| s.id == id)
                    .ok_or_else(|| DWaveError::SolverConfigError(format!("Solver {} not found", id)))?
            } else {
            ...
            if matches!(solver.solver_type, SolverType::QuantumProcessor) {
                ...
                let legacy_params = params.map(|p| ProblemParams {
                    ...
                    flux_bias_map: p.flux_biases.clone().map(|fb| {
                        fb.into_iter().map(|(k, v)| (k, serde_json::to_value(v).unwrap())).collect()
                    }),
                    other: serde_json::Value::Object(serde_json::Map::new()),
                }).unwrap_or_default();
        ...
        fn submit_with_auto_embedding(
            ...
            let embedding = if let Some(custom_emb) = &embedding_config.custom_embedding {
                ...
                embedder.find_embedding(&logical_edges, model.num_qubits, &hardware_graph)
                    .map_err(|e| DWaveError::EmbeddingError(e.to_string()))?
            };

            // Calculate chain strength
            let chain_strength = self.calculate_chain_strength(model, &embedding_config.chain_strength_method);
        ...
        fn get_solver_topology(&self, solver_id: &str) -> DWaveResult<HardwareGraph> {
            ...
            if let Some(edges) = properties["couplers"].as_array() {
                ...
                let num_qubits = properties["qubits"].as_array()
                    .map(|arr| arr.len())
                    .unwrap_or(0);
        ...
        fn calculate_chain_strength(
            ...
            match method {
                ChainStrengthMethod::Auto => {
                    ...
                    let max_coupling = model.couplings()
                        .iter()
                ...
                ChainStrengthMethod::Adaptive(multiplier) => {
                    let avg_coupling = model.couplings()
                        .iter()
                        .map(|c| c.strength.abs())
                        .sum::<f64>() / model.couplings().len().max(1) as f64;
        ...
        fn embed_problem(
            ...
            let max_qubit = embedding.chains.values()
            ...
            for (var, chain) in &embedding.chains {
                if let Ok(bias) = model.get_bias(*var) {
                    if bias != 0.0 {
                        ...
                        for &qubit in chain {
                            embedded_model.set_bias(qubit, bias_per_qubit)
                                .map_err(|e| DWaveError::EmbeddingError(e.to_string()))?;
                        }
                    }
                }
            ...
            for coupling in model.couplings() {
                if let (Some(chain1), Some(chain2)) = (
                    ...
                    for &q1 in chain1 {
                        for &q2 in chain2 {
                            embedded_model.set_coupling(q1, q2, coupling.strength)
                                .map_err(|e| DWaveError::EmbeddingError(e.to_string()))?;
                        }
                    }
                }
            ...
            for chain in embedding.chains.values() {
                for window in chain.windows(2) {
                    if let [q1, q2] = window {
                        embedded_model.set_coupling(*q1, *q2, -chain_strength)
                            .map_err(|e| DWaveError::EmbeddingError(e.to_string()))?;
                    }
                }
            }
        ...
        fn submit_embedded_problem(
            ...
            let legacy_params = ProblemParams {
                ...
                annealing_time: params.anneal_schedule
                    .as_ref()
        ...
        pub fn submit_hybrid(
            ...
            if let Some(time_limit) = params.time_limit {
                hybrid_params.insert("time_limit".to_string(), serde_json::to_value(time_limit).unwrap());
            }
            ...
            let problem = Problem {
                ...
                params: ProblemParams {
                    ...
                    flux_biases: None,
                    flux_bias_map: None,
                    other: serde_json::Value::Object(hybrid_params.into_iter().map(|(k, v)| (k, v)).collect()),
                },
            };
        ...
        pub fn get_problem_metrics(&self, problem_id: &str) -> DWaveResult<ProblemMetrics> {
            ...
            let queue_time = Duration::from_micros(
                timing["qpu_access_overhead_time"].as_u64().unwrap_or(0)
            );
            let programming_time = Duration::from_micros(
                timing["qpu_programming_time"].as_u64().unwrap_or(0)
            );
            let sampling_time = Duration::from_micros(
                timing["qpu_sampling_time"].as_u64().unwrap_or(0)
            );
            let readout_time = Duration::from_micros(
                timing["qpu_readout_time"].as_u64().unwrap_or(0)
            );
            ...
            let best_energy = solution.energies.iter()
                ...
                match status.status {
                    ...
                    ProblemStatus::Failed => {
                        return Err(DWaveError::StatusError(format!(
                            "Problem {} failed", problem_id
                        )));
                    }
                    ProblemStatus::Cancelled => {
                        return Err(DWaveError::StatusError(format!(
                            "Problem {} was cancelled", problem_id
                        )));
                    }
                    ProblemStatus::InProgress | ProblemStatus::Pending => {
                        if start_time.elapsed() > self.problem_timeout {
                            return Err(DWaveError::TimeoutError(format!(
                                "Timeout waiting for problem {} completion", problem_id
                            )));
                        }
