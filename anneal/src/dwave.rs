...
mod client {
    ...
    use crate::ising::{IsingModel, QuboModel, IsingError};
    ...
    impl DWaveClient {
        ...
        pub fn new(token: impl Into<String>, endpoint: Option<String>) -> DWaveResult<Self> {
            ...
            let runtime = Runtime::new()
                .map_err(|e| DWaveError::RuntimeError(e.to_string()))?;

            // Default endpoint if not provided
            let endpoint = endpoint.unwrap_or_else(||
                "https://cloud.dwavesys.com/sapi/v2".to_string()
            );
        ...
        pub fn submit_ising(&self, model: &IsingModel, solver_id: &str, params: ProblemParams) -> DWaveResult<Solution> {
        ...
        pub fn submit_qubo(&self, model: &QuboModel, solver_id: &str, params: ProblemParams) -> DWaveResult<Solution> {
        ...
        fn submit_problem(&self, problem: &Problem) -> DWaveResult<Solution> {
            ...
            self.runtime.block_on(async {
                ...
                let problem_id = submit_response["id"].as_str()
                    .ok_or_else(|| {
                ...
                while attempts < MAX_ATTEMPTS {
                    ...
                    let status_response = self.client.get(&result_url)
                        .header("Authorization", format!("token {}", self.token))
                        .send()
                        .await?;
                ...
                Err(DWaveError::ApiError("Timeout waiting for problem solution".into()))
            })
        }
    }
}
