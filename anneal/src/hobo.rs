...
impl HoboProblem {
    ...
    pub fn add_quadratic(&mut self, var1: usize, var2: usize, coefficient: f64) {
        ...
        if var1 == var2 {
            ...
            let (i, j) = if var1 < var2 { (var1, var2) } else { (var2, var1) };
    ...
    pub fn max_order(&self) -> usize {
        self.higher_order_terms
            ...
            .max()
            .unwrap_or(2)
            .max(if !self.quadratic_terms.is_empty() { 2 } else { 0 })
    ...
    fn reduce_term_substitution(
        ...
        if current_vars.len() == 2 {
            reduction.qubo.add_quadratic(current_vars[0], current_vars[1], current_coeff);
        ...
        for aux in &self.auxiliary_vars {
            ...
            match &aux.reduction_type {
                ...
                ReductionType::MultiProduct(vars) => {
                    let expected = vars.iter()
                        .all(|&v| v < solution.len() && solution[v]);
                    if aux_value != expected {
...
impl HoboAnalyzer {
    ...
    pub fn analyze(problem: &HoboProblem) -> HoboStats {
        ...
        stats.estimated_aux_vars_substitution = problem.higher_order_terms.iter()
