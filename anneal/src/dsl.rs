...
use crate::ising::{IsingModel, IsingError, QuboModel};
...
impl OptimizationModel {
    ...
    pub fn add_binary(&mut self, name: impl Into<String>) -> DslResult<Variable> {
        ...
        if self.variables.contains_key(&var_name) {
            return Err(DslError::InvalidConstraint(format!("Variable {} already exists", var_name)));
        }
    ...
    pub fn add_binary_vector(&mut self, name: impl Into<String>, size: usize) -> DslResult<VariableVector> {
    ...
    pub fn add_integer(&mut self, name: impl Into<String>, min: i32, max: i32) -> DslResult<Variable> {
        let var_name = name.into();
        ...
        if min > max {
            return Err(DslError::InvalidRange(format!("Invalid range [{}, {}]", min, max)));
        }
...
pub mod patterns {
    ...
    pub fn knapsack(
        ...
        model.add_constraint(
            selection.weighted_sum(weights).less_than_or_equal(capacity)
        )?;
        ...
        for &(u, v) in edges {
            for c in 0..num_colors {
                if let (Some(var_u), Some(var_v)) = (x[u].get(c), x[v].get(c)) {
                    ...
                    model.add_constraint(Constraint {
                        expression: BooleanExpression::AtMostOne(vec![var_u.clone(), var_v.clone()]),
...
mod tests {
    ...
    fn test_integer_variable_creation() {
        ...
        assert!(matches!(var.var_type, VariableType::Integer { min: 0, max: 7 }));
    }
