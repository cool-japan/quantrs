...
impl NonStoquasticHamiltonian {
    ...
    pub fn new(num_qubits: usize, hamiltonian_type: HamiltonianType) -> Self {
        let has_sign_problem = match hamiltonian_type {
            ...
            HamiltonianType::MixedModel { stoquastic_fraction } => stoquastic_fraction < 1.0,
        };
    ...
    pub fn add_complex_coupling(&mut self, coupling: ComplexCoupling) -> NonStoquasticResult<()> {
        if coupling.sites.0 >= self.num_qubits || coupling.sites.1 >= self.num_qubits {
            return Err(NonStoquasticError::InvalidHamiltonian(
                format!("Invalid coupling sites: ({}, {}) for {} qubits", 
    ...
    pub fn set_local_field(&mut self, site: usize, field: f64) -> NonStoquasticResult<()> {
        if site >= self.num_qubits {
            return Err(NonStoquasticError::InvalidHamiltonian(
                format!("Invalid site index: {} for {} qubits", site, self.num_qubits)
            ));
        }
    ...
    pub fn sign_problem_severity(&self) -> f64 {
        ...
        for coupling in &self.complex_couplings {
            ...
            match coupling.interaction_type {
                InteractionType::XX | InteractionType::YY | InteractionType::XY | 
...
impl NonStoquasticSimulator {
    ...
    fn population_annealing_simulation(&mut self) -> NonStoquasticResult<NonStoquasticResults> {
        ...
        for _ in 0..self.config.population_size {
            let mut state = QuantumState::new(self.hamiltonian.num_qubits, self.config.num_time_slices);
        ...
        let final_energies: Vec<NComplex<f64>> = population.iter()
            .map(|state| self.calculate_state_energy(state).unwrap_or(NComplex::new(0.0, 0.0)))
            .collect();
        
        let ground_state_energy = final_energies.iter()
    ...
    fn calculate_local_energy(&self, time_slice: usize, site: usize) -> NonStoquasticResult<NComplex<f64>> {
...
pub fn xy_to_ising_approximation(
    ...
    if !matches!(xy_hamiltonian.hamiltonian_type, HamiltonianType::XYModel { .. }) {
        return Err(NonStoquasticError::InvalidHamiltonian(
...
pub fn create_xy_chain(num_qubits: usize, j_x: f64, j_y: f64) -> NonStoquasticResult<NonStoquasticHamiltonian> {
...
pub fn create_frustrated_xy_triangle(j_xy: f64) -> NonStoquasticResult<NonStoquasticHamiltonian> {
    let mut hamiltonian = NonStoquasticHamiltonian::new(3, HamiltonianType::XYModel { j_x: j_xy, j_y: j_xy });
...
mod tests {
    ...
    fn test_sign_problem_detection() {
        ...
        let ising_like = NonStoquasticHamiltonian::new(4, HamiltonianType::MixedModel { stoquastic_fraction: 1.0 });
    ...
    fn test_complex_coupling_addition() {
        ...
        assert_eq!(hamiltonian.complex_couplings[0].strength, NComplex::new(0.5, 0.3));
    }
