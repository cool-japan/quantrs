...
impl Coupling {
    ...
    pub fn new(i: usize, j: usize, strength: f64) -> IsingResult<Self> {
        ...
        if !strength.is_finite() {
            return Err(IsingError::InvalidValue(format!("Coupling strength must be finite, got {}", strength)));
        }
...
impl IsingModel {
    ...
    pub fn set_bias(&mut self, qubit: usize, bias: f64) -> IsingResult<()> {
        ...
        if !bias.is_finite() {
            return Err(IsingError::InvalidValue(format!("Bias must be finite, got {}", bias)));
        }
    ...
    pub fn energy(&self, spins: &[i8]) -> IsingResult<f64> {
        if spins.len() != self.num_qubits {
            return Err(IsingError::InvalidValue(
                format!("Spin configuration must have {} elements, got {}", self.num_qubits, spins.len())
            ));
        }
        ...
        for (i, &spin) in spins.iter().enumerate() {
            if spin != -1 && spin != 1 {
                return Err(IsingError::InvalidValue(
                    format!("Spin values must be -1 or 1, got {} at index {}", spin, i)
                ));
            }
        }
        
        // Calculate energy from biases
        let bias_energy: f64 = self.biases.iter()
            .map(|(&qubit, &bias)| bias * (spins[qubit] as f64))
            .sum();
        
        // Calculate energy from couplings
        let coupling_energy: f64 = self.couplings.iter()
...
impl QuboModel {
    ...
    pub fn set_linear(&mut self, var: usize, value: f64) -> IsingResult<()> {
        ...
        if !value.is_finite() {
            return Err(IsingError::InvalidValue(format!("Linear term must be finite, got {}", value)));
        }
    ...
    pub fn set_quadratic(&mut self, var1: usize, var2: usize, value: f64) -> IsingResult<()> {
        ...
        if !value.is_finite() {
            return Err(IsingError::InvalidValue(format!("Quadratic term must be finite, got {}", value)));
        }
        
        // Always store with var1 < var2 for consistency
        let (var1, var2) = if var1 < var2 { (var1, var2) } else { (var2, var1) };
    ...
    pub fn objective(&self, binary_vars: &[bool]) -> IsingResult<f64> {
        if binary_vars.len() != self.num_variables {
            return Err(IsingError::InvalidValue(
                format!("Binary configuration must have {} elements, got {}", self.num_variables, binary_vars.len())
            ));
        }
        
        // Calculate from linear terms
        let linear_value: f64 = self.linear_terms.iter()
            .map(|(&var, &value)| if binary_vars[var] { value } else { 0.0 })
            .sum();
        
        // Calculate from quadratic terms
        let quadratic_value: f64 = self.quadratic_terms.iter()
            .map(|(&(var1, var2), &value)| {
...
mod tests {
    ...
    fn test_ising_model_energy() {
        ...
        // Check energy for [+1, -1, +1]
        let energy = model.energy(&[1, -1, 1]).unwrap();
        assert_eq!(energy, 1.0 * 1.0 + (-0.5) * (-1.0) + (-1.0) * 1.0 * (-1.0) + 0.5 * (-1.0) * 1.0);
    }
    ...
    fn test_ising_to_qubo_conversion() {
        ...
        println!("Ising model: biases = {:?}, couplings = {:?}",
                 ising.biases(), ising.couplings());
        println!("Resulting QUBO model: linear terms = {:?}, quadratic terms = {:?}, offset = {}",
                 qubo.linear_terms(), qubo.quadratic_terms(), qubo.offset);
    ...
    fn test_qubo_to_ising_conversion() {
        ...
        println!("QUBO model: linear terms = {:?}, quadratic terms = {:?}, offset = {}",
                 qubo.linear_terms(), qubo.quadratic_terms(), qubo.offset);
        println!("Ising model: biases = {:?}, couplings = {:?}, offset = {}",
                 ising.biases(), ising.couplings(), offset);
        ...
        println!("Back to QUBO model: linear terms = {:?}, quadratic terms = {:?}, offset = {}",
                 qubo2.linear_terms(), qubo2.quadratic_terms(), qubo2.offset);
