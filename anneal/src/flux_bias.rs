...
impl FluxBiasOptimizer {
    ...
    pub fn optimize_ising(
        ...
        for qubit in hardware_qubits {
            result.flux_biases.insert(qubit, self.config.initial_flux_bias);
        }
        ...
        let final_energy = self.compute_average_energy_with_flux(
            model, 
            samples, 
            &result.flux_biases
        )?;
    ...
    fn optimize_with_gradients(
        ...
        for iteration in 0..max_iterations {
            ...
            // Compute gradients
            let gradients = self.compute_gradients(model, embedding, samples, &result.flux_biases)?;
        ...
        for _ in 0..10 { // Max iterations
            ...
            for &qubit in &hardware_qubits {
                ...
                for delta in [-self.config.step_size, 0.0, self.config.step_size] {
                    ...
                    let energy = self.compute_average_energy_with_flux(
                        model, 
                        samples, 
                        &result.flux_biases
                    )?;
    ...
    fn compute_gradients(
        ...
        for (qubit, &current_flux) in flux_biases {
            ...
            let energy_minus = self.compute_average_energy_with_flux(model, samples, &flux_minus)?;
...
impl MLFluxBiasOptimizer {
    ...
    pub fn learn_pattern(&mut self, problem_type: &str, flux_biases: &HashMap<usize, f64>) {
        ...
        self.learned_patterns.insert(problem_type.to_string(), pattern);
    }
