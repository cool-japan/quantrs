...
impl PenaltyOptimizer {
    ...
    pub fn optimize_ising_penalties(
        ...
        for (var, chain) in &embedding.chains {
            stats.chain_strengths.insert(*var, self.config.initial_chain_strength);
        }
    ...
    fn apply_static_penalties(
        ...
        for (var, chain) in &embedding.chains {
            let strength = chain_strengths.get(var)
                .copied()
...
impl AdvancedPenaltyOptimizer {
    ...
    pub fn optimize_with_gradients(
        ...
        let mut chain_strengths: HashMap<usize, f64> = embedding.chains.keys()
        ...
        for iteration in 0..max_iterations {
            ...
            self.base_optimizer.apply_static_penalties(model, embedding, &chain_strengths)?;
        ...
        Ok(PenaltyStats {
            ...
            violations: HashMap::new(),
            energy_improvement: 0.0,
            chain_break_rate: self.base_optimizer.compute_chain_break_rate(samples, embedding),
        })
    }
    ...
    fn compute_gradients(
        ...
        for (var, &current_strength) in chain_strengths {
            ...
            let energy_plus = {
                ...
                self.base_optimizer.apply_static_penalties(&mut model_copy, embedding, &strengths_plus)?;
                self.base_optimizer.compute_average_energy(&model_copy, samples)
            };
            ...
            let energy_minus = {
                let mut model_copy = model.clone();
                self.base_optimizer.apply_static_penalties(&mut model_copy, embedding, &strengths_minus)?;
                self.base_optimizer.compute_average_energy(&model_copy, samples)
            };
...
impl ConstraintPenaltyOptimizer {
    ...
    pub fn add_constraint(&mut self, constraint: Constraint) {
        ...
        self.penalty_weights.insert(constraint.name.clone(), default_weight);
    ...
    fn check_constraint(&self, constraint: &Constraint, sample: &[i8]) -> bool {
        let sum: i8 = constraint.variables.iter()
