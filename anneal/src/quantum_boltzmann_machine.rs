...
impl QuantumRestrictedBoltzmannMachine {
    ...
    pub fn new(
        ...
        if visible_config.num_units == 0 || hidden_config.num_units == 0 {
            return Err(QbmError::InvalidModel("Layer sizes must be > 0".to_string()));
        }
    ...
    pub fn train(&mut self, dataset: &[TrainingSample]) -> QbmResult<()> {
        ...
        for (i, sample) in dataset.iter().enumerate() {
            if sample.data.len() != expected_size {
                return Err(QbmError::DataError(
                    format!("Sample {} has {} features, expected {}", 
                           i, sample.data.len(), expected_size)
                ));
            }
        }
        ...
        // Momentum terms
        let mut weight_momentum = vec![vec![0.0; self.hidden_config.num_units]; self.visible_config.num_units];
        ...
        for epoch in 0..self.training_config.epochs {
            ...
            for batch_start in (0..dataset.len()).step_by(self.training_config.batch_size) {
                ...
                let batch_samples: Vec<&TrainingSample> = batch_indices.iter()
                    .map(|&i| &dataset[i])
                    .collect();
            ...
            if epoch % self.training_config.log_frequency == 0 {
                println!("Epoch {}: Error = {:.6}, FE Diff = {:.6}, Time = {:.2?}",
                         epoch, avg_error, avg_fe_diff, epoch_start.elapsed());
            }
        ...
        self.training_stats = Some(QbmTrainingStats {
            ...
            converged: self.training_config.error_threshold
                .map_or(false, |t| reconstruction_errors.last().unwrap_or(&f64::INFINITY) < &t),
    ...
    fn contrastive_divergence_batch(
        ...
        for (i, sample) in batch.iter().enumerate() {
            ...
            // Negative phase
            let (visible_recon, hidden_probs_neg, sampling_stats) = if self.training_config.persistent_cd {
                if let Some(ref mut chains) = persistent_chains {
                    ...
                    return Err(QbmError::TrainingError("Persistent chains not initialized".to_string()));
                }
            } else {
                // Standard CD-k
            ...
            let error = sample.data.iter().zip(visible_recon.iter())
                .map(|(orig, recon)| (orig - recon).powi(2))
                .sum::<f64>() / sample.data.len() as f64;
    ...
    fn quantum_sample_hidden(&mut self, probabilities: &[f64]) -> QbmResult<(Vec<f64>, QuantumSamplingStats)> {
        let start_time = Instant::now();
    ...
    fn quantum_sample_visible(&mut self, hidden_states: &[f64]) -> QbmResult<(Vec<f64>, QuantumSamplingStats)> {
    ...
    fn quantum_annealing_sample(&self, model: &IsingModel) -> QbmResult<Vec<i8>> {
        let mut simulator = QuantumAnnealingSimulator::new(self.training_config.annealing_params.clone())
    ...
    fn free_energy(&self, visible: &[f64]) -> QbmResult<f64> {
        ...
        // Visible bias term
        let visible_term: f64 = visible.iter().zip(self.visible_biases.iter())
            .map(|(&v, &b)| v * b)
            .sum();
...
impl QuantumSamplingStats {
    ...
    fn merge(&mut self, other: &QuantumSamplingStats) {
        ...
        if self.sampling_calls > 0 {
            ...
            self.average_annealing_energy = 
                (self.average_annealing_energy * (total_calls - other.sampling_calls as f64) + 
                 other.average_annealing_energy * other.sampling_calls as f64) / total_calls;
            
            self.success_rate = 
                (self.success_rate * (total_calls - other.sampling_calls as f64) + 
                 other.success_rate * other.sampling_calls as f64) / total_calls;
            
            self.classical_fallback_rate = 
                (self.classical_fallback_rate * (total_calls - other.sampling_calls as f64) + 
                 other.classical_fallback_rate * other.sampling_calls as f64) / total_calls;
        }
    }
}
