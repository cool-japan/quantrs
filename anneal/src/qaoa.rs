...
pub enum QuantumGate {
    ...
    ZZ { qubit1: usize, qubit2: usize, angle: f64 },
...
impl QaoaOptimizer {
    ...
    pub fn solve(&mut self, problem: &IsingModel) -> QaoaResult<QaoaResults> {
        ...
        let final_state = self.simulate_qaoa_circuit(problem, &optimization_result.optimal_parameters)?;
        ...
        println!("Best energy: {:.6}, Approximation ratio: {:.3}", best_energy, approximation_ratio);
    ...
    fn initialize_parameters(&mut self, problem: &IsingModel) -> QaoaResult<Vec<f64>> {
        ...
        match param_init {
            ...
            ParameterInitialization::Linear { gamma_max, beta_max } => {
                // Interleaved gamma and beta parameters
                for i in 0..num_parameters {
                    if i % 2 == 0 {
                        // Gamma parameters (problem evolution)
                        let layer = i / 2;
                        parameters[i] = gamma_max * (layer + 1) as f64 / self.get_num_layers() as f64;
                    } else {
            ...
            ParameterInitialization::TransferLearning { previous_parameters } => {
    ...
    fn get_num_parameters(&self) -> usize {
        match &self.config.variant {
            ...
            QaoaVariant::QaoaPlus { layers, multi_angle } => {
                if *multi_angle {
            ...
            QaoaVariant::MultiAngle { layers, angles_per_layer } => layers * angles_per_layer,
    ...
    fn build_qaoa_circuit(&self, problem: &IsingModel, parameters: &[f64]) -> QaoaResult<QaoaCircuit> {
        ...
        for layer in 0..num_layers {
            ...
            let gamma = if gamma_idx < parameters.len() { parameters[gamma_idx] } else { 0.0 };
            let beta = if beta_idx < parameters.len() { parameters[beta_idx] } else { 0.0 };
    ...
    fn optimize_parameters(
        ...
        match &self.config.optimizer {
            QaoaClassicalOptimizer::NelderMead { initial_size, tolerance, max_iterations } => {
                self.optimize_nelder_mead(problem, initial_parameters, *initial_size, *tolerance, *max_iterations)
            }
            
            QaoaClassicalOptimizer::GradientBased { learning_rate, gradient_step, max_iterations } => {
                self.optimize_gradient_based(problem, initial_parameters, *learning_rate, *gradient_step, *max_iterations)
            }
    ...
    fn optimize_nelder_mead(
        ...
        for iteration in 0..max_iterations {
            ...
            if function_values[best_idx] <= reflected_value && reflected_value < function_values[second_worst_idx] {
                // Accept reflection
            ...
            if iteration % 10 == 0 && self.config.detailed_logging {
                println!("Nelder-Mead iter {}: Best energy = {:.6}", iteration, best_energy);
            }
        }
    ...
    fn optimize_gradient_based(
        ...
        for iteration in 0..max_iterations {
            ...
            let gradients = self.compute_finite_difference_gradients(problem, &parameters, gradient_step)?;
            ...
            // Logging
            if iteration % 10 == 0 && self.config.detailed_logging {
                println!("Gradient iter {}: Energy = {:.6}, Grad norm = {:.6}", 
    ...
    fn evaluate_qaoa_energy(&mut self, problem: &IsingModel, parameters: &[f64]) -> QaoaResult<f64> {
        self.optimization_history.function_evaluations += 1;
        
        // Record parameter history
        if self.config.track_optimization_history {
            self.optimization_history.parameters.push(parameters.to_vec());
        }
    ...
    fn apply_gate(&self, state: &mut QuantumState, gate: &QuantumGate) -> QaoaResult<()> {
        match gate {
            ...
            QuantumGate::ZZ { qubit1, qubit2, angle } => {
    ...
    fn apply_rx_gate(&self, state: &mut QuantumState, qubit: usize, angle: f64) {
        ...
        for i in 0..(1 << n) {
            ...
            if bit == 0 {
                ...
                new_amplitudes[j] = new_amplitudes[j] + state.amplitudes[i] * complex::Complex64::new(0.0, -sin_half);
            } else {
                // |1⟩ component
    ...
    fn apply_zz_gate(&self, state: &mut QuantumState, qubit1: usize, qubit2: usize, angle: f64) {
        for i in 0..state.amplitudes.len() {
            ...
            let phase = if parity == 0 { -angle / 2.0 } else { angle / 2.0 };
    ...
    fn calculate_circuit_stats(&self, circuit: &QaoaCircuit) -> QaoaCircuitStats {
        ...
        for layer in &circuit.layers {
            for gate in &layer.problem_gates {
                match gate {
                    QuantumGate::RX { .. } | QuantumGate::RY { .. } | QuantumGate::RZ { .. } | QuantumGate::H { .. } => {
                        single_qubit_gates += 1;
                        *gate_counts.entry(format!("{:?}", gate).split(' ').next().unwrap().to_string()).or_insert(0) += 1;
                    }
    ...
    fn calculate_quantum_stats(&self, state: &QuantumState, problem: &IsingModel) -> QuantumStateStats {
    ...
    fn calculate_performance_metrics(
        ...
        QaoaPerformanceMetrics {
            ...
            optimization_efficiency: (optimization_result.optimal_energy.abs()) / self.optimization_history.function_evaluations as f64,
...
pub fn create_warm_start_qaoa_config(layers: usize, initial_solution: Vec<i8>, shots: usize) -> QaoaConfig {
    QaoaConfig {
