    ...
    pub fn new(name: String, lower_bound: f64, upper_bound: f64, precision_bits: usize) -> ContinuousVariableResult<Self> {
        if lower_bound >= upper_bound {
            return Err(ContinuousVariableError::InvalidVariable(
                format!("Invalid bounds: {} >= {}", lower_bound, upper_bound)
            ));
        }
...
impl ContinuousOptimizationProblem {
    ...
    pub fn add_variable(&mut self, variable: ContinuousVariable) -> ContinuousVariableResult<()> {
        if self.variables.contains_key(&variable.name) {
            return Err(ContinuousVariableError::InvalidVariable(
                format!("Variable '{}' already exists", variable.name)
            ));
        }
        ...
        for (var_name, var) in &self.variables {
            ...
            if indices.iter().any(|&i| i >= binary_solution.len()) {
                return Err(ContinuousVariableError::DiscretizationError(
                    format!("Binary solution too short for variable '{}'", var_name)
                ));
            }
...
impl ContinuousVariableAnnealer {
    ...
    pub fn solve(&mut self, problem: &ContinuousOptimizationProblem) -> ContinuousVariableResult<ContinuousSolution> {
        ...
        for iteration in 0..self.config.max_refinement_iterations {
            ...
            // Adaptive refinement
            if self.config.adaptive_discretization && iteration < self.config.max_refinement_iterations - 1 {
                current_problem = self.refine_discretization(problem, &continuous_values)?;
            }
        }
        
        let (final_binary, mut final_continuous, annealing_time) = best_solution
            .ok_or_else(|| ContinuousVariableError::OptimizationFailed("No solution found".to_string()))?;
        ...
        let constraint_violations = self.calculate_constraint_violations(problem, &final_continuous);
        ...
        let final_resolution = problem.variables.iter()
    ...
    fn create_discretized_problem(&self, _problem: &ContinuousOptimizationProblem) -> ContinuousVariableResult<DiscretizedProblem> {
    ...
    fn solve_discretized_problem(&mut self, _problem: &DiscretizedProblem) -> ContinuousVariableResult<Vec<i8>> {
    ...
    fn local_search(
        ...
        for _ in 0..self.config.local_search_iterations {
            ...
            for (var_name, var) in &problem.variables {
                ...
                let step_size = (var.upper_bound - var.lower_bound) * self.config.local_search_step_size;
...
pub fn create_quadratic_problem(
    ...
    let objective: ObjectiveFunction = Box::new(move |vars: &HashMap<String, f64>| {
        ...
        // Linear term
        let linear_term: f64 = linear_coeffs.iter().zip(x.iter()).map(|(c, xi)| c * xi).sum();
    ...
    for (i, &(lower, upper)) in bounds.iter().enumerate() {
        let var = ContinuousVariable::new(
            format!("x{}", i),
            lower,
            upper,
            precision_bits,
...
mod tests {
    ...
    fn test_quadratic_problem_creation() {
        ...
        let problem = create_quadratic_problem(&linear_coeffs, &quadratic_matrix, &bounds, 6).unwrap();
    ...
    fn test_constraint_evaluation() {
        ...
        let constraint = ContinuousConstraint::new("sum_constraint".to_string(), constraint_fn, 10.0);
