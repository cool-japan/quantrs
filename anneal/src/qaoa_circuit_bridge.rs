...
use crate::qaoa::{QaoaCircuit, QaoaLayer, QuantumGate as QaoaGate, QaoaError, QaoaResult};
...
impl QaoaCircuitBridge {
    ...
    pub fn qaoa_to_circuit_gates(&self, qaoa_circuit: &QaoaCircuit) -> BridgeResult<Vec<Box<dyn GateOp>>> {
        let mut gates = Vec::new();
        
        // Add initial Hadamard gates for superposition state
        for qubit in 0..qaoa_circuit.num_qubits {
            gates.push(Box::new(Hadamard { target: QubitId(qubit as u32) }) as Box<dyn GateOp>);
        }
    ...
    pub fn convert_qaoa_gate_to_circuit_gates(&self, qaoa_gate: &QaoaGate) -> BridgeResult<Vec<Box<dyn GateOp>>> {
        match qaoa_gate {
            ...
            QaoaGate::ZZ { qubit1, qubit2, angle } => {
    ...
    pub fn create_measurement_circuit(&self, num_qubits: usize) -> BridgeResult<Vec<Box<dyn GateOp>>> {
    ...
    pub fn estimate_optimization_benefit(
        ...
        OptimizationMetrics {
            ...
            gate_count_reduction: original_circuit.gates.len().saturating_sub(optimized_circuit.gates.len()),
...
impl EnhancedQaoaOptimizer {
    ...
    pub fn build_optimized_circuit(
        ...
        let mut circuit = self.bridge.build_optimizable_qaoa_circuit(
            problem, 
            parameters, 
            layers
        )?;
    ...
    pub fn estimate_circuit_cost(&self, circuit: &CircuitBridgeRepresentation) -> CircuitCostEstimate {
        ...
        CircuitCostEstimate {
            ...
            estimated_execution_time_ms: single_qubit_gates as f64 * 0.001 + two_qubit_gates as f64 * 0.1,
        }
    }
}
...
mod tests {
    ...
    fn test_gate_conversion() {
        ...
        let qaoa_gate = QaoaGate::RX { qubit: 0, angle: PI / 2.0 };
        let circuit_gates = bridge.convert_qaoa_gate_to_circuit_gates(&qaoa_gate).unwrap();
    ...
    fn test_zz_gate_decomposition() {
        ...
        let qaoa_gate = QaoaGate::ZZ { qubit1: 0, qubit2: 1, angle: PI / 4.0 };
        let circuit_gates = bridge.convert_qaoa_gate_to_circuit_gates(&qaoa_gate).unwrap();
    ...
    fn test_invalid_qubit_index() {
        ...
        let qaoa_gate = QaoaGate::RX { qubit: 3, angle: PI / 2.0 };
    ...
    fn test_circuit_compatibility_validation() {
        let circuit = CircuitBridgeRepresentation {
            gates: vec![
                ...
                Box::new(RotationX { target: QubitId(0), theta: PI / 2.0 }) as Box<dyn GateOp>,
            ],
            parameter_map: vec![],
            num_qubits: 2,
            num_parameters: 2,
