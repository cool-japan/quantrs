...
impl std::fmt::Debug for PumpSchedule {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            PumpSchedule::Linear { initial_power, final_power } => {
                f.debug_struct("Linear")
                    .field("initial_power", initial_power)
                    .field("final_power", final_power)
                    .finish()
            }
            PumpSchedule::Exponential { initial_power, final_power, time_constant } => {
                f.debug_struct("Exponential")
                    .field("initial_power", initial_power)
                    .field("final_power", final_power)
                    .field("time_constant", time_constant)
                    .finish()
            }
            PumpSchedule::Sigmoid { initial_power, final_power, steepness, midpoint } => {
...
impl CoherentIsingMachine {
    ...
    fn initialize_topology(&mut self) -> CimResult<()> {
        ...
        match topology {
            ...
            NetworkTopology::Random { connectivity } => {
                let num_possible_edges = self.config.num_oscillators * (self.config.num_oscillators - 1) / 2;
            ...
            NetworkTopology::SmallWorld { ring_connectivity, rewiring_probability } => {
    ...
    pub fn solve(&mut self, problem: &IsingModel) -> CimResult<CimResults> {
        if problem.num_qubits != self.config.num_oscillators {
            return Err(CimError::SimulationError(
                format!("Problem size {} doesn't match CIM size {}", 
        ...
        for step in 0..num_steps {
            ...
            if step % 1000 == 0 && self.config.detailed_logging {
                println!("Step {}: Time = {:.3}, Energy = {:.6}, Power = {:.3}",
        ...
        let results = CimResults {
            ...
            optical_evolution: self.evolution_history.iter().map(|(_, state)| state.clone()).collect(),
    ...
    fn get_pump_power(&self, time: f64) -> f64 {
        ...
        match &self.config.pump_schedule {
            ...
            PumpSchedule::Exponential { initial_power, final_power, time_constant } => {
                initial_power + (final_power - initial_power) * 
                    (1.0 - (-time / time_constant).exp())
            }
            ...
            PumpSchedule::Custom { power_function } => {
                power_function(time)
            }
        }
    }
    ...
    fn add_noise(&mut self) {
        ...
        for osc in &mut self.oscillators {
            ...
            // Amplitude noise
            let amplitude_noise = 1.0 + self.rng.gen_range(-1.0..1.0) * noise_config.amplitude_noise;
    ...
    fn measure_solution(&self) -> CimResult<Vec<i8>> {
        ...
        for osc in &self.oscillators {
            ...
            let measurement = osc.amplitude.re * self.config.measurement_config.detection_efficiency;
    ...
    fn check_convergence(&self) -> CimResult<bool> {
        ...
        let oscillation_stable = self.oscillators.iter()
            .all(|osc| osc.amplitude.magnitude() > self.config.convergence_config.oscillation_threshold);
    ...
    fn calculate_optical_statistics(&self) -> OpticalStatistics {
        ...
        // Average power
        let total_power: f64 = self.oscillators.iter()
        ...
        // Power variance
        let power_variance = {
            let powers: Vec<f64> = self.oscillators.iter()
                .map(|osc| osc.amplitude.magnitude_squared())
                .collect();
            let mean = powers.iter().sum::<f64>() / powers.len() as f64;
            powers.iter()
                .map(|&p| (p - mean).powi(2))
                .sum::<f64>() / powers.len() as f64
        };
        
        // Phase coherence (simplified)
        let phase_coherence: Vec<f64> = self.oscillators.iter()
            ...
            for j in 0..num_oscillators {
                if i == j {
                    ...
                    let phase_diff = (self.oscillators[i].amplitude.phase() - 
                                    self.oscillators[j].amplitude.phase()).abs();
    ...
    fn calculate_performance_metrics(&self, best_energy: f64, convergence_time: f64) -> CimPerformanceMetrics {
        ...
        // Power efficiency
        let average_power = self.oscillators.iter()
            .map(|osc| osc.amplitude.magnitude_squared())
