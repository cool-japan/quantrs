...
pub fn create_test_problem(
    ...
    match problem_type {
        ProblemType::RandomIsing => {
            create_random_ising_problem(&mut problem, density, rng_seed)?
        }
        ProblemType::MaxCut => {
            create_max_cut_problem(&mut problem, density, rng_seed)?
        }
        ProblemType::VertexCover => {
            create_vertex_cover_problem(&mut problem, density, rng_seed)?
        }
        ProblemType::TSP => {
            create_tsp_problem(&mut problem, density, rng_seed)?
        }
        ProblemType::Portfolio => {
            create_portfolio_problem(&mut problem, density, rng_seed)?
        }
        ProblemType::Custom(ref name) => {
            return Err(ApplicationError::ConfigurationError(
                format!("Custom problem type not implemented: {}", name)
            ));
        }
    }
...
fn create_random_ising_problem(problem: &mut IsingModel, density: f64, seed: u64) -> ApplicationResult<()> {
...
fn create_max_cut_problem(problem: &mut IsingModel, density: f64, seed: u64) -> ApplicationResult<()> {
    let size = problem.num_qubits;
    let mut local_seed = seed;
...
fn create_vertex_cover_problem(problem: &mut IsingModel, density: f64, seed: u64) -> ApplicationResult<()> {
    let size = problem.num_qubits;
    let mut local_seed = seed;
...
fn create_portfolio_problem(problem: &mut IsingModel, _density: f64, seed: u64) -> ApplicationResult<()> {
    let size = problem.num_qubits;
    let mut local_seed = seed;
...
pub fn calculate_test_quality_metrics(
    ...
    let variance_quality = qualities.iter()
        .map(|q| (q - mean_quality).powi(2))
        .sum::<f64>() / (qualities.len() - 1).max(1) as f64;
    ...
    let median_quality = if sorted_qualities.len() % 2 == 0 {
        (sorted_qualities[sorted_qualities.len() / 2 - 1] + sorted_qualities[sorted_qualities.len() / 2]) / 2.0
    } else {
    ...
    let variance_time = execution_times.iter()
        .map(|t| (t.as_secs_f64() - mean_time_secs).powi(2))
        .sum::<f64>() / (execution_times.len() - 1).max(1) as f64;
...
pub fn generate_test_summary(results: &TestSuiteResults) -> String {
    ...
    summary.push_str(&format!("**Overall Success:** {}\n\n", if results.overall_success { "✅" } else { "❌" }));
    
    // Scenario tests summary
    if !results.scenario_results.is_empty() {
        let scenario_success = results.scenario_results.iter().filter(|r| r.success).count();
    ...
    if !results.regression_results.is_empty() {
        let regressions_detected = results.regression_results.iter().filter(|r| r.regression_detected).count();
        summary.push_str(&format!(
    ...
    if !results.platform_results.is_empty() {
        let avg_compatibility = results.platform_results.iter()
            .map(|r| r.compatibility_score)
            .sum::<f64>() / results.platform_results.len() as f64;
    ...
    if !results.stress_results.is_empty() {
        let avg_success_rate = results.stress_results.iter()
            .map(|r| r.success_rate)
            .sum::<f64>() / results.stress_results.len() as f64;
    ...
    if !results.property_results.is_empty() {
        let total_cases = results.property_results.iter().map(|r| r.cases_tested).sum::<usize>();
        let total_passed = results.property_results.iter().map(|r| r.cases_passed).sum::<usize>();
