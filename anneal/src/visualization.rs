...
use crate::simulator::{AnnealingParams, QuantumAnnealingSimulator, AnnealingSolution};
...
impl LandscapeAnalyzer {
    ...
    pub fn sample_landscape(&self, model: &IsingModel) -> VisualizationResult<Vec<LandscapePoint>> {
        ...
        for _ in 0..num_samples {
            ...
            let hamming_distance = self.reference_config.as_ref()
    ...
    pub fn enumerate_landscape(&self, model: &IsingModel) -> VisualizationResult<Vec<LandscapePoint>> {
...
pub fn plot_energy_landscape<P: AsRef<Path>>(
    ...
    for (i, point) in points.iter().enumerate() {
        writeln!(
            ...
            i,
            point.energy,
            point.hamming_distance.map_or("".to_string(), |d| d.to_string()),
...
pub fn plot_energy_histogram<P: AsRef<Path>>(
    ...
    if points.is_empty() {
        return Err(VisualizationError::InvalidParameter("No points to plot".to_string()));
    }
    ...
    if min_energy == max_energy {
        return Err(VisualizationError::InvalidParameter("All energies are the same".to_string()));
    }
        ...
        for i in 0..points.len() {
            ...
            while let Some(current_idx) = stack.pop() {
                ...
                for j in 0..points.len() {
                    ...
                    if energy_diff <= self.energy_tolerance && hamming_dist <= self.hamming_threshold {
