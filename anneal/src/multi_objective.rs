...
pub enum ScalarizationMethod {
    ...
    WeightedChebyshev { weights: Vec<f64>, reference_point: Vec<f64> },
    
    /// Augmented Chebyshev with penalty term
    AugmentedChebyshev { weights: Vec<f64>, reference_point: Vec<f64>, rho: f64 },
    
    /// Achievement scalarizing function
    Achievement { weights: Vec<f64>, reference_point: Vec<f64> },
    
    /// ε-constraint: optimize one objective subject to constraints on others
    EpsilonConstraint { primary_objective: usize, constraints: Vec<f64> },
}
...
impl ScalarizationMethod {
    ...
    pub fn scalarize(&self, objectives: &[f64]) -> MultiObjectiveResult<f64> {
        match self {
            ScalarizationMethod::WeightedSum { weights } => {
                ...
                let weighted_sum = weights.iter().zip(objectives.iter())
                    .map(|(w, obj)| w * obj)
                    .sum();
            ...
            ScalarizationMethod::WeightedChebyshev { weights, reference_point } => {
                ...
                let max_weighted_deviation = weights.iter()
                    .zip(objectives.iter())
            ...
            ScalarizationMethod::AugmentedChebyshev { weights, reference_point, rho } => {
                let chebyshev_value = self.scalarize_chebyshev(objectives, weights, reference_point)?;
                
                let augmentation_term = rho * weights.iter()
                    .zip(objectives.iter())
                    .zip(reference_point.iter())
            ...
            ScalarizationMethod::Achievement { weights, reference_point } => {
                let achievement_value = weights.iter()
                    .zip(objectives.iter())
                    .zip(reference_point.iter())
            ...
            ScalarizationMethod::EpsilonConstraint { primary_objective, constraints } => {
    ...
    fn scalarize_chebyshev(&self, objectives: &[f64], weights: &[f64], reference_point: &[f64]) -> MultiObjectiveResult<f64> {
        if weights.len() != objectives.len() || reference_point.len() != objectives.len() {
            return Err(MultiObjectiveError::ScalarizationError(
                "Dimension mismatch in Chebyshev scalarization".to_string()
            ));
...
impl MultiObjectiveOptimizer {
    ...
    pub fn solve(
        ...
        for (run_idx, scalarization) in scalarizations.into_iter().enumerate() {
            ...
            println!("Running scalarization {}/{}", run_idx + 1, self.config.num_pareto_runs);
            
            // Solve single-objective problem
            let solution = self.solve_scalarized_problem(model, &objective_function, &scalarization)?;
        ...
        let pareto_front: Vec<MultiObjectiveSolution> = all_solutions.iter()
    ...
    fn generate_scalarizations(&mut self, num_objectives: usize) -> MultiObjectiveResult<Vec<ScalarizationMethod>> {
        ...
        match self.config.scalarization.clone() {
            ...
            ScalarizationMethod::WeightedChebyshev { reference_point, .. } => {
            ...
            ScalarizationMethod::EpsilonConstraint { primary_objective, .. } => {
    ...
    fn generate_weight_vector(&mut self, num_objectives: usize, run_index: usize, num_runs: usize) -> Vec<f64> {
        if num_objectives == 2 {
    ...
    fn solve_scalarized_problem(
        ...
        let solution = simulator.solve(&scalarized_model)
            .map_err(|e| MultiObjectiveError::OptimizationFailed(e.to_string()))?;
    ...
    /// Remove duplicate solutions
    fn remove_duplicates(&self, mut solutions: Vec<MultiObjectiveSolution>) -> Vec<MultiObjectiveSolution> {
        solutions.sort_by(|a, b| {
            a.variables.partial_cmp(&b.variables).unwrap_or(std::cmp::Ordering::Equal)
        });
    ...
    fn perform_pareto_analysis(&self, solutions: &mut [MultiObjectiveSolution]) -> MultiObjectiveResult<()> {
    ...
    fn fast_non_dominated_sort(&self, solutions: &mut [MultiObjectiveSolution]) -> MultiObjectiveResult<()> {
        let n = solutions.len();
    ...
    fn calculate_crowding_distances(&self, solutions: &mut [MultiObjectiveSolution]) -> MultiObjectiveResult<()> {
                ...
                if range > 0.0 {
                    ...
                    for i in 1..indices.len()-1 {
                        let distance = (solutions[indices[i+1]].objectives[obj_idx] - 
                                      solutions[indices[i-1]].objectives[obj_idx]) / range;
    ...
    /// Calculate objective bounds
    fn calculate_objective_bounds(&self, solutions: &[MultiObjectiveSolution], num_objectives: usize) -> Vec<(f64, f64)> {
    ...
    fn calculate_statistics(
        ...
        let average_crowding_distance = if pareto_front.is_empty() {
            ...
            pareto_front.iter()
                ...
                .sum::<f64>() / pareto_front.len() as f64
        };
        
        let mut convergence_metrics = HashMap::new();
        convergence_metrics.insert("pareto_ratio".to_string(), 
...
impl QualityMetrics {
    ...
    pub fn hypervolume(solutions: &[MultiObjectiveSolution], reference_point: &[f64]) -> MultiObjectiveResult<f64> {
        if solutions.is_empty() {
            return Ok(0.0);
        }
        ...
        if num_objectives == 2 {
            ...
            for solution in &sorted_solutions {
                if solution.objectives[0] < reference_point[0] && solution.objectives[1] < reference_point[1] {
                    let width = solution.objectives[0] - prev_x;
                    let height = reference_point[1] - solution.objectives[1];
                    volume += width * height;
                    prev_x = solution.objectives[0];
                }
            }
    ...
    pub fn spacing(solutions: &[MultiObjectiveSolution]) -> f64 {
        ...
        for (i, sol1) in solutions.iter().enumerate() {
            ...
            for (j, sol2) in solutions.iter().enumerate() {
                if i != j {
                    let distance: f64 = sol1.objectives.iter()
...
mod tests {
    ...
    fn test_solution_dominance() {
        ...
        // sol3 dominates both sol1 and sol2 (both objectives are better)
    ...
    fn test_chebyshev_scalarization() {
        ...
        let scalarization = ScalarizationMethod::WeightedChebyshev { weights, reference_point };
