...
use quantrs2_anneal::{
    ...
    reverse_annealing::{
        ReverseAnnealingSimulator, ReverseAnnealingParams, 
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (name, s_target, pause) in strategies {
        println!("Strategy: {} (s_target={}, pause={})", name, s_target, pause);
        ...
        println!("  Best energy: {:.4} (improvement: {:.4})", 
    ...
    println!("  Improvement over forward: {:.4}", 
    ...
    for iteration in 1..=5 {
        ...
        if iter_result.best_energy < current_energy {
            ...
            println!("  Iteration {}: Improved to {:.4}", iteration, current_energy);
        } else {
            println!("  Iteration {}: No improvement (energy: {:.4})", 
                iteration, iter_result.best_energy);
        }
    }
    
    println!("\nFinal results:");
    println!("  Initial energy (forward annealing): {:.4}", forward_result.best_energy);
    println!("  Final energy (after reverse annealing): {:.4}", current_energy);
    println!("  Total improvement: {:.4}", forward_result.best_energy - current_energy);
