...
fn basic_photonic_annealing_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Display metrics
    println!("\n  Performance metrics:");
    println!("    Success probability: {:.1}%", results.metrics.success_probability * 100.0);
    println!("    Average quality: {:.3}", results.metrics.average_quality);
    println!("    Quantum advantage estimate: {:.2}x", results.metrics.quantum_advantage);
    println!("    Photon loss: {:.1}%", results.metrics.photon_loss * 100.0);
    println!("    Measurement efficiency: {:.1}%", results.metrics.measurement_efficiency * 100.0);
...
fn measurement_strategy_comparison() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let strategies = vec![
        ("Homodyne (0°)", MeasurementStrategy::Homodyne { local_oscillator_phase: 0.0 }),
        ("Homodyne (45°)", MeasurementStrategy::Homodyne { local_oscillator_phase: std::f64::consts::PI / 4.0 }),
        ("Heterodyne", MeasurementStrategy::Heterodyne),
        ("Photon Counting", MeasurementStrategy::PhotonCounting { threshold: 0.5 }),
        ("Parity", MeasurementStrategy::Parity),
        ("Adaptive", MeasurementStrategy::Adaptive { feedback_strength: 0.3 }),
    ];
...
fn squeezed_state_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let states = vec![
        ...
        ("Squeezed (r=0.5)", InitialStateType::SqueezedVacuum { squeezing: 0.5 }),
        ("Squeezed (r=1.0)", InitialStateType::SqueezedVacuum { squeezing: 1.0 }),
        ("Thermal (n=0.5)", InitialStateType::Thermal { mean_photons: 0.5 }),
    ];
...
fn measurement_based_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut fidelities: Vec<f64> = results.measurement_outcomes.iter()
        .map(|o| o.fidelity)
        .collect();
    ...
    let mut solution_qualities: Vec<f64> = results.measurement_outcomes.iter()
        .map(|o| {
            let worst_energy = results.measurement_outcomes.iter()
                .map(|o2| o2.energy)
    ...
    for i in 0..bins.len()-1 {
        let count = solution_qualities.iter()
            .filter(|&&q| q <= bins[i] && q > bins[i+1])
            .count();
        let percentage = count as f64 / solution_qualities.len() as f64 * 100.0;
        println!("    Quality {:.0}%-{:.0}%: {:.1}%", 
                bins[i+1] * 100.0, bins[i] * 100.0, percentage);
    }
...
fn realistic_experimental_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("    Best energy: {:.6}", results.best_energy);
    println!("    Runtime: {:.2?}", runtime);
    println!("    Effective temperature: {:.2e} K", results.metrics.effective_temperature);
    ...
    println!("    Energy gap: {:.6}", results.best_energy - ideal_results.best_energy);
    println!("    Success probability ratio: {:.2}x", 
            ideal_results.metrics.success_probability / results.metrics.success_probability);
    
    // Analyze impact of noise
    println!("\n  Noise impact analysis:");
    println!("    Photon loss: {:.1}%", results.metrics.photon_loss * 100.0);
    println!("    Final state purity: {:.3}", results.final_state.purity());
