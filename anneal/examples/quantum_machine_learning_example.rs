...
fn variational_quantum_classifier_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("    Final loss: {:.6}", vqc.training_history.losses.last().unwrap_or(&0.0));
    println!("    Test prediction: class {}", prediction);
    println!("    Class probabilities: [{:.3}, {:.3}]", probabilities[0], probabilities[1]);
    
    // Evaluate accuracy on training data
    let mut correct = 0;
    for sample in &training_data[..20] { // Test on subset
...
fn quantum_neural_network_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("    Training samples: {}", training_data.len());
    println!("    Final loss: {:.6}", qnn.training_history.losses.last().unwrap_or(&0.0));
    println!("    Test input: [{:.2}, {:.2}, {:.2}]", test_input[0], test_input[1], test_input[2]);
    ...
    let error: f64 = output.iter().zip(expected.iter())
        .map(|(o, e)| (o - e).abs())
...
fn quantum_feature_maps_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let feature_maps = vec![
        ...
        ("Pauli (Linear)", FeatureMapType::PauliFeatureMap { entanglement: EntanglementType::Linear }),
        ("Pauli (Circular)", FeatureMapType::PauliFeatureMap { entanglement: EntanglementType::Circular }),
        ("ZZ Feature Map", FeatureMapType::ZZFeatureMap { repetitions: 2 }),
    ];
    
    println!("    Input data: [{:.2}, {:.2}, {:.2}, {:.2}]", test_data[0], test_data[1], test_data[2], test_data[3]);
    ...
    let feature_map = QuantumFeatureMap::new(4, 4, FeatureMapType::ZZFeatureMap { repetitions: 2 })?;
...
fn quantum_kernel_methods_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let kernel_methods = vec![
        ("SVM", KernelMethodType::SupportVectorMachine { c_parameter: 1.0 }),
        ("Ridge Regression", KernelMethodType::RidgeRegression { regularization: 0.1 }),
    ...
    for (name, method_type) in kernel_methods {
        ...
        let feature_map = QuantumFeatureMap::new(
            2, 3, 
            FeatureMapType::PauliFeatureMap { entanglement: EntanglementType::Circular }
        )?;
        ...
        println!("      Training samples: {}", training_data.len());
        println!("      Support vectors: {}", kernel_method.support_vectors.len());
...
fn quantum_gan_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("    Generator architecture: {} -> {} -> {}", 
    ...
    if !history.generator_losses.is_empty() {
        println!("    Final generator loss: {:.4}", history.generator_losses.last().unwrap());
        println!("    Final discriminator loss: {:.4}", history.discriminator_losses.last().unwrap());
    }
    ...
    let gen_mean_x: f64 = generated_samples.iter().map(|s| s[0]).sum::<f64>() / generated_samples.len() as f64;
    let gen_mean_y: f64 = generated_samples.iter().map(|s| s[1]).sum::<f64>() / generated_samples.len() as f64;
    
    println!("    Real data mean: [{:.3}, {:.3}]", real_mean_x, real_mean_y);
...
fn quantum_reinforcement_learning_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for episode in 0..num_episodes {
        ...
        if episode % 2 == 0 {
            println!("      Episode {}: Reward = {:.2}, Buffer size = {}", 
                     episode, episode_reward, agent.experience_buffer.len());
        }
    }
    ...
    println!("      Test state: [{:.2}, {:.2}, {:.2}, {:.2}]", 
             test_state[0], test_state[1], test_state[2], test_state[3]);
    println!("      Selected action: {}", test_action);
    
    // Performance statistics
    let avg_reward: f64 = agent.stats.episode_rewards.iter().sum::<f64>() / agent.stats.episode_rewards.len() as f64;
    let max_reward = agent.stats.episode_rewards.iter().fold(f64::NEG_INFINITY, |a, &b| a.max(b));
    
    println!("      Average episode reward: {:.2}", avg_reward);
    println!("      Best episode reward: {:.2}", max_reward);
    println!("      Experience buffer utilization: {:.1}%", 
...
fn quantum_autoencoder_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("    Training completed in {:.2?}", training_time);
    println!("    Architecture: 8 -> 3 -> 8 (compression ratio: {:.1}x)", 
             8.0 / 3.0);
    println!("    Training samples: {}", training_data.len());
    
    if !autoencoder.training_history.losses.is_empty() {
        println!("    Final reconstruction loss: {:.6}", 
    ...
    for sample in test_samples {
        ...
        let error: f64 = sample.iter().zip(reconstructed.iter())
    ...
    let input_str = test_sample.iter()
        .map(|x| format!("{:.2}", x))
        .collect::<Vec<_>>()
        .join(", ");
    let encoded_str = encoded.iter()
        ...
        .collect::<Vec<_>>()
        .join(", ");
    let reconstructed_str = reconstructed.iter()
        .map(|x| format!("{:.2}", x))
        .collect::<Vec<_>>()
        .join(", ");
...
fn quantum_classical_comparison_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for _ in 0..100 {
        ...
        let label = if x1*x1 + x2*x2 - x3.sin() > 0.0 { 1 } else { 0 };
    ...
    println!("    Dataset: {} training, {} test samples", training_data.len(), test_data.len());
    ...
    println!("    ├─────────────────┼─────────────┼─────────────┼─────────────┤");
    println!("    │ Quantum VQC     │ {:>9.1}%  │ {:>9.2?}  │ {:>9}   │", 
             quantum_accuracy * 100.0, quantum_training_time, quantum_classifier.parameters.len());
    println!("    │ Classical NN    │ {:>9.1}%  │ {:>9.2?}  │ {:>9}   │", 
             classical_accuracy * 100.0, classical_training_time, classical_weights.len());
    println!("    └─────────────────┴─────────────┴─────────────┴─────────────┘");
    ...
    println!("      Original features: [{:.2}, {:.2}, {:.2}]", test_sample[0], test_sample[1], test_sample[2]);
    println!("      Quantum encoding: [{:.3}, {:.3}, {:.3}]", encoded[0], encoded[1], encoded[2]);
