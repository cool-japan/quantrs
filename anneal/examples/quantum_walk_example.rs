...
use quantrs2_anneal::{
    ising::IsingModel,
    quantum_walk::{
        QuantumWalkOptimizer, QuantumWalkConfig, QuantumWalkAlgorithm,
        CoinOperator, AdiabaticHamiltonian, QuantumState,
    },
...
fn amplitude_amplification_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("Amplification overhead: {:.2?}", time_with_amp - time_no_amp);
...
fn performance_comparison_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("  QW advantage: {}", if energy_diff > 0.0 { "Better energy" } else { "Faster time" });
...
fn print_state_info(state: &QuantumState) {
    ...
    for (i, amp) in state.amplitudes.iter().take(8).enumerate() {
        ...
        println!("      |{:?}⟩: ({:.3} + {:.3}i), P = {:.3}", 
                 bits.iter().map(|&b| if b > 0 { 1 } else { 0 }).collect::<Vec<_>>(),
                 amp.re, amp.im, prob);
    }
