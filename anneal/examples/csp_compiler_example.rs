...
use quantrs2_anneal::{
    csp_compiler::{
        CspProblem, CspVariable, CspConstraint, Domain, CspValue, CompilationParams,
        ComparisonOp,
    },
...
fn boolean_sat_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("  Constraints compiled: {}", compilation_info.constraints_compiled);
...
fn graph_coloring_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for i in 0..3 {
        let var = CspVariable::new(
            format!("vertex_{}", i),
            Domain::Discrete(colors.clone())
    ...
    println!("  Solution found with energy: {:.4}", result.best_energy + offset);
...
fn n_queens_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for i in 0..n {
        ...
        let constraint = CspConstraint::ExactlyOne { variables: row_vars };
        problem.add_constraint(constraint)?;
    }
    ...
    for j in 0..n {
        ...
        let constraint = CspConstraint::ExactlyOne { variables: col_vars };
        problem.add_constraint(constraint)?;
    }
...
fn sudoku_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for i in 0..2 {
        ...
        let constraint = CspConstraint::AllDifferent { variables: row_vars };
        problem.add_constraint(constraint)?;
    }
    ...
    for j in 0..2 {
        ...
        let constraint = CspConstraint::AllDifferent { variables: col_vars };
        problem.add_constraint(constraint)?;
    }
...
fn scheduling_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for i in 0..3 {
        let var = CspVariable::new(
            format!("task_{}", i),
            Domain::Discrete(time_slots.clone())
    ...
    for task in 0..3 {
        ...
        let assigned_slot = if slot0_idx < result.best_spins.len() && result.best_spins[slot0_idx] > 0 {
            0
...
fn map_coloring_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for region in &regions {
        let var = CspVariable::new(
            region.to_string(),
            Domain::Categorical(colors.clone())
