...
use quantrs2_anneal::{
    ising::IsingModel,
    qaoa::{QaoaOptimizer, QaoaConfig, QaoaVariant, MixerType, ProblemEncoding, QaoaClassicalOptimizer, ParameterInitialization},
    qaoa_circuit_bridge::{
        QaoaCircuitBridge, EnhancedQaoaOptimizer, OptimizationLevel, 
        create_qaoa_bridge_for_problem, validate_circuit_compatibility
    },
};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("Built QAOA circuit with {} gates and {} parameters", 
             circuit.gates.len(), circuit.num_parameters);
    ...
    println!("Built optimized QAOA circuit with {} gates", optimized_circuit.gates.len());
    ...
    println!("  - Single-qubit gates: {}", cost_estimate.single_qubit_gates);
    println!("  - Two-qubit gates: {}", cost_estimate.two_qubit_gates);
    println!("  - Estimated execution time: {:.3} ms", cost_estimate.estimated_execution_time_ms);
    ...
    let qaoa_gates = vec![
        QaoaGate::RX { qubit: 0, angle: PI / 2.0 },
        QaoaGate::RY { qubit: 1, angle: PI / 4.0 },
        QaoaGate::RZ { qubit: 2, angle: PI / 6.0 },
        QaoaGate::CNOT { control: 0, target: 1 },
        QaoaGate::ZZ { qubit1: 1, qubit2: 2, angle: PI / 8.0 },
        QaoaGate::H { qubit: 0 },
    ];
    ...
    for (i, qaoa_gate) in qaoa_gates.iter().enumerate() {
        match bridge.convert_qaoa_gate_to_circuit_gates(qaoa_gate) {
            Ok(circuit_gates) => {
                println!("Gate {}: {:?} -> {} circuit gates", i + 1, qaoa_gate, circuit_gates.len());
                for (j, gate) in circuit_gates.iter().enumerate() {
                    println!("  Gate {}.{}: {} on qubits {:?}", 
                             i + 1, j + 1, gate.name(), gate.qubits());
                }
            }
            Err(e) => {
    ...
    println!("  - Parameter references: {}", multi_circuit.parameter_map.len());
    ...
    println!("  - Quadratic terms: {}", problem_repr.quadratic_terms.len());
    
    for (i, quad_term) in problem_repr.quadratic_terms.iter().enumerate() {
        println!("    Quadratic term {}: J_{{{},{}}} = {}", 
                 i + 1, quad_term.qubit1, quad_term.qubit2, quad_term.coefficient);
    }
...
mod tests {
    ...
    fn test_circuit_building() {
        ...
        let circuit = bridge.build_optimizable_qaoa_circuit(&problem, &parameters, 1).unwrap();
