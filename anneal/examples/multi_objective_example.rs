...
use quantrs2_anneal::{
    ising::IsingModel,
    multi_objective::{
        MultiObjectiveOptimizer, MultiObjectiveConfig, ScalarizationMethod,
        QualityMetrics, MultiObjectiveSolution,
    },
...
fn portfolio_optimization_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let objective_function = Box::new(move |spins: &[i8]| -> Vec<f64> {
        ...
        for i in 0..4 {
            for j in 0..4 {
                risk += weights[i] * weights[j] * risks_clone[i] * risks_clone[j] * correlations_clone[i][j];
            }
        }
        
        // Calculate portfolio return
        let portfolio_return: f64 = weights.iter().zip(expected_returns_clone.iter())
            .map(|(w, r)| w * r).sum();
    ...
    if let Some(min_risk_sol) = results.all_solutions.iter().min_by(|a, b| a.objectives[0].partial_cmp(&b.objectives[0]).unwrap()) {
        let weights = spins_to_portfolio_weights(&min_risk_sol.variables);
        println!("\n  Minimum risk portfolio:");
        println!("    Weights: {:?}", weights.iter().map(|w| format!("{:.3}", w)).collect::<Vec<_>>());
        println!("    Risk: {:.4}", min_risk_sol.objectives[0]);
        println!("    Return: {:.4}", -min_risk_sol.objectives[1]);
    }
    
    if let Some(max_return_sol) = results.all_solutions.iter().min_by(|a, b| a.objectives[1].partial_cmp(&b.objectives[1]).unwrap()) {
    ...
    for (i, (min_val, max_val)) in results.objective_bounds.iter().enumerate() {
        ...
        println!("      {}: [{:.4}, {:.4}]", obj_name, display_min, display_max);
    }
...
fn engineering_design_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let objective_function = Box::new(move |spins: &[i8]| -> Vec<f64> {
        ...
        for i in 0..4 {
            if spins[i] > 0 {  // Component is selected
    ...
    for (i, solution) in results.pareto_front.iter().take(5).enumerate() {
        let selected_components: Vec<char> = solution.variables.iter().enumerate()
            .filter(|(_, &spin)| spin > 0)
            .map(|(idx, _)| (b'A' + idx as u8) as char)
            .collect();
        
        println!("    Design {}: Components {:?}, Cost: ${:.0}, Performance: {:.1}",
                 i + 1,
                 selected_components,
...
fn network_design_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let objective_function = Box::new(move |spins: &[i8]| -> Vec<f64> {
        let selected_edges: Vec<(usize, usize)> = spins.iter().enumerate()
            .filter(|(_, &spin)| spin > 0)
            .map(|(idx, _)| edges[idx])
            .collect();
    ...
    for (i, solution) in results.pareto_front.iter().take(3).enumerate() {
        ...
        println!("    Topology {}: {} edges, Latency: {:.2}, Reliability: {:.2}",
                 i + 1, num_edges, solution.objectives[0], -solution.objectives[1]);
    }
...
fn scalarization_comparison_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let objective_function = Box::new(|spins: &[i8]| -> Vec<f64> {
        let obj1 = spins.iter().enumerate().map(|(i, &s)| if s > 0 { i as f64 } else { 0.0 }).sum::<f64>();
        let obj2 = spins.iter().filter(|&&s| s > 0).count() as f64;
        vec![obj1, obj2]
    });
    
    let methods = vec![
        ("Weighted Sum", ScalarizationMethod::WeightedSum { weights: vec![0.7, 0.3] }),
        ("Chebyshev", ScalarizationMethod::WeightedChebyshev { 
            weights: vec![1.0, 1.0], 
            reference_point: vec![20.0, 8.0] 
        }),
        ("ε-Constraint", ScalarizationMethod::EpsilonConstraint { 
            primary_objective: 0, 
    ...
    for (method_name, scalarization) in methods {
        ...
        println!("\n  {} Method:", method_name);
        println!("    Pareto front size: {}", results.pareto_front.len());
        println!("    Avg crowding distance: {:.4}", results.stats.average_crowding_distance);
        println!("    Runtime: {:.2?}", runtime);
        
        if !results.pareto_front.is_empty() {
            let best = &results.pareto_front[0];
            println!("    Best solution: Obj1={:.2}, Obj2={:.2}", best.objectives[0], best.objectives[1]);
        }
    }
...
fn quality_metrics_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (i, sol) in solutions.iter().enumerate() {
        println!("    Sol {}: ({:.2}, {:.2})", i + 1, sol.objectives[0], sol.objectives[1]);
    }
    
    // Calculate ranges
    let obj1_range = solutions.iter().map(|s| s.objectives[0]).fold((f64::INFINITY, f64::NEG_INFINITY), 
        |(min, max), val| (min.min(val), max.max(val)));
    let obj2_range = solutions.iter().map(|s| s.objectives[1]).fold((f64::INFINITY, f64::NEG_INFINITY), 
        |(min, max), val| (min.min(val), max.max(val)));
    
    println!("\n  Objective ranges:");
    println!("    Objective 1: [{:.2}, {:.2}]", obj1_range.0, obj1_range.1);
    println!("    Objective 2: [{:.2}, {:.2}]", obj2_range.0, obj2_range.1);
...
fn three_objective_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let objective_function = Box::new(|spins: &[i8]| -> Vec<f64> {
        ...
        let energy = spins.iter().enumerate().map(|(i, &s)| s as f64 * (i as f64 - 3.5) * 0.2).sum::<f64>();
    ...
    for (i, solution) in results.pareto_front.iter().take(8).enumerate() {
        let config_str = solution.variables.iter()
            .map(|&s| if s > 0 { "↑" } else { "↓" })
            .collect::<String>();
        println!("    Sol {}: {} | Energy: {:.2}, Sparsity: {:.1}, Fragmentation: {:.1}",
                 i + 1, config_str, 
