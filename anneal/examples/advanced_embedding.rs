...
use quantrs2_anneal::{
    ...
    penalty_optimization::{PenaltyOptimizer, PenaltyConfig, AdvancedPenaltyOptimizer},
    ising::IsingModel,
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("   Logical graph: {} variables, {} edges", num_vars, logical_edges.len());
    ...
    let standard_embedding = standard_embedder.find_embedding(&logical_edges, num_vars, &hardware)?;
    
    println!("   Standard embedding found:");
    for (var, chain) in &standard_embedding.chains {
        println!("     Variable {}: chain {:?} (length {})", var, chain, chain.len());
    }
    ...
    let (layout_embedding, layout_stats) = layout_embedder.find_embedding(&logical_edges, num_vars, &hardware)?;
    
    println!("   Layout-aware embedding found:");
    println!("   - Average chain length: {:.2}", layout_stats.avg_chain_length);
    println!("   - Maximum chain length: {}", layout_stats.max_chain_length);
    println!("   - Long chains (>{} qubits): {}", 4, layout_stats.long_chains);
    ...
    for &(i, j) in &logical_edges {
        if let (Some(chain_i), Some(chain_j)) = (layout_embedding.chains.get(&i), layout_embedding.chains.get(&j)) {
            // Find a valid coupling between the chains
            'outer: for &qi in chain_i {
                for &qj in chain_j {
                    if qi < hardware.num_qubits && qj < hardware.num_qubits && hardware.are_connected(qi, qj) {
    ...
    let penalty_stats = penalty_optimizer.optimize_ising_penalties(
        &mut ising,
        &layout_embedding,
        &samples,
    )?;
    ...
    println!("   - Chain break rate: {:.2}%", penalty_stats.chain_break_rate * 100.0);
