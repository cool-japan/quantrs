...
use quantrs2_anneal::{
    ...
    coherent_ising_machine::{
        CoherentIsingMachine, CimConfig, PumpSchedule, NetworkTopology, NoiseConfig,
        MeasurementConfig, ConvergenceConfig, create_standard_cim_config, 
        create_low_noise_cim_config, create_realistic_cim_config,
    },
};
...
fn standard_cim_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("    Average power: {:.6}", results.optical_stats.average_power);
    println!("    Power variance: {:.6}", results.optical_stats.power_variance);
    println!("    Pump efficiency: {:.1}%", results.optical_stats.pump_efficiency * 100.0);
    
    // Final optical state
    println!("\n  Final optical amplitudes:");
    for (i, amplitude) in results.final_optical_state.iter().enumerate() {
        println!("    Oscillator {}: magnitude = {:.6}, phase = {:.3} rad", 
                 i, amplitude.magnitude(), amplitude.phase());
    }
    ...
    if results.energy_history.len() >= 10 {
        ...
        for (i, &energy) in results.energy_history.iter().enumerate() {
            if i % (results.energy_history.len() / 5).max(1) == 0 || i == results.energy_history.len() - 1 {
...
fn topology_comparison_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let topologies = vec![
        ...
        ("2×2 Lattice", NetworkTopology::Lattice2D { width: 2, height: 2 }),
        ("Random (70%)", NetworkTopology::Random { connectivity: 0.7 }),
        ("Small World", NetworkTopology::SmallWorld { 
            ring_connectivity: 2, 
...
fn pump_schedule_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let schedules = vec![
        ("Linear", PumpSchedule::Linear {
            initial_power: 0.3,
            final_power: 2.0,
        }),
        ("Exponential", PumpSchedule::Exponential {
            initial_power: 0.3,
            final_power: 2.0,
            time_constant: 3.0,
        }),
        ("Sigmoid", PumpSchedule::Sigmoid {
            initial_power: 0.3,
            final_power: 2.0,
            steepness: 5.0,
            midpoint: 0.6,
    ...
    for (schedule_name, schedule) in schedules {
        ...
        println!("    Convergence time: {:.3}", results.convergence_time);
        println!("    Runtime: {:.2?}", runtime);
        println!("    Final power efficiency: {:.1}%", 
    ...
    for (noise_name, noise_config) in noise_levels {
        ...
        if results.energy_history.len() > 10 {
            ...
            let energy_std = {
                ...
                let variance = last_10_energies.iter()
                    .map(|&e| (e - mean).powi(2))
                    .sum::<f64>() / 10.0;
                variance.sqrt()
            };
...
fn large_scale_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("  Problem: {}-qubit random graph (~40% edge density)", problem_size);
    ...
    println!("    Average optical power: {:.6}", results.optical_stats.average_power);
    println!("    Power efficiency: {:.1}%", results.performance_metrics.power_efficiency * 100.0);
    println!("    Solution quality score: {:.3}", results.performance_metrics.solution_quality);
    println!("    Time to convergence: {:.1}%", results.performance_metrics.time_to_convergence * 100.0);
    ...
    println!("    Average cross-correlation: {:.3}", avg_cross_correlation);
...
fn convergence_analysis_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let convergence_configs = vec![
        ("Tight Convergence", ConvergenceConfig {
            energy_tolerance: 1e-8,
            stagnation_time: 0.5,
            oscillation_threshold: 0.05,
            phase_stability: 0.005,
        }),
        ("Standard Convergence", ConvergenceConfig {
            energy_tolerance: 1e-6,
            stagnation_time: 1.0,
            oscillation_threshold: 0.1,
            phase_stability: 0.01,
        }),
        ("Relaxed Convergence", ConvergenceConfig {
    ...
    for (config_name, conv_config) in convergence_configs {
        ...
        if results.energy_history.len() > 5 {
            ...
            let oscillation_count = final_5_energies.windows(2)
                .filter(|pair| (pair[1] - pair[0]).abs() > 1e-6)
                .count();
            println!("    Energy oscillations in final 5 steps: {}", oscillation_count);
        }
        
        // Phase stability analysis
        let final_optical_state = &results.final_optical_state;
        let phase_stability = final_optical_state.iter()
...
fn custom_pump_schedule_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (schedule_name, schedule) in custom_schedules {
        ...
        if results.energy_history.len() > 10 {
            ...
            println!("    Second half improvement: {:.6}", second_half_improvement);
