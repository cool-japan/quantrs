...
use quantrs2_anneal::{
    ...
    qubo::QuboBuilder,
    visualization::{
        LandscapeAnalyzer, BasinAnalyzer, calculate_landscape_stats, 
        plot_energy_landscape, plot_energy_histogram
    },
};
use std::fs;
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    analyze_and_visualize(&frustrated_model, "frustrated_triangle", "Frustrated Triangle")?;
...
fn analyze_and_visualize(
    ...
    println!("  Energy range: [{:.4}, {:.4}]", stats.min_energy, stats.max_energy);
    println!("  Mean energy: {:.4} ± {:.4}", stats.mean_energy, stats.energy_std);
    println!("  Ground state degeneracy: {}", stats.ground_state_degeneracy);
...
fn compare_landscapes() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (name, model) in problems {
        ...
        println!("{:<15} {:<10.3} {:<10.3} {:<10.3} {:<10.3} {:<8.3}",
                 name,
                 stats.min_energy,
                 stats.max_energy,
                 stats.mean_energy,
