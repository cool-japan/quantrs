...
use quantrs2_anneal::{
    qubo::QuboBuilder,
    qubo_decomposition::{QuboDecomposer, DecompositionConfig, DecompositionStrategy},
...
fn block_decomposition_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("  Average sub-problem size: {:.1}", result.stats.avg_subproblem_size);
    println!("  Sub-problem size range: [{}, {}]", 
    ...
    println!("  Decomposition time: {:.2?}", result.stats.decomposition_time);
    println!("  Sub-problem solving time: {:.2?}", result.stats.solving_time);
    println!("  Refinement iterations: {}", result.stats.refinement_iterations);
...
fn spectral_decomposition_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("  Best objective value: {:.4}", result.objective_value);
    println!("  Total solving time: {:.2?}", solving_time);
    println!("  Convergence: {}", if result.stats.converged { "Yes" } else { "No" });
    
    // Analyze sub-problem balance
    let sizes: Vec<usize> = result.sub_solutions.iter()
...
fn hierarchical_decomposition_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("  Problem size: {} variables", qubo.num_variables);
    println!("  Number of leaf sub-problems: {}", result.stats.num_subproblems);
    println!("  Average sub-problem size: {:.1}", result.stats.avg_subproblem_size);
    println!("  Best objective value: {:.4}", result.objective_value);
    println!("  Total solving time: {:.2?}", solving_time);
...
fn clustering_decomposition_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("  Problem size: {} variables", qubo.num_variables);
    println!("  Number of clusters found: {}", result.stats.num_subproblems);
    println!("  Best objective value: {:.4}", result.objective_value);
    println!("  Total solving time: {:.2?}", solving_time);
...
fn portfolio_optimization_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let selected_assets: Vec<usize> = result.variable_values.iter()
        .filter(|(_, &value)| value)
        .map(|(&idx, _)| idx)
        .collect();
    
    println!("  Selected assets: {:?}", &selected_assets[..std::cmp::min(10, selected_assets.len())]);
...
fn create_chain_qubo(n: usize) -> Result<quantrs2_anneal::ising::QuboModel, Box<dyn std::error::Error>> {
    let mut builder = QuboBuilder::new();
    
    // Create variables
    let vars: Vec<_> = (0..n).map(|i| builder.add_variable(format!("x{}", i)).unwrap()).collect();
...
fn create_grid_qubo(rows: usize, cols: usize) -> Result<quantrs2_anneal::ising::QuboModel, Box<dyn std::error::Error>> {
    let n = rows * cols;
    let mut builder = QuboBuilder::new();
...
fn create_hierarchical_qubo(n: usize) -> Result<quantrs2_anneal::ising::QuboModel, Box<dyn std::error::Error>> {
    let mut builder = QuboBuilder::new();
...
fn create_clustered_qubo(n: usize, num_clusters: usize) -> Result<quantrs2_anneal::ising::QuboModel, Box<dyn std::error::Error>> {
    let mut builder = QuboBuilder::new();
...
fn create_portfolio_qubo(n_assets: usize, target_count: usize) -> Result<quantrs2_anneal::ising::QuboModel, Box<dyn std::error::Error>> {
    let mut builder = QuboBuilder::new();
