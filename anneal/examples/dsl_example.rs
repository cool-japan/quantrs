...
use quantrs2_anneal::{
    dsl::{OptimizationModel, Expression, patterns},
...
fn portfolio_optimization_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let high_risk_stocks: Vec<f64> = risks.iter()
        .map(|&r| if r > 0.25 { 1.0 } else { 0.0 })
        .collect();
    
    model.add_constraint(
        selection.weighted_sum(&high_risk_stocks).less_than_or_equal(1.0)
    )?;
    ...
    for (i, stock) in stocks.iter().enumerate() {
        if result.best_spins[i] == 1 {
            println!("    - {} (return: {:.1}%, risk: {:.1}%)", 
                     stock, expected_returns[i] * 100.0, risks[i] * 100.0);
        }
    }
    
    let total_return: f64 = stocks.iter().enumerate()
        .filter(|(i, _)| result.best_spins[*i] == 1)
        .map(|(i, _)| expected_returns[i])
        .sum();
    
    let total_risk: f64 = stocks.iter().enumerate()
        .filter(|(i, _)| result.best_spins[*i] == 1)
        .map(|(i, _)| risks[i])
        .sum::<f64>() / 3.0;
...
fn sudoku_solver_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for row in 0..4 {
        for col in 0..4 {
            model.add_constraint(
                Expression::Sum(
                    (0..4).map(|v| Expression::Variable(x[row][col].variables[v].clone()))
                        .collect()
                ).equals(1.0)
            )?;
        }
    ...
    // Compile and solve
    println!("Sudoku model created with {} qubits", model.summary().num_qubits);
...
fn nurse_scheduling_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for n in 0..nurses.len() {
        ...
        for d in 0..days {
            let day_shifts = model.add_binary_vector(
                format!("nurse_{}_day_{}", n, d),
                shifts.len()
            )?;
    ...
    for n in 0..nurses.len() {
        for d in 0..days {
            ...
            satisfaction = satisfaction.add(
                x[n][d].weighted_sum(shift_preferences)
            );
        }
    }
...
fn multi_objective_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    model.add_constraint(
        produce.weighted_sum(&resources).less_than_or_equal(resource_limit)
    )?;
    ...
    println!("    Total resources: {:.1}/{:.1} units", total_resources, resource_limit);
    
    println!("\n  Analysis:");
    println!("    Profit per emission: ${:.2}/unit", total_profit / total_emissions);
    println!("    Resource utilization: {:.1}%", total_resources / resource_limit * 100.0);
