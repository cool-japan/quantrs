...
fn xy_model_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("    Hamiltonian type: {:?}", xy_hamiltonian.hamiltonian_type);
    println!("    Has sign problem: {}", xy_hamiltonian.has_sign_problem);
    println!("    Number of coupling terms: {}", xy_hamiltonian.complex_couplings.len());
    
    // Analyze coupling structure
    println!("\n    Coupling structure:");
    for (i, coupling) in xy_hamiltonian.complex_couplings.iter().enumerate() {
        println!("      Coupling {}: sites ({}, {}), type {:?}, strength {:.3}",
                i + 1,
                coupling.sites.0,
                coupling.sites.1,
    ...
    if num_qubits <= 4 {
        ...
        println!("      Hermiticity check: max error = {:.2e}", max_hermiticity_error);
        
        // Sample matrix elements
        println!("      Sample matrix elements:");
        for i in 0..matrix.len().min(4) {
            let elements: Vec<String> = matrix[i].iter().take(4)
                .map(|z| format!("{:.2}+{:.2}i", z.re, z.im))
                .collect();
            println!("        Row {}: [{}{}]", i, elements.join(", "), 
...
fn xyz_model_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for coupling in &xyz_hamiltonian.complex_couplings {
        *interaction_counts.entry(format!("{:?}", coupling.interaction_type))
            .or_insert(0) += 1;
    }
    ...
    for (name, jx, jy, jz) in test_cases {
        ...
        println!("      {}: sign problem = {:.2}, stoquastic = {}", 
...
fn sign_problem_analysis_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let models = vec![
        ("Pure Ising (Z-Z)", HamiltonianType::XYZModel { j_x: 0.0, j_y: 0.0, j_z: 1.0 }),
        ("XY Ferromagnetic", HamiltonianType::XYModel { j_x: 1.0, j_y: 1.0 }),
        ("XY Antiferromagnetic", HamiltonianType::XYModel { j_x: -1.0, j_y: -1.0 }),
        ("Isotropic Heisenberg", HamiltonianType::XYZModel { j_x: 1.0, j_y: 1.0, j_z: 1.0 }),
        ("XXZ Easy-axis", HamiltonianType::XYZModel { j_x: 1.0, j_y: 1.0, j_z: 2.0 }),
        ("XXZ Easy-plane", HamiltonianType::XYZModel { j_x: 1.0, j_y: 1.0, j_z: 0.5 }),
    ...
    for &delta in &delta_values {
        ...
        println!("      δ = {:.1}: sign problem severity = {:.3}", delta, sign_severity);
    }
    ...
    for &size in &[3, 4, 6, 8, 10] {
        ...
        println!("      N = {:2}: {} couplings, sign severity = {:.3}", 
...
fn quantum_monte_carlo_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let strategies = vec![
        ...
        ("Population Annealing", SignMitigationStrategy::PopulationAnnealing),
    ...
    for (name, strategy) in strategies {
        ...
        println!("    │ {:16} │ {:11.4} │ {:12.3} │ {:11.2?} │ {:11} │",
                name,
                result.ground_state_energy.re,
                result.average_sign.norm(),
                runtime,
                if result.convergence_info.converged { "Yes" } else { "No" });
    }
    println!("    └──────────────────┴─────────────┴──────────────┴─────────────┴─────────────┘");
    ...
    println!("      Ground state energy: {:.6} + {:.6}i", 
             detailed_result.ground_state_energy.re, 
             detailed_result.ground_state_energy.im);
    println!("      Energy variance: {:.6}", detailed_result.energy_variance);
    println!("      Average sign magnitude: {:.4}", detailed_result.average_sign.norm());
    println!("      Sign problem severity: {:.3}", detailed_result.sign_problem_severity);
...
fn stoquastic_comparison_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("    ├─────────────────────┼─────────────────┼─────────────────┤");
    println!("    │ Non-stoquastic QMC  │ {:15.4} │ {:15.2?} │", 
             ns_result.ground_state_energy.re, ns_runtime);
    println!("    │ Stoquastic approx   │ {:15.4} │ {:15.2?} │", 
    ...
    println!("      Sign problem severity: {:.3}", ns_result.sign_problem_severity);
    ...
    println!("      Stoquastic ground state:     {:?}", &classical_result.best_spins[..ns_result.ground_state.as_ref().map_or(classical_result.best_spins.len(), |gs| gs.len().min(classical_result.best_spins.len()))]);
    
    // Calculate overlap if possible
    if let Some(ref ns_gs) = ns_result.ground_state {
        let overlap = ns_gs.iter().zip(classical_result.best_spins.iter())
            .map(|(&a, &b)| if a == b { 1.0 } else { 0.0 })
            .sum::<f64>() / ns_gs.len() as f64;
...
fn complex_hamiltonian_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("    Creating complex Ising model with {} qubits", num_qubits);
    ...
    for (i, ((site1, site2), strength)) in complex_couplings.iter().enumerate() {
        ...
        println!("      Coupling {}: ({}, {}) → {:.2} + {:.2}i (|J| = {:.3})",
                i + 1, site1, site2, strength.re, strength.im, strength.norm());
    }
    ...
    println!("        Non-zero elements: {} ({:.1}%)", nnz_count, 100.0 * nnz_count as f64 / (dim * dim) as f64);
    ...
    for i in 0..4.min(dim) {
        let row_elements: Vec<String> = matrix[i].iter().take(4)
            .map(|z| {
                if z.norm() < 1e-10 {
                    "    0    ".to_string()
...
fn frustrated_systems_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut disordered_xy = NonStoquasticHamiltonian::new(4, HamiltonianType::XYModel { j_x: 1.0, j_y: 1.0 });
    ...
    for &j in &j_values {
        ...
        println!("      J = {:.1}: E = {:8.4}, |<sign>| = {:.3}, severity = {:.3}",
                j,
                result.ground_state_energy.re,
                result.average_sign.norm(),
