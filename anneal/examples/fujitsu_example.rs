...
use quantrs2_anneal::{
    ising::IsingModel,
    qubo::QuboBuilder,
    fujitsu::{FujitsuClient, FujitsuAnnealingParams, GuidanceConfig},
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("\n{}Example 2: Guidance Mode{}", "=".repeat(10), "=".repeat(10));
    ...
    println!("  Improvement: {:.4}", sim_result.best_energy - guided_result.best_energy);
    
    // Example 3: Solve a QUBO problem directly
    println!("\n{}Example 3: QUBO Problem{}", "=".repeat(10), "=".repeat(10));
    ...
    println!("  Assets selected: {}/{}", selected_count, qubo_solution.len());
...
fn create_maxcut_instance(n: usize) -> Result<IsingModel, Box<dyn std::error::Error>> {
    ...
    for i in 0..n {
        for j in (i+1)..n {
            if rand::random::<f64>() < 0.4 {  // 40% edge density
...
#[cfg(feature = "fujitsu")]
fn create_portfolio_optimization(n_assets: usize) -> Result<quantrs2_anneal::qubo::QuboFormulation, Box<dyn std::error::Error>> {
    ...
    let risks: Vec<f64> = (0..n_assets).map(|_| rand::random::<f64>() * 0.05).collect();
    ...
    for i in 0..n_assets {
        for j in (i+1)..n_assets {
            if rand::random::<f64>() < 0.2 {  // 20% correlation
