...
use quantrs2_anneal::{
    ising::IsingModel,
    problem_schedules::{ProblemSpecificScheduler, ProblemType, AdaptiveScheduleOptimizer},
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let problems = vec![
        ...
        ("Number Partitioning", create_number_partitioning_problem(10)?),
    ...
    for (name, model) in problems {
        ...
        println!("  Initial temperature: {:.2}", specific_params.initial_temperature);
        println!("  Initial transverse field: {:.2}", specific_params.initial_transverse_field);
...
fn create_maxcut_problem(n: usize) -> Result<IsingModel, Box<dyn std::error::Error>> {
    ...
    for i in 0..n {
        for j in (i+1)..n {
            if rand::random::<f64>() < 0.3 {  // 30% edge probability
...
fn create_number_partitioning_problem(n: usize) -> Result<IsingModel, Box<dyn std::error::Error>> {
    ...
    let vars: Result<Vec<_>, _> = (0..n).map(|i| builder.add_variable(format!("x_{}", i))).collect();
    let vars = vars?;
...
fn create_graph_coloring_problem(n: usize) -> Result<IsingModel, Box<dyn std::error::Error>> {
    ...
    for i in 0..n {
        for j in (i+1)..n {
            if rand::random::<f64>() < 0.4 {  // 40% edge probability
                edges.push((i, j));
            }
        }
    }
