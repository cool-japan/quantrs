...
use quantrs2_anneal::{
    continuous_variable::{
        ...
        ContinuousVariable, ContinuousConstraint, create_quadratic_problem,
    },
...
fn portfolio_optimization_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let sum_constraint = ContinuousConstraint::new("weights_sum".to_string(), sum_constraint_fn, 1000.0);
    ...
    let return_constraint = ContinuousConstraint::new("target_return".to_string(), return_constraint_fn, 1000.0);
    ...
    println!("    Portfolio risk (variance): {:.6}", result.objective_value);
    println!("    Portfolio volatility (std dev): {:.4}", result.objective_value.sqrt());
    println!("    Runtime: {:.2?}", runtime);
...
fn engineering_design_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let objective = Box::new(|vars: &HashMap<String, f64>| {
        ...
        0.6224 * x1 * x3 * x4 + 1.7781 * x2 * x3.powi(2) + 3.1661 * x1.powi(2) * x4 + 19.84 * x1.powi(2) * x3
    });
    
    let mut problem = ContinuousOptimizationProblem::new(objective);
    
    // Add design variables with engineering bounds
    let shell_thickness = ContinuousVariable::new("shell_thickness".to_string(), 0.0625, 6.1875, 10)?
    ...
    problem.add_constraint(ContinuousConstraint::new("pressure".to_string(), pressure_constraint, 100.0));
    ...
    problem.add_constraint(ContinuousConstraint::new("stress".to_string(), stress_constraint, 100.0));
    
    // Constraint 3: Volume constraint (minimum volume requirement)
    let volume_constraint = Box::new(|vars: &HashMap<String, f64>| {
        let x3 = vars["inner_radius"];
        let x4 = vars["length"];
        let volume = std::f64::consts::PI * x3.powi(2) * x4 + (4.0/3.0) * std::f64::consts::PI * x3.powi(3);
        1296000.0 - volume  // Minimum volume requirement
    });
    problem.add_constraint(ContinuousConstraint::new("volume".to_string(), volume_constraint, 1.0));
    ...
    println!("    Shell thickness: {:.4}", result.variable_values["shell_thickness"]);
    println!("    Head thickness: {:.4}", result.variable_values["head_thickness"]);
    println!("    Inner radius: {:.4}", result.variable_values["inner_radius"]);
...
fn precision_study_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for &precision in &precision_levels {
        ...
        println!("  {} bits: x = {:.6}, error = {:.6}, resolution = {:.6}, levels = {}",
                 precision,
