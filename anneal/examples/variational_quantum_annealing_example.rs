...
use quantrs2_anneal::{
    ...
    variational_quantum_annealing::{
        VariationalQuantumAnnealer, VqaConfig, AnsatzType, ClassicalOptimizer,
        EntanglingGateType, MixerType, QuantumGate, ParameterRef,
...
fn qaoa_inspired_vqa_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("  Parameters: {} (2 per layer)", results.optimal_parameters.len());
    ...
    // Performance metrics
    println!("\n  Performance metrics:");
    println!("    Function evaluations: {}", results.statistics.function_evaluations);
    println!("    Gradient evaluations: {}", results.statistics.gradient_evaluations);
    println!("    Average energy: {:.6}", results.statistics.average_energy);
    println!("    Energy variance: {:.6}", results.statistics.energy_variance);
...
fn hardware_efficient_vqa_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let param_magnitudes: Vec<f64> = results.optimal_parameters.iter().map(|&p| p.abs()).collect();
    let avg_param_magnitude = param_magnitudes.iter().sum::<f64>() / param_magnitudes.len() as f64;
    let max_param_magnitude = param_magnitudes.iter().cloned().fold(f64::NEG_INFINITY, f64::max);
    
    println!("    Average parameter magnitude: {:.6}", avg_param_magnitude);
    println!("    Maximum parameter magnitude: {:.6}", max_param_magnitude);
...
fn adiabatic_inspired_vqa_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("  Parameters: {} (s-parameters)", results.optimal_parameters.len());
    println!("  Best energy: {:.6}", results.best_energy);
    println!("  Best solution: {:?}", results.best_solution);
    println!("  Iterations: {}", results.iterations_completed);
    println!("  Converged: {}", results.converged);
    ...
    println!("    Final s: {:.6}", *results.optimal_parameters.last().unwrap());
    ...
    println!("    Ground state quality: {:.1}%", 
...
fn optimizer_comparison_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let optimizers = vec![
        ("Adam", ClassicalOptimizer::Adam {
            learning_rate: 0.01,
            beta1: 0.9,
            beta2: 0.999,
            epsilon: 1e-8,
        }),
        ("Gradient Descent", ClassicalOptimizer::GradientDescent {
            learning_rate: 0.05,
        }),
        ("RMSprop", ClassicalOptimizer::RMSprop {
            learning_rate: 0.01,
            decay_rate: 0.9,
            epsilon: 1e-8,
        }),
    ...
    for (optimizer_name, optimizer) in optimizers {
        ...
        println!("    Converged: {}", results.converged);
        println!("    Runtime: {:.2?}", runtime);
        println!("    Function evals: {}", results.statistics.function_evaluations);
        println!("    Gradient evals: {}", results.statistics.gradient_evaluations);
...
fn ansatz_depth_study_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for depth in depths {
        ...
        println!("    Significant parameters: {}/{}", significant_params, param_magnitudes.len());
        println!("    Parameter utilization: {:.1}%", utilization * 100.0);
        
        // Expressivity analysis
        let energy_range = if results.energy_history.len() > 1 {
            results.energy_history.iter().cloned().fold(f64::NEG_INFINITY, f64::max) -
            results.energy_history.iter().cloned().fold(f64::INFINITY, f64::min)
        } else {
            0.0
        };
...
fn custom_ansatz_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let custom_structure = vec![
        ...
        QuantumGate::RY { qubit: 0, angle: ParameterRef::new(0) },
        QuantumGate::RY { qubit: 1, angle: ParameterRef::new(1) },
        QuantumGate::RY { qubit: 2, angle: ParameterRef::new(2) },
        ...
        QuantumGate::RZ { qubit: 0, angle: ParameterRef::scaled(3, 0.5) },
        QuantumGate::RZ { qubit: 1, angle: ParameterRef::new(4) },
        QuantumGate::RZ { qubit: 2, angle: ParameterRef::scaled(5, 0.8) },
        
        // Final mixing layer
        QuantumGate::RX { qubit: 0, angle: ParameterRef::new(6) },
        QuantumGate::RX { qubit: 1, angle: ParameterRef::new(7) },
        QuantumGate::RX { qubit: 2, angle: ParameterRef::new(8) },
    ];
    ...
    println!("  Parameters: {} with scaling factors", results.optimal_parameters.len());
    println!("  Best energy: {:.6}", results.best_energy);
    println!("  Best solution: {:?}", results.best_solution);
    println!("  Iterations: {}", results.iterations_completed);
    println!("  Converged: {}", results.converged);
    ...
    println!("    Parameter range: [{:.6}, {:.6}]",
             results.optimal_parameters.iter().cloned().fold(f64::INFINITY, f64::min),
             results.optimal_parameters.iter().cloned().fold(f64::NEG_INFINITY, f64::max));
...
fn large_scale_optimization_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("  Problem: {}-qubit random graph (~30% edge density)", problem_size);
    println!("  Ansatz: QAOA-inspired (4 layers)");
    println!("  Parameters: {} (8 total: 4×2)", results.optimal_parameters.len());
    ...
    println!("    Runtime per iteration: {:.2?}", 
    ...
    println!("    Energy consistency: {}", 
             if (solution_energy - results.best_energy).abs() < 1e-3 { "✓" } else { "✗" });
    ...
    if results.energy_history.len() >= 20 {
        ...
        println!("    Convergence pattern: {}", 
                 if early_progress > late_progress * 5.0 { "Fast early" } else { "Steady" });
    }
