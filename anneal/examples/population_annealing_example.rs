...
use quantrs2_anneal::{
    ising::IsingModel,
    population_annealing::{PopulationAnnealingSimulator, PopulationAnnealingConfig, MpiConfig},
    simulator::{QuantumAnnealingSimulator, AnnealingParams, TemperatureSchedule},
};
...
fn basic_population_annealing() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("  Final ESS: {:.2}", result.ess_history.last().unwrap_or(&0.0));
    
    // Print energy evolution
    println!("\nEnergy evolution:");
    for (i, stats) in result.energy_history.iter().enumerate() {
        if i % 10 == 0 {
            println!("  Step {}: T={:.3}, E_min={:.3}, E_mean={:.3}, ESS={:.1}",
                     i, stats.temperature, stats.min_energy, stats.mean_energy, stats.effective_sample_size);
        }
    }
...
fn large_population_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let max_energy = final_energies.iter().copied().fold(f64::NEG_INFINITY, f64::max);
    ...
    println!("    Ground state copies: {}", 
             final_energies.iter().filter(|&&e| (e - min_energy).abs() < 1e-6).count());
...
fn comparison_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("  Final ESS: {:.2}", pop_result.ess_history.last().unwrap_or(&0.0));
    ...
    println!("Population annealing improvement: {:.4}", energy_improvement);
...
fn mpi_configuration_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("  Number of processes: {}", config.mpi_config.as_ref().unwrap().num_processes);
    println!("  Load balancing enabled: {}", config.mpi_config.as_ref().unwrap().load_balancing);
    println!("  Communication frequency: {} steps", config.mpi_config.as_ref().unwrap().communication_frequency);
    println!("  Population per process: {}", config.population_size / config.mpi_config.as_ref().unwrap().num_processes);
...
fn complex_landscape_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let min_ess = result.ess_history.iter().copied().fold(f64::INFINITY, f64::min);
    let max_ess = result.ess_history.iter().copied().fold(f64::NEG_INFINITY, f64::max);
    ...
    let low_ess_count = result.ess_history.iter().filter(|&&ess| ess < 150.0).count();
    println!("    Steps with ESS < 150: {}/{}", low_ess_count, result.ess_history.len());
    
    Ok(())
}

/// Create a random spin glass model
fn create_random_spin_glass(n: usize, density: f64) -> Result<IsingModel, Box<dyn std::error::Error>> {
