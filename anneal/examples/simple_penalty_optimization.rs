...
use quantrs2_anneal::{
    ...
    penalty_optimization::{PenaltyOptimizer, PenaltyConfig, ConstraintPenaltyOptimizer, Constraint, ConstraintType},
    ising::IsingModel,
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let stats = penalty_optimizer.optimize_ising_penalties(
        &mut ising,
        &embedding,
        &samples,
    )?;
    ...
    println!("   - Chain break rate: {:.2}%", stats.chain_break_rate * 100.0);
    ...
    let binary_samples: Vec<Vec<i8>> = samples.iter()
        .map(|s| s.iter().map(|&spin| if spin == 1 { 1 } else { 0 }).collect())
        .collect();
