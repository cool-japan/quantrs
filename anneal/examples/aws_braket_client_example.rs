...
use quantrs2_anneal::{
    ising::IsingModel,
    braket::{
        BraketClient, DeviceSelector, DeviceType, DeviceStatus, AdvancedAnnealingParams,
        CostTracker,
    },
};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match client.select_device(Some(&qpu_selector)) {
        Ok(device) => {
            println!("✓ Selected QPU device: {} ({})", device.device_name, device.provider_name);
        }
        Err(e) => {
    ...
    match client.select_device(Some(&simulator_selector)) {
        Ok(device) => {
            println!("✓ Selected simulator: {} ({})", device.device_name, device.provider_name);
        }
        Err(e) => {
    ...
    println!("Batch submission simulation (would submit {} problems):", batch_problems.len());
    for (i, _) in batch_problems.iter().enumerate() {
        println!("  Problem {}: 3-qubit triangle with coupling strength {:.1}",
                 i + 1, -1.0 - i as f64 * 0.2);
    }
