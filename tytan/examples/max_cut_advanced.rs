...
use quantrs2_tytan::{
    ...
    analysis::graph::{
        generate_graph, generate_complete_graph, generate_regular_graph,
        generate_grid_graph, analyze_graph
    },
    visualization::{
    ...
    optimization::{
        ...
        penalty::{PenaltyOptimizer, PenaltyConfig},
    },
    benchmark::{
...
fn qubo_to_matrix_format(qubo: &quantrs2_tytan::QuboModel) -> Result<(Array2<f64>, HashMap<String, usize>), Box<dyn std::error::Error>> {
...
fn analyze_cut_quality(
    ...
    let total_weight: f64 = weights.map(|w| w.iter().sum()).unwrap_or(edges.len() as f64);
...
fn run_max_cut_experiments() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (graph_name, edges, weights) in experiments {
        ...
        let n_nodes = edges.iter()
            .flat_map(|&(i, j)| vec![i, j])
            .max()
            .unwrap_or(0) + 1;
        
        let props = analyze_graph(n_nodes, &edges);
        println!("Nodes: {}, Edges: {}, Avg Degree: {:.2}, Density: {:.3}",
        ...
        let sa_metrics = runner.benchmark_sampler(
            "Simulated Annealing",
            sa_sampler,
            &qubo,
        )?;
        ...
        // GA sampler
        let ga_sampler = GaSampler::new();
        let ga_metrics = runner.benchmark_sampler(
            "Genetic Algorithm",
            ga_sampler,
            &qubo,
        )?;
        ...
        // Analyze results
        let best_sa = &sa_metrics.best_solution;
        let best_ga = &ga_metrics.best_solution;
        ...
        println!("  Time: {:.3}s", sa_metrics.total_time.as_secs_f64());
        ...
        println!("  Time: {:.3}s", ga_metrics.total_time.as_secs_f64());
...
/// Generate comparison report
fn generate_comparison_report(results: &[ExperimentResult]) -> Result<(), Box<dyn std::error::Error>> {
    use std::io::Write;
    ...
    // Summary table
    writeln!(&mut report, "| Graph | Nodes | Edges | Best Cut | Best Method | Time (s) |")?;
    writeln!(&mut report, "|-------|-------|-------|----------|-------------|----------|")?;
    
    for result in results {
        let (best_cut, best_method, best_time) = if result.sa_analysis.cut_value >= result.ga_analysis.cut_value {
            (result.sa_analysis.cut_value, "SA", result.sa_metrics.total_time.as_secs_f64())
        } else {
            (result.ga_analysis.cut_value, "GA", result.ga_metrics.total_time.as_secs_f64())
        };
        
        writeln!(&mut report, "| {} | {} | {} | {:.2} | {} | {:.3} |",
                 result.graph_name,
    ...
    for result in results {
        ...
        writeln!(&mut report, "- Average Degree: {:.2}", result.graph_props.avg_degree)?;
        writeln!(&mut report, "- Density: {:.3}", result.graph_props.density)?;
        writeln!(&mut report, "- Connected: {}", result.graph_props.is_connected)?;
        
        writeln!(&mut report, "\n**Simulated Annealing:**")?;
        writeln!(&mut report, "- Cut Value: {:.2}", result.sa_analysis.cut_value)?;
        writeln!(&mut report, "- Cut Ratio: {:.3}", result.sa_analysis.cut_ratio)?;
        writeln!(&mut report, "- Partition Balance: {:.3} ({}:{})", 
        ...
        writeln!(&mut report, "- Energy: {:.4}", result.sa_metrics.best_energy)?;
        writeln!(&mut report, "- Time: {:.3}s", result.sa_metrics.total_time.as_secs_f64())?;
        
        writeln!(&mut report, "\n**Genetic Algorithm:**")?;
        writeln!(&mut report, "- Cut Value: {:.2}", result.ga_analysis.cut_value)?;
        writeln!(&mut report, "- Cut Ratio: {:.3}", result.ga_analysis.cut_ratio)?;
        writeln!(&mut report, "- Partition Balance: {:.3} ({}:{})", 
        ...
        writeln!(&mut report, "- Energy: {:.4}", result.ga_metrics.best_energy)?;
        writeln!(&mut report, "- Time: {:.3}s", result.ga_metrics.total_time.as_secs_f64())?;
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("  Set sizes: {} vs {}", analysis.set_a_size, analysis.set_b_size);
    
    // Verify all edges
    println!("\nEdge breakdown:");
    for (idx, &(i, j)) in edges.iter().enumerate() {
        let in_cut = partition[i] != partition[j];
        println!("  Edge ({},{}) weight={:.1}: {}",
                 i, j, weights[idx],
