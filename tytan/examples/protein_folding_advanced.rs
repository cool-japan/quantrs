...
impl Protein {
    fn from_sequence(name: &str, seq: &str) -> Option<Self> {
        let sequence: Option<Vec<_>> = seq.chars()
...
fn create_folding_model(
    ...
    for i in 0..n {
        for (idx, &pos) in positions.iter().enumerate() {
            let var = model.add_variable(&format!("x_{}_{}_{}_{}",
                i, pos.0, pos.1, pos.2))?;
        ...
        for (idx1, &pos1) in positions.iter().enumerate() {
            for &(dx, dy, dz) in &neighbors {
                ...
                if let Some(idx2) = positions.iter().position(|&p| p == pos2) {
                    ...
                    let aux_var = model.add_variable(&format!("seq_{}_{}_to_{}",
                        i, idx1, idx2))?;
                    
                    // aux_var = 1 iff residue i at idx1 AND residue i+1 at idx2
                    // This requires: aux_var <= x_{i,idx1} and aux_var <= x_{i+1,idx2}
                    // and aux_var >= x_{i,idx1} + x_{i+1,idx2} - 1
                    model.add_constraint_le(
                        &format!("seq_aux1_{}_{}_{}",i, idx1, idx2),
                        aux_var.clone(),
                        position_vars[&(i, idx1)].clone(),
                    )?;
                    
                    model.add_constraint_le(
                        &format!("seq_aux2_{}_{}_{}",i, idx1, idx2),
                        aux_var.clone(),
                        position_vars[&(i+1, idx2)].clone(),
                    )?;
                    
                    let sum_expr = position_vars[&(i, idx1)].clone() + 
                                   position_vars[&(i+1, idx2)].clone() + 
                                   SimpleExpr::constant(-1.0);
                    model.add_constraint_ge(
                        &format!("seq_aux3_{}_{}_{}",i, idx1, idx2),
                        aux_var.clone(),
                        sum_expr,
                    )?;
    ...
    for i in 0..n {
        for j in i+2..n { // Skip adjacent residues
...
fn validate_conformation(
    ...
    for i in 0..n-1 {
        ...
        let is_neighbor = neighbors.iter().any(|&(dx, dy, dz)| {
            pos2 == (pos1.0 + dx, pos1.1 + dy, pos1.2 + dz)
        });
...
fn run_folding_experiment(
    ...
    println!("Sequence: {}", protein.sequence.iter()
        .map(|aa| match aa {
    ...
    sampler.set_parameters(HashMap::from([
        ("initial_temp".to_string(), 50.0),
        ("final_temp".to_string(), 0.001),
        ("num_sweeps".to_string(), 20000.0),
    ]));
    ...
    println!("\nResults:");
    println!("  Valid conformations: {} / {}", valid_conformations.len(), samples.len());
        ...
        for (i, &pos1) in conformation.iter().enumerate() {
            for (j, &pos2) in conformation.iter().enumerate().skip(i+2) {
                if protein.sequence[i] == AminoAcid::Hydrophobic &&
                   protein.sequence[j] == AminoAcid::Hydrophobic {
                    let neighbors = lattice.neighbors();
                    if neighbors.iter().any(|&(dx, dy, dz)| {
                        pos2 == (pos1.0 + dx, pos1.1 + dy, pos1.2 + dz)
                    }) {
                        hh_contacts += 1;
                    }
        ...
        for (i, &pos1) in conformation.iter().enumerate() {
            for (j, &pos2) in conformation.iter().enumerate() {
                if i != j {
                    ...
                    if dist_sq <= 2 { // Adjacent or diagonal
                        contact_map[[i, j]] = 1.0;
                    }
                }
            }
    ...
    if !valid_conformations.is_empty() {
        let energies: Vec<f64> = valid_conformations.iter()
            .map(|(_, e)| *e)
            .collect();
        ...
        let unique_energies: HashSet<_> = energies.iter()
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (name, seq) in &test_proteins[..3] { // First 3 sequences
    ...
    for (name, seq) in &[("Alpha", "HHHHPPPPHHHHPPPP"), 
                         ("Beta", "HPHPHPHPHPHPHPH")] {
        if let Some(protein) = Protein::from_sequence(name, seq) {
    ...
    if let Some(protein) = Protein::from_sequence("Kinetics", "HPHPPHHPH") {
        ...
        for run in 0..5 {
            ...
            let mut sampler = SASampler::new();
            sampler.set_parameters(HashMap::from([
                ("initial_temp".to_string(), 100.0),
                ("final_temp".to_string(), 0.001),
                ("num_sweeps".to_string(), 5000.0),
                ("seed".to_string(), run as f64),
            ]));
        ...
        for (i, pathway) in pathways.iter().enumerate() {
            ...
            println!("    Pathway {}: {} steps, E_initial = {:.1}, E_final = {:.1}",
                     i + 1, pathway.len(), initial_e, final_e);
        }
    }
