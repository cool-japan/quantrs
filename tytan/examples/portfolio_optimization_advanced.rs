...
fn generate_assets(n_assets: usize, seed: u64) -> (Vec<Asset>, Array2<f64>) {
    ...
    for i in 0..n_assets {
        for j in 0..n_assets {
            covariance[[i, j]] = correlation[[i, j]] * 
                                 assets[i].volatility * 
                                 assets[j].volatility;
        }
    }
...
fn create_portfolio_model(
    ...
    for i in 0..n_assets {
        ...
        // weight_sum = selection_var
        model.add_constraint_eq(
            &format!("weight_selection_{}", i),
            weight_sum,
            selection_vars[i].clone(),
        )?;
    }
    ...
    for i in 0..n_assets {
        for j in 1..=n_bins {
            ...
            total_weight = total_weight + 
                SimpleExpr::constant(weight_value) * weight_vars[&(i, j)].clone();
        }
    }
    
    // Add slack variables for weight constraint
    let weight_slack = model.add_variable("weight_slack")?;
    model.add_constraint_eq(
        "total_weight",
        total_weight + weight_slack.clone(),
        SimpleExpr::constant(1.0),
    )?;
    
    // Constraint: min/max number of assets
    if let Some(min_assets) = constraints.min_assets {
        let asset_count: SimpleExpr = selection_vars.iter()
            .map(|v| v.clone())
            .sum();
        model.add_constraint_ge(
            "min_assets",
            asset_count,
            SimpleExpr::constant(min_assets as f64),
        )?;
    }
    ...
    if let Some(max_assets) = constraints.max_assets {
        ...
        model.add_constraint_le(
            "max_assets",
            asset_count,
            SimpleExpr::constant(max_assets as f64),
        )?;
    }
    
    // Constraint: min/max weight per asset
    if constraints.min_weight.is_some() || constraints.max_weight.is_some() {
        for i in 0..n_assets {
            let mut asset_weight = SimpleExpr::constant(0.0);
            for j in 1..=n_bins {
                let weight_value = j as f64 / n_bins as f64;
            ...
            // If selected, weight must be within bounds
            if let Some(min_weight) = constraints.min_weight {
                // asset_weight >= min_weight * selection_var
                let min_expr = SimpleExpr::constant(min_weight) * selection_vars[i].clone();
                model.add_constraint_ge(
                    &format!("min_weight_{}", i),
                    asset_weight.clone(),
                    min_expr,
                )?;
            }
            
            if let Some(max_weight) = constraints.max_weight {
                // asset_weight <= max_weight * selection_var
                let max_expr = SimpleExpr::constant(max_weight) * selection_vars[i].clone();
                model.add_constraint_le(
                    &format!("max_weight_{}", i),
                    asset_weight,
                    max_expr,
                )?;
            }
    ...
    // Sector constraints
    for (sector, (min_pct, max_pct)) in &constraints.sector_limits {
        let mut sector_weight = SimpleExpr::constant(0.0);
        ...
        model.add_constraint_ge(
            &format!("sector_min_{}", sector),
            sector_weight.clone(),
            SimpleExpr::constant(*min_pct),
        )?;
        
        model.add_constraint_le(
            &format!("sector_max_{}", sector),
            sector_weight,
            SimpleExpr::constant(*max_pct),
        )?;
    }
    ...
    if let Some(current_weights) = &constraints.current_weights {
        for i in 0..n_assets {
            let current_w = current_weights.get(i).copied().unwrap_or(0.0);
            
            // Add auxiliary variables for absolute difference
            let buy_var = model.add_variable(&format!("buy_{}", i))?;
            let sell_var = model.add_variable(&format!("sell_{}", i))?;
            
            // new_weight - current_weight = buy - sell
            let mut new_weight = SimpleExpr::constant(0.0);
            for j in 1..=n_bins {
                let weight_value = j as f64 / n_bins as f64;
            ...
            model.add_constraint_eq(
                ...
                new_weight + SimpleExpr::constant(-current_w),
                buy_var.clone() + SimpleExpr::constant(-1.0) * sell_var.clone(),
            )?;
            
            // Add transaction costs
            objective = objective + 
                SimpleExpr::constant(constraints.transaction_cost) * buy_var + 
                SimpleExpr::constant(constraints.transaction_cost) * sell_var;
        }
    }
    
    // Add penalty for weight slack
    objective = objective + SimpleExpr::constant(1000.0) * weight_slack;
...
fn extract_portfolio(
    ...
    for i in 0..n_assets {
        for j in 1..=n_bins {
            ...
            if solution.assignments.get(&var_name).copied().unwrap_or(false) {
...
fn run_portfolio_optimization(
    ...
    for &risk_aversion in risk_aversions {
        ...
        // Create model
        let model = create_portfolio_model(
            assets,
            covariance,
            constraints,
            risk_aversion,
        ...
        // Parameter tuning
        let mut tuner = ParameterTuner::new(TuningConfig {
            max_evaluations: 20,
            initial_samples: 5,
            ..Default::default()
        });
        ...
        let cov_clone = covariance.clone();
        let n_assets = assets.len();
        let tuning_result = tuner.tune_sampler(
            ...
            move |samples| {
                ...
                for sample in samples {
                    let weights = extract_portfolio(sample, n_assets, n_bins);
                    let metrics = calculate_portfolio_metrics(&weights, &assets_clone, &cov_clone);
                    
                    if metrics.n_holdings > 0 {
                        best_sharpe = best_sharpe.max(metrics.sharpe_ratio);
                    }
                }
                
                -best_sharpe // Minimize negative Sharpe
            },
        )?;
        
        // Run with optimized parameters
        let mut sampler = SASampler::new();
        sampler.set_parameters(tuning_result.best_parameters);
...
fn plot_efficient_frontier(
    ...
    writeln!(&mut csv_data, "risk_aversion,return,volatility,sharpe_ratio,n_holdings")?;
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("\nAsset Universe:");
    println!("{:<10} {:<15} {:<10} {:<10}", "Symbol", "Sector", "Return", "Vol");
    println!("{:-<45}", "");
    ...
    let efficient_frontier = run_portfolio_optimization(
        &assets,
        &covariance,
        &basic_constraints,
        &risk_aversions,
    ...
    // Add sector limits
    sector_constraints.sector_limits.insert("Technology".to_string(), (0.10, 0.30));
    sector_constraints.sector_limits.insert("Finance".to_string(), (0.15, 0.25));
    sector_constraints.sector_limits.insert("Healthcare".to_string(), (0.10, 0.25));
    
    let sector_portfolios = run_portfolio_optimization(
        &assets,
        &covariance,
        &sector_constraints,
        &[1.0, 2.0, 5.0],
    ...
    let rebalanced = run_portfolio_optimization(
        &assets,
        &covariance,
    ...
    let all_portfolios: Vec<_> = efficient_frontier.iter()
        .chain(sector_portfolios.iter())
        .collect();
    
    let returns: Vec<f64> = all_portfolios.iter()
        .map(|(_, p)| p.expected_return)
        .collect();
    let volatilities: Vec<f64> = all_portfolios.iter()
        .map(|(_, p)| p.volatility)
        .collect();
    let sharpes: Vec<f64> = all_portfolios.iter()
        .map(|(_, p)| p.sharpe_ratio)
        .collect();
    
    println!("Return range: {:.1}% - {:.1}%",
             returns.iter().min_by(|a, b| a.partial_cmp(b).unwrap()).unwrap() * 100.0,
             returns.iter().max_by(|a, b| a.partial_cmp(b).unwrap()).unwrap() * 100.0);
    println!("Volatility range: {:.1}% - {:.1}%",
             volatilities.iter().min_by(|a, b| a.partial_cmp(b).unwrap()).unwrap() * 100.0,
             volatilities.iter().max_by(|a, b| a.partial_cmp(b).unwrap()).unwrap() * 100.0);
    println!("Sharpe ratio range: {:.3} - {:.3}",
             sharpes.iter().min_by(|a, b| a.partial_cmp(b).unwrap()).unwrap(),
             sharpes.iter().max_by(|a, b| a.partial_cmp(b).unwrap()).unwrap());
