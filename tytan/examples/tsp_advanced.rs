    ...
    fn distance_to(&self, other: &City, metric: DistanceMetric) -> f64 {
        match metric {
            ...
            DistanceMetric::Manhattan => {
                ...
                (self.latitude - other.latitude).abs()
            }
        }
    }
}
...
fn run_tsp_experiments() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let grid_cities: Vec<City> = (0..9).map(|i| {
        let row = i / 3;
        let col = i % 3;
        City::new(&format!("Grid_{}{}", row, col), row as f64 * 10.0, col as f64 * 10.0)
    }).collect();
...
fn solve_tsp(
    ...
    // Tune for TSP-specific objective
    ...
    let tuning_result = tuner.tune_sampler(
        ...
        move |samples| {
            ...
            let mut valid_tours = 0;
            
            for sample in samples {
                if let Ok(tour) = extract_tour(sample, n_cities) {
                    valid_tours += 1;
                    let length = calculate_tour_length(&tour, &distances_clone);
                    best_length = best_length.min(length);
                }
            }
            
            // Penalize invalid tours
            if valid_tours == 0 {
                1e6
            } else {
                best_length
    ...
    println!("Best parameters: {:?}", tuning_result.best_parameters);
    ...
    let samples = sampler.run_qubo(&qubo, 5000)?;
    ...
    if let Some(tour) = best_tour {
        ...
        let max_segment = segment_lengths.iter().fold(0.0, |a, &b| a.max(b));
