...
fn quick_benchmark_demo() -> Result<(), Box<dyn Error>> {
    ...
    println!("   - Peak memory: {} MB", metrics.memory.peak_memory / 1_048_576);
    
    let efficiency = metrics.calculate_efficiency();
    println!("   - Samples per second: {:.2}", efficiency.samples_per_second);
    println!("   - Memory efficiency: {:.4}", efficiency.memory_efficiency);
...
fn custom_benchmark_demo() -> Result<(), Box<dyn Error>> {
    ...
    // Display summary
    println!("\n   Benchmark Summary:");
    println!("   - Total benchmarks: {}", report.metadata.total_benchmarks);
    println!("   - Duration: {:?}", report.metadata.total_duration);
    println!("   - Best configuration: {}-{}", 
        report.summary.most_efficient_backend,
        report.summary.most_efficient_sampler
    );
