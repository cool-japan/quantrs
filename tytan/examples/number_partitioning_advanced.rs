...
use quantrs2_tytan::{
    ...
    visualization::{
        solution_analysis::{analyze_solution_distribution, DistributionConfig, ClusteringMethod},
        convergence::plot_convergence,
    },
    optimization::{
        penalty::{PenaltyOptimizer, PenaltyConfig, PenaltyFunction},
        constraints::{ConstraintBuilder, ConstraintType},
    },
};
...
fn create_two_way_partition_model(
    numbers: &[i32],
    ...
    for i in 0..n {
        partition_sum = partition_sum + 
            SimpleExpr::constant(numbers[i] as f64) * vars[i].clone();
    }
    ...
    for i in 0..n {
        for j in 0..n {
            ...
            if i == j {
                ...
                objective = objective + SimpleExpr::constant(coeff) * vars[i].clone() * vars[j].clone();
            }
        }
    }
...
fn create_k_way_partition_model(
    ...
    for p in 0..k {
        ...
        for i in 0..n {
            partition_sum = partition_sum + 
                SimpleExpr::constant(numbers[i] as f64) * vars[&(i, p)].clone();
        }
        ...
        for i in 0..n {
            for j in 0..n {
                ...
                if i == j {
                    ...
                    objective = objective + 
                        SimpleExpr::constant(coeff) * vars[&(i, p)].clone() * vars[&(j, p)].clone();
                }
            }
        }
        ...
        for i in 0..n {
            objective = objective + 
                SimpleExpr::constant(-2.0 * numbers[i] as f64 * target_sum) * vars[&(i, p)].clone();
        }
    }
...
fn run_partition_experiment(
    ...
    sa_sampler.set_parameters(HashMap::from([
    ...
    if let PartitioningType::TwoWayEqual = partition_type {
        ...
        match solve_partition_dp(numbers) {
            Some((dp_partition, target)) => {
                ...
                let sa_optimal = sa_analysis.partition_sums.iter()
                    .all(|&s| s == target || s == sa_analysis.total_sum - target);
                let ga_optimal = ga_analysis.partition_sums.iter()
    ...
    println!("  Mean energy: {:.4}", distribution_analysis.statistics.mean_energy);
    println!("  Energy std dev: {:.4}", distribution_analysis.statistics.std_energy);
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (n, vars, dev, time) in results {
        println!("{:<10} | {:>10} | {:>12.2} | {:>10.3}",
                 n, vars, dev, time.as_secs_f64());
    }
