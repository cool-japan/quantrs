...
use crate::sampler::{Sampler, SampleResult, SamplerError};
...
impl AdaptiveOptimizer {
    ...
    pub fn new() -> Self {
        Self {
            samplers: vec![
                ("SA".to_string(), Box::new(crate::sampler::SASampler::new(None))),
                ("GA".to_string(), Box::new(crate::sampler::GASampler::new(None))),
            ],
            performance_history: PerformanceHistory {
    ...
    pub fn optimize(
        ...
        if let Some(best) = best_result {
            ...
            self.update_best_solution(problem_id.clone(), &best, &algorithm, total_time.as_secs_f64());
    ...
    fn select_algorithm_and_params(
        ...
        match &self.strategy_selector.strategy {
            ...
            SelectionStrategy::ThompsonSampling => {
                self.thompson_sampling_select(features)
            }
            SelectionStrategy::UCB { c } => {
                self.ucb_select(features, *c)
            }
    ...
    fn check_convergence(&self, history: &[(f64, f64)]) -> bool {
        ...
        let best_recent = recent.iter().map(|(_, e)| e).min_by(|a, b| a.partial_cmp(b).unwrap()).unwrap();
        let best_overall = history.iter().map(|(_, e)| e).min_by(|a, b| a.partial_cmp(b).unwrap()).unwrap();
    ...
    fn record_run(&mut self, record: RunRecord) {
        ...
        perf.success_rate = (perf.success_rate * n + if record.success { 1.0 } else { 0.0 }) / (n + 1.0);
        perf.n_runs += 1;
    ...
    fn update_best_solution(
        ...
        let entry = self.performance_history.best_solutions
    ...
    fn thompson_sampling_select(&self, features: &ProblemFeatures) -> Result<(String, HashMap<String, f64>), String> {
        // Simplified Thompson sampling
        self.random_select()
    }
    
    fn ucb_select(&self, features: &ProblemFeatures, c: f64) -> Result<(String, HashMap<String, f64>), String> {
        // Simplified UCB
        self.select_by_heuristics(features)
    }
    ...
    fn greedy_select(&self, features: &ProblemFeatures) -> Result<(String, HashMap<String, f64>), String> {
...
impl FeatureExtractor for BasicFeatureExtractor {
    fn extract(&self, qubo: &Array2<f64>) -> HashMap<String, f64> {
        ...
        features.insert("mean".to_string(), values.iter().sum::<f64>() / values.len() as f64);
        features.insert("max".to_string(), values.iter().fold(f64::NEG_INFINITY, |a, &b| a.max(b)));
        features.insert("min".to_string(), values.iter().fold(f64::INFINITY, |a, &b| a.min(b)));
...
impl FeatureExtractor for StructureFeatureExtractor {
    fn extract(&self, qubo: &Array2<f64>) -> HashMap<String, f64> {
        ...
        features.insert("diagonal_dominance".to_string(), 
            if total_sum > 0.0 { diag_sum / total_sum } else { 0.0 });
        ...
        features.insert("symmetry".to_string(), symmetry_score / ((n * (n - 1)) / 2) as f64);
