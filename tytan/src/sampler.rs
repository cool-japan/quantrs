...
impl SASampler {
        ...
        D: ndarray::Dimension + 'static,
    {
        ...
        if matrix_or_tensor.ndim() == 2 {
            ...
            return self.run_hobo_tensor(matrix_or_tensor, var_map, shots);
        }
    }
        ...
        D: ndarray::Dimension + 'static,
    {
        ...
        #[cfg(feature = "parallel")]
        {
            ...
            // Run in parallel
            all_solutions = seeds.into_par_iter().map(|seed| {
        ...
        #[cfg(not(feature = "parallel"))]
        {
            for _ in 0..total_runs {
                ...
                for sweep in 0..sweeps {
                    ...
                    for _ in 0..n_vars {
                        // Select random bit to flip
                        let idx = rng.gen_range(0..n_vars);
                        ...
                        // Metropolis acceptance criterion
                        let accept = delta_e <= 0.0 ||
                            rng.gen_range(0.0..1.0) < (-delta_e / temp).exp();
...
impl Sampler for GASampler {
    ...
    fn run_qubo(&self,
        ...
        #[cfg(feature = "parallel")]
        let eval_energy = |indiv: &Vec<bool>| -> f64 {
            calculate_energy(indiv, matrix)
        };
...
impl ArminSampler {
    ...
    fn run_gpu_annealing(
        ...
        if n_vars > 2048 {
            if self.verbose {
                println!("Problem size too large for standard OpenCL kernel. Using chunked approach.");
            }
        ...
        if self.verbose {
            println!("Initializing GPU with {} variables and {} shots", n_vars, num_shots);
        }
        ...
        let sweeps = if n_vars < 100 { 1000 } else if n_vars < 500 { 2000 } else { 5000 };

        if self.verbose {
            println!("Running {} sweeps with temperature range [{}, {}]",
        ...
        if use_parallel_tempering {
            ...
            solutions_buffer.read(&mut solutions_data[0..(num_shots * n_vars)]).enq()?;
    ...
    fn run_gpu_annealing_chunked(
        ...
        if self.verbose {
            println!("Using chunked approach for large problem: {} variables", n_vars);
        }
        ...
        if self.verbose {
            println!("Processing in {} chunks of at most {} variables each",
        ...
        for chunk_idx in 0..num_chunks {
            ...
            if self.verbose {
                println!("Processing chunk {}/{}: variables {}..{}",
                        chunk_idx + 1, num_chunks, start_var, end_var - 1);
            }
            ...
            for (sol_idx, solution) in solutions.iter().enumerate() {
                ...
                // Update energy
                energies[sol_idx] = evaluate_qubo_energy(&solutions[sol_idx], h_vector, j_matrix, n_vars);
            }
        }

        // Sort solutions by energy
        let mut solution_pairs: Vec<(Vec<bool>, f64)> = solutions.into_iter()
    ...
    fn optimize_chunk(
        ...
        let h_vec_f32: Vec<f32> = h_vector.iter().map(|&x| x as f32).collect();
        let j_mat_f32: Vec<f32> = j_matrix.iter().map(|&x| x as f32).collect();
        let initial_u8: Vec<u8> = initial_state.iter().map(|&b| if b { 1 } else { 0 }).collect();
...
impl Sampler for ArminSampler {
            ...
            Ok(binary_solutions) => {
                ...
                let mut results: Vec<SampleResult> = solution_counts.into_iter()
                    .map(|(bin_solution, (energy, count))| {
                        // Convert to variable assignments
                        let assignments: HashMap<String, bool> = bin_solution.iter()
                            .enumerate()
                            .map(|(idx, &value)| {
                                let var_name = idx_to_var.get(&idx).unwrap().clone();
                                (var_name, value)
    ...
    fn run_hobo(&self,
        ...
        if tensor.ndim() > 3 {
            if self.verbose {
                println!("Higher-order ({}D) HOBO problem, using tensor decomposition approach",
                        tensor.ndim());
            }
        ...
        for _ in 0..shots {
            let solution = (0..n_vars).map(|_| rng.gen_bool(0.5)).collect();
...
impl ArminSampler {
    ...
    fn run_hobo_3rd_order(
        ...
        if self.verbose {
            println!("Processing 3rd-order HOBO problem with {} variables", n_vars);
        }
        ...
        if !cubic_terms.is_empty() {
            ...
            if self.verbose {
                println!("Reformulating {} cubic terms with auxiliary variables", cubic_terms.len());
            }
    ...
    fn run_hobo_high_order(
        ...
        if self.verbose {
            println!("Processing high-order ({}D) HOBO problem with {} variables",
                     tensor_order, n_vars);
        ...
        let temperatures: Vec<f64> = (0..n_replicas)
            .map(|i| {
                temp_max * (temp_min / temp_max).powf(i as f64 / (n_replicas - 1) as f64)
            })
            .collect();
        ...
        for sweep in 0..n_sweeps {
            // Update each replica
            for (replica_idx, (state, &temperature)) in replicas.iter_mut().zip(temperatures.iter()).enumerate() {
                // Perform n_vars steps for each replica
                for _ in 0..n_vars {
                    // Select a random bit to flip
                    let bit_idx = rng.gen_range(0..n_vars);
                    ...
                    let accept = delta_e <= 0.0 ||
                                 rng.gen_range(0.0..1.0) < (-delta_e / temperature).exp();
            ...
            if sweep % exchange_interval == 0 {
                for i in 0..n_replicas-1 {
                    ...
                    let delta = (1.0/temperatures[i] - 1.0/temperatures[i+1]) * (energies[i+1] - energies[i]);
        ...
        #[cfg(feature = "parallel")]
        let final_eval = |state: &Vec<bool>| -> f64 {
            evaluate_hobo_energy(state)
        };
        ...
        #[cfg(feature = "scirs")]
        {
            ...
            let evaluate_energy = |state: &[bool]| -> f64 {
                ...
                if n_vars <= 32 {
                    ...
                    let state_f32: Vec<f32> = state.iter().map(|&b| if b { 1.0 } else { 0.0 }).collect();
