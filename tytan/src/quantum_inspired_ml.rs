...
impl QuantumSVM {
    ...
    pub fn fit(
        ...
        let results = sampler.run_qubo(&(qubo, var_map.clone()), 100)
    ...
    pub fn predict(&self, x: &Array2<f64>) -> Result<Array1<f64>, String> {
        ...
        let alphas = self.alphas.as_ref()
            .ok_or("Model not trained")?;
        let sv_labels = self.sv_labels.as_ref()
            .ok_or("Model not trained")?;
        ...
        for i in 0..n_samples {
            ...
            for j in 0..support_vectors.shape()[0] {
                let kernel_val = self.kernel_function(
                    &x.row(i),
                    &support_vectors.row(j)
                )?;
    ...
    fn kernel_function(
        ...
        match &self.kernel {
            ...
            KernelType::Polynomial { degree, coef0 } => {
                Ok((x.dot(y) + coef0).powi(*degree as i32))
            }
...
impl QuantumBoltzmannMachine {
    ...
    pub fn train(
        ...
        for epoch in 0..epochs {
            ...
            // Update weights
            self.weights += &((pos_associations - neg_associations) * self.learning_rate / batch_size as f64);
            ...
            let reconstruction_error = ((data - &neg_visible).mapv(|x| x * x)).sum() / batch_size as f64;
    ...
    fn sample_hidden_given_visible(
        ...
        for i in 0..batch_size {
            ...
            let results = sampler.run_qubo(&(qubo, var_map), 1)
                .map_err(|e| format!("Sampling error: {:?}", e))?;
...
mod tests {
    ...
    fn test_quantum_clustering() {
        ...
        let clustering = QuantumClustering::new(2)
