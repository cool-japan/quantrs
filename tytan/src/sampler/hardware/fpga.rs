...
impl FPGASampler {
    ...
    pub fn new(config: FPGAConfig) -> Self {
        let max_problem_size = match &config.platform {
            FPGAPlatform::XilinxAlveo { model } => {
                match model.as_str() {
                    "U280" => 8192,
                    "U250" => 4096,
                    _ => 2048,
            ...
            FPGAPlatform::Custom { resources, .. } => {
                (resources.logic_elements / 100) as usize
            }
        };
    ...
    fn select_bitstream(&self) -> Result<String, SamplerError> {
        match &self.config.algorithm {
            FPGAAlgorithm::SimulatedBifurcation { .. } => {
                Ok("sbm_optimizer_v2.bit".to_string())
            }
            FPGAAlgorithm::DigitalAnnealing { .. } => {
                Ok("digital_annealing_v3.bit".to_string())
            }
            FPGAAlgorithm::MomentumAnnealing { .. } => {
                Ok("momentum_annealing_v1.bit".to_string())
            }
            FPGAAlgorithm::ParallelTempering { .. } => {
                Ok("parallel_tempering_v2.bit".to_string())
            }
            FPGAAlgorithm::Custom { name, .. } => {
                Ok(format!("{}_custom.bit", name))
            }
        }
    }
    ...
    fn execute_on_fpga(
        ...
        let results = match &self.config.algorithm {
            FPGAAlgorithm::SimulatedBifurcation { time_step, damping, pressure } => {
...
impl Sampler for FPGASampler {
    ...
    fn run_hobo(
        ...
        Err(SamplerError::NotImplemented("HOBO not supported by FPGA hardware".to_string()))
    }
