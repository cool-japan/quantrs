...
impl Default for HitachiConfig {
    fn default() -> Self {
        Self {
            ...
            hardware_version: HardwareVersion::Gen4 { king_graph_size: 512 },
        }
    }
}
...
impl HitachiCMOSSampler {
    ...
    fn create_king_graph_embedding(&self, qubo: &Array2<f64>) -> Result<KingGraphEmbedding, SamplerError> {
        let n = qubo.shape()[0];
        let king_size = match &self.config.hardware_version {
            HardwareVersion::Gen4 { king_graph_size } => *king_graph_size,
            HardwareVersion::Gen5 { king_graph_size, .. } => *king_graph_size,
        };
...
mod tests {
    ...
    fn test_hitachi_config() {
        let config = HitachiConfig::default();
