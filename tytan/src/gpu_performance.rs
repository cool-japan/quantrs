...
#[cfg(feature = "scirs")]
use crate::scirs_stub::scirs2_core::gpu::{GpuContext, DeviceInfo};
    ...
    fn generate_recommendations(&self, metrics: &GpuPerformanceMetrics) -> Vec<String> {
        ...
        if metrics.device_utilization < 0.7 {
            recommendations.push(
                "Low GPU utilization detected. Consider increasing batch size or workload.".to_string()
            );
        }
        ...
        if metrics.memory_bandwidth_util > 0.9 {
            recommendations.push(
                "High memory bandwidth usage. Consider memory access optimization or compression.".to_string()
            );
        }
        ...
        if metrics.cache_hit_rate < 0.8 {
            recommendations.push(
                "Low cache hit rate. Consider data locality optimizations.".to_string()
            );
        }
...
impl MemoryAccessAnalyzer {
    ...
    pub fn analyze_pattern(&mut self, addresses: &[usize], access_type: AccessType) {
        ...
        let (common_stride, frequency) = stride_counts.iter()
...
impl KernelFusionOptimizer {
    ...
    pub fn add_kernel(
        ...
        self.kernel_info.insert(name.to_string(), KernelInfo {
            compute_intensity,
            memory_required,
            fusable: true,
        });
    ...
    pub fn find_fusion_opportunities(&self) -> Vec<FusionOpportunity> {
        ...
        opportunities.sort_by(|a, b| {
            b.benefit_score.partial_cmp(&a.benefit_score).unwrap()
        });
    ...
    fn can_fuse(
        ...
        let direct_dep = deps2.contains(&kernel1.to_string()) || 
                        deps1.contains(&kernel2.to_string());
        
        // Check if both kernels are fusable
        let both_fusable = self.kernel_info.get(kernel1).map(|k| k.fusable).unwrap_or(false) &&
                          self.kernel_info.get(kernel2).map(|k| k.fusable).unwrap_or(false);
