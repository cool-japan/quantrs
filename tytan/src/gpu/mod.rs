...
pub fn gpu_solve_qubo(
    ...
    #[cfg(feature = "scirs")]
    {
        use scirs2_core::gpu::{GpuDevice, GpuArray};
        ...
        let gpu_states = device.random_array::<f32>((shots, n_vars))
            .map_err(|e| GpuError::MemoryTransfer(e.to_string()))?;
        
        // Binarize states (threshold at 0.5)
        let gpu_binary = device.binarize(&gpu_states, 0.5)
        ...
        // Compute energies
        let gpu_energies = device.qubo_energy(&gpu_binary, &gpu_matrix)
            .map_err(|e| GpuError::KernelExecution(e.to_string()))?;
        
        // Transfer results back to CPU
        let binary_states: Array<bool, Ix2> = gpu_binary.to_ndarray()
            .map_err(|e| GpuError::MemoryTransfer(e.to_string()))?;
        
        let energies: Array<f64, Ix2> = gpu_energies.to_ndarray()
            .map_err(|e| GpuError::MemoryTransfer(e.to_string()))?;
        ...
        for result in results {
            let entry = consolidated.entry(result.assignments.clone())
                .or_insert((result.energy, 0));
            entry.1 += 1;
        }
        
        // Convert back to SampleResults
        let mut final_results: Vec<SampleResult> = consolidated.into_iter()
            .map(|(assignments, (energy, occurrences))| {
                SampleResult {
                    assignments,
                    energy,
                    occurrences,
    ...
    #[cfg(not(feature = "scirs"))]
    {
        ...
        // Execute kernel
        let kernel = ocl_pq.kernel_builder("qubo_energy")
            .arg(&binary_buffer)
            .arg(&matrix_buffer)
            .arg(&energies_buffer)
            .arg(n_vars as i32)
        ...
        energies_buffer.read(&mut energies)
            .enq()
            .map_err(|e| GpuError::MemoryTransfer(e.to_string()))?;
...
pub fn gpu_solve_qubo(
    ...
    Err(GpuError::NotAvailable("GPU acceleration not enabled. Rebuild with '--features gpu_accelerated'".to_string()))
}
