...
use crate::scirs_stub::{
    scirs2_optimization::{Optimizer, OptimizationProblem},
...
impl PenaltyOptimizer {
    ...
    pub fn initialize_weights(&mut self, constraints: &[String]) {
        for constraint in constraints {
            self.constraint_weights.insert(
                constraint.clone(),
    ...
    pub fn optimize_penalties(
        ...
        while iteration < self.config.max_iterations && !converged {
            ...
            // Check convergence
            let max_violation = violations.values()
                .map(|v| v.abs())
                .fold(0.0, f64::max);
    ...
    fn evaluate_violations(
        ...
        for (constraint_name, constraint_expr) in model.get_constraints() {
            ...
            violations.insert(
                ...
                if count > 0 { total_violation / count as f64 } else { 0.0 },
            );
        }
        ...
        for (constraint_name, &violation) in violations {
            if let Some(weight) = self.constraint_weights.get_mut(constraint_name) {
                if violation.abs() > self.config.violation_tolerance {
                    ...
                    *weight = (*weight * self.config.adjustment_factor)
    ...
    fn update_weights_optimized(
        ...
        use crate::scirs_stub::scirs2_optimization::{ObjectiveFunction, Bounds};
        
        // Define optimization problem
        let constraint_names: Vec<_> = violations.keys().cloned().collect();
        let current_weights: Array1<f64> = constraint_names.iter()
            .map(|name| self.constraint_weights[name])
            .collect();
        
        // Objective: minimize total weighted violations
        let violations_vec: Array1<f64> = constraint_names.iter()
    ...
    fn calculate_objective(
        ...
        for (assignment, energy) in sample_results {
            ...
            for (constraint_name, constraint_expr) in model.get_constraints() {
                ...
                let weight = self.constraint_weights.get(constraint_name)
                    .copied()
                    .unwrap_or(1.0);
    ...
    fn calculate_satisfaction_rate(&self, violations: &HashMap<String, f64>) -> f64 {
        let satisfied = violations.values()
    ...
    pub fn export_config(&self) -> PenaltyExport {
        PenaltyExport {
            ...
            final_violations: self.violation_history.iter()
                .filter(|v| v.iteration == self.violation_history.iter()
                    .map(|h| h.iteration)
                    .max()
                    .unwrap_or(0))
...
pub fn analyze_penalty_landscape(
    config: &PenaltyConfig,
    violations: &[f64],
) -> PenaltyAnalysis {
    ...
    for &weight in weights.iter() {
        let penalty_values: Vec<f64> = violations.iter()
...
fn find_optimal_weight(
    ...
    for &weight in weights.iter() {
        let total_penalty: f64 = violations.iter()
            .map(|&v| calculate_penalty(v, weight, config.penalty_type))
            .sum();
...
fn calculate_sensitivity(violations: &[f64], config: &PenaltyConfig) -> f64 {
    ...
    let penalties: Vec<f64> = violations.iter()
        .map(|&v| calculate_penalty(v, weight, config.penalty_type))
        .collect();
    
    let delta = 0.01 * weight;
    let penalties_delta: Vec<f64> = violations.iter()
        .map(|&v| calculate_penalty(v, weight + delta, config.penalty_type))
        .collect();
    
    let derivatives: Vec<f64> = penalties.iter().zip(penalties_delta.iter())
