...
impl ConstraintHandler {
    ...
    pub fn add_integer_encoding(
        ...
        // Store encoding info
        self.encoding_cache.insert(base_name.clone(), EncodingInfo {
    ...
    pub fn generate_penalty_terms(
        ...
        for constraint in &self.constraints {
            let weight = penalty_weights.get(&constraint.name)
            ...
            let penalty_expr = match &constraint.constraint_type {
                ...
                ConstraintType::LessThanOrEqual { bound } => {
                    ...
                    if let Some(slack_var) = constraint.slack_variables.first() {
                        let expr_with_slack = constraint.expression.clone() 
                            + Variable::new(slack_var.clone()).into();
                ...
                ConstraintType::Range { lower, upper } => {
                    ...
                    let lower_penalty = self.generate_inequality_penalty(
                        &constraint.expression, *lower, false
                    )?;
                    let upper_penalty = self.generate_inequality_penalty(
                        &constraint.expression, *upper, true
                    )?;
