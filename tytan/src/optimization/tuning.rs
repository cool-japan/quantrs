...
use crate::scirs_stub::scirs2_optimization::bayesian::{
    BayesianOptimizer, AcquisitionFunction, GaussianProcess,
};
...
impl ParameterTuner {
    ...
    pub fn tune_sampler<S: Sampler>(
        ...
        let default_objective = self.evaluate_configuration(
            &default_params,
            &sampler_factory,
            model,
            &objective,
        ...
        for i in 0..self.config.initial_samples {
            ...
            let obj_value = self.evaluate_configuration(
                &params,
                &sampler_factory,
                model,
                &objective,
        ...
        let mut best_objective = self.evaluation_history.iter()
        ...
        // Analyze results
        let best_eval = self.evaluation_history.iter()
            .min_by(|a, b| a.objective_value.partial_cmp(&b.objective_value).unwrap())
            .unwrap();
        
        let convergence_history: Vec<f64> = self.evaluation_history.iter()
        ...
        Ok(TuningResult {
            ...
            improvement_over_default: (default_objective - best_eval.objective_value) / default_objective.abs(),
    ...
    fn get_next_parameters(&mut self) -> Result<HashMap<String, f64>, Box<dyn std::error::Error>> {
        #[cfg(feature = "scirs")]
        {
            ...
            let x_data: Vec<Array1<f64>> = self.evaluation_history.iter()
                .map(|e| {
                    Array1::from_vec(
                        self.parameter_bounds.iter()
            ...
            let y_data: Array1<f64> = self.evaluation_history.iter()
                .map(|e| e.objective_value)
                .collect();
    ...
    fn transform_from_unit(&self, unit_values: &Array1<f64>) -> HashMap<String, f64> {
        self.parameter_bounds.iter().enumerate()
            .map(|(i, b)| {
                let unit_val = unit_values[i].clamp(0.0, 1.0);
                let value = match b.scale {
                    ParameterScale::Linear => {
                        b.min + unit_val * (b.max - b.min)
                    }
                    ParameterScale::Logarithmic => {
    ...
    fn calculate_parameter_importance(&self) -> Result<HashMap<String, f64>, Box<dyn std::error::Error>> {
        #[cfg(feature = "scirs")]
        {
    ...
    fn calculate_importance_gp(&self) -> Result<HashMap<String, f64>, Box<dyn std::error::Error>> {
        ...
        Ok(self.parameter_bounds.iter()
            .map(|b| (b.name.clone(), 1.0 / self.parameter_bounds.len() as f64))
            .collect())
    }
    
    /// Calculate importance using correlation
    fn calculate_importance_correlation(&self) -> Result<HashMap<String, f64>, Box<dyn std::error::Error>> {
        let mut importance = HashMap::new();
        
        for bounds in &self.parameter_bounds {
            // Calculate correlation between parameter and objective
            let param_values: Vec<f64> = self.evaluation_history.iter()
                .map(|e| e.parameters[&bounds.name])
                .collect();
            
            let obj_values: Vec<f64> = self.evaluation_history.iter()
                .map(|e| e.objective_value)
                .collect();
...
#[cfg(feature = "scirs")]
impl From<AcquisitionType> for crate::scirs_stub::scirs2_optimization::bayesian::AcquisitionFunction {
