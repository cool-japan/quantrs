...
impl PortfolioOptimizer {
    ...
    pub fn build_qubo(&self, num_bits_per_asset: usize) -> Result<(Array2<f64>, HashMap<String, usize>), String> {
    ...
    fn add_return_constraint(
            ...
            for j in 0..n_assets {
                for k1 in 0..bits_per_asset {
                    for k2 in 0..bits_per_asset {
                        ...
                        qubo[[var_idx1, var_idx2]] += penalty * self.returns[i] * self.returns[j] * w1 * w2;
                    }
                }
            }
        }
        ...
        for i in 0..n_assets {
            for k in 0..bits_per_asset {
                let weight = (1 << k) as f64 / ((1 << bits_per_asset) - 1) as f64;
                let var_idx = i * bits_per_asset + k;
                qubo[[var_idx, var_idx]] -= 2.0 * penalty * target_return * self.returns[i] * weight;
            }
        }
            ...
            for j in 0..n_assets {
                for k1 in 0..bits_per_asset {
                    for k2 in 0..bits_per_asset {
                        ...
                        qubo[[var_idx1, var_idx2]] += risk_scale * self.covariance[[i, j]] * w1 * w2;
                    }
                }
            }
        }
...
impl RiskParityOptimizer {
    ...
    pub fn build_qubo(&self, num_bits_per_asset: usize) -> Result<(Array2<f64>, HashMap<String, usize>), String> {
        ...
        let targets = self.target_contributions.as_ref()
        ...
        for i in 0..p.shape()[0] {
            ...
            if !view_assets.is_empty() {
                ...
                for &asset in &view_assets {
                    posterior[asset] += confidence * (view_return - posterior[asset]) * p[[i, asset]];
                }
            }
        }
...
pub enum TransactionCostModel {
    ...
    /// Fixed + proportional
    FixedPlusProportional { fixed: f64, proportional: Array1<f64> },
    /// Custom function
    Custom,
}
        ...
        for trigger in &self.triggers {
            match trigger {
                RebalancingTrigger::Deviation { threshold } => {
                    let max_deviation = current_weights.iter()
