...
pub enum VRPVariant {
    ...
    /// Pickup and Delivery
    VRPPD { pickups: Vec<usize>, deliveries: Vec<usize> },
    /// VRP with Backhauls
...
impl VehicleRoutingOptimizer {
    ...
    fn add_cvrp_constraints(
        ...
        for j in 1..n_locations { // Skip depot
                            ...
                            if let Some(&idx) = var_map.get(&var) {
                                ...
                                if j < time_windows.len() {
                                    let earliest_arrival = if i < time_windows.len() {
                                        time_windows[i].start + time_windows[i].service_time + travel_time
                                    } else {
                                        travel_time
                                    };
        ...
        for v in 0..self.num_vehicles {
            ...
            loop {
                ...
                if let Some(next) = next_location {
                    ...
                    let arrival_time = route.arrival_times.last().unwrap() + self.distance_matrix[[current, next]];
            ...
            if route.path.len() > 1 {
                ...
                route.arrival_times.push(route.arrival_times.last().unwrap() + self.distance_matrix[[current, self.depot]]);
                routes.push(route);
            }
        }
...
pub enum AssignmentPolicy {
    ...
    Random,
    /// ABC classification
    ABC { a_locations: Vec<usize>, b_locations: Vec<usize>, c_locations: Vec<usize> },
    /// Dedicated storage
    Dedicated,
    /// Class-based storage
    ClassBased,
...
impl WarehouseOptimizer {
    ...
    pub fn optimize_picking(&self) -> Result<PickingPlan, String> {
        match &self.policies.picking {
            PickingPolicy::Batch { size } => {
                self.optimize_batch_picking(*size)
            }
    ...
    fn optimize_picking_route(&self, orders: &[Order]) -> Result<PickingRoute, String> {
        ...
        for (i, &loc) in pick_locations.iter().enumerate() {
            ...
            distances[[0, i+1]] = self.calculate_distance(
                (station.0, station.1, 0),
                loc_pos,
            );
        ...
        match self.layout.distance_type {
            ...
            DistanceType::Euclidean => {
                (((pos1.0 as f64 - pos2.0 as f64).powi(2) +
                  ...
                  (pos1.2 as f64 - pos2.2 as f64).powi(2))).sqrt()
            }
            _ => 0.0,
        }
    }
