...
use crate::{
    benchmark::{
        hardware::{HardwareBackend, CpuBackend},
        metrics::{BenchmarkMetrics, TimingMetrics, MemoryMetrics, QualityMetrics, UtilizationMetrics},
        analysis::PerformanceReport,
    },
    sampler::{SASampler, GASampler},
};
...
impl BenchmarkRunner {
        ...
        for backend_name in &config.backends {
            match backend_name.as_str() {
                "cpu" => {
                    ...
                    let sampler = Box::new(SASampler::new(None)) as Box<dyn crate::sampler::Sampler + Send + Sync>;
                ...
                for sampler_config in &self.config.sampler_configs {
                    for backend_idx in 0..self.backends.len() {
                        ...
                        println!(
                            ...
                            completed + 1, total_benchmarks,
                            backend_name, sampler_config.name,
                            problem_size, density
                        );
    ...
    fn run_single_benchmark(
        ...
        for _ in 0..num_repetitions {
            ...
            // Run benchmark
            let results = backend.run_qubo(
                matrix,
                num_reads,
                sampler_config.params.clone(),
            )?;
    ...
    fn count_unique_solutions(results: &[crate::sampler::SampleResult]) -> usize {
        ...
        let unique: HashSet<Vec<bool>> = results.iter()
            .map(|r| {
...
pub fn quick_benchmark(problem_size: usize) -> Result<BenchmarkMetrics, Box<dyn std::error::Error>> {
