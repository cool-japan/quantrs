...
impl HardwareBackend for CpuBackend {
    ...
    fn run_qubo(
        ...
        Ok(self.sampler.run_qubo(&(matrix.clone(), var_map), num_reads)?)
    }
    ...
    fn get_metrics(&self) -> HashMap<String, f64> {
        ...
        #[cfg(feature = "scirs")]
        {
            metrics.insert("simd_enabled".to_string(), if self.simd_enabled { 1.0 } else { 0.0 });
        }
...
impl HardwareBackend for GpuBackend {
    ...
    fn initialize(&mut self) -> Result<(), Box<dyn std::error::Error>> {
        #[cfg(feature = "scirs")]
        {
            self.gpu_context = Some(crate::scirs_stub::scirs2_core::gpu::GpuContext::new(self.device_id)?);
        }
        Ok(())
    }
