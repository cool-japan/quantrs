...
use crate::benchmark::{
    metrics::{BenchmarkMetrics, aggregation, statistics},
...
impl PerformanceReport {
    ...
    fn calculate_summary(results: &[BenchmarkResult]) -> SummaryStatistics {
        let total_samples: usize = results.iter()
            .map(|r| r.metrics.quality.unique_solutions)
            .sum();
        
        let best_time_per_sample = results.iter()
            .map(|r| r.metrics.timings.time_per_sample)
            .min()
            .unwrap_or(std::time::Duration::ZERO);
        
        let best_energy_found = results.iter()
        ...
        for result in results {
            ...
            backend_efficiency.entry(result.backend_name.clone())
                .and_modify(|e| *e += efficiency.samples_per_second)
                .or_insert(efficiency.samples_per_second);
            
            sampler_efficiency.entry(result.sampler_name.clone())
                .and_modify(|e| *e += efficiency.samples_per_second)
                .or_insert(efficiency.samples_per_second);
        }
        
        let most_efficient_backend = backend_efficiency.iter()
            .max_by(|a, b| a.1.partial_cmp(b.1).unwrap())
            .map(|(k, _)| k.clone())
            .unwrap_or_default();
        
        let most_efficient_sampler = sampler_efficiency.iter()
            .max_by(|a, b| a.1.partial_cmp(b.1).unwrap())
            .map(|(k, _)| k.clone())
            .unwrap_or_default();
    ...
    fn analyze_backends(results: &[BenchmarkResult]) -> HashMap<String, BackendAnalysis> {
        ...
        for result in results {
            by_backend.entry(result.backend_name.clone())
        ...
        for (backend_name, backend_results) in by_backend {
            ...
            let avg_time_per_sample = backend_results.iter()
                .map(|r| r.metrics.timings.time_per_sample.as_secs_f64())
                .sum::<f64>() / backend_results.len() as f64;
            
            let avg_memory_usage = backend_results.iter()
                .map(|r| r.metrics.memory.peak_memory)
                .sum::<usize>() / backend_results.len();
            
            // Find best problem size
            let mut size_performance: HashMap<usize, Vec<f64>> = HashMap::new();
            for result in &backend_results {
                let efficiency = result.metrics.calculate_efficiency();
                size_performance.entry(result.problem_size)
                    .or_insert_with(Vec::new)
                    .push(efficiency.samples_per_second);
            }
            
            let best_problem_size = size_performance.iter()
                .map(|(size, perfs)| {
            ...
            let efficiency_by_size: HashMap<usize, f64> = size_performance.iter()
                .map(|(size, perfs)| {
            ...
            for result in &backend_results {
                ...
                let efficiency = result.metrics.calculate_efficiency();
                density_performance.entry(density_str)
                    .or_insert_with(Vec::new)
                    .push(efficiency.samples_per_second);
            }
            
            let efficiency_by_density: HashMap<String, f64> = density_performance.iter()
                .map(|(density, perfs)| {
                    let avg = perfs.iter().sum::<f64>() / perfs.len() as f64;
                    (density.clone(), avg)
                })
            ...
            analysis.insert(backend_name.clone(), BackendAnalysis {
                name: backend_name,
                num_benchmarks,
                success_rate,
    ...
    fn analyze_samplers(results: &[BenchmarkResult]) -> HashMap<String, SamplerAnalysis> {
        ...
        for result in results {
            by_sampler.entry(result.sampler_name.clone())
                .or_insert_with(Vec::new)
                .push(result);
        }
        ...
        for (sampler_name, sampler_results) in by_sampler {
            ...
            let avg_solution_quality = sampler_results.iter()
                .map(|r| r.metrics.quality.best_energy)
                .sum::<f64>() / sampler_results.len() as f64;
            
            // Simple convergence rate estimate
            let convergence_rate = sampler_results.iter()
            ...
            for result in &sampler_results {
                ...
                problem_type_performance.entry(problem_type.to_string())
                    .or_insert_with(Vec::new)
                    .push(result.metrics.quality.best_energy);
            }
            
            let performance_by_problem_type: HashMap<String, f64> = problem_type_performance.iter()
                .map(|(ptype, energies)| {
            ...
            analysis.insert(sampler_name.clone(), SamplerAnalysis {
                name: sampler_name,
                num_benchmarks,
    ...
    fn analyze_scaling(results: &[BenchmarkResult]) -> ScalingAnalysis {
        ...
        for result in results {
            let efficiency = result.metrics.calculate_efficiency();
            size_efficiencies.entry(result.problem_size)
        ...
        let optimal_problem_sizes: Vec<usize> = optimal_sizes.into_iter()
            .take(3)
            .map(|(size, _)| size)
            .collect();
    ...
    fn comparative_analysis(results: &[BenchmarkResult]) -> ComparativeAnalysis {
        ...
        for result in results {
            ...
            config_performance.entry(config.clone())
            ...
            efficiency_scores.entry(config)
                .and_modify(|e| *e += efficiency.samples_per_second)
                .or_insert(efficiency.samples_per_second);
        }
        ...
        for config1 in &configs {
            for config2 in &configs {
                if let (Some((perf1, _)), Some((perf2, _))) = 
                    (config_performance.get(config1), config_performance.get(config2)) {
        ...
        for (config, (performance, quality)) in config_performance {
            ...
            if !is_dominated {
                ...
                pareto_points.retain(|p| {
                    !(quality_score >= p.quality_score && performance_score >= p.performance_score &&
                      (quality_score > p.quality_score || performance_score > p.performance_score))
                });
    ...
    fn generate_recommendations(
        ...
        if scaling.time_complexity.order.contains("³") || scaling.time_complexity.order.contains("⁴") {
            recommendations.push(Recommendation {
        ...
        if summary.best_energy_found > -100.0 { // Arbitrary threshold
            recommendations.push(Recommendation {
        ...
        if scaling.weak_scaling_efficiency < 0.7 {
            recommendations.push(Recommendation {
                ...
                message: "Parallel efficiency is low - consider optimizing communication patterns".to_string(),
                impact: ImpactLevel::High,
                details: HashMap::from([
                    ("efficiency".to_string(), scaling.weak_scaling_efficiency.to_string()),
                ]),
            });
        }
...
impl PerformanceReport {
    pub fn generate_summary(&self) -> String {
        ...
        summary.push_str(&format!("- Best time per sample: {:?}\n", self.summary.best_time_per_sample));
        summary.push_str(&format!("- Best energy found: {:.4}\n", self.summary.best_energy_found));
        summary.push_str(&format!("- Most efficient backend: {}\n", self.summary.most_efficient_backend));
        summary.push_str(&format!("- Most efficient sampler: {}\n\n", self.summary.most_efficient_sampler));
