...
impl BenchmarkMetrics {
    ...
    fn calculate_energy_efficiency(&self) -> Option<f64> {
        self.utilization.power_consumption.map(|power| {
            power * self.timings.time_per_sample.as_secs_f64()
        })
    }
...
pub mod aggregation {
    ...
    pub fn aggregate_metrics(metrics: &[BenchmarkMetrics]) -> AggregatedMetrics {
        ...
        let total_times: Vec<f64> = metrics.iter()
            ...
            .collect();
        aggregated.avg_total_time = Duration::from_secs_f64(
            total_times.iter().sum::<f64>() / total_times.len() as f64
        );
        aggregated.min_total_time = Duration::from_secs_f64(
            total_times.iter().copied().fold(f64::INFINITY, f64::min)
        );
        aggregated.max_total_time = Duration::from_secs_f64(
            total_times.iter().copied().fold(0.0, f64::max)
        );
        
        // Quality aggregation
        aggregated.best_energy_overall = metrics.iter()
            .map(|m| m.quality.best_energy)
            .fold(f64::INFINITY, f64::min);
        aggregated.avg_success_rate = metrics.iter()
            .map(|m| m.quality.success_probability)
            .sum::<f64>() / metrics.len() as f64;
        
        // Memory aggregation
        aggregated.peak_memory_overall = metrics.iter()
