...
impl BenchmarkVisualizer {
    ...
    fn generate_scaling_plot(&self, output_dir: &str) -> Result<(), Box<dyn std::error::Error>> {
        #[cfg(feature = "scirs")]
        {
            use crate::scirs_stub::scirs2_plot::{Plot, Line, Scatter};
    ...
    fn generate_efficiency_heatmap(&self, output_dir: &str) -> Result<(), Box<dyn std::error::Error>> {
        #[cfg(feature = "scirs")]
        {
            use crate::scirs_stub::scirs2_plot::{Plot, Heatmap};
            ...
            for (i, backend) in backends.iter().enumerate() {
                for (j, sampler) in samplers.iter().enumerate() {
                    ...
                    if let Some(efficiency) = self.report.comparison.efficiency_ranking.iter()
                        .find(|(c, _)| c == &config)
                        .map(|(_, e)| *e) {
    ...
    fn generate_efficiency_csv(&self, output_dir: &str) -> Result<(), Box<dyn std::error::Error>> {
        ...
        for (i, (config, efficiency)) in self.report.comparison.efficiency_ranking.iter().enumerate() {
    ...
    fn generate_pareto_plot(&self, output_dir: &str) -> Result<(), Box<dyn std::error::Error>> {
        #[cfg(feature = "scirs")]
        {
            ...
            // All points
            let all_configs: Vec<String> = self.report.comparison.quality_comparison.keys()
                .cloned().collect();
            ...
            for config in &all_configs {
                if let Some(&quality) = self.report.comparison.quality_comparison.get(config) {
                    ...
                    // Find performance
                    let perf = self.report.comparison.efficiency_ranking.iter()
                        .find(|(c, _)| c == config)
                        .map(|(_, e)| *e)
                        .unwrap_or(0.0);
            ...
            // Pareto frontier
            let pareto_quality: Vec<f64> = self.report.comparison.pareto_frontier.iter()
                .map(|p| p.quality_score)
                .collect();
            let pareto_performance: Vec<f64> = self.report.comparison.pareto_frontier.iter()
                .map(|p| p.performance_score)
                .collect();
            let pareto_labels: Vec<String> = self.report.comparison.pareto_frontier.iter()
    ...
    fn generate_pareto_csv(&self, output_dir: &str) -> Result<(), Box<dyn std::error::Error>> {
        ...
        let mut csv_content = String::from("configuration,quality_score,performance_score,is_pareto\n");
        ...
        for (config, &quality) in &self.report.comparison.quality_comparison {
            ...
            let is_pareto = self.report.comparison.pareto_frontier.iter()
    ...
    fn generate_comparison_chart(&self, output_dir: &str) -> Result<(), Box<dyn std::error::Error>> {
        #[cfg(feature = "scirs")]
        {
            use crate::scirs_stub::scirs2_plot::{Plot, Bar};
            
            let mut plot = Plot::new();
            
            // Backend comparison
            let backend_names: Vec<String> = self.report.backend_analysis.keys()
                .cloned().collect();
            let backend_times: Vec<f64> = backend_names.iter()
                .map(|name| {
                    self.report.backend_analysis[name]
                        .avg_time_per_sample.as_secs_f64()
                })
                .collect();
            
            plot.add_trace(
                Bar::new(backend_names.clone(), backend_times)
