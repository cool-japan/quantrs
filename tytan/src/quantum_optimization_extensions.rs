...
use crate::hybrid_algorithms::{Hamiltonian, PauliTerm, ClassicalOptimizer};
...
impl AdaptQAOA {
    ...
    pub fn adapt_circuit(&mut self, hamiltonian: &Hamiltonian) -> Result<AdaptCircuit, String> {
        ...
        while circuit.depth() < self.max_depth && !converged {
            ...
            let (best_op_idx, max_gradient) = gradients.iter()
                .enumerate()
    ...
    fn estimate_commutator_norm(
        ...
        for h_term in &hamiltonian.terms {
            let commutes = self.pauli_strings_commute(
...
impl OperatorPool {
    ...
    fn get_operator(&self, idx: usize) -> Result<&PauliOperator, String> {
        let total_ops = self.single_qubit_ops.len() + self.two_qubit_ops.len() + self.custom_ops.len();
        ...
        for layer in 0..self.p {
            for qubit in 0..num_qubits {
                ...
                if idx + 1 < control_points.len() {
                    angles[layer][qubit] = control_points[idx] * (1.0 - frac) + control_points[idx + 1] * frac;
                } else {
...
mod tests {
    ...
    fn test_adapt_qaoa() {
        ...
        let adapt = AdaptQAOA::new(10, optimizer)
            .with_gradient_threshold(1e-4);
    ...
    fn test_qaoa_plus() {
        ...
        let qaoa_plus = QAOAPlus::new(3, optimizer)
            .with_mixers(vec![
                MixerOperator::StandardX,
                MixerOperator::XYMixer { coupling_strength: 0.5 },
                MixerOperator::StandardX,
            ]);
    ...
    fn test_multi_angle_qaoa() {
        let multi = MultiAngleQAOA::new(5)
            .with_parameterization(
                AngleParameterization::FourierSeries { num_frequencies: 3 }
            );
