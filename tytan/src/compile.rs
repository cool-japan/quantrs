...
impl Model {
    ...
    pub fn add_constraint_eq_one(&mut self, name: &str, variables: Vec<Expr>) -> CompileResult<()> {
        ...
        let sum_expr = variables.iter().fold(Expr::from(0), |acc, v| acc + v.clone());
    ...
    pub fn add_constraint_at_most_one(&mut self, name: &str, variables: Vec<Expr>) -> CompileResult<()> {
    ...
    pub fn add_constraint_implies_any(&mut self, name: &str, conditions: Vec<Expr>, result: Expr) -> CompileResult<()> {
                ...
                Constraint::AtMostOne { variables, .. } => {
                    ...
                    for i in 0..variables.len() {
                        for j in (i + 1)..variables.len() {
                            final_expr = final_expr + Expr::from(penalty_weight) * variables[i].clone() * variables[j].clone();
                        }
                    }
                }
                Constraint::ImpliesAny { conditions, result, .. } => {
                    // If any condition is true, result must be true
                    // Penalty: (max(conditions) - result)^2 where max is approximated by sum
                    let conditions_sum = conditions.iter().fold(Expr::from(0), |acc, c| acc + c.clone());
                    // Penalty when conditions_sum > 0 and result = 0
                    final_expr = final_expr + Expr::from(penalty_weight) * conditions_sum * (Expr::from(1) - result.clone());
                }
            }
        }
...
impl Model {
    ...
    /// Add constraint: exactly one of the variables must be 1
    pub fn add_constraint_eq_one(&mut self, name: &str, variables: Vec<SimpleExpr>) -> CompileResult<()> {
    ...
    /// Add constraint: at most one of the variables can be 1
    pub fn add_constraint_at_most_one(&mut self, name: &str, variables: Vec<SimpleExpr>) -> CompileResult<()> {
        self.constraints.push(Constraint::AtMostOne {
            name: name.to_string(),
            variables,
        });
    ...
    /// Add constraint: if any condition is true, then result must be true
    pub fn add_constraint_implies_any(&mut self, name: &str, conditions: Vec<SimpleExpr>, result: SimpleExpr) -> CompileResult<()> {
        self.constraints.push(Constraint::ImpliesAny {
            name: name.to_string(),
            conditions,
            result,
        ...
        for constraint in &self.constraints {
            match constraint {
                Constraint::Equality { expr, value, .. } => {
                    ...
                    self.add_expr_squared_to_qubo(expr, penalty_weight, &mut qubo_terms, &mut offset)?;
                    self.add_expr_to_qubo(expr, -2.0 * penalty_weight * value, &mut qubo_terms, &mut offset)?;
                ...
                Constraint::AtMostOne { variables, .. } => {
                    ...
                    for i in 0..variables.len() {
                        for j in (i + 1)..variables.len() {
                            if let (SimpleExpr::Var(vi), SimpleExpr::Var(vj)) = (&variables[i], &variables[j]) {
                                let key = if vi < vj { (vi.clone(), vj.clone()) } else { (vj.clone(), vi.clone()) };
                ...
                Constraint::ImpliesAny { conditions, result, .. } => {
                    ...
                    for cond in conditions {
                        if let (SimpleExpr::Var(c), SimpleExpr::Var(r)) = (cond, result) {
                            let key = if c < r { (c.clone(), r.clone()) } else { (r.clone(), c.clone()) };
                            *qubo_terms.entry(key).or_insert(0.0) -= penalty_weight;
                        }
                        // Also add linear term for condition
                        if let SimpleExpr::Var(c) = cond {
                            *qubo_terms.entry((c.clone(), c.clone())).or_insert(0.0) += penalty_weight;
                        }
                    }
                }
            }
    ...
    fn add_expr_to_qubo(&self, expr: &SimpleExpr, coeff: f64, terms: &mut HashMap<(String, String), f64>, offset: &mut f64) -> CompileResult<()> {
        match expr {
            SimpleExpr::Var(name) => {
            ...
            SimpleExpr::Mul(left, right) => {
                if let (SimpleExpr::Var(v1), SimpleExpr::Var(v2)) = (left.as_ref(), right.as_ref()) {
                    let key = if v1 < v2 { (v1.clone(), v2.clone()) } else { (v2.clone(), v1.clone()) };
                    *terms.entry(key).or_insert(0.0) += coeff;
                } else if let (SimpleExpr::Const(c), var) | (var, SimpleExpr::Const(c)) = (left.as_ref(), right.as_ref()) {
    ...
    fn add_expr_squared_to_qubo(&self, expr: &SimpleExpr, coeff: f64, terms: &mut HashMap<(String, String), f64>, offset: &mut f64) -> CompileResult<()> {
