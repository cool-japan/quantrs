...
impl QuantumBoltzmannMachine {
    ...
    pub fn generate_samples(&self, num_samples: usize) -> Vec<Array1<bool>> {
        ...
        for _ in 0..num_samples {
            ...
            let initial_visible = Array1::from_shape_fn(
                self.n_visible,
                |_| rng.gen_bool(0.5)
            );
...
impl QuantumVAE {
    ...
    fn quantum_encode(&self, input: &Array1<bool>) -> (Array1<f64>, Array1<f64>) {
        ...
        let log_var = state.slice(ndarray::s![self.latent_dim..2*self.latent_dim.min(self.input_dim)]).to_owned();
    ...
    fn classical_encode(&self, input: &Array1<bool>) -> (Array1<f64>, Array1<f64>) {
        ...
        let mean = Array1::from_vec(
            encoded.iter().take(self.latent_dim).cloned().collect()
        );
        let log_var = Array1::from_vec(
            encoded.iter().skip(self.latent_dim).take(self.latent_dim).cloned().collect()
        );
    ...
    pub fn train(&mut self, batch_size: usize) -> Result<f64, String> {
        ...
        for _ in 0..batch_size {
            ...
            let target = if experience.done {
                ...
                experience.reward + self.gamma * next_q_values.iter().cloned().fold(f64::NEG_INFINITY, f64::max)
            };
...
impl QuantumQNetwork {
    ...
    fn forward(&self, state: &Array1<f64>) -> Array1<f64> {
        ...
        quantum_state.slice_mut(ndarray::s![..self.input_dim]).assign(state);
        ...
        // Extract Q-values
        quantum_state.slice(ndarray::s![self.input_dim..]).to_owned()
    }
