    ...
    fn simulate_cim(
        ...
        for step in 0..steps {
            ...
            for i in 0..n {
                ...
                for j in 0..n {
                    if i != j {
                        ...
                        if self.use_feedback {
                            ...
                            let delayed_step = (step as f64 - self.feedback_delay / self.dt).max(0.0) as usize;
                ...
                let deterministic = (pump_term - 1.0) * amplitudes[i] 
                    - nonlinear_term 
                    + detuning_term 
                    + coupling_term;
        ...
        let spins: Vec<f64> = amplitudes.iter()
            .map(|amp| amp.re.signum())
            .collect();
    ...
    fn run_qubo(
        ...
        if n != self.n_spins {
            return Err(SamplerError::InvalidParameter(
                format!("CIM configured for {} spins but QUBO has {} variables", self.n_spins, n)
            ));
        }
        ...
        for _ in 0..shots {
            ...
            // Count occurrences
            let entry = solution_counts.entry(binary.clone()).or_insert((qubo_energy, 0));
            entry.1 += 1;
        }
...
impl AdvancedCIM {
    ...
    pub fn new(n_spins: usize) -> Self {
        Self {
            ...
            pulse_shape: PulseShape::Gaussian { width: 1.0, amplitude: 1.0 },
    ...
    fn apply_pulse_shaping(&self, t: f64) -> f64 {
        match &self.pulse_shape {
            ...
            PulseShape::Sech { width, amplitude } => {
                amplitude / (t / width).cosh()
            }
            PulseShape::Custom { .. } => {
    ...
    fn compute_bifurcation_param(&self, t: f64) -> f64 {
        ...
        match self.bifurcation_control.ramp_type {
            RampType::Linear => {
                initial + (final_param - initial) * progress
            }
...
impl NetworkedCIM {
    ...
    fn get_neighbors(&self, module_idx: usize) -> Vec<usize> {
        match &self.topology {
...
mod tests {
    ...
    fn test_advanced_cim() {
        let cim = AdvancedCIM::new(3)
            .with_pulse_shape(PulseShape::Gaussian { width: 1.0, amplitude: 1.5 })
            .with_num_rounds(2);
