...
impl<'a> Auto_array<'a> {
    ...
    /// - A vector of indices for each dimension
    pub fn get_dframe(&self, format: &str) -> AutoArrayResult<(Array<i32, ndarray::Ix2>, Vec<Vec<String>>)> {
        // Count the number of dimensions from format placeholders
        let dim_count = format.matches("{}").count();
    ...
    /// - A vector of indices for each dimension
    pub fn get_image(&self, format: &str) -> AutoArrayResult<(Array<u8, ndarray::Ix2>, Vec<Vec<String>>)> {
        // Count the number of dimensions from format placeholders
        let dim_count = format.matches("{}").count();
