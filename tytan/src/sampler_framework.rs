...
use crate::scirs_stub::{
    scirs2_optimization::bayesian::{BayesianOptimizer, AcquisitionFunction, KernelType},
...
impl HyperparameterOptimizer {
        ...
        F: Fn(&HashMap<String, f64>) -> Box<dyn Sampler>,
    {
        match &self.method {
            OptimizationMethod::RandomSearch => {
                self.random_search(objective, validation_problems)
            }
            OptimizationMethod::GridSearch { resolution } => {
                self.grid_search(objective, validation_problems, *resolution)
            }
            #[cfg(feature = "scirs")]
            OptimizationMethod::Bayesian { kernel, acquisition, exploration } => {
            ...
            OptimizationMethod::Evolutionary { population_size, mutation_rate } => {
        ...
        F: Fn(&HashMap<String, f64>) -> Box<dyn Sampler>,
    {
        ...
        // Find best
        let (best_idx, &best_score) = y_data.iter()
            .enumerate()
    ...
    #[cfg(feature = "scirs")]
    fn params_to_array(&self, params: &HashMap<String, f64>) -> Result<ndarray::Array1<f64>, String> {
    ...
    #[cfg(feature = "scirs")]
    fn array_to_params(&self, array: &ndarray::Array1<f64>) -> Result<HashMap<String, f64>, String> {
        let mut params = HashMap::new();
...
impl EnsembleSampler {
    ...
    fn voting_ensemble(
        ...
        for result in all_results {
            let state: Vec<bool> = qubo.1.iter()
        ...
        let mut final_results: Vec<SampleResult> = vote_counts.into_iter()
            .map(|(state, (energy, count))| {
                let assignments: HashMap<String, bool> = qubo.1.iter()
                    .zip(state.iter())
    ...
    fn weighted_voting_ensemble(
        ...
        let weights = self.weights.as_ref()
            .ok_or_else(|| SamplerError::InvalidParameter(
                "Weights not set for weighted voting".to_string()
            ))?;
    ...
    fn aggregate_results(
        ...
        for result in results {
            let state: Vec<bool> = var_map.iter()
                .map(|(var, _)| result.assignments[var])
                .collect();
            
            let entry = aggregated.entry(state)
        ...
        let mut final_results: Vec<SampleResult> = aggregated.into_iter()
            .map(|(state, (energy, count))| {
                let assignments: HashMap<String, bool> = var_map.iter()
                    .zip(state.iter())
                    .map(|((var, _), &val)| (var.clone(), val))
                    .collect();
...
impl<S: Sampler> AdaptiveSampler<S> {
    ...
    fn adapt_parameters(&mut self) -> HashMap<String, f64> {
        ...
        match &self.strategy {
            AdaptationStrategy::TemperatureAdaptive { initial_range, adaptation_rate } => {
                let mut params = HashMap::new();
                
                // Adapt temperature based on acceptance rate
                let (min_temp, max_temp) = initial_range;
                let temp = if history.improvements.len() > 10 {
                    let recent_improvements: f64 = history.improvements
                        .iter()
                        .rev()
                        .take(10)
                        .sum::<f64>() / 10.0;
        ...
        match &self.metric {
            ...
            EvaluationMetric::TimeToSolution(threshold) => {
                let found = results.iter()
                    .any(|r| r.energy <= *threshold);
                Ok(if found { elapsed.as_secs_f64() } else { f64::INFINITY })
            }
...
mod tests {
    ...
    fn test_hyperparameter_space() {
        ...
        optimizer.add_parameter(
            "temperature",
            ParameterSpace::Continuous { min: 0.1, max: 10.0, log_scale: true }
        );
        
        optimizer.add_parameter(
            "sweeps",
            ParameterSpace::Discrete { values: vec![100.0, 500.0, 1000.0] }
        );
