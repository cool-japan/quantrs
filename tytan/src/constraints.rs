...
impl ConstraintLibrary {
    ...
    pub fn sudoku() -> Vec<GlobalConstraint> {
        ...
        // Row constraints
        for row in 0..9 {
            let vars: Vec<String> = (0..9)
                .map(|col| format!("cell_{}_{}", row, col))
                .collect();
        ...
        for col in 0..9 {
            let vars: Vec<String> = (0..9)
                .map(|row| format!("cell_{}_{}", row, col))
                .collect();
            constraints.push(GlobalConstraint::AllDifferent { variables: vars });
        }
...
pub fn constraints_to_penalties(
    ...
    for constraint in constraints {
        match &constraint.constraint {
            ConstraintExpression::LinearInequality { coefficients, bound } => {
...
mod tests {
    ...
    fn test_alldifferent_propagation() {
        let mut propagator = AllDifferentPropagator::new(
