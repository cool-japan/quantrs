...
impl ParallelTemperingSampler {
    ...
    fn run_parallel_tempering(&mut self, matrix: &Array<f64, Ix2>, var_map: &HashMap<String, usize>, num_reads: usize) -> SamplerResult<Vec<SampleResult>> {
        let n = matrix.nrows();
        ...
        for _ in 0..num_reads {
            ...
            let mut chains: Vec<Vec<i32>> = (0..self.num_chains)
                .map(|_| (0..n).map(|_| if self.rng.gen::<f64>() < 0.5 { 0 } else { 1 }).collect())
                .collect();
            ...
            for _ in 0..self.sweeps {
                ...
                for i in 0..(self.num_chains - 1) {
                    if self.rng.gen::<f64>() < 0.1 { // 10% exchange probability
                        let energy_i = self.calculate_energy(&chains[i], matrix);
                        let energy_j = self.calculate_energy(&chains[i + 1], matrix);
                        
                        let delta = (energy_j - energy_i) * (1.0 / self.temperatures[i] - 1.0 / self.temperatures[i + 1]);
    ...
    fn metropolis_update(&mut self, chain: &mut Vec<i32>, matrix: &Array<f64, Ix2>, temperature: f64) {
        let n = chain.len();
...
impl Sampler for ParallelTemperingSampler {
    fn run_qubo(&self, qubo: &(Array<f64, Ix2>, HashMap<String, usize>), num_reads: usize) -> SamplerResult<Vec<SampleResult>> {
    ...
    fn run_hobo(&self, hobo: &(Array<f64, IxDyn>, HashMap<String, usize>), shots: usize) -> SamplerResult<Vec<SampleResult>> {
        ...
        if matrix_dyn.ndim() != 2 {
            return Err(SamplerError::InvalidParameter("HOBO matrix must be 2D for parallel tempering".into()));
        }
        
        let matrix_2d = matrix_dyn.clone().into_dimensionality::<Ix2>()
