...
impl VisualizationExporter {
    ...
    fn export_energy_csv(
        ...
        for i in 0..data.x_coords.len() {
            let density = data.density.as_ref()
                .and_then(|d| d.get(i))
                .copied()
                .unwrap_or(0.0);
    ...
    fn export_matplotlib_energy(
        ...
        writeln!(file, "scatter = plt.scatter(x, y, c=energies, cmap='viridis', s=50)")?;
    ...
    fn export_html_energy(
        ...
        writeln!(file, "    <ul>")?;
        writeln!(file, "        <li>Number of samples: {}</li>", data.x_coords.len())?;
        writeln!(file, "        <li>Min energy: {:.4}</li>", 
                 data.energies.iter().fold(f64::INFINITY, |a, &b| a.min(b)))?;
        writeln!(file, "        <li>Max energy: {:.4}</li>",
                 data.energies.iter().fold(f64::NEG_INFINITY, |a, &b| a.max(b)))?;
        writeln!(file, "        <li>Projection method: {}</li>", data.projection_method)?;
        writeln!(file, "    </ul>")?;
    ...
    fn export_plotly_distribution(&self, data: &DistributionData, path: &str) 
        -> Result<(), Box<dyn std::error::Error>> {
        // Implementation for distribution Plotly export
        Ok(())
    }
    
    fn export_matplotlib_distribution(&self, data: &DistributionData, path: &str) 
        -> Result<(), Box<dyn std::error::Error>> {
        // Implementation for distribution Matplotlib export
        Ok(())
    }
    
    fn export_html_distribution(&self, data: &DistributionData, path: &str) 
        -> Result<(), Box<dyn std::error::Error>> {
        // Implementation for distribution HTML export
        Ok(())
    }
    
    fn export_plotly_convergence(&self, data: &ConvergenceData, path: &str) 
        -> Result<(), Box<dyn std::error::Error>> {
        // Implementation for convergence Plotly export
        Ok(())
    }
    
    fn export_matplotlib_convergence(&self, data: &ConvergenceData, path: &str) 
        -> Result<(), Box<dyn std::error::Error>> {
        // Implementation for convergence Matplotlib export
        Ok(())
    }
    
    fn export_html_convergence(&self, data: &ConvergenceData, path: &str) 
        -> Result<(), Box<dyn std::error::Error>> {
...
pub fn export_visualization<T: Serialize>(
    ...
    match format {
        ...
        _ => {
            return Err(format!("Export format {:?} not implemented for generic data", format).into());
        }
    }
