...
use crate::scirs_stub::{
    scirs2_plot::{Plot2D, NetworkPlot, ColorMap},
...
impl ProblemVisualizer {
    ...
    pub fn visualize(&self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        match &self.problem_type {
            VisualizationType::TSP { coordinates, city_names } => {
                self.visualize_tsp(coordinates, city_names)?
            }
            VisualizationType::GraphColoring { adjacency_matrix, node_names, max_colors } => {
                self.visualize_graph_coloring(adjacency_matrix, node_names, *max_colors)?
            }
            VisualizationType::MaxCut { adjacency_matrix, node_names } => {
                self.visualize_max_cut(adjacency_matrix, node_names)?
            }
            VisualizationType::JobShop { n_jobs, n_machines, time_horizon } => {
            ...
            VisualizationType::Knapsack { weights, values, capacity } => {
                self.visualize_knapsack(weights, values, *capacity)?
            }
            VisualizationType::Portfolio { asset_names, expected_returns, risk_matrix } => {
                self.visualize_portfolio(asset_names, expected_returns, risk_matrix)?
            }
            VisualizationType::Custom { plot_function, metadata } => {
    ...
    fn visualize_tsp(
            ...
            for (idx, sample) in best_samples.iter().enumerate() {
                ...
                if let Some(names) = city_names {
                    for (i, name) in names.iter().enumerate() {
                        subplot.text(coordinates[i].0, coordinates[i].1, name)
                            .set_fontsize(8)
                            .set_ha("center");
                    }
                }
                
                subplot.set_title(&format!("Tour {}: Distance = {:.2}", idx + 1, sample.energy));
        ...
        #[cfg(not(feature = "scirs"))]
        {
            ...
            let export = TSPExport {
                ...
                best_tours: best_samples.iter()
                    .take(self.config.top_k)
                    .map(|s| self.extract_tsp_tour(s, n_cities))
                    .collect::<Result<Vec<_>, _>>()?,
                tour_lengths: best_samples.iter()
                    .take(self.config.top_k)
                    .map(|s| s.energy)
                    .collect(),
            };
    ...
    fn extract_tsp_tour(&self, sample: &SampleResult, n_cities: usize) 
    ...
    fn visualize_graph_coloring(
        ...
        #[cfg(feature = "scirs")]
        {
            ...
            ax.set_title(&format!("Graph Coloring: {} colors used", 
    ...
    fn extract_node_colors(&self, sample: &SampleResult, n_nodes: usize, max_colors: usize) 
        -> Result<Vec<usize>, Box<dyn std::error::Error>> {
    ...
    fn extract_partition(&self, sample: &SampleResult, n_nodes: usize) 
    ...
    fn visualize_number_partition(
        ...
        #[cfg(feature = "scirs")]
        {
            ...
            ax.set_ylabel("Sum");
            ax.set_title(&format!("Number Partition: |{:.2} - {:.2}| = {:.2}", 
    ...
    fn visualize_knapsack(
        ...
        for i in 0..n_items {
            let var_name = format!("x_{}", i);
            if best_sample.assignments.get(&var_name).copied().unwrap_or(false) {
                selected[i] = true;
        ...
        #[cfg(feature = "scirs")]
        {
            ...
            let colors: Vec<&str> = selected.iter()
                .map(|&s| if s { "green" } else { "red" })
                .collect();
            
            ax1.bar(&x_pos, values)
                .set_color(&colors)
                .set_alpha(0.7);
            ...
            ax1.set_title(&format!("Selected Items (Green): Value = {:.2}, Weight = {:.2}/{:.2}", 
    ...
    fn visualize_portfolio(
        ...
        let portfolio_variance: f64 = weights.iter().enumerate()
            .map(|(i, wi)| {
        ...
        #[cfg(feature = "scirs")]
        {
            ...
            let nonzero_weights: Vec<(String, f64)> = asset_names.iter()
                .zip(weights.iter())
                .filter(|(_, &w)| w > 0.01)
                .map(|(n, &w)| (n.clone(), w))
                .collect();
            ...
            if !nonzero_weights.is_empty() {
                let labels: Vec<String> = nonzero_weights.iter()
                    .map(|(n, _)| n.clone())
                    .collect();
                let sizes: Vec<f64> = nonzero_weights.iter()
                    .map(|(_, w)| *w)
                    .collect();
                
                ax1.pie(&sizes, &labels)
                    .set_autopct("%1.1f%%");
    ...
    fn extract_portfolio_weights(&self, sample: &SampleResult, n_assets: usize) 
    ...
    fn visualize_custom(
        ...
        println!("Custom visualization: {} with metadata: {:?}", plot_function, metadata);
        Ok(())
    }
    ...
    fn compute_graph_layout(&self, adjacency: &Array2<f64>) 
