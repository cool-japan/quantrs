...
use crate::optimization::{
    penalty::ConstraintViolation,
    tuning::TuningEvaluation,
...
#[cfg(feature = "scirs")]
use crate::scirs_stub::scirs2_plot::{Plot2D, MultiPlot, Annotation};
...
impl ConvergencePlot {
    ...
    pub fn analyze(&self) -> Result<ConvergenceAnalysis, Box<dyn std::error::Error>> {
        ...
        let best_objective = self.objective_history.iter()
        ...
        let total_time = self.time_history.last().copied()
    ...
    fn analyze_constraint_satisfaction(&self) -> Result<ConstraintSatisfaction, Box<dyn std::error::Error>> {
        let final_violations = self.constraint_history.last()
            .cloned()
            .unwrap_or_default();
        
        let satisfied_count = final_violations.values()
            .filter(|&&v| v.abs() < 1e-6)
            .count();
        ...
        let max_violation = final_violations.values()
            .map(|v| v.abs())
            .fold(0.0, f64::max);
    ...
    /// Analyze parameter stability
    fn analyze_parameter_stability(&self) -> Result<ParameterStability, Box<dyn std::error::Error>> {
        ...
        let final_values = self.parameter_history.last()
            .cloned()
            .unwrap_or_default();
        ...
        for (param_name, _) in &final_values {
            ...
            let values: Vec<f64> = self.parameter_history.iter()
    ...
    fn find_parameter_convergence_window(&self, values: &[f64]) 
        ...
        for start in 0..values.len() - window {
            ...
            let max_dev = window_values.iter()
                .map(|v| (v - mean).abs())
                .fold(0.0, f64::max);
    ...
    /// Calculate convergence metrics
    fn calculate_convergence_metrics(&self) -> Result<ConvergenceMetrics, Box<dyn std::error::Error>> {
    ...
    fn estimate_convergence_rate(&self, objectives: &[f64]) 
    ...
    fn calculate_oscillation_index(&self, objectives: &[f64]) 
        -> Result<f64, Box<dyn std::error::Error>> {
        if objectives.len() < 3 {
            return Ok(0.0);
        }
    ...
    /// Find plateau regions
    fn find_plateau_regions(&self, objectives: &[f64]) 
    ...
    fn identify_improvement_events(&self, objectives: &[f64]) 
    ...
    fn estimate_optimum(&self, objectives: &[f64]) 
    ...
    pub fn plot(&self) -> Result<(), Box<dyn std::error::Error>> {
        #[cfg(feature = "scirs")]
        {
            ...
            // Raw objectives
            subplot.plot(&iterations, &self.objective_history)
                .set_label("Objective")
                .set_alpha(0.5);
            ...
            if self.config.show_best {
                ...
                subplot.plot(&iterations, &best_so_far)
            ...
            if self.config.smoothing_window > 1 {
                ...
                subplot.plot(&iterations, &smoothed)
                    .set_label("Smoothed")
                    .set_linestyle("--");
            }
            ...
            if self.config.show_constraints && !self.constraint_history.is_empty() {
                ...
                let mut constraint_names: Vec<_> = self.constraint_history
                    .iter()
                    .flat_map(|c| c.keys())
                ...
                for name in constraint_names {
                    let violations: Vec<f64> = self.constraint_history.iter()
                        .map(|c| c.get(name).copied().unwrap_or(0.0).abs())
                        .collect();
                    
                    subplot.plot(&iterations, &violations)
                        .set_label(name);
                }
            ...
            if !self.parameter_history.is_empty() {
                ...
                for name in param_names {
                    ...
                    if max_val > min_val {
                        ...
                        subplot.plot(&iterations, &normalized)
                            .set_label(name);
                    }
                }
...
pub fn track_adaptive_convergence(
    ...
    for (i, (perf, params)) in performance_history.iter()
