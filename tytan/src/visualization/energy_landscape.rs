...
use crate::scirs_stub::{
    scirs2_plot::{Plot2D, Plot3D, Heatmap, ColorMap},
...
impl EnergyLandscape {
    ...
    pub fn energy_histogram(&self) -> Result<HistogramData, Box<dyn std::error::Error>> {
        let energies: Vec<f64> = self.samples.iter()
            .map(|s| s.energy)
            .collect();
    ...
    pub fn project_2d(&mut self) -> Result<ProjectedLandscape, Box<dyn std::error::Error>> {
        ...
        let x_coords: Vec<f64> = (0..projected.nrows())
            .map(|i| projected[[i, 0]])
            .collect();
        let y_coords: Vec<f64> = (0..projected.nrows())
            .map(|i| projected[[i, 1]])
            .collect();
        let energies: Vec<f64> = self.samples.iter()
            .map(|s| s.energy)
    ...
    /// PCA projection
    fn project_pca(&self, data: &Array2<f64>, n_components: usize) 
    ...
    /// Random projection
    fn project_random(&self, data: &Array2<f64>, n_components: usize) 
        -> Result<Array2<f64>, Box<dyn std::error::Error>> {
    ...
    fn project_hamming(&self, data: &Array2<f64>) 
        -> Result<Array2<f64>, Box<dyn std::error::Error>> {
        let n_samples = data.nrows();
        ...
        for i in 0..n_samples {
            let hamming_dist: f64 = data.row(i).iter()
...
pub fn plot_energy_landscape(
    ...
    #[cfg(feature = "scirs")]
    {
        ...
        subplot.scatter(&proj_data.x_coords, &proj_data.y_coords)
