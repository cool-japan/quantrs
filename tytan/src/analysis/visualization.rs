...
pub fn prepare_energy_landscape(
    ...
    if results.is_empty() {
        return Err(VisualizationError::DataError("No results to analyze".to_string()));
    }
    ...
    let correlations = if config.compute_correlations {
        ...
        for i in 0..n_vars {
            for j in (i+1)..n_vars {
                ...
                if corr.abs() > 0.01 {  // Only store non-negligible correlations
                    corr_map.insert(
                        (variable_names[i].clone(), variable_names[j].clone()),
                        corr
                    );
                }
            }
...
pub fn extract_tsp_tour(
    result: &SampleResult,
    n_cities: usize,
) -> VisualizationResult<Vec<usize>> {
...
pub fn analyze_convergence(
    ...
    if iteration_results.is_empty() {
        return Err(VisualizationError::DataError("No iteration data".to_string()));
    }
...
fn calculate_correlation_matrix(data: &Array2<f64>) -> VisualizationResult<Array2<f64>> {
    ...
    for i in 0..n_vars {
        for j in 0..n_vars {
            ...
            let (mean_i, std_i) = calculate_mean_std(col_i.as_slice().unwrap());
            let (mean_j, std_j) = calculate_mean_std(col_j.as_slice().unwrap());
            
            if std_i > 0.0 && std_j > 0.0 {
                let cov: f64 = col_i.iter()
                    .zip(col_j.iter())
                    .map(|(&x, &y)| (x - mean_i) * (y - mean_j))
                    .sum::<f64>() / data.nrows() as f64;
...
fn simple_pca(data: &Array2<f64>, n_components: usize) -> VisualizationResult<(Array2<f64>, Vec<f64>)> {
...
pub fn spring_layout(
    ...
    let scale_x = if max_x > min_x { 0.9 / (max_x - min_x) } else { 1.0 };
    let scale_y = if max_y > min_y { 0.9 / (max_y - min_y) } else { 1.0 };
    
    positions.iter()
        .map(|&(x, y)| (
            (x - min_x) * scale_x + 0.05,
            (y - min_y) * scale_y + 0.05
        ))
        .collect()
}
