...
pub fn cluster_solutions(
    ...
    if results.is_empty() {
        return Err(AnalysisError::DataProcessingError("Empty results list".to_string()));
    }
    
    // Extract all variable names
    let variable_names: Vec<String> = results[0].assignments.keys()
        .cloned()
        .collect();
    ...
    for (i, &label) in labels.iter().enumerate() {
        ...
        cluster_energies.entry(label).or_default().push(results[i].energy);
    }
...
pub fn cluster_solutions(
    ...
    let mut variable_names: Vec<String> = results[0].assignments.keys()
        .cloned()
        .collect();
    variable_names.sort();
    ...
    for (i, result) in results.iter().enumerate() {
        ...
        let binary: Vec<bool> = variable_names.iter()
            .map(|name| *result.assignments.get(name).unwrap_or(&false))
            .collect();
        
        groups.entry(binary.clone()).or_default().push(i);
        group_energies.entry(binary).or_default().push(result.energy);
    }
...
pub fn calculate_diversity(results: &[SampleResult]) -> AnalysisResult<HashMap<String, f64>> {
    ...
    if !energies.is_empty() {
        let min_energy = *energies.iter().min_by(|a, b| a.partial_cmp(b).unwrap()).unwrap_or(&0.0);
        let max_energy = *energies.iter().max_by(|a, b| a.partial_cmp(b).unwrap()).unwrap_or(&0.0);
...
pub fn visualize_energy_distribution(
    ...
    let root = BitMapBackend::new(file_path, (800, 600))
        .into_drawing_area();
    
    root.fill(&WHITE)
        .map_err(|e| AnalysisError::VisualizationError(e.to_string()))?;
    
    let min_energy = *energies.iter().min_by(|a, b| a.partial_cmp(b).unwrap()).unwrap();
