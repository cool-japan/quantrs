...
#[cfg(feature = "dwave")]
use crate::symbol::{Symbol, Expression};
...
impl EncodedVariable {
    ...
    fn generate_binary_vars(name: &str, scheme: &EncodingScheme) -> Vec<String> {
        match scheme {
            ...
            EncodingScheme::Unary { num_values } => {
                (0..*num_values - 1)
                    .map(|i| format!("{}_u{}", name, i))
                    .collect()
            }
            EncodingScheme::OrderEncoding { min_value, max_value } => {
                let range = max_value - min_value;
                (0..range)
                    .map(|i| format!("{}_ord{}", name, i))
                    .collect()
            }
    ...
    pub fn decode(&self, binary_values: &HashMap<String, bool>) -> Option<i32> {
        match &self.scheme {
            ...
            EncodingScheme::Direct => {
                binary_values.get(&self.name).map(|&b| if b { 1 } else { 0 })
            }
        }
    }
    ...
    pub fn get_penalty_matrix(&self, var_indices: &HashMap<String, usize>) -> Array2<f64> {
        ...
        match &self.scheme {
            EncodingScheme::OneHot { .. } => {
                ...
                let indices: Vec<usize> = self.binary_vars.iter()
...
impl EncodingOptimizer {
    ...
    pub fn add_variable(&mut self, name: &str, min_value: i32, max_value: i32) {
        self.domains.insert(name.to_string(), (min_value, max_value));
    }
    
    /// Add constraint between variables
    pub fn add_constraint(&mut self, var1: &str, var2: &str) {
        self.constraint_graph.entry(var1.to_string())
            .or_insert_with(Vec::new)
            .push(var2.to_string());
        self.constraint_graph.entry(var2.to_string())
            .or_insert_with(Vec::new)
            .push(var1.to_string());
    }
    ...
    pub fn optimize_encodings(&self) -> HashMap<String, EncodingScheme> {
        ...
        for (var, &(min_val, max_val)) in &self.domains {
            ...
            let neighbors = self.constraint_graph.get(var)
                .map(|v| v.len())
                .unwrap_or(0);
            ...
            let encoding = if domain_size == 2 {
                ...
                EncodingScheme::OneHot { num_values: domain_size }
            } else if domain_size <= 8 {
                // Medium domain: binary or gray code
                if self.has_ordering_constraints(var) {
                    EncodingScheme::GrayCode { num_values: domain_size }
                } else {
                    EncodingScheme::Binary { num_values: domain_size }
                }
            } else if self.has_ordering_constraints(var) {
                // Large ordered domain: order encoding or domain wall
                if domain_size <= 32 {
                    EncodingScheme::OrderEncoding { min_value: min_val, max_value: max_val }
                } else {
                    EncodingScheme::DomainWall { num_values: domain_size }
                }
            } else {
...
impl EncodingConverter {
    ...
    pub fn build_qubo_matrix(&self, base_matrix: Array2<f64>) -> Array2<f64> {
        ...
        let var_indices: HashMap<String, usize> = binary_vars.iter()
            .enumerate()
            .map(|(i, v)| (v.clone(), i))
            .collect();
...
pub fn compare_encodings(
    ...
    results.insert("one-hot".to_string(), EncodingMetrics {
    ...
    results.insert("binary".to_string(), EncodingMetrics {
    ...
    results.insert("domain-wall".to_string(), EncodingMetrics {
        num_bits: dw_bits,
