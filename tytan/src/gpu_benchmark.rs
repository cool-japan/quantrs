...
use crate::sampler::{Sampler, SampleResult};
...
use crate::scirs_stub::{
    scirs2_core::gpu::{GpuContext, get_device_count},
    scirs2_plot::{Plot, Line, Scatter, Bar},
};
...
impl<S: Sampler> GpuBenchmark<S> {
    ...
    fn benchmark_problem_sizes(&mut self, results: &mut BenchmarkResults) -> Result<(), String> {
        ...
        for &size in &self.config.problem_sizes {
            ...
            results.size_results.insert(size, SizeResults {
                avg_time,
                std_dev,
                throughput,
                best_energy,
    ...
    fn benchmark_batch_sizes(&mut self, results: &mut BenchmarkResults) -> Result<(), String> {
        ...
        for &batch_size in &self.config.batch_sizes {
            ...
            results.batch_results.insert(batch_size, BatchResults {
                exec_time: elapsed,
    ...
    fn benchmark_temperature_schedules(&mut self, results: &mut BenchmarkResults) -> Result<(), String> {
        if self.config.verbose {
        ...
        for &(initial, final_) in &self.config.temperature_schedules {
            ...
            let final_quality = solutions.first()
                .map(|s| s.energy)
                .unwrap_or(f64::INFINITY);
            
            results.temp_results.insert(schedule_name, TempResults {
    ...
    fn benchmark_energy_efficiency(&mut self, results: &mut BenchmarkResults) -> Result<(), String> {
        if self.config.verbose {
        ...
        let total_time: Duration = results.size_results.values()
            .map(|r| r.avg_time)
            .sum();
    ...
    fn get_device_info(&self) -> String {
        #[cfg(feature = "scirs")]
        {
            if let Ok(ctx) = GpuContext::new(0) {
                ...
                return format!("GPU: {} MB, {} compute units @ {} MHz",
    ...
    fn generate_report(&self, results: &BenchmarkResults) -> Result<(), String> {
        ...
        for (size, res) in &results.size_results {
            writeln!(file, "  Size {}: {:.2} ms avg, {:.0} samples/sec", 
                size, res.avg_time.as_secs_f64() * 1000.0, res.throughput).unwrap();
        }
        ...
        if let Some(energy) = &results.energy_metrics {
            ...
            writeln!(file, "  Energy per Sample: {:.3} mJ", energy.energy_per_sample * 1000.0).unwrap();
            writeln!(file, "  Performance per Watt: {:.1} samples/J", energy.perf_per_watt).unwrap();
        }
    ...
    fn plot_scaling_results(&self, results: &BenchmarkResults) -> Result<(), String> {
        #[cfg(feature = "scirs")]
        {
            ...
            let time_line = Line::new(sizes.clone(), times)
                .name("Execution Time (ms)");
            let throughput_line = Line::new(sizes, throughputs)
    ...
    fn plot_temperature_comparison(&self, results: &BenchmarkResults) -> Result<(), String> {
        #[cfg(feature = "scirs")]
        {
            ...
            let qualities: Vec<f64> = schedules.iter()
                .map(|s| results.temp_results[s].final_quality)
                .collect();
            
            let bar = Bar::new(schedules, qualities)
                .name("Final Solution Quality");
...
impl GpuComparison {
    ...
    pub fn run_comparison(&mut self) -> Result<ComparisonResults, String> {
        ...
        for config in &mut self.configs {
            ...
            // Placeholder results
            results.implementations.insert(config.name.clone(), 
        ...
        results.best_performer = results.implementations.iter()
            .max_by(|a, b| {
                a.1.avg_performance.partial_cmp(&b.1.avg_performance).unwrap()
            })
