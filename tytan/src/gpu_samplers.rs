...
use crate::scirs_stub::{
    scirs2_core::gpu::{GpuContext, DeviceInfo, get_device_count},
...
impl EnhancedArminSampler {
    ...
    fn run_gpu_optimized(
        ...
        if self.verbose {
            ...
            println!("GPU Device: {} MB memory, {} compute units",
        ...
        for batch in 0..num_batches {
            ...
            if self.verbose {
                println!("Processing batch {}/{} ({} samples)", 
                    batch + 1, num_batches, batch_size);
            }
    ...
    fn launch_annealing_kernel(
        ...
        d_states.copy_to_host(&mut host_states)
        ...
        for i in 0..batch_size {
            ...
            let state: Vec<bool> = host_states[start..end]
                .iter()
                .map(|&x| x != 0)
                .collect();
            results.push(state);
...
impl MIKASAmpler {
    ...
    fn run_hobo_gpu(
        ...
        use crate::scirs_stub::scirs2_linalg::tensor_contraction::{cp_decomposition, optimize_contraction_order};
        
        let n_vars = var_map.len();
        let order = tensor.ndim();
        
        if self.base_config.verbose {
            println!("Processing {}-order tensor with {} variables", order, n_vars);
        }
...
impl AsyncGpuPipeline {
    ...
    pub fn run_pipelined(
        ...
        self.sampler.run_qubo(&(qubo.clone(), var_map.clone()), shots)
    }
}
