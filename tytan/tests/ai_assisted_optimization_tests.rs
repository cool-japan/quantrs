...
fn test_ai_optimization_small_problem() {
    ...
    assert!(!optimization_result.problem_info.structure_patterns.is_empty());
    let has_block_pattern = optimization_result.problem_info.structure_patterns
        .iter()
    ...
    assert!(optimization_result.predicted_quality.confidence_interval.0 <= optimization_result.predicted_quality.expected_quality);
    assert!(optimization_result.predicted_quality.expected_quality <= optimization_result.predicted_quality.confidence_interval.1);
    ...
    println!("Recommended algorithm: {}", optimization_result.recommended_algorithm);
    println!("Predicted quality: {:.3}", optimization_result.predicted_quality.expected_quality);
...
fn test_problem_feature_extraction() {
    ...
    // 1. Dense problem
    let dense_qubo = Array2::ones((5, 5));
...
fn test_structure_recognition() {
    let recognizer = ProblemStructureRecognizer::new();
    ...
    let has_grid = grid_patterns.iter().any(|p| matches!(p, StructurePattern::Grid { .. }));
    assert!(has_grid);
    ...
    let has_block = block_patterns.iter().any(|p| matches!(p, StructurePattern::Block { .. }));
    assert!(has_block);
    ...
    let has_chain = chain_patterns.iter().any(|p| matches!(p, StructurePattern::Chain { .. }));
    assert!(has_chain);
...
fn test_algorithm_selection() {
    let config = AIOptimizerConfig::default();
    let selector = AutomatedAlgorithmSelector::new(config);

    // Test selection for different problem characteristics
    
    // Small problem
    let small_features = Array1::from(vec![50.0, 0.0, 1.0, 1.0, 0.3, 5.0, 1.0, 3.0, 1.0, 0.5]);
    let small_patterns = vec![];
    let small_alg = selector.select_algorithm(&small_features, &small_patterns).unwrap();
    assert_eq!(small_alg, "BranchAndBound"); // Should prefer exact methods for small problems

    // Dense problem
    let dense_features = Array1::from(vec![200.0, 0.0, 1.0, 1.0, 0.9, 5.0, 1.0, 8.0, 1.0, 2.0]);
    let dense_patterns = vec![];
    let dense_alg = selector.select_algorithm(&dense_features, &dense_patterns).unwrap();
    assert_eq!(dense_alg, "SimulatedAnnealing"); // Should prefer SA for dense problems

    // Tree-structured problem
    let tree_features = Array1::from(vec![100.0, 0.0, 1.0, 1.0, 0.2, 3.0, 1.0, 2.5, 1.0, 1.0]);
    let tree_patterns = vec![StructurePattern::Tree { depth: 5, branching_factor: 2.0 }];
    let tree_alg = selector.select_algorithm(&tree_features, &tree_patterns).unwrap();
    assert_eq!(tree_alg, "DynamicProgramming"); // Should prefer DP for tree structures

    // Large general problem
    let large_features = Array1::from(vec![1000.0, 0.0, 2.0, 1.5, 0.4, 8.0, 2.0, 6.0, 1.0, 3.0]);
    let large_patterns = vec![];
    let large_alg = selector.select_algorithm(&large_features, &large_patterns).unwrap();
...
fn test_parameter_optimization() {
    ...
    let sa_params = optimizer_net.optimize_parameters(&features, "SimulatedAnnealing", Some(0.9)).unwrap();
    ...
    let ga_params = optimizer_net.optimize_parameters(&features, "GeneticAlgorithm", Some(0.85)).unwrap();
...
fn test_quality_prediction() {
    let config = AIOptimizerConfig::default();
    let predictor = SolutionQualityPredictor::new(&config);

    // Test quality prediction for different scenarios
    let features = Array1::from(vec![50.0, 0.0, 1.0, 1.0, 0.3, 3.0, 1.0, 2.5, 1.0, 1.0]);
    ...
    for algorithm in algorithms {
        let prediction = predictor.predict_quality(&features, algorithm, &params).unwrap();
        ...
        println!("Algorithm {}: quality = {:.3}, confidence interval = [{:.3}, {:.3}]",
                 algorithm, prediction.expected_quality, 
...
fn test_difficulty_assessment() {
    ...
    let easy_assessment = optimizer.assess_difficulty(&easy_qubo, &easy_features, &easy_patterns).unwrap();
    
    assert!(easy_assessment.difficulty_score < 0.5);
    assert!(!easy_assessment.recommended_resources.gpu_recommended);
    assert!(!easy_assessment.recommended_resources.distributed_recommended);
    ...
    let hard_assessment = optimizer.assess_difficulty(&hard_qubo, &hard_features, &hard_patterns).unwrap();
    ...
    println!("Easy problem difficulty: {:.3}", easy_assessment.difficulty_score);
    println!("Hard problem difficulty: {:.3}", hard_assessment.difficulty_score);
}
...
fn test_training_components() {
    ...
    for i in 0..20 {
        ...
        let features = Array1::from(vec![
            size as f64, 0.0, 1.0, 1.0, 0.3, 3.0, 1.0, 2.5, 1.0, 1.0
        ]);
...
fn test_comprehensive_optimization_workflow() {
    ...
    for (name, qubo) in test_cases {
        ...
        println!("  Problem type: {}", optimization_result.problem_info.problem_type);
    }
...
fn create_large_structured_qubo() -> Array2<f64> {
    ...
    for i in 0..size-1 {
        if i % 10 == 9 { // Connect last element of each block to first of next
            qubo[[i, i + 1]] = 0.1;
            qubo[[i + 1, i]] = 0.1;
        }
    }
