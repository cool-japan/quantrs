...
fn test_graph_partitioner_kernighan_lin() {
    let config = GraphPartitionerConfig {
        algorithm: PartitioningAlgorithm::KernighanLin {
            max_iterations: 100,
            improvement_threshold: 1e-6,
            initial_partitioning: InitialPartitioning::Random,
        },
        max_subproblem_size: 50,
        overlap_strategy: OverlapStrategy::MinimalOverlap { overlap_size: 2 },
        load_balancing: LoadBalancingStrategy::Strict { tolerance: 0.1 },
        quality_metric: QualityMetric::Modularity,
    };
    ...
    for subproblem in &subproblems {
        assert!(subproblem.variables.len() <= config.max_subproblem_size);
...
fn test_graph_partitioner_spectral() {
    let config = GraphPartitionerConfig {
        ...
        overlap_strategy: OverlapStrategy::AdaptiveOverlap { 
            base_overlap: 1,
            complexity_factor: 0.1,
        },
        load_balancing: LoadBalancingStrategy::Flexible { max_imbalance: 0.2 },
        quality_metric: QualityMetric::CutSize,
    };
...
fn test_graph_partitioner_multilevel() {
    let config = GraphPartitionerConfig {
        algorithm: PartitioningAlgorithm::Multilevel {
            ...
            refinement_method: RefinementMethod::FiducciaMattheyses,
            max_levels: 5,
            coarsening_ratio: 0.5,
        },
        max_subproblem_size: 20,
        overlap_strategy: OverlapStrategy::MinimalOverlap { overlap_size: 1 },
        load_balancing: LoadBalancingStrategy::Strict { tolerance: 0.05 },
        quality_metric: QualityMetric::EdgeCut,
    };
...
fn test_decomposition_integration() {
    ...
    let partition_config = GraphPartitionerConfig {
        algorithm: PartitioningAlgorithm::KernighanLin {
            max_iterations: 50,
            improvement_threshold: 1e-5,
            initial_partitioning: InitialPartitioning::Random,
        ...
        overlap_strategy: OverlapStrategy::MinimalOverlap { overlap_size: 1 },
        load_balancing: LoadBalancingStrategy::Flexible { max_imbalance: 0.15 },
        quality_metric: QualityMetric::Modularity,
    };
